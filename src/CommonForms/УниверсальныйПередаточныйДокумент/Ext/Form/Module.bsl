...
Процедура ЗагрузитьТаблицуПродавцыИзВременногоХранилища(АдресХранилища)
	...
	Если ТипЗнч(ТаблицаПродавцы) = Тип("ТаблицаЗначений") Тогда
		Продавцы.Загрузить(ТаблицаПродавцы);
	ИначеЕсли ТипЗнч(ТаблицаПродавцы) = Тип("СправочникСсылка.Контрагенты") Тогда
		НоваяСтрока = Продавцы.Добавить();
		НоваяСтрока.Продавец = ТаблицаПродавцы;
	КонецЕсли;
