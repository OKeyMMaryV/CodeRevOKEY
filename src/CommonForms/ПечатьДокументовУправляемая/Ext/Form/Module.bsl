...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ИсточникДанных") Тогда
		...
		Если НЕ бит_БК_Общий.ПечатьПоВнешнемуИсточнику(
					Параметры.ИсточникДанных,
					Параметры.ПараметрыИсточника,
					КоллекцияПечатныхФорм,
					ОбъектыПечати,
					ПараметрыВывода,
					Параметры.ПараметрКоманды) Тогда
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		бит_БК_Общий.СформироватьПечатныеФормы(ИмяМенеджераПечати, ИменаМакетов, ПараметрКоманды, ПараметрыПечати,
...
Процедура ПечатьВыполнить()
	...
	бит_БК_ОбщийКлиент.РаспечататьТабличныеДокументы(ТабличныеДокументы, ОбъектыПечати,
...
Процедура ОтправитьПоПочтеВыполнить()
	Outlook = Истина;
	Если Outlook Тогда
		ОтправитьПоПочтеВыполнить_Outlook();
	иначе
		ОтправитьПоПочтеВыполнить_Вн();
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьПоПочтеВыполнить_Outlook()
	бит_БК_ПараметрыОтправкиСообщения = бит_БК_Общий.ПолучитьПараметрыОтправкиСообщения(ОбъектыПечати[0].Значение);
	ТабличныеДокументы = Новый СписокЗначений;
	Для Каждого ТабДокумент Из ИменаТабДокументов Цикл
		ТабличныеДокументы.Добавить(ЭтаФорма["Таб" + ТабДокумент.Значение], ТабДокумент.Представление);
	КонецЦикла;
	СписокВложений = бит_БК_ОбщийКлиент.ПолучитьСписокВложений(ТабличныеДокументы, ОбъектыПечати);
	бит_БК_ОбщийКлиент.СоздатьНовоеСообщениеАУТЛУК(СписокВложений, бит_БК_ПараметрыОтправкиСообщения);	
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьПоПочтеВыполнить_Вн()
