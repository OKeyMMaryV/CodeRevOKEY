...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если КлючНазначенияИспользования = "РасходМатериалов" 
		Или (КлючНазначенияИспользования = "ВыплатыСамозанятым" И ФормаОткрытаИзШапки) Тогда
...
Процедура РасположениеСчетовЗатратПриИзменении(Элемент)
	...
	Если Элементы.ГруппаСчетаУчета.Видимость Тогда
		...
		БухгалтерскийУчетКлиентСервер.ПриВыбореСчета(СчетЗатрат, ЭтотОбъект, ПоляФормы);
	КонецЕсли;
