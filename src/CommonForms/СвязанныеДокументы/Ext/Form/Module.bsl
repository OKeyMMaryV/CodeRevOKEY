...
Процедура ВывестиРодительскиеОбъекты(ТекущийОбъект, ДеревоРодитель)
	...
	//	|ГДЕ
	//	|	(ВЫРАЗИТЬ(бит_ДополнительныеАналитики.ЗначениеАналитики КАК Документ."+МетаданныеОбъекта.Имя+")) = &ТекущийДокумент";	
	//	Запрос.УстановитьПараметр("ТекущийДокумент",ТекущийОбъект);
	...
	//	|	РегистрСведений.бит_ДополнительныеАналитики КАК бит_ДополнительныеАналитики
	//	|ГДЕ
	//	|	(ВЫРАЗИТЬ(бит_ДополнительныеАналитики.Объект КАК Документ."+МетаданныеОбъекта.Имя+")) = &ТекущийДокумент";	
	//	Запрос.УстановитьПараметр("ТекущийДокумент",ТекущийОбъект);
	//	РезультатДопА = Запрос.Выполнить();
	//	Если НЕ РезультатДопА.Пустой() Тогда
	//		ВыборкаИзДопА = РезультатДопА.Выбрать();
...
Функция ОбъектыПоКритериюОтбора(ЗначениеКритерияОтбора)
	...
	// BIT AMerkulov ++
	Если ЗначениеКритерияОтбора.Метаданные().Реквизиты.Найти("ОК_ID_Разноска") <> Неопределено И ТипЗнч(ЗначениеКритерияОтбора) <> Тип("ДокументСсылка.бит_БюджетнаяОперация") Тогда
		...
		ID = ЗначениеКритерияОтбора.ОК_ID_Разноска;
