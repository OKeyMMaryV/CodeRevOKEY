...
Процедура ОрганизацииОтметкаПриИзменении(Элемент)
	...
	ИзменитьНастройкуПоОрганизации(ТекущиеДанные.Организация, ТекущиеДанные.Отметка);
...
Процедура ОбновитьСведенияНастроекНаСервере()
	...
	Для Каждого СтрокаПоОрганизации Из Организации Цикл
		...
		Если СтрокаНастройки <> Неопределено Тогда
			СтрокаПоОрганизации.Отметка    = СтрокаНастройки.Отметка;
...
Функция НастройкаОтправкиИСостояниеПоследнейОтправки()
	...
	Запрос.Текст =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ЕСТЬNULL(НастройкиОтправкиОтчетностиВБанк.ОтправлятьОтчеты, ЛОЖЬ) КАК Отметка,
	...
	Для Каждого РегЗадание Из Задания Цикл
		Если РегЗадание.Параметры.Количество() > 0 Тогда
			...
			Если СтрокаНастройки <> Неопределено Тогда
				СтрокаНастройки.Отметка = РегЗадание.Использование;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура ИзменитьНастройкуПоОрганизации(Организация, ОтправлятьОтчеты)
	...
	ЗаписьНастройки.ОтправлятьОтчеты = ОтправлятьОтчеты;
