...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.КлючНазначенияИспользования = "ДокументыПоставщиков" Тогда
		РаботаСДО_ОбновитьКоманды();
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Список, "ДополнительныеПоляИзРССостоянияЭД", Истина, Истина);
		Элементы.СписокСтатусЭДО.Видимость = Истина;
		Элементы.СписокВидДО.Видимость = Истина;
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Список, "ДополнительныеПоляИзРССостоянияЭД", Ложь, Истина);
		Элементы.СписокСтатусЭДО.Видимость = Ложь;
		Элементы.СписокВидДО.Видимость = Ложь;
	КонецЕсли;
...
//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-04-30 (#3614)
&НаКлиенте
Процедура КомандаРаботаСДО_ОткрытьДокумент(Команда)
	
	ТекущиеДанные = Элементы.Список.ТекущиеДанные;
	
	Если ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Для выполнения команды необходимо выбрать документ.'");
	...
	СписокАктуальныхЭлектронныхДокументов = ПолучитьСписокАктуальныхЭлектронныхДокументов(СсылкаНаОбъект);
	Если СписокАктуальныхЭлектронныхДокументов.Количество() > 0 Тогда
		ЭлектронныйДокумент = СписокАктуальныхЭлектронныхДокументов[0].Значение;
	КонецЕсли; 
	...
	Иначе
		...
		СсылкаНаОбъект_СтруктураРеквизитов = бит_ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаОбъект, "Номер, Дата");
		
		СообщениеПользователю = Новый СообщениеПользователю;
		СообщениеПользователю.Текст = "У документа учета №%1 от %2 нет ссылки на входящий ЭД и БД";
		СообщениеПользователю.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СообщениеПользователю.Текст, СсылкаНаОбъект_СтруктураРеквизитов.Номер, Формат(СсылкаНаОбъект_СтруктураРеквизитов.Дата, "ДФ=dd.MM.yyyy"));
...
&НаКлиенте
Процедура КомандаРаботаСДО_РаботаСАрхивом(Команда)
	
	Режим = РежимДиалогаВопрос.ДаНет;
	Оповещение = Новый ОписаниеОповещения("РаботаСАрхивом_ПослеЗакрытияВопроса", ЭтаФорма, Параметры);
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Открыть архив ДО с установленными отборами?'"), Режим, 0);
	
КонецПроцедуры

&НаКлиенте
Процедура РаботаСАрхивом_ПослеЗакрытияВопроса(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если ТипЗнч(ВыделенныеСтроки) <> Тип("Массив")
			ИЛИ ВыделенныеСтроки.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Для выполнения команды необходимо выбрать хотя бы один документ.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
		
		СписокАктуальныхЭлектронныхДокументов = ПолучитьСписокАктуальныхЭлектронныхДокументов(ВыделенныеСтроки);
		СтруктураПараметров.Вставить("СписокАктуальныхЭлектронныхДокументов", СписокАктуальныхЭлектронныхДокументов);
...
Функция ПолучитьСписокАктуальныхЭлектронныхДокументов(СписокВладельцев, ВходящиеДокументы = Неопределено)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		|	ЭлектронныйДокумент.Ссылка.Дата КАК Дата,
		|	ЭлектронныйДокумент.ДокументОснование КАК ДокументОснование,
		|	ЭлектронныйДокумент.Ссылка КАК ЭлектронныйДокумент
		|ПОМЕСТИТЬ ВТ_ПолныйСписокЭлектронныхДокументов
		|ИЗ
		|	Документ.ЭлектронныйДокументВходящий.ДокументыОснования КАК ЭлектронныйДокумент
		|ГДЕ
		|	&ВходящиеДокументы = ИСТИНА
		|	И ЭлектронныйДокумент.ДокументОснование В(&СписокВладельцев)
		...
		|
		|ВЫБРАТЬ
		|	СостоянияЭД.ЭлектронныйДокумент.Дата,
		...
		|	И (&ВходящиеДокументы = НЕОПРЕДЕЛЕНО
		|			ИЛИ &ВходящиеДокументы = ЛОЖЬ
		|				И СостоянияЭД.СсылкаНаОбъект ССЫЛКА Документ.ЭлектронныйДокументИсходящий
		|			ИЛИ &ВходящиеДокументы = ИСТИНА
		|				И СостоянияЭД.СсылкаНаОбъект ССЫЛКА Документ.ЭлектронныйДокументВходящий)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПолныйСписокЭлектронныхДокументов.ДокументОснование КАК ДокументОснование,
		|	МАКСИМУМ(ПолныйСписокЭлектронныхДокументов.Дата) КАК Дата
		|ПОМЕСТИТЬ ВТ_ОтборПоДате
		|ИЗ
		|	ВТ_ПолныйСписокЭлектронныхДокументов КАК ПолныйСписокЭлектронныхДокументов
		|
		|СГРУППИРОВАТЬ ПО
		|	ПолныйСписокЭлектронныхДокументов.ДокументОснование
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ПолныйСписокЭлектронныхДокументов.ЭлектронныйДокумент КАК ЭлектронныйДокумент
		|ИЗ
		|	ВТ_ОтборПоДате КАК ВТ_ОтборПоДате
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПолныйСписокЭлектронныхДокументов КАК ВТ_ПолныйСписокЭлектронныхДокументов
		|		ПО ВТ_ОтборПоДате.ДокументОснование = ВТ_ПолныйСписокЭлектронныхДокументов.ДокументОснование
		|			И ВТ_ОтборПоДате.Дата = ВТ_ПолныйСписокЭлектронныхДокументов.Дата
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ПолныйСписокЭлектронныхДокументов.ЭлектронныйДокумент";
	
	Если ТипЗнч(СписокВладельцев) <> Тип("Массив") Тогда
		МассивВладельцев = Новый Массив;
		МассивВладельцев.Добавить(СписокВладельцев);
	Иначе
		МассивВладельцев = СписокВладельцев;
	КонецЕсли; 
	
	Запрос.УстановитьПараметр("СписокВладельцев", МассивВладельцев);
	Запрос.УстановитьПараметр("ВходящиеДокументы", ВходящиеДокументы);
	...
	Пока Выборка.Следующий() Цикл
		...
		СписокЗначений.Добавить(Выборка.ЭлектронныйДокумент);
...
//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-04-30 (#3614)
&НаСервере
Процедура РаботаСДО_ОбновитьКоманды()
	
	ГруппаВажныеКоманды = Элементы.Найти("ГруппаВажныеКоманды");
	Если ГруппаВажныеКоманды <> Неопределено Тогда
		
		ИмяПодменю_РаботаСДО = "Подменю_РаботаСДО";
		Подменю_РаботаСДО = Элементы.Найти(ИмяПодменю_РаботаСДО);
		Если Подменю_РаботаСДО = Неопределено Тогда
			Подменю_РаботаСДО = Элементы.Добавить(ИмяПодменю_РаботаСДО, Тип("ГруппаФормы"), ГруппаВажныеКоманды);
			Подменю_РаботаСДО.Вид         = ВидГруппыФормы.Подменю;
			Подменю_РаботаСДО.Заголовок   = "Работа с ДО";
			Подменю_РаботаСДО.Видимость   = Истина;
			Подменю_РаботаСДО.Доступность = Истина;
		КонецЕсли;
		
		КомандаРаботаСДО_ОткрытьДокумент = ЭтаФорма.Команды.Найти("КомандаРаботаСДО_ОткрытьДокумент");
		Если КомандаРаботаСДО_ОткрытьДокумент = Неопределено Тогда
			КомандаРаботаСДО_ОткрытьДокумент = ЭтаФорма.Команды.Добавить("КомандаРаботаСДО_ОткрытьДокумент");
			КомандаРаботаСДО_ОткрытьДокумент.Заголовок = НСтр("ru = 'Открыть документ'");
			КомандаРаботаСДО_ОткрытьДокумент.Действие = "КомандаРаботаСДО_ОткрытьДокумент";
		КонецЕсли;
		
		Если Элементы.Найти(КомандаРаботаСДО_ОткрытьДокумент.Имя) = Неопределено Тогда
			КнопкаФормы = Элементы.Добавить(КомандаРаботаСДО_ОткрытьДокумент.Имя, Тип("КнопкаФормы"), Подменю_РаботаСДО);
			КнопкаФормы.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
			КнопкаФормы.ИмяКоманды  = КомандаРаботаСДО_ОткрытьДокумент.Имя;
			КнопкаФормы.Видимость   = Истина;
			КнопкаФормы.Доступность = Истина;
		КонецЕсли;
		
		КомандаРаботаСДО_РаботаСАрхивом = ЭтаФорма.Команды.Найти("КомандаРаботаСДО_РаботаСАрхивом");
		Если КомандаРаботаСДО_РаботаСАрхивом = Неопределено Тогда
			КомандаРаботаСДО_РаботаСАрхивом = ЭтаФорма.Команды.Добавить("КомандаРаботаСДО_РаботаСАрхивом");
			КомандаРаботаСДО_РаботаСАрхивом.Заголовок = НСтр("ru = 'Работа с архивом'");
			КомандаРаботаСДО_РаботаСАрхивом.Действие = "КомандаРаботаСДО_РаботаСАрхивом";
		КонецЕсли;
		
		Если Элементы.Найти(КомандаРаботаСДО_РаботаСАрхивом.Имя) = Неопределено Тогда
			КнопкаФормы = Элементы.Добавить(КомандаРаботаСДО_РаботаСАрхивом.Имя, Тип("КнопкаФормы"), Подменю_РаботаСДО);
			КнопкаФормы.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
			КнопкаФормы.ИмяКоманды  = КомандаРаботаСДО_РаботаСАрхивом.Имя;
			КнопкаФормы.Видимость   = Истина;
			КнопкаФормы.Доступность = Истина;
		КонецЕсли;
...
&НаСервере
Процедура РаботаСДО_ДополнитьЗапрос()
