...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	//ОКЕЙ Наумов А.Н.(СофтЛаб) Начало 2019-04-12 (#3103)
	Если ок_ЭтоPDF(ДокументДляПечати) Тогда
		ВидОперации = НСтр("ru = 'Печать электронного документа'");
		ТекстСообщения = НСтр("ru = 'Файл с расширением ""pdf"" отправляется на печать через окно приложения'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ТекстСообщения, ТекстСообщения);
		Возврат;
	КонецЕсли;
	//ОКЕЙ Наумов А.Н.(СофтЛаб) Конец 2019-04-12 (#3103)
...
//ОКЕЙ Наумов А.Н.(СофтЛаб) Начало 2019-04-12 (#3103)
&НаСервере
Функция ок_ЭтоPDF(ПараметрКоманды)
	
	Возврат НРег(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрКоманды,"Расширение")) = "pdf";
