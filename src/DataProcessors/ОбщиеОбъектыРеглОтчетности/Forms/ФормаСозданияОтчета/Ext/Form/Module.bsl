...
Процедура ОбновитьШаблоныЧерезВебСервис(СписокШаблонов)
	...
	ПараметрыЗапуска = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор); // результат выполнения не нужен
	ПараметрыЗапуска.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение списка XML-шаблонов через веб-сервис Росстата'");
	ПараметрыЗапуска.КлючФоновогоЗадания = "ОбновитьШаблоныСпискомЧерезВебСервисНаСервереВФоне"; // выполняем только одно задание в единицу времени
