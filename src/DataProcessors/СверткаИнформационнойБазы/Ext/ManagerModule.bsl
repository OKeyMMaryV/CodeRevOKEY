	...
	Если НЕ Результат.Пустой() Тогда
		Если Параметры.НаличиеИзмеренияОрганизация Тогда
			...
			Пока ВыборкаПоОрганизациям.Следующий() Цикл
				...
				// бит_финанс. Изменение кода. Начало. 08.09.2016{{
				Если бит_ОбщегоНазначенияКлиентСервер.ЭтоОбъектБИТ(Параметры.ТекущийРегистр.Имя) Тогда
					Продолжить;
				КонецЕсли; 
				// бит_финанс. Изменение кода. Конец. 08.09.2016}}
...
Процедура СвернутьИнформационнуюБазу(ПараметрыОбработки, АдресХранилища) Экспорт
	...
	Попытка
		...
		Для Каждого ТипДокумента Из ТипыДокументов Цикл
			// бит_финанс. Изменение кода. Начало. 16.10.2015{{
			Если бит_ОбщегоНазначенияКлиентСервер.ЭтоОбъектБИТ(ТипДокумента.Имя) Тогда
				Продолжить;
			КонецЕсли; 
			// бит_финанс. Изменение кода. Конец. 16.10.2015}}
		...
		Для Каждого ДанныеРегистра из СпособыСверткиОбъектовМетаданных Цикл
			// бит_финанс. Изменение кода. Начало. 16.10.2015{{
			Если бит_ОбщегоНазначенияКлиентСервер.ЭтоОбъектБИТ(ДанныеРегистра.ОбъектМД) Тогда
		...
		Для Каждого ТекСтрока из СпособыСверткиОбъектовМетаданных Цикл
			// бит_финанс. Изменение кода. Начало. 16.10.2015{{
			Если бит_ОбщегоНазначенияКлиентСервер.ЭтоОбъектБИТ(ТекСтрока.ОбъектМД) Тогда
...
Процедура ПометитьДокументыНаУдаление(ТипДокумента, ПараметрыОбработки)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		// бит_финанс. Изменение кода. Начало. 09.09.2016{{
		...
		КонецЕсли; 
		// бит_финанс. Изменение кода. Конец. 09.09.2016}}
	КонецЦикла;
