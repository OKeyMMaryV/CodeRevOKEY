...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ СВВ {[+](фрагмент добавлен), Сапожников Вадим 23.09.2015 13:14:54
	Элементы.ДокументыКИмпортуДоговор.УстановитьДействие("ПриИзменении","ДокументыКИмпортуДоговорПриИзменении");
	//-- СВВ}
...
Процедура ПриОткрытии(Отказ)
	...
	СБ_КазначействоКлиент.КлиентБанкПриОткрытии(ЭтаФорма);
	// -СБ. Широков Николай
...
Процедура ДокументыКИмпортуДоговорНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОтбора.Вставить("Владелец",      ТекущиеДанные.Контрагент);
	ПараметрыОтбора.Вставить("Организация",   Объект.Организация);
	//++ СВВ {[+](фрагмент добавлен), Сапожников Вадим 23.09.2015 13:08:21
...
// +СБ. Широков Николай. 2014-12-01. ТЗ "Казначейство" 2.2.10.1  
		// ++ 1c-izhtc Клинцов 2016.03.16 (
		//Если ТекущиеДанные.ВидДокумента = "ПоступлениеНаРасчетныйСчет" Тогда
		Если ТекущиеДанные.ПоступилоСписано = "Поступило" Тогда
		// -- 1c-izhtc Клинцов 2016.03.16 )
			
			Договор = ТекущиеДанные.Договор;
			//++ СВВ {[+](фрагмент добавлен), Сапожников Вадим 23.09.2015 13:41:36
			ВходящиеПараметры = Новый Структура;
			ВходящиеПараметры.Вставить("Договор",                      ТекущиеДанные.Договор);
			ВходящиеПараметры.Вставить("СБ_ПрофильРазноски",           ТекущиеДанные.СБ_ПрофильРазноски);
			ВходящиеПараметры.Вставить("СтатьяДДС",                    ТекущиеДанные.СтатьяДДС);
			//Профиль разноски
			УстановитьПрофильРазностки(ВходящиеПараметры);
			ТекущиеДанные.СБ_ПрофильРазноски = ВходящиеПараметры.СБ_ПрофильРазноски;
			СБ_ПрофильРазноскиОбработатьИзменение(ТекущиеДанные, ",Договор");
			//СтатьяДДС
			ПодобратьСтатьюДДС(ВходящиеПараметры);
			ТекущиеДанные.СтатьяДДС = ВходящиеПараметры.СтатьяДДС;
			//ЗаполнитьЗначенияСвойств(ТекущиеДанные, ВходящиеПараметры);
			//-- СВВ}
			
		КонецЕсли; 
// -СБ. Широков Николай 	

		//ижтиси, шадрин, 23.03.2016( типовой код, которого в новой версии не стало
		//ТекущиеДанные.Контрагент = ИзменитьКонтрагентаНаСервере(ТекущиеДанные.Договор);
		//ДокументыКИмпортуКонтрагентПриИзменении(неопределено);
		//ижтиси, шадрин, 23.03.2016)
	КонецЕсли;
...
Процедура ДокументыКИмпортуВидОперацииПриИзмененииНаСервере(ИдентификаторСтроки)
	...
	//++ СВВ {[+](фрагмент добавлен), Сапожников Вадим 23.09.2015 13:02:13
// +СБ. Широков Николай. 2015-04-20. Доработка #1465
	ТекущиеДанные = ДокументыКИмпорту.НайтиПоИдентификатору(ИдентификаторСтроки);
	Если ТекущиеДанные.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступление") Тогда
		ТекущиеДанные.СБ_ПрофильРазноски 	= "57.01";
		ТекущиеДанные.СтатьяДДС				= СБ_КазначействоПовтИсп.ПолучитьЗначениеКонстанты("СтатьяДДС_Инкассация");
		
		УточнитьХозяйственнуюОперациюДаннымиВведеннымиПользователем(
				ТекущиеДанные,
				"СБ_ПрофильРазноски, СтатьяДДС")
	КонецЕсли; 
// -СБ. Широков Николай 
	//-- СВВ}
...
Процедура УстановитьУсловноеОформлениеВыгрузка() Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
		ЭлементУО.Отбор,
		"Объект.ПлатежныеДокументы.ок_ЕстьРасхожденияСуммППиЗнРДС",
		ВидСравненияКомпоновкиДанных.Равно,
		Истина);
	...
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ПлатежныеДокументыВыгружать");
...
Процедура ЗагрузитьПлатежныеДокументыПослеОбновленияВФоне(Результат)
	...
	//ОКЕЙ Рычаков А.С.(СофтЛаб) 2020-11-02 Начало (#3936)
	СтрокиСРасхождениями = Объект.ПлатежныеДокументы.НайтиСтроки(Новый Структура("ок_ЕстьРасхожденияСуммППиЗнРДС",Истина));
	КоличествоСРасхождениями = СтрокиСРасхождениями.Количество(); 
	Для Каждого ТекСтрока Из СтрокиСРасхождениями Цикл 
...
Процедура ПлатежныеДокументыУстановитьОтметку(Отметка)
	...
	//ОК+ Аверьянова 04.05.17 #AT-1539497# для возможности установки отметки выделенных строк
	//Для каждого СтрокаДокумента Из Объект.ПлатежныеДокументы Цикл
	//	СтрокаДокумента.Выгружать = Отметка;
	//КонецЦикла;
	Если Элементы.ПлатежныеДокументы.ВыделенныеСтроки.Количество() > 1 Тогда 
		Для каждого СтрокаДокумента Из Элементы.ПлатежныеДокументы.ВыделенныеСтроки Цикл
			//ОКЕЙ Рычаков А.С.(СофтЛаб) 2020-11-02 Начало (#3936)
			//Объект.ПлатежныеДокументы.НайтиПоИдентификатору(СтрокаДокумента).Выгружать = Отметка;
			мСтрокаДокумента = Объект.ПлатежныеДокументы.НайтиПоИдентификатору(СтрокаДокумента); 
			Если мСтрокаДокумента.ок_ЕстьРасхожденияСуммППиЗнРДС Тогда
				Продолжить;
			Иначе 	
				мСтрокаДокумента.Выгружать = Отметка;
			КонецЕсли;	
			//ОКЕЙ Рычаков А.С.(СофтЛаб) 2020-11-02 Конец (#3936)
	   КонецЦикла;
	иначе  	
		Для каждого СтрокаДокумента Из Объект.ПлатежныеДокументы Цикл
			//ОКЕЙ Рычаков А.С.(СофтЛаб) 2020-11-02 Начало (#3936)
			Если СтрокаДокумента.ок_ЕстьРасхожденияСуммППиЗнРДС Тогда
				Продолжить;
			КонецЕсли;
			//ОКЕЙ Рычаков А.С.(СофтЛаб) 2020-11-02 Конец (#3936)
			СтрокаДокумента.Выгружать = Отметка;
...
Процедура ЗаполнитьДокументыКИмпортуПоРаспознаннымДаннымИзБанка(ДокументыКИмпорту, РаспознанныеДанныеИзБанка, ВыводитьСообщения)
	...
	Для Каждого КлючИЗначение Из РаспознанныеДанныеИзБанка.Выписки Цикл
		...
		Для Каждого Операция Из Выписка.Операции Цикл
			...
			КонецЕсли;
			// ++ 1c-izhtc Клинцов 2016.03.16 (
			СтрокаТаблицы.ПоступилоСписано = Операция.ПоступилоСписано;
			Если СтрокаТаблицы.ПоступилоСписано = "Поступило" Тогда
				Если ЛЕВ(СтрокаТаблицы.НазначениеПлатежа, 2) = "02" Тогда	
...
Процедура ЗаполнитьСтрокуПоХозяйственнойОперации(РаспознанныеДанныеИзБанка, СтрокаТаблицы, ХозяйственнаяОперация)
	...
	Если ХозяйственнаяОперация.Свойство("СБ_ПрофильРазноски") Тогда
		СтрокаТаблицы.СБ_ПрофильРазноски = ХозяйственнаяОперация.СБ_ПрофильРазноски;
	КонецЕсли;
	// -- 1c-izhtc Клинцов 2016.03.17 )
...
Процедура ЗаполнитьСтрокуПоДаннымДокумента(СтрокаТаблицы, ДанныеДокумента)
	...
	// ++ 1c-izhtc Клинцов 2016.03.17 (
	СтрокаТаблицы.СБ_ПрофильРазноски = ДанныеДокумента.СБ_ПрофильРазноски;
	// -- 1c-izhtc Клинцов 2016.03.17 )
...
&НаСервере
Процедура ДокументыКИмпортуУстановитьОтметку(Отметка, ПервоначальноеЗаполнение = Ложь)
		//++СВВ
//Процедура ДокументыКИмпортуУстановитьОтметку(Отметка)
	...
	Для каждого СтрокаДокумента Из ДокументыКИмпорту Цикл
		...
		//++СВВ
		Если Лев(СтрокаДокумента.НазначениеПлатежа,2) = "02" и ПервоначальноеЗаполнение Тогда
...
Процедура УточнитьХозяйственнуюОперациюДаннымиВведеннымиПользователем(СтрокаДокументыКИмпорту, ИменаВведенныхДанных)
	...
	Если ЗначениеЗаполнено(СтрокаДокументыКИмпорту.Документ) Тогда
		...
		// +СБ. Широков Николай. 2016-05-24. 
		ВведенныеДанные.Вставить("СБ_ПрофильРазноски");
		// -СБ. Широков Николай
	Иначе
...
Процедура СБ_ПрофильРазноскиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	мТекущийПрофиль = ДанныеСтроки.СБ_ПрофильРазноски; 
	
	СтандартнаяОбработка = Ложь;
	
	ПараметрыФормы = Новый Структура("Организация, СБ_ПрофильРазноски", Объект.Организация, мТекущийПрофиль);
	ВыбраннаяСтрока = ОткрытьФормуМодально("ОбщаяФорма.СБ_ФормаВыбораПрофиляРазноски", ПараметрыФормы);
	
	Если ВыбраннаяСтрока <> Неопределено Тогда
		ДанныеСтроки.СБ_ПрофильРазноски = ВыбраннаяСтрока.Код;
	КонецЕсли; 
	
	Если мТекущийПрофиль <> ДанныеСтроки.СБ_ПрофильРазноски Тогда
		СБ_ПрофильРазноскиОбработатьИзменение(ДанныеСтроки);
	КонецЕсли; 
...
// +СБ. Широков Николай. 2014-12-01. ТЗ "Казначейство" 2.2.10.1  
&НаКлиенте
Процедура СБ_ПрофильРазноскиАвтоПодборТекста(Элемент, Текст, ДанныеВыбора, Параметры, Ожидание, СтандартнаяОбработкаа)
	...
	МассивПрофилей 		= СБ_КазначействоСервер.НайтиПрофильРазноски(Текст, Объект.Организация);
	...
	ИначеЕсли КоличествоЭлементов = 1 Тогда
		ДанныеСтроки.СБ_ПрофильРазноски = МассивПрофилей[0];
		НеВыбирать = Истина;	
	КонецЕсли; 
	
	Если НеВыбирать Тогда
		Если мТекущийПрофиль <> ДанныеСтроки.СБ_ПрофильРазноски Тогда
			СБ_ПрофильРазноскиОбработатьИзменение(ДанныеСтроки);
		КонецЕсли; 
		Возврат;
	...
	СписокВыбора.ЗагрузитьЗначения(МассивПрофилей);
	
	ВыбранныйЭлемент = ВыбратьИзСписка(СписокВыбора, Элемент);
	Если ВыбранныйЭлемент <> Неопределено Тогда
		ДанныеСтроки.СБ_ПрофильРазноски = ВыбранныйЭлемент.Значение;
	КонецЕсли; 
...
Процедура СБ_ПрофильРазноскиОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Если КоличествоЭлементов = 0 Тогда
		ДанныеСтроки.СБ_ПрофильРазноски = "";
		НеВыбирать = Истина;	
	ИначеЕсли КоличествоЭлементов = 1 Тогда
		ДанныеСтроки.СБ_ПрофильРазноски = МассивПрофилей[0];
		НеВыбирать = Истина;	
...
// ++ 1c-izhtc Клинцов 2016.03.17 (
&НаСервере
Процедура СБ_ПрофильРазноскиОбработатьИзменениеНаСервере(ИдентификаторСтроки, ИменаВведенныхДанных = "")
	
	ТекущиеДанные = ДокументыКИмпорту.НайтиПоИдентификатору(ИдентификаторСтроки);
	
	УточнитьХозяйственнуюОперациюДаннымиВведеннымиПользователем(ТекущиеДанные, "СБ_ПрофильРазноски" + ИменаВведенныхДанных);
...
// +СБ. Широков Николай. 2014-12-01. ТЗ "Казначейство" 2.2.10.1 
&НаКлиенте
Процедура СБ_ПрофильРазноскиОбработатьИзменение(ДанныеСтроки, ИменаВведенныхДанных = "") Экспорт
	
	// ++ 1c-izhtc Клинцов 2016.03.17 (
	СБ_ПрофильРазноскиОбработатьИзменениеНаСервере(Элементы.ДокументыКИмпорту.ТекущаяСтрока, ИменаВведенныхДанных);
	...
	// -- 1c-izhtc Клинцов 2016.03.17 )
	// ++ 1c-izhtc Клинцов 2016.03.16 (
	//Если ДанныеСтроки.ВидДокумента = "ПоступлениеНаРасчетныйСчет" Тогда
	Если ДанныеСтроки.ПоступилоСписано = "Поступило" Тогда
	// -- 1c-izhtc Клинцов 2016.03.16 )
		ДанныеСтроки.ВидОперации = СБ_КазначействоСервер.ВидОперацииПоступленияДСПоПрофилюРазноски(ДанныеСтроки.СБ_ПрофильРазноски);	
	// ++ 1c-izhtc Клинцов 2016.03.16 (
	//ИначеЕсли ДанныеСтроки.ВидДокумента = "СписаниеСРасчетногоСчета" Тогда
	ИначеЕсли ДанныеСтроки.ПоступилоСписано = "Списано" Тогда
	// -- 1c-izhtc Клинцов 2016.03.16 )
		ДанныеСтроки.ВидОперации = СБ_КазначействоСервер.ВидОперацииСписанияДСПоПрофилюРазноски(ДанныеСтроки.СБ_ПрофильРазноски);	
	КонецЕсли; 
	
КонецПроцедуры // -СБ. Широков Николай

//++ СВВ {[+](фрагмент добавлен), Сапожников Вадим 23.10.2015 16:22:17
&НаСервере
Функция ИзменитьКонтрагентаНаСервере(Договор)
	Возврат Договор.Владелец;
КонецФункции
//-- СВВ}

&НаСервере
Процедура ПодобратьСтатьюДДС(ТекущиеДанные)
	Запрос = Новый Запрос;
	Запрос.Текст = 
	...
	|		ПО ДоговорыКонтрагентовбит_СтатьиОборотов.СтатьяОборотов = бит_СтатьиОборотов_СтатьиРегл.СтатьяОборотов
	|ГДЕ
	|	ДоговорыКонтрагентовбит_СтатьиОборотов.РасходДоход = ЗНАЧЕНИЕ(Перечисление.бит_РасходДоход.Поступление)
	|	И ДоговорыКонтрагентовбит_СтатьиОборотов.Ссылка = &Договор";
	Запрос.УстановитьПараметр("Договор",	ТекущиеДанные.Договор);	 //Договоры контрагентов
	РезультатЗапроса = Запрос.Выполнить();
	
	Если РезультатЗапроса.Пустой() Тогда
		Сообщить("Не удалось определить статью ДДС. В договоре не указана статья оборотов с видом ""Поступление""");  
	Иначе
		
		Выборка = РезультатЗапроса.Выбрать();
		Выборка.Следующий();
		
		Если НЕ ЗначениеЗаполнено(Выборка.СтатьяРегл) Тогда
			Сообщить("Не удалось определить статью ДДС. Статья оборотов не задано соответствие");
		Иначе
			ТекущиеДанные.СтатьяДДС = Выборка.СтатьяРегл;	
		КонецЕсли; 
	КонецЕсли; 
КонецПроцедуры

&НаСервере
Процедура УстановитьПрофильРазностки(ТекущиеДанные)
	
	ТекущиеДанные.СБ_ПрофильРазноски = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущиеДанные.Договор, "СБ_ПрофильРазноски");
