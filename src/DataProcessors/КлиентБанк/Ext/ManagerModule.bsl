...
Функция Выгрузить(ТаблицаДокументов, ПлатежныеДокументы, Знач Кодировка, Программа, НачПериода, КонПериода, БанковскийСчет,
	...
	//++ СВВ {[+](фрагмент добавлен), Сапожников Вадим 23.09.2015 14:07:53
	...
	Для каждого СтрокаСекции из ПлатежныеДокументы Цикл
		...
		ДокументОбъект = СтрокаСекции.Документ.ПолучитьОбъект();
		ДокументОбъект.СБ_ВыгруженВКлиентБанка = Истина;
...
Процедура ЗаписатьЗаголовокФайлаВыгрузки(ПотокВыгрузки, ПараметрыВыгрузки)
	...
	ПотокВыгрузки.ЗаписатьСтроку("ВерсияФормата=" + ПараметрыВыгрузки.ВерсияФормата); // Поддерживаются версии "1.01" и "1.02"
...
Функция ПолучитьТаблицуДокументовДляЭкспорта(ПлатежныеПоручения, ПлатежныеТребования) Экспорт
	...
	"ВЫБРАТЬ
	|	ПлатежноеПоручение.Ссылка КАК ПП,
	|	ВЫБОР
	|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПлатежноеПоручениебит_ПлатежныеПозиции.ПлатежнаяПозиция.ДокументОснование) = 1
	|			ТОГДА МАКСИМУМ(ПлатежноеПоручениебит_ПлатежныеПозиции.ПлатежнаяПозиция.ДокументОснование)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.бит_ЗаявкаНаРасходованиеСредств.ПустаяСсылка)
	|	КОНЕЦ КАК ЗнРДС,
	//ОКЕЙ Василенко В.В. (СофтЛаб) Начало 2021-10-29 (#4439)
	//|	СУММА(ПлатежноеПоручениебит_ПлатежныеПозиции.ПлатежнаяПозиция.ДокументОснование.СуммаДокумента) КАК СуммаЗнРДС
	|	СУММА(ПлатежноеПоручениебит_ПлатежныеПозиции.ПлатежнаяПозиция.ДокументОснование.СуммаРегл) КАК СуммаЗнРДС
	//ОКЕЙ Василенко В.В. (СофтЛаб) Конец 2021-10-29 (#4439)
	...
	|		ПО ПлатежноеПоручение.Ссылка = ПлатежноеПоручениебит_ПлатежныеПозиции.Ссылка
	|ГДЕ
	...
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-07-13 (#3795)
	|	,ПлатежноеПоручение.ок_КодНазначенияПлатежа КАК ок_КодНазначенияПлатежа
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2020-07-13 (#3795)
	//ОКЕЙ Рычаков А.С.(СофтЛаб) 2020-11-02 Начало (#3936)
	|	,ВТ_ПлатежныеПоручения.ЗнРДС КАК ок_ЗнРДС
	|	,ВТ_ПлатежныеПоручения.СуммаЗнРДС КАК ок_СуммаЗнРДС
	|	,ВЫБОР КОГДА ВТ_ПлатежныеПоручения.СуммаЗнРДС <> ПлатежноеПоручение.СуммаДокумента 
	|    ТОГДА Истина ИНАЧЕ Ложь КОНЕЦ КАК ок_ЕстьРасхожденияСуммППиЗнРДС
	//ОКЕЙ Рычаков А.С.(СофтЛаб) 2020-11-02 Конец (#3936)
	|ИЗ
	|	Документ.ПлатежноеПоручение КАК ПлатежноеПоручение
	//ОКЕЙ Рычаков А.С.(СофтЛаб) 2020-11-02 Начало (#3936)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ПлатежныеПоручения КАК ВТ_ПлатежныеПоручения
	|		ПО ВТ_ПлатежныеПоручения.ПП = ПлатежноеПоручение.Ссылка
	//ОКЕЙ Рычаков А.С.(СофтЛаб) 2020-11-02 Конец (#3936)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияБанковскихДокументов КАК СостоянияБанковскихДокументов
	|		ПО ПлатежноеПоручение.Ссылка = СостоянияБанковскихДокументов.СсылкаНаОбъект
	//ОКЕЙ Рычаков А.С.(СофтЛаб) 2020-11-02 Начало (#3936)
...
Функция СформироватьСтруктуруЭкспорта(ВерсияФормата)
	...
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-07-13 (#3795)
	СтруктураЭкспорта.Вставить("КодНазПлатежа",			 ""); // "Код назначения платежа"
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2020-07-13 (#3795)
	...
	Если ВыборкаДляЭкспорта.ПеречислениеВБюджет Тогда
		...
		Если ПоказательТипаПредусмотренСтандартамиОбмена Тогда
			Если ВыборкаДляЭкспорта.Дата >= ПлатежиВБюджетКлиентСервер.НачалоДействияПриказа126н() Тогда
				...
				СтрокаЭкспорта.ПоказательТипа = ""; //изменения по задаче 2355 БИТ АКриштопов 060416
			Иначе
...
//НазначениеПлатежа, НазначениеПлатежа1 НазначениеПлатежа2 НазначениеПлатежа3 НазначениеПлатежа4 НазначениеПлатежа5 НазначениеПлатежа6
	...
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-07-13 (#3795)
	Если ЗначениеЗаполнено(ВыборкаДляЭкспорта.ок_КодНазначенияПлатежа) Тогда
		СтрокаЭкспорта.КодНазПлатежа = Строка(ВыборкаДляЭкспорта.ок_КодНазначенияПлатежа);
	Иначе
		Если СтрокаЭкспорта.Свойство("КодНазПлатежа") Тогда
...
// +СБ. Широков Николай. 2015-05-19. 
Процедура РаспознатьДанныеПоТаблицеИмпорта(ДокументыКИмпорту, Организация, БанковскийСчет)
	...
	Для каждого СтрокаДокумента Из ДокументыКИмпорту Цикл
		...
		Если НЕ ПустаяСТрока(СтрокаДокумента.ДатаСписано) Тогда
			Результат = ПолучитьДатуИзСтроки(СтрокаДокумента.ДатаСписано);
		ИначеЕсли НЕ ПустаяСТрока(СтрокаДокумента.ДатаПоступило) Тогда
			Результат = ПолучитьДатуИзСтроки(СтрокаДокумента.ДатаПоступило);
		Иначе
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТаблицаИмпорта.ПлательщикСчет КАК ПлательщикСчет,
	|	ТаблицаИмпорта.ПлательщикИНН КАК ПлательщикИНН,
	|	ТаблицаИмпорта.Плательщик1 КАК Плательщик1,
	|	ТаблицаИмпорта.ПлательщикКПП КАК ПлательщикКПП,
	|	ТаблицаИмпорта.ПолучательСчет КАК ПолучательСчет,
	|	ТаблицаИмпорта.ПолучательИНН КАК ПолучательИНН,
	|	ТаблицаИмпорта.ПолучательКПП КАК ПолучательКПП,
	|	ТаблицаИмпорта.Получатель1 КАК Получатель1,
	|	ТаблицаИмпорта.НомерДок КАК НомерДок,
	|	ТаблицаИмпорта.ДатаПлПоручения КАК ДатаПлПоручения,
	|	ТаблицаИмпорта.ДатаДок КАК ДатаДок,
	|	ТаблицаИмпорта.НомерСтроки КАК НомерСтроки2
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВнешняяТаблица.ДатаДок,
	|	ВнешняяТаблица.ДатаПлПоручения,
	|	ВнешняяТаблица.НомерДок,
	|	ВЫБОР
	|		КОГДА ВнешняяТаблица.ПлательщикСчет = &НомерСчета
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоСписание,
	|	ВЫБОР
	|		КОГДА ВнешняяТаблица.ПлательщикСчет = &НомерСчета
	|			ТОГДА ВнешняяТаблица.ПолучательСчет
	|		ИНАЧЕ ВнешняяТаблица.ПлательщикСчет
	|	КОНЕЦ КАК КонтрагентНомерСчета,
	|	ВЫБОР
	|		КОГДА ВнешняяТаблица.ПлательщикСчет = &НомерСчета
	|			ТОГДА ВнешняяТаблица.ПолучательИНН
	|		ИНАЧЕ ВнешняяТаблица.ПлательщикИНН
	|	КОНЕЦ КАК КонтрагентИНН,
	|	ВЫБОР
	|		КОГДА ВнешняяТаблица.ПлательщикСчет = &НомерСчета
	|			ТОГДА ВнешняяТаблица.ПолучательКПП
	|		ИНАЧЕ ВнешняяТаблица.ПлательщикКПП
	|	КОНЕЦ КАК КонтрагентКПП,
	|	ВЫБОР
	|		КОГДА ВнешняяТаблица.ПлательщикСчет = &НомерСчета
	|			ТОГДА ВнешняяТаблица.Получатель1
	|		ИНАЧЕ ВнешняяТаблица.Плательщик1
	|	КОНЕЦ КАК КонтрагентНаименование,
	|	ВнешняяТаблица.НомерСтроки2,
	|	НАЧАЛОПЕРИОДА(ВнешняяТаблица.ДатаДок, ДЕНЬ) КАК ДатаДокНачалоДня,
	|	КОНЕЦПЕРИОДА(ВнешняяТаблица.ДатаДок, ДЕНЬ) КАК ДатаДокКонецДня,
	|	НАЧАЛОПЕРИОДА(ВнешняяТаблица.ДатаПлПоручения, ДЕНЬ) КАК ДатаПлПорученияНачалоДня,
	|	КОНЕЦПЕРИОДА(ВнешняяТаблица.ДатаПлПоручения, ДЕНЬ) КАК ДатаПлПорученияКонецДня
	|ПОМЕСТИТЬ Таблица
	|ИЗ
	...
	|ПОМЕСТИТЬ НайденныеДокументыБезФильтра
	|ИЗ
	|	Таблица КАК Таблица
	...
	|			ПО СписаниеСРасчетногоСчета.Ссылка = СписаниеСРасчетногоСчетаРасшифровкаПлатежа.Ссылка
	|				И (СписаниеСРасчетногоСчетаРасшифровкаПлатежа.НомерСтроки = 1)
	|		ПО Таблица.ДатаДокНачалоДня <= СписаниеСРасчетногоСчета.Дата
	|			И Таблица.ДатаДокКонецДня >= СписаниеСРасчетногоСчета.Дата
	|			И Таблица.НомерДок = СписаниеСРасчетногоСчета.НомерВходящегоДокумента
	|ГДЕ
	|	Таблица.ЭтоСписание
	...
	|			ПО СписаниеСРасчетногоСчета.Ссылка = СписаниеСРасчетногоСчетаРасшифровкаПлатежа.Ссылка
	|				И (СписаниеСРасчетногоСчетаРасшифровкаПлатежа.НомерСтроки = 1)
	|		ПО Таблица.ДатаПлПорученияНачалоДня <= СписаниеСРасчетногоСчета.ДатаВходящегоДокумента
	|			И Таблица.ДатаПлПорученияКонецДня >= СписаниеСРасчетногоСчета.ДатаВходящегоДокумента
	|			И Таблица.НомерДок = СписаниеСРасчетногоСчета.НомерВходящегоДокумента
	|ГДЕ
	|	Таблица.ЭтоСписание
	|	И СписаниеСРасчетногоСчета.Организация = &Организация
	...
	|	ТЧ_РасшифровкаПлатежа.ДоговорКонтрагента,
	|	ТЧ_РасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств,
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеНаРасчетныйСчет.РасшифровкаПлатежа КАК ТЧ_РасшифровкаПлатежа
	|			ПО ПоступлениеНаРасчетныйСчет.Ссылка = ТЧ_РасшифровкаПлатежа.Ссылка
	|				И (ТЧ_РасшифровкаПлатежа.НомерСтроки = 1)
	|		ПО Таблица.ДатаДокНачалоДня <= ПоступлениеНаРасчетныйСчет.Дата
	|			И Таблица.ДатаДокКонецДня >= ПоступлениеНаРасчетныйСчет.Дата
	|			И Таблица.НомерДок = ПоступлениеНаРасчетныйСчет.НомерВходящегоДокумента
	|ГДЕ
	|	НЕ Таблица.ЭтоСписание
	...
	|			ПО ПоступлениеНаРасчетныйСчет.Ссылка = ТЧ_РасшифровкаПлатежа.Ссылка
	|				И (ТЧ_РасшифровкаПлатежа.НомерСтроки = 1)
	|		ПО Таблица.ДатаПлПорученияНачалоДня <= ПоступлениеНаРасчетныйСчет.ДатаВходящегоДокумента
	|			И Таблица.ДатаПлПорученияКонецДня >= ПоступлениеНаРасчетныйСчет.ДатаВходящегоДокумента
	|			И Таблица.НомерДок = ПоступлениеНаРасчетныйСчет.НомерВходящегоДокумента
	|ГДЕ
	|	НЕ Таблица.ЭтоСписание
	|	И ПоступлениеНаРасчетныйСчет.Организация = &Организация
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НайденныеДокументы.НомерСтроки2,
	|	МИНИМУМ(НайденныеДокументы.Ссылка) КАК ДокументСсылка,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ НайденныеДокументы.Ссылка) КАК КоличествоДокументов,
	|	НайденныеДокументы.Номер,
	|	НайденныеДокументы.Дата,
	|	НайденныеДокументы.Проведен,
	|	НайденныеДокументы.ВидОперации,
	|	НайденныеДокументы.СчетКонтрагента,
	|	НайденныеДокументы.СчетОрганизации,
	|	НайденныеДокументы.Организация,
	|	НайденныеДокументы.Контрагент,
	|	НайденныеДокументы.ДокументОснование,
	|	НайденныеДокументы.ДоговорКонтрагента,
	|	НайденныеДокументы.СтатьяДвиженияДенежныхСредств,
	|	НайденныеДокументы.СБ_ПрофильРазноски
	|ПОМЕСТИТЬ НайденныеДокументы
	|ИЗ
	|	НайденныеДокументыБезФильтра КАК НайденныеДокументы
	|
	|СГРУППИРОВАТЬ ПО
	|	НайденныеДокументы.НомерСтроки2,
	|	НайденныеДокументы.Номер,
	...
	|ВЫБРАТЬ
	|	Таблица.ДатаДок,
	|	Таблица.ДатаПлПоручения,
	|	Таблица.НомерДок,
	|	Таблица.ЭтоСписание,
	|	Таблица.КонтрагентНомерСчета,
	|	Таблица.КонтрагентИНН,
	|	Таблица.КонтрагентКПП,
	|	Таблица.КонтрагентНаименование,
	|	Таблица.НомерСтроки2,
	|	Таблица.ДатаДокНачалоДня,
	|	Таблица.ДатаДокКонецДня,
	|	Таблица.ДатаПлПорученияНачалоДня,
	|	Таблица.ДатаПлПорученияКонецДня
	|ПОМЕСТИТЬ ТаблицаНайтиАналитику
	|ИЗ
	|	Таблица КАК Таблица
	|		ЛЕВОЕ СОЕДИНЕНИЕ НайденныеДокументы КАК НайденныеДокументы
	|		ПО Таблица.НомерСтроки2 = НайденныеДокументы.НомерСтроки2
	|ГДЕ
	|	НайденныеДокументы.НомерСтроки2 ЕСТЬ NULL 
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	Контрагенты.КПП,
	|	ВЫБОР
	|		КОГДА Контрагенты.НаименованиеПолное ПОДОБНО Таблица.КонтрагентНаименование
	|			ТОГДА 1
	|		ИНАЧЕ 2
	|	КОНЕЦ КАК Приоритет,
	|	Таблица.КонтрагентНаименование,
	|	Таблица.КонтрагентНомерСчета
	|ПОМЕСТИТЬ КонтрагентыБезФильтров
	|ИЗ
	|	ТаблицаНайтиАналитику КАК Таблица
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
	|		ПО Таблица.КонтрагентИНН = Контрагенты.ИНН
	|ГДЕ
	|	Контрагенты.Ссылка В ИЕРАРХИИ(&ГруппаКонтрагентов_Использовать)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Таблица.НомерСтроки2,
	|	МИНИМУМ(ЕСТЬNULL(КонтрагентыПоКПП.Ссылка, ЕСТЬNULL(КонтрагентыПоНаименованию.Ссылка, КонтрагентыПоИНН.Ссылка))) КАК КонтрагентСсылка,
	|	КОЛИЧЕСТВО(ЕСТЬNULL(КонтрагентыПоКПП.Ссылка, ЕСТЬNULL(КонтрагентыПоНаименованию.Ссылка, КонтрагентыПоИНН.Ссылка))) КАК КоличествоКонтрагентов
	|ПОМЕСТИТЬ НайденныеКонтрагенты
	|ИЗ
	|	ТаблицаНайтиАналитику КАК Таблица
	|		ЛЕВОЕ СОЕДИНЕНИЕ КонтрагентыБезФильтров КАК КонтрагентыПоКПП
	|		ПО Таблица.НомерСтроки2 = КонтрагентыПоКПП.НомерСтроки2
	|			И Таблица.КонтрагентИНН = КонтрагентыПоКПП.ИНН
	|			И Таблица.КонтрагентКПП = КонтрагентыПоКПП.КПП
	|		ЛЕВОЕ СОЕДИНЕНИЕ КонтрагентыБезФильтров КАК КонтрагентыПоНаименованию
	|		ПО Таблица.НомерСтроки2 = КонтрагентыПоНаименованию.НомерСтроки2
	|			И (КонтрагентыПоНаименованию.Приоритет = 1)
	|		ЛЕВОЕ СОЕДИНЕНИЕ КонтрагентыБезФильтров КАК КонтрагентыПоИНН
	|		ПО Таблица.НомерСтроки2 = КонтрагентыПоИНН.НомерСтроки2
	|
	|СГРУППИРОВАТЬ ПО
	|	Таблица.НомерСтроки2
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	КонтрагентыБезФильтров.НомерСтроки2,
	|	МАКСИМУМ(БанковскиеСчета.Ссылка) КАК БанковскийСчетСсылка
	|ПОМЕСТИТЬ НайденныеСчета
	|ИЗ
	|	КонтрагентыБезФильтров КАК КонтрагентыБезФильтров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.БанковскиеСчета КАК БанковскиеСчета
	|		ПО КонтрагентыБезФильтров.Ссылка = БанковскиеСчета.Владелец
	|			И КонтрагентыБезФильтров.КонтрагентНомерСчета = БанковскиеСчета.НомерСчета
	|
	|СГРУППИРОВАТЬ ПО
	|	КонтрагентыБезФильтров.НомерСтроки2
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаНайтиАналитику.НомерСтроки2,
	|	НайденныеСчета.БанковскийСчетСсылка,
	|	ВЫБОР
	|		КОГДА НайденныеСчета.БанковскийСчетСсылка ЕСТЬ NULL 
	|			ТОГДА НайденныеКонтрагенты.КонтрагентСсылка
	|		ИНАЧЕ НайденныеСчета.БанковскийСчетСсылка.Владелец
	|	КОНЕЦ КАК КонтрагентСсылка,
	|	ЕСТЬNULL(НайденныеКонтрагенты.КоличествоКонтрагентов, 0) КАК КоличествоКонтрагентов,
	|	NULL КАК ДокументСсылка,
	|	0 КАК КоличествоДокументов,
	|	NULL КАК Номер,
	|	NULL КАК Дата,
	|	NULL КАК Проведен,
	|	ТаблицаНайтиАналитику.КонтрагентНомерСчета,
	|	ТаблицаНайтиАналитику.ЭтоСписание,
	|	NULL КАК ВидОперации,
	|	NULL КАК СчетОрганизации,
	|	NULL КАК Организация,
	|	NULL КАК ДокументОснование,
	|	NULL КАК ДоговорКонтрагента,
	|	NULL КАК СтатьяДвиженияДенежныхСредств,
	|	NULL КАК СБ_ПрофильРазноски
	|ИЗ
	|	ТаблицаНайтиАналитику КАК ТаблицаНайтиАналитику
	|		ЛЕВОЕ СОЕДИНЕНИЕ НайденныеСчета КАК НайденныеСчета
	|		ПО ТаблицаНайтиАналитику.НомерСтроки2 = НайденныеСчета.НомерСтроки2
	|		ЛЕВОЕ СОЕДИНЕНИЕ НайденныеКонтрагенты КАК НайденныеКонтрагенты
	|		ПО ТаблицаНайтиАналитику.НомерСтроки2 = НайденныеКонтрагенты.НомерСтроки2
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	НайденныеДокументы.Контрагент,
	|	0,
	|	НайденныеДокументы.ДокументСсылка,
	|	НайденныеДокументы.КоличествоДокументов,
	|	НайденныеДокументы.Номер,
	|	НайденныеДокументы.Дата,
	|	НайденныеДокументы.Проведен,
	|	NULL,
	...
	Запрос.УстановитьПараметр("ГруппаКонтрагентов_Использовать",	СБ_КазначействоПовтИсп.ПолучитьЗначениеКонстанты("ГруппаКонтрагентов_Использовать"));	 //Контрагенты
	Запрос.УстановитьПараметр("НомерСчета",							ОбщегоНазначения.ЗначениеРеквизитаОбъекта(БанковскийСчет, "НомерСчета"));	 //Строка
	Запрос.УстановитьПараметр("Организация",						Организация);	 //Организации
	Запрос.УстановитьПараметр("СчетОрганизации",					БанковскийСчет);	 //Банковские счета
	Запрос.УстановитьПараметр("ТаблицаИмпорта",						ДокументыКИмпорту);
	РезультатЗапроса = Запрос.Выполнить();
	
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		
		СтрокаДокумента = ДокументыКИмпорту[Выборка.НомерСтроки2 - 1];
		...
		Если ЗначениеЗаполнено(Выборка.ДокументСсылка) Тогда
			...
			СтрокаДокумента.НомерДок = Выборка.Номер;
			СтрокаДокумента.ДатаДок  = Выборка.Дата;
			
			СтрокаДокумента.ВидОперации 	= Выборка.ВидОперации; 
			СтрокаДокумента.СтатьяДДС 		= Выборка.СтатьяДвиженияДенежныхСредств; 
			СтрокаДокумента.СчетКонтрагента = Выборка.БанковскийСчетСсылка;
			СтрокаДокумента.СчетОрганизации = БанковскийСчет;
			
			Если Выборка.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПереводНаДругойСчет Тогда
				СтрокаДокумента.Контрагент = Выборка.Организация;
			Иначе
				СтрокаДокумента.Контрагент = Выборка.КонтрагентСсылка;
			КонецЕсли;
			
			СтрокаДокумента.Договор				= Выборка.ДоговорКонтрагента;
			СтрокаДокумента.ДокументОснование 	= Выборка.ДокументОснование;
			СтрокаДокумента.СБ_ПрофильРазноски 	= Выборка.СБ_ПрофильРазноски;
						
			Если Выборка.КоличествоДокументов > 1 Тогда
				ДобавитьЗамечание(СтрокаДокумента, 1, "В информационной базе найдено несколько(" + Выборка.КоличествоДокументов + ") соответствующих документов!");
			КонецЕсли; 		
		...
		Иначе
			...
			СчетКонтрагента = Выборка.КонтрагентНомерСчета;
			
			Если НЕ ЗначениеЗаполнено(СтрокаДокумента.СчетКонтрагента) Тогда
				
				СтрокаДокумента.СчетКонтрагента = "  Не найден ("+ СчетКонтрагента +")";
				Если Выборка.ЭтоСписание Тогда
					ДобавитьЗамечание(СтрокаДокумента, 2, "Не найден счет контрагента ("+ СчетКонтрагента +").");
				КонецЕсли;
				
				СчетКонтрагентаНайден = Ложь;
				
			Иначе
				СчетКонтрагентаНайден = Истина;
			КонецЕсли; 
			
			Если НЕ СчетКонтрагентаНайден И Выборка.КоличествоКонтрагентов > 1 Тогда
				ДобавитьЗамечание(СтрокаДокумента, 2, "В информационной базе найдено несколько("+Выборка.КоличествоКонтрагентов+") контрагентов с одинаковым ИНН!");
			КонецЕсли; 
