...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПравоДоступаОбменЭДО = ПравоДоступа("Использование", Метаданные.Обработки.ОбменСКонтрагентами);
...
Процедура ОткрытьВходящиеДокументыЭДО(Команда)
	...
	ОткрытьФорму("Обработка.ОбменСКонтрагентами.Форма.ТекущиеДелаПоЭДО", ПараметрыОткрытия, ЭтотОбъект);
...
Процедура ОбновитьСпискиДокументовЭДО()
	...
	Если Не ПравоДоступа("Использование", Метаданные.Обработки.ОбменСКонтрагентами) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьДокумент(Таблица, ОбъектМетаданных, Заголовок, Оформите, Отработайте, Ожидайте)
	...
	Если ОбъектМетаданных = Метаданные.Документы.ПеремаркировкаТоваровИСМП
		И Не ПолучитьФункциональнуюОпцию("ВестиУчетОбувнойПродукцииИСМП") Тогда
		ОпцияВключена = Ложь;
...
//
// Параметры:
//   Организации - фильтр по организациям
