...
Перем КоличествоДокЗагруженных, ВремяНачалаЗагрузки, ВремяОкончанияЗагрузки; // (Извещение на e-mail о рез. выполнения регл. задания)
...
Процедура Загрузка(ЗагрузитьГруппу=Истина, ДатаНачала=Неопределено, ДатаОкончания=Неопределено, ТипыОпераций=Неопределено, Порция = 0, УсловиеRecNoВаучеры="", ПроверятьЗакрытыйПериод = Ложь) Экспорт
	...
	Если ЗагрузитьГруппу Тогда
		...
		Попытка
    		...
    		СформироватьИПередатьОтчет(ТаблицаОшибок, СтруктураНастройки, ДатаНачалаЗагрузки, ДатаОкончанияЗагрузки);
		Исключение
			ЗаписьЖурналаРегистрации("Неудачный запуск отправки отчета о работе обработки загрузки из аксапты (Лоджистик)" 
			,УровеньЖурналаРегистрации.Ошибка
			, 
			,
			,ОписаниеОшибки());
...
Функция СформироватьИПередатьОтчет(ТаблицаОшибок, СтруктураНастройки, ДатаНачалаЗагрузки, ДатаОкончанияЗагрузки)
	...
	времСообщение_Заголовок = "Информация по работе обработки по загрузке данных из Аксапты (Лоджистик).";
	...
	времСообщение_Подпись   = "Информация о работе обработки ""Загрузка данных из Аксапты Лоджистик"". Пользователь: "+ПользователиИнформационнойБазы.ТекущийПользователь();
...
Процедура ЗагрузитьГруппу(RecNo) Экспорт
	...
	Отбор = Новый Структура();
	Отбор.Вставить("ТипОперацииАксапты",Справочники.бит_ок_типыОперацийАксапты.НайтиПоКоду(360));
	ТЗСтроки = ТЗРезультат.НайтиСтроки(Отбор);
	Для каждого СтрокаУдаления Из ТЗСтроки Цикл
		ТЗРезультат.Удалить(СтрокаУдаления);
	КонецЦикла;	
	Отбор = Новый Структура();
	Отбор.Вставить("ТипОперацииАксапты",Справочники.бит_ок_типыОперацийАксапты.НайтиПоКоду(420));
	
	ТЗСтроки = ТЗРезультат.НайтиСтроки(Отбор);
	Для каждого СтрокаУдаления Из ТЗСтроки Цикл
		ТЗРезультат.Удалить(СтрокаУдаления);
	КонецЦикла;
	времТипыОперацийАксапты510 =  Справочники.бит_ок_типыОперацийАксапты.НайтиПоКоду(510);	
    Стр = ТЗРезультат.Добавить();
    Стр.ТипОперацииАксапты = времТипыОперацийАксапты510;
	ТЗРезультат.Сортировать("ТипОперацииАксапты Возр");
	// Ок Видяйкин В.И. 20170808 -
...
мТаймаут 				= 0;
Организация 			= ?(ЗначениеЗаполнено(Организация), Организация, Справочники.Организации.НайтиПоКоду("000000056"));  //***БИТ***Теплова***
ЗаполнитьНастройки();
