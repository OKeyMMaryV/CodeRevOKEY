...
Процедура ОбработатьРезультатВыполненияФоновогоЗаданияЗаполненияДоходовВычетов(АдресРезультата)
	...
	Если Результат.Свойство("СтандартныйВычет") Тогда
		
		Для Каждого ТекущийВычет Из Результат.СтандартныйВычет Цикл
			ЗаполнитьЗначенияСвойств(Объект.ДоходыВычеты.Добавить(), ТекущийВычет);
		КонецЦикла;
