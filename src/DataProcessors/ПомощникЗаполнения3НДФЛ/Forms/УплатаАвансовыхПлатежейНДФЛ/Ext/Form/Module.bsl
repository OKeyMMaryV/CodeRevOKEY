...
&НаСервереБезКонтекста
Функция ОписаниеДействияРасшифровкаРасчетовПрошлыхЛет(Организация, Период, АдресПоказателей, ВключатьНачисления)
	
	НачалоПериода = НачалоГода(Период);
	КонецПериода  = КонецГода(Период);
	
	ИмяФормы = "Отчет.РасшифровкаПлатежейВБюджет.Форма.ФормаОтчета";
	ПараметрыФормы = ПараметрыФормыРасшифровкиПлатежейВБюджет(Организация, НачалоПериода, КонецПериода, ВключатьНачисления);
...
Процедура ОткрытьРасшифровкуРасчетовПрошлыхЛет(ВключатьНачисления = Ложь)
	...
	ОписаниеДействия = ОписаниеДействияРасшифровкаРасчетовПрошлыхЛет(
		Объект.Организация, ДатаПрошлогоГода, АдресВоВременномХранилище, ВключатьНачисления);
...
Процедура ПередНачаломДлительнойОперации()
	...
	ДанныеУчетаАктуальны = Показатели.ДеятельностьОтсутствует;
	ВозможнаБыстраяАктуализация = Показатели.МожноАктуализироватьДанные;
...
Процедура УправлениеФормой(Форма)
	...
	Иначе
		...
		Элементы.АктуализироватьДанныеУчета.Видимость  = Форма.ТребуетсяПроверятьАктуальность
			Или Не Форма.ДанныеУчетаАктуальны И Не Форма.ВозможнаБыстраяАктуализация И ЕстьДанныеДляРасчета;
