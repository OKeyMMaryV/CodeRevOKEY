...
Процедура ЗаполнитьДоходыРасходыОтПредпринимательскойДеятельностиВФоне(ПараметрыЗаполнения, АдресРезультата) Экспорт
	...
	Иначе
		РезультатВыполнения.Вставить("АвансовыеПлатежи",
			УчетНДФЛПредпринимателя.УплаченныйНалогЗаПериод(Организация, НачалоПериода, НачалоКвартала(КонецПериода) - 1));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтандартныеВычеты) Тогда
		РезультатВыполнения.Вставить("СтандартныйВычет", СтандартныеВычеты);
	КонецЕсли;
...
Функция ПоясненияКРасчету(Показатели) Экспорт
	...
	СтандартныйВычетНаДетей       = Формат(Показатели.СтандартныйВычетНаДетей, ФорматЧиселИтоги);
	СтандартныйВычетНаНалогоплательщика = Формат(Показатели.СтандартныйВычетНаНалогоплательщика, ФорматЧиселИтоги);
	
	СтандартныйВычет = Показатели.СтандартныйВычетНаДетей + Показатели.СтандартныйВычетНаНалогоплательщика;
	Показатели.НалоговаяБаза = Макс(0, Показатели.Доходы - Показатели.ПрофессиональныйВычет - СтандартныйВычет);
	Показатели.НалогИсчисленныйВсего = Показатели.НалоговаяБаза * СтавкаНалога / 100;
	...
	ДополнительныйТекст = СтрШаблон("%1 - %2 - %3", Доходы, ПрофессиональныйВычет, СтандартныйВычет);
	...
	// Этап №4 - окончательный расчет налога к уплате
	Если НомерКвартала > 1 Тогда
		...
		Если НомерКвартала = 2 И ЕстьПереплата Тогда
			
			ТекстАванс = СтрШаблон(НСтр("ru = 'Налог за %1 - Авансовый платеж за %2 - Переплата'"),
				Показатели.ПредставлениеПериода, ПредставлениеПериодаАванса);
			ДополнительныйТекст = СтрШаблон("%1 - %2 - %3",
				Формат(Показатели.НалогИсчисленныйВсего, ФорматЧиселВПояснении), АвансовыеПлатежи, Показатели.Переплата);
			
		ИначеЕсли НомерКвартала = 2 Тогда
			
			ТекстАванс = СтрШаблон(НСтр("ru = 'Налог за %1 - Авансовый платеж за %2'"),
				Показатели.ПредставлениеПериода, ПредставлениеПериодаАванса);
			ДополнительныйТекст = СтрШаблон("%1 - %2",
				Формат(Показатели.НалогИсчисленныйВсего, ФорматЧиселВПояснении), Показатели.АвансовыеПлатежи);
			
		ИначеЕсли ЕстьПереплата Тогда
			
			ТекстАванс = СтрШаблон(НСтр("ru = 'Налог за %1 - Авансовые платежи за %2 - Переплата'"),
				Показатели.ПредставлениеПериода, ПредставлениеПериодаАванса);
			ДополнительныйТекст = СтрШаблон("%1 - %2 - %3",
				Формат(Показатели.НалогИсчисленныйВсего, ФорматЧиселВПояснении), АвансовыеПлатежи, Показатели.Переплата);
		...
		Иначе
			...
			ТекстАванс = СтрШаблон(НСтр("ru = 'Налог за %1 - Авансовые платежи за %2'"),
				Показатели.ПредставлениеПериода, ПредставлениеПериодаАванса);
			ДополнительныйТекст = СтрШаблон("%1 - %2",
				Формат(Показатели.НалогИсчисленныйВсего, ФорматЧиселВПояснении), Показатели.АвансовыеПлатежи);
		...
		СодержимоеСтроки.Добавить(ТекстАванс);
...
Процедура ЗаполнитьНачальныеПоказатели(Показатели)
	...
	Если ЗначениеЗаполнено(ГраницаОтчетности) И Не Показатели.НалоговыйПериодПропущен Тогда
		
		Показатели.ЭтоПериодПрошлыхЛет = Показатели.Период <= ГраницаОтчетности
	...
	Показатели.ДеятельностьОтсутствует = Показатели.Доходы = 0 И Показатели.АвансовыеПлатежи = 0
	...
	ОстатокРасчетовЗаПредыдущиеПериоды = УчетНДФЛПредпринимателя.ОстатокРасчетовПоНалогуЗаПредыдущиеПериоды(
		Организация, Показатели.НачалоПериода);
