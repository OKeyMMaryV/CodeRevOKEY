	...
	Для каждого ЭлементНоменклатуры Из СписокНоменклатуры Цикл
		...
		Для каждого ПолеМакета Из СписокПолейМакета Цикл
			...
			Если ПолеМакета.Тип = "Barcode" Тогда
				...
				Если НЕ ЗначениеЗаполнено(ЗначениеРеквизита) И ПолеМакета.Значение = "Штрихкод" Тогда
				...
				ИначеЕсли ПолеМакета.Значение = "ШтрихкодМаркировки" И СоответствиеТиповКодовМаркировки[ПолеМакета.ТипШтрихкода] <> Неопределено Тогда
					
					КодGS1ДляКомпонентыПечати = 
						ШтрихкодированиеИС.КодGS1ДляКомпонентыПечати(ЗначениеРеквизита, СоответствиеТиповКодовМаркировки[ПолеМакета.ТипШтрихкода]);
						
					ЗначенияПолей.Вставить("Значение", КодGS1ДляКомпонентыПечати);
				Иначе
