...
Процедура СформироватьСервер( )
	...
	//	// ++ BIT AMerkulov 03042014 // Доработан запрос по датам проводок МСФО
	//	//ОК+ Аверьянова 12092014  //доработан запрос для сравнения с началом открытого периода
	//	Запрос.Текст =
	//	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	//	|	И &Условие3
	//	|	И ВЫБОР
	//	|			КОГДА &ВидОперацииРО = НЕОПРЕДЕЛЕНО
	//	|				ТОГДА ИСТИНА
	//	|			ИНАЧЕ бит_БК_РазнесениеОборотов.Ссылка.ВидОперации = &ВидОперацииРО
	//	|		КОНЕЦ
	//	|
	//	|СГРУППИРОВАТЬ ПО
	...
	//	|	МИНИМУМ(бит_Дополнительный_2Обороты.Период) КАК МинимальныйПериодМСФО,
	//	|	МИНИМУМ(рс_ДокументыПоID.Регистратор) КАК РегистраторДокПоID
	//	|ПОМЕСТИТЬ ВТ_Итог
	//	|ИЗ
	...
	//	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.бит_Дополнительный_2.Обороты(, , Регистратор, , , , , ) КАК бит_Дополнительный_2Обороты
	//	|			ПО рс_ДокументыПоID.Документ = бит_Дополнительный_2Обороты.Регистратор
	...
	//	|	РегистрСведений.бит_НачалоОткрытогоПериода КАК бит_НачалоОткрытогоПериода
	//	|ГДЕ
	//	|	рс_АккруалПоID.Ссылка ЕСТЬ NULL 
	//	|	И бит_НачалоОткрытогоПериода.Организация = &Организация
	//	|
	//	|СГРУППИРОВАТЬ ПО
	...
	//	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	//	|	ВТ_Итог.ID,
	//	|	ВТ_Итог.ВидДокумента,
	//	|	ВТ_Итог.НомерДокумента,
	//	|	ВТ_Итог.ДатаДокумента,
	//	|	ВТ_Итог.ДокументОснование,
	//	|	ВТ_Итог.АккруалПоID,
	//	|	ВТ_Итог.Контрагент,
	//	|	ВТ_Итог.Сумма,
	//	|	ВТ_Итог.АккруалСсылка,
	//	|	ВТ_Итог.СтатьяОборотов,
	//	|	ВТ_Итог.Период, ВТ_Итог.РегистраторДокПоID
	//	|ИЗ
	//	|	ВТ_Итог КАК ВТ_Итог
	//	  | ГДЕ
	//	|	(ВТ_Итог.РегистраторДокПоID ЕСТЬ NULL 
	//	|			ИЛИ ВТ_Итог.МинимальныйПериодМСФО > &ДатаКон)
	//	
	//	|
	//	|СГРУППИРОВАТЬ ПО
	//	|	ВТ_Итог.ID,
	...
	//	|	ВТ_Итог.СтатьяОборотов,
	//	|	ВТ_Итог.Период,
	//	|	ВТ_Итог.АккруалСсылка,ВТ_Итог.РегистраторДокПоID,
	//	|	ВТ_Итог.Сумма";
	//// --	
	//	
	//ИначеЕсли Объект.ВидОперации = Перечисления.рс_ВидыОперацийФормированиеАккруаловПоID.ЗаявкаНаДоговор Тогда
		...
		|	МИНИМУМ(бит_Дополнительный_2Обороты.Период) КАК ДатаПроводкиМСФО,
		|	бит_Дополнительный_2Обороты.Регистратор КАК РегистраторМСФО,		
		|	бит_Дополнительный_2Обороты.Регистратор.Дата КАК ДатаДокументаРегистратора
		|ПОМЕСТИТЬ ВТ_Итог
		|ИЗ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.рс_ДокументыПоID КАК рс_ДокументыПоID
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.бит_Дополнительный_2.Обороты(, , Регистратор, , , , , ) КАК бит_Дополнительный_2Обороты
		|			ПО (бит_Дополнительный_2Обороты.Регистратор = рс_ДокументыПоID.Документ)
		|		ПО рс_ЗаявкаНаДоговорБаза.ID = рс_ДокументыПоID.ID,
		|	РегистрСведений.бит_НачалоОткрытогоПериода КАК бит_НачалоОткрытогоПериода
		|ГДЕ
		|	рс_АккруалПоID.Ссылка ЕСТЬ NULL 
		|	И (рс_ДокументыПоID.Регистратор ЕСТЬ NULL 
		|			ИЛИ бит_Дополнительный_2Обороты.Период >= бит_Дополнительный_2Обороты.Регистратор.Дата)
		|	И бит_НачалоОткрытогоПериода.Организация = &Организация
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	бит_Дополнительный_2Обороты.Регистратор.Дата
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	ВТ_Итог.Сумма,
		|	ВТ_Итог.АккруалСсылка,
		|	ВТ_Итог.ДокументПоID,
		|	ВТ_Итог.Период,
		|	ВТ_Итог.ДатаПроводкиМСФО
		|ИЗ
		|	ВТ_Итог КАК ВТ_Итог
		|ГДЕ
		|	(ВТ_Итог.ДокументПоID ЕСТЬ NULL 
		|			ИЛИ ВТ_Итог.ДатаПроводкиМСФО > &ДатаКон)
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_Итог.Период,
		|	ВТ_Итог.ДатаПроводкиМСФО,
	...
	СтруктураКурсаРегл				=	РаботаСКурсамиВалют.ПолучитьКурсВалюты(ВалютаРегламентированногоУчета, Объект.ДатаКон);
	...
	Для Каждого СтрокаЗапрос Из мОбъект.База Цикл
		...
		//ОКЕЙ Первухин В.С. (ПервыйБИТ) Начало 2020-01-18 (#НТП_БП03_ФР04)
		СтрокаЗапрос.ок_ВалютаДокумента			=	СтрокаЗапрос.ДокументОснование.ДоговорКонтрагента.ВалютаВзаиморасчетов;
		
		СтруктураКурса							=	РаботаСКурсамиВалют.ПолучитьКурсВалюты(СтрокаЗапрос.ок_ВалютаДокумента, Объект.ДатаКон);
		
		СтрокаЗапрос.ок_КурсВзаиморасчетов		=	СтруктураКурса.Курс;
		СтрокаЗапрос.ок_КратностьВзаиморасчетов	=	СтруктураКурса.Кратность;
		
		СтрокаЗапрос.ок_СуммаРегл				=	РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(
														СтрокаЗапрос.Сумма, СтрокаЗапрос.ок_ВалютаДокумента, ВалютаРегламентированногоУчета, СтрокаЗапрос.ок_КурсВзаиморасчетов,
														СтруктураКурсаРегл.Курс, СтрокаЗапрос.ок_КратностьВзаиморасчетов, СтруктураКурсаРегл.Кратность);
		//ОКЕЙ Первухин В.С. (ПервыйБИТ) Конец 2020-01-18 (#НТП_БП03_ФР04)
...
//ОКЕЙ Первухин В.С. (ПервыйБИТ) Начало 2020-03-10 (#НТП_БП03_ФР04)
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	//ОКЕЙ Первухин В.С. (ПервыйБИТ) Начало 2020-03-10 (#НТП_БП03_ФР04)
	Элементы.СуммаИтог.Видимость	=	Ложь;
	//ОКЕЙ Первухин В.С. (ПервыйБИТ) Конец 2020-03-10 (#НТП_БП03_ФР04)
КонецПроцедуры
