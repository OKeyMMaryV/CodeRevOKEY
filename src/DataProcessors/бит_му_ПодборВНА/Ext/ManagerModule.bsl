...
Функция ПолучитьТекстЗапроса(Режим, СтруктураПараметров = Неопределено) Экспорт
	...
	ИначеЕсли Режим = Перечисления.бит_му_РежимыПодбораВНА.ок_ПринятиеКУчетуОборудования Тогда
		
		ТекстЗапроса = бит_му_Общий.СформироватьТекстЗапросаПринятиеКУчетуОСПодборОборудования();
		
	ИначеЕсли Режим = Перечисления.бит_му_РежимыПодбораВНА.ок_МодернизацияОборудования Тогда
		
		ТекстЗапроса = бит_му_Общий.СформироватьТекстЗапросаМодернизацияОСПодборОборудования();
...
Функция ПолучитьПереченьОбъектов(АдресСКД, Компоновщик, СтруктураПараметров) Экспорт
	...
	// ОКЕЙ Конаков Ю.М. (СофтЛаб) Начало 2021-10-21 (#4297) 
	ЭтоМодернизацияОборудования	   = Режим = Перечисления.бит_му_РежимыПодбораВНА.ок_МодернизацияОборудования;
	// ОКЕЙ Конаков Ю.М. (СофтЛаб) Конец 2021-10-21 (#4297) 
	...
	Если ЭтоПринятиеКУчетуОС ИЛИ ЭтоПринятиеКУчетуОСПоОстаткам ИЛИ ЭтоВводНачальныхОстатковОС
		// ОКЕЙ Конаков Ю.М. (СофтЛаб) Начало 2021-10-21 (#4297)
		Или ЭтоМодернизацияОборудования
		// ОКЕЙ Конаков Ю.М. (СофтЛаб) Конец 2021-10-21 (#4297) 
		...
		// ОКЕЙ Конаков Ю.М. (СофтЛаб) Начало 2021-10-21 (#4338) 
		...
		Если ЭтоПринятиеКУчетуОС 
			Или ЭтоПринятиеКУчетуОСПоОстаткам
			Или ЭтоВводНачальныхОстатковОС
			Или ЭтоМодернизацияОС
			Или ЭтоМодернизацияОборудования Тогда	
		// ОКЕЙ Конаков Ю.М. (СофтЛаб) Конец 2021-10-21 (#4338) 
			...
			// ОКЕЙ Конаков Ю.М. (СофтЛаб) Начало 2021-10-21 (#4297) 
			ИначеЕсли ЭтоМодернизацияОборудования Тогда
				
				МассивСубконтоКт = Новый Массив();
				
				МассивСубконтоКт.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОбъектыСтроительства);
				МассивСубконтоКт.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат);
...
Функция СформироватьТекстЗапросаПеремещениеОС()
	...
	Иначе	
		...
		// При запросе из документа перемещения ОС нужно подтягивать не старые данные местоположения, а новые. 
		//ИмяРеквизитаПодразделение = "ПодразделениеОрганизации";
		//ИмяРеквизитаМОЛ = "МОЛОрганизации";
		ИмяРеквизитаПодразделение	= "ПодразделениеПолучатель";
		ИмяРеквизитаМОЛ				= "МОЛПолучатель";
	...
	ТекстЗапроса = 
		...
		///БИТ Тртилек
		// ОКЕЙ Конаков Ю.М. (СофтЛаб) Начало 2021-10-20 (#4297) 
		|	ТабЧасть.ОсновноеСредство.БИТ_ГруппаОСУУ.ок_АвтоматическийПараллельныйУчет КАК АвтоматическийПараллельныйУчет,
		// ОКЕЙ Конаков Ю.М. (СофтЛаб) Конец 2021-10-20 (#4297) 
...
Функция СформироватьТекстЗапросаВыбытиеОС(СтруктураПараметров)
	...
	|	СписаниеОСОС.Ссылка.Организация.*,
	// ОКЕЙ Конаков Ю.М. (СофтЛаб) Начало 2021-10-20 (#4297) 
	|	СписаниеОСОС.ОсновноеСредство.БИТ_ГруппаОСУУ.ок_АвтоматическийПараллельныйУчет КАК АвтоматическийПараллельныйУчет,
	// ОКЕЙ Конаков Ю.М. (СофтЛаб) Конец 2021-10-20 (#4297) 
	...
	|	ПередачаОСОС.Ссылка.Организация.*,
	// ОКЕЙ Конаков Ю.М. (СофтЛаб) Начало 2021-10-20 (#4297) 
	|	ПередачаОСОС.ОсновноеСредство.БИТ_ГруппаОСУУ.ок_АвтоматическийПараллельныйУчет КАК АвтоматическийПараллельныйУчет,
	// ОКЕЙ Конаков Ю.М. (СофтЛаб) Конец 2021-10-20 (#4297) 
	...
	Если НЕ ПодготовкаКПередачеОСОбработан Тогда
		...
		|	ПодготовкаКПередачеОСОС.Ссылка.Организация.*,
		// ОКЕЙ Конаков Ю.М. (СофтЛаб) Начало 2021-10-20 (#4297) 
		|	ПодготовкаКПередачеОСОС.ОсновноеСредство.БИТ_ГруппаОСУУ.ок_АвтоматическийПараллельныйУчет КАК АвтоматическийПараллельныйУчет,
		// ОКЕЙ Конаков Ю.М. (СофтЛаб) Конец 2021-10-20 (#4297)
	...
	// ОКЕЙ Конаков Ю.М. (СофтЛаб) Начало 2021-10-20 (#4297) 
	|	ВЫРАЗИТЬ(бит_Дополнительный_2Остатки.Субконто1 КАК Справочник.ОсновныеСредства).БИТ_ГруппаОСУУ.ок_АвтоматическийПараллельныйУчет КАК АвтоматическийПараллельныйУчет,
	// ОКЕЙ Конаков Ю.М. (СофтЛаб) Конец 2021-10-20 (#4297) 
	|	ПараметрыОС_МУ.МОЛ.*,
	|	ПараметрыОС_МУ.Местонахождение.*,
	|	ПараметрыОС_МУ.КлассОС.*,
	...
	|	ВыбытиеОСОС_МУ.Ссылка.Организация.*,
	// ОКЕЙ Конаков Ю.М. (СофтЛаб) Начало 2021-10-20 (#4297) 
	|	ВыбытиеОСОС_МУ.ОсновноеСредство.БИТ_ГруппаОСУУ.ок_АвтоматическийПараллельныйУчет КАК АвтоматическийПараллельныйУчет,
	// ОКЕЙ Конаков Ю.М. (СофтЛаб) Конец 2021-10-20 (#4297) 
