...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Начало 2021-07-09 (#ТП_БП05_ФР07)
	Если ТипВладельца = Тип("ДокументСсылка.бит_ЗаявкаНаРасходованиеСредств") Тогда
		Если ВладелецФайла.ВидОперации = Перечисления.бит_ВидыОперацийЗаявкаНаРасходование.ПеречислениеДенежныхСредствПодотчетнику 
				И Не Пользователи.РолиДоступны("ок_ПросмотрПрисоединенныеФайлыЗнРДС") Тогда
			ВызватьИсключение НСтр("ru = 'Нет прав на просмотр файлов для ЗнРДС с видом операции
										 |""Перечисление подотчетному лицу""'");
		КонецЕсли; 			
	КонецЕсли;
	//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Конец 2021-07-09 (#ТП_БП05_ФР07)
