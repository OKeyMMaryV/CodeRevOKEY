...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Элементы.ИнформацияОбновлениеЗавершено.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ЗаголовокОбновлениеЗавершено, бит_ОбщегоНазначения.МетаданныеВерсия());
	КонецЕсли;
	...
	Элементы.ЗаголовокИнформации.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Выполняются дополнительные процедуры обработки данных на версию %1
			|Работа с этими данными временно ограничена'"), бит_ОбщегоНазначения.МетаданныеВерсия());
...
Процедура СкрытьЛишниеГруппыНаФорме(ОткрытиеИзПанелиАдминистрирования, Сведения)
	...
	Если ЭтоПолноправныйПользователь
		...
		И бит_ОбщегоНазначения.МетаданныеВерсия() = Сведения.ВерсияУдалениеПатчей Тогда
