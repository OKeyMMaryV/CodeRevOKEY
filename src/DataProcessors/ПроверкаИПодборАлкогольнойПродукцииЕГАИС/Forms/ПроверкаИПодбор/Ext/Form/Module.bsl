...
Процедура УправлениеДоступностьюКонтекстныхКоманд()
	...
	Если Элементы.ДеревоМаркированнойПродукцииПринятьКакПартионную.Видимость Тогда
		Элементы.ДеревоМаркированнойПродукцииПринятьКакПартионную.Доступность = ДоступностьКомандУпаковки И СимволовАкцизнойМарки = 68;
	КонецЕсли;
...
Функция РезультатыПроверкиУспешноСохранены()
	...
	Исключение
		...
		ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки(),, "ПроверяемыйДокумент");
		Возврат Ложь;
...
//	ИначеЕсли ЗагрузкаДанныхТСД.ТребуетсяУточнениеДанных Тогда
//		
//		ДанныеШтрихкода = ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукцияДопУказание[0].ДанныеШтрихкода;
//		ДополнительныеПараметры = Новый Структура("ДанныеШтрихкода", ДанныеШтрихкода);
//		
//		ОткрытьФорму(
//			"Обработка.РаботаСАкцизнымиМаркамиЕГАИС.Форма.ФормаВводаАкцизнойМаркиПоискНоменклатуры",
//			ЗагрузкаДанныхТСД.ПараметрыУточненияДанных,
//			ЭтотОбъект,,,,
	...
	ИначеЕсли СледующаяОперация = "Упаковка" Тогда
		...
		Если ЗагрузкаДанныхТСД.Упаковки.Количество() <> ЗагрузкаДанныхТСД.ИндексОбрабатываемогоЭлемента Тогда
			...
			Если ЗагрузкаДанныхТСД.ЕстьИерархия Тогда
				...
				ЗагрузкаДанныхТСД.Упаковки[ЗагрузкаДанныхТСД.ИндексОбрабатываемогоЭлемента - 1].ДанныеШтрихКода.Штрихкод = 
				ЗагрузкаДанныхТСД.Упаковки[ЗагрузкаДанныхТСД.ИндексОбрабатываемогоЭлемента - 1].ДанныеШтрихКода.Штрихкод.Штрихкод;
			КонецЕсли;
