...
Функция ПроверитьДоступнуюИерархиюШтрихкодов(Штрихкоды)
	...
	Если Упаковок = 0 И Блоков = 1 И Пачек > 0 Тогда
		...
		ПоказатьВопрос(ОповещениеОЗавершении,
			НСтр("ru = 'Отсканирован один код блока и коды маркировки. Загрузить блок с вложенным составом?'"), РежимДиалогаВопрос.ДаНет);
		Возврат Истина;
	ИначеЕсли Упаковок = 1 И Блоков > 0 И Пачек = 0 Тогда
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ПредложитьЗагрузитьВУпаковке", ЭтотОбъект, Штрихкоды);
		ПоказатьВопрос(ОповещениеОЗавершении,
			НСтр("ru = 'Отсканирована одна упаковка и коды табачных блоков. Загрузить упаковку с вложенным составом?'"), РежимДиалогаВопрос.ДаНет);
		Возврат Истина;
	ИначеЕсли Упаковок = 1 И Блоков = 0 И Пачек > 0 Тогда
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ПредложитьЗагрузитьВУпаковке", ЭтотОбъект, Штрихкоды);
		ПоказатьВопрос(ОповещениеОЗавершении,
