...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АдресСхемыКомпоновкиДанных = Обработки.бит_мто_ПодборЗаявокНаПотребность.ИнициализироватьКомпоновщик(Объект.Компоновщик, УникальныйИдентификатор, "СКД");
...
Процедура ЗаполнитьКэшЗначений()
	...
	фКэшЗначений.Вставить("НастраиваемыйОбъект", "Обработка.бит_мто_ПодборЗаявокНаПотребность");
...
Процедура ПолучитьРезультатЗапроса()
	...
	Результат = Обработки.бит_мто_ПодборЗаявокНаПотребность.ВыполнитьЗапросПоНастройке(Объект.Компоновщик, АдресСхемыКомпоновкиДанных, ЭлементыУпорядочивания);
...
Процедура УстановитьЗаголовокФормы()
	...
	Если ЗначениеЗаполнено(ТекущаяНастройка) Тогда
		...
		ЭтаФорма.Заголовок = "Подбор заявок на потребность ("+ТекущаяНастройка+")";
		
		
	Иначе	
		
		ЭтаФорма.Заголовок = "Подбор заявок на потребность";
...
Процедура ВидимостьНазначенныхАналитик()
	...
	Для каждого Реквизит Из Метаданные.Обработки.бит_мто_ПодборЗаявокНаПотребность.ТабличныеЧасти.Товары.Реквизиты Цикл
...
// Процедура переносит заявки на потребность.
// 
// Параметры:
//  ИмяКоманды - Строка.
// 
...
Процедура ЗаполнитьПользовательскиеНастройкиМакета()
	...
	Для каждого Настр Из ПользоватНастройки.Отбор.Элементы Цикл
		...
		Если ИмяНастройки = "СтатьяОборотов" 
			И ЗначениеЗаполнено(фСтруктураПараметров.Документ["СтатьяОборотов"]) Тогда
		
			Настр.Использование = Истина;
			Настр.ПравоеЗначение = фСтруктураПараметров.Документ["СтатьяОборотов"];
