...
Процедура Заполнить(Команда)
	...
	Если ДатаПервогоПлатежа > ДатаОкончания Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru='Дата первого платежа не должна быть позже %1'"),
			Формат(ДатаОкончания, "ДЛФ=D"));
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "ДатаПервогоПлатежа", , Отказ);
	КонецЕсли;
	Если ДатаПоследнегоПлатежа > ДатаОкончания Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru='Дата последнего платежа не должна быть позже %1'"),
			Формат(ДатаОкончания, "ДЛФ=D"));
