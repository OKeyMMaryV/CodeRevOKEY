...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоРассрочка Тогда
		Элементы.ГрафикПлатежейЗаполнить.Видимость = Ложь;
		Элементы.ЗаголовокГрафикПлатежей.Видимость = Ложь;
		Элементы.ГрафикПлатежей.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСверху;
	Иначе
		Элементы.ГрафикПлатежейЗаполнить.Видимость = Истина;
		Элементы.ЗаголовокГрафикПлатежей.Видимость = Истина;
		Элементы.ГрафикПлатежей.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
		Элементы.ЗаголовокГрафикПлатежей.Заголовок = СтрШаблон(НСтр("ru='Платежи за период по %1'"), 
...
&НаКлиенте
Процедура ГрафикПлатежейДатаПлатежаПриИзменении(Элемент)
	...
	Если ТекущиеДанные.ДатаПлатежа <= ДатаОкончания Тогда
		Возврат;
	КонецЕсли;
	
	ТекущиеДанные.ДатаПлатежа = ДатаОкончания;
	ТекстПредупреждения = СтрШаблон(НСтр("ru = 'Дата платежа не должна быть позже %1'"),
		Формат(ДатаОкончания, "ДЛФ=D"));
