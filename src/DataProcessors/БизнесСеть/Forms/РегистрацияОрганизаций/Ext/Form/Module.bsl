...
Процедура ЗарегистрироватьОрганизации(Команда)
	...
	Иначе
		ЗарегистрироватьОрганизацииПродолжение();
	КонецЕсли;
...
Процедура ОтобразитьСостояниеПодключенияИПП(Форма, Логин)
	...
	КонецЕсли;
	Форма.Элементы.ДекорацияЛогинИПП.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(ШаблонЛогина);
	Форма.Элементы.ДекорацияЛогинИПП.РасширеннаяПодсказка.Заголовок	= СтроковыеФункцииКлиент.ФорматированнаяСтрока(ШаблонПодсказки);

КонецПроцедуры

&НаКлиенте
Процедура ЗарегистрироватьОрганизацииПродолжение(Знач ПараметрыАутентификации = Неопределено,
