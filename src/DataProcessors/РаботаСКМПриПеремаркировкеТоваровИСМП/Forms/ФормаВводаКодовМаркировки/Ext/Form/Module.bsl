...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.РезультатВзятьИзПула Тогда
		Если Не ЗначениеЗаполнено(Объект.НоваяНоменклатура) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "НовоеПредставление", , Отказ);
		КонецЕсли;
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	ОповещениеПриЗавершении = Новый ОписаниеОповещения(
		"ПоискПоШтрихкодуЗавершение", ЭтотОбъект, Элементы.СтраницыДанных.ТекущаяСтраница);
...
Процедура РаспечататьНовыйКодЗавершение(ДанныеОтветаРезервированияИПечати, ДополнительныеПараметры) Экспорт
	...
	Если ДанныеОтветаРезервированияИПечати.РезультатРезервирования.Количество() Тогда
		...
		Объект.НовыйКодМаркировки = СтрокаРезультата.ШтрихкодУпаковки;
		Отказ = Ложь;
		Объект.РезультатВзятьИзПула = Ложь;
		ЗавершитьВвод(Отказ, Истина);
...
Процедура ШагДалееПроверитьИПереключитьСтраницу(НоваяСтраница)
	...
	Если СтрукутраВозможныхПроверок.КодМаркировки И Не ЗначениеЗаполнено(Объект.КодМаркировки) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,, "Объект.КодМаркировки", Отказ);
	КонецЕсли;
	...
	Если СтрукутраВозможныхПроверок.ПричинаПеремаркировки И Не ЗначениеЗаполнено(Объект.ПричинаПеремаркировки) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,, "Объект.ПричинаПеремаркировки", Отказ);
	КонецЕсли;
	...
	Если СтрукутраВозможныхПроверок.НовыйКодМаркировки И Не ЗначениеЗаполнено(Объект.НовыйКодМаркировки) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,, "Объект.НовыйКодМаркировки", Отказ);
	КонецЕсли;
	...
	Если Не Отказ Тогда
		...
		Если Элементы.СтраницыДанных.ТекущаяСтраница = НоваяСтраница Тогда
			ЗавершитьВвод();
		Иначе
			Элементы.СтраницыДанных.ТекущаяСтраница = НоваяСтраница;
			УстановитьДоступностьЭлементовФормы();
		КонецЕсли;
...
Процедура ОпределитьТекущийШаг(Форма)
	...
	Если Не ЗначениеЗаполнено(Форма.Объект.КодМаркировки) Тогда
		Форма.Элементы.СтраницыДанных.ТекущаяСтраница = Форма.Элементы.СтраницаКодМаркировки;
	Иначе
		Форма.Элементы.СтраницыДанных.ТекущаяСтраница = Форма.Элементы.СтраницаНовыйКодМаркировки;
	КонецЕсли;
...
Процедура РучнойВводШтрихкодаЗавершение(ДанныеШтрихкода, ТекущаяСтраница) Экспорт
	...
	ОповещениеПриЗавершении = Новый ОписаниеОповещения("ПоискПоШтрихкодуЗавершение", ЭтотОбъект, ТекущаяСтраница);
...
Функция ПараметрыСканированияШтрихкода(ТекущаяСтраница)
	...
	ПараметрыСканирования.ЭтоМаркировкаОстатков   = Объект.МаркировкаОстатков;
...
&НаКлиенте
Процедура ПоискПоШтрихкодуЗавершение(ДанныеШтрихкода, ТекущаяСтраница) Экспорт
	...
	Если ТекущаяСтраница = Элементы.СтраницаКодМаркировки Тогда
		СтруктураПолей = СтруктураПолейПоРежиму();
		СледующийШаг   = Истина;
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаНовыйКодМаркировки Тогда
		СтруктураПолей = СтруктураПолейПоРежиму(Истина);
		СледующийШаг   = Ложь;
	Иначе
		Возврат;
...
Процедура ОбновитьПредставленияНоменклатуры(Форма)
	...
	Форма.Представление = ИнтеграцияИСКлиентСервер.ПредставлениеНоменклатурыФорматированнойСтрокой(
		Форма.Объект.Номенклатура,
		Форма.Объект.Характеристика,
		Форма.Объект.ХарактеристикиИспользуются,
		Форма.Объект.БлокировкаРедактированияСтарогоКода);
	...
	Форма.НовоеПредставление = ИнтеграцияИСКлиентСервер.ПредставлениеНоменклатурыФорматированнойСтрокой(
		Форма.Объект.НоваяНоменклатура,
		Форма.Объект.НоваяХарактеристика,
		Форма.Объект.НоваяХарактеристикаИспользуется,
		ТолькоПросмотрНовойНоменклатуры);
