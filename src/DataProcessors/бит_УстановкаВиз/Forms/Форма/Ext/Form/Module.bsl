...
// ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2021-10-28 (#4391)
&НаКлиенте
Процедура ок_УстановленныеВизыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	
	ТекущиеДанные = Элементы.УстановленныеВизы.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено
		ИЛИ ОтменаРедактирования Тогда 
		
		Возврат;
		
	КонецЕсли;
	
	Если ТипЗнч(Объект.ОбъектВизирования) = Тип("ДокументСсылка.ЭлектронныйДокументВходящий")
		И ТипЗнч(ТекущиеДанные.ФизическоеЛицо) = Тип("СправочникСсылка.бит_БК_Инициаторы") Тогда
		
		ШаблонОшибки 		= "Согласующее лицо %1 уволен(а) от %2. Рекомендовано заменить согласующее лицо.";
		РеквизитыИнициатора = ОК_ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ТекущиеДанные.ФизическоеЛицо, "ок_Уволен,ок_ДатаУвольнения");
		
		Если РеквизитыИнициатора.ок_Уволен Тогда 
			
			ПоказатьПредупреждение(,СтрШаблон(ШаблонОшибки, ТекущиеДанные.ФизическоеЛицо, Формат(РеквизитыИнициатора.ок_ДатаУвольнения,"ДФ=dd.MM.yyyy")));
			
			ТекущиеДанные.ФизическоеЛицо = ПредопределенноеЗначение("Справочник.бит_БК_Инициаторы.ПустаяСсылка");
