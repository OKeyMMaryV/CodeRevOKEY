...
Процедура ИзменитьОформлениеСтраницыОжидаетсяРезультатРассмотрения(Форма)
	...
	Элементы.ОжидаетсяРезультатРассмотрения_НеБудутПриняты.Видимость = 
		Форма.ЭтоПредупреждениеИзНастроек
		И НЕ Форма.СостояниеЭДОсПФР.Неодобренное.Вид = Форма.НаОтключение;
	
	Элементы.ОжидаетсяРезультатРассмотрения_НаПодключениеИлиСертификат.Видимость = 
		Форма.СостояниеЭДОсПФР.Неодобренное.Вид = Форма.НаПодключение
		ИЛИ Форма.СостояниеЭДОсПФР.Неодобренное.Вид = Форма.НаСертификат;
	
	Элементы.ОжидаетсяРезультатРассмотрения_НаОтключение.Видимость = 
		Форма.СостояниеЭДОсПФР.Неодобренное.Вид = Форма.НаОтключение;
	
	Если Форма.СостояниеЭДОсПФР.Неодобренное.Вид = Форма.НаПодключение Тогда
		ПредставлениеВидаЗаявления = НСтр("ru = ' на переход к использованию электронных трудовых книжек '");
	ИначеЕсли Форма.СостояниеЭДОсПФР.Неодобренное.Вид = Форма.НаСертификат Тогда
		ПредставлениеВидаЗаявления = НСтр("ru = ' с новым сертификатом '");
	ИначеЕсли Форма.СостояниеЭДОсПФР.Неодобренное.Вид = Форма.НаОтключение Тогда
