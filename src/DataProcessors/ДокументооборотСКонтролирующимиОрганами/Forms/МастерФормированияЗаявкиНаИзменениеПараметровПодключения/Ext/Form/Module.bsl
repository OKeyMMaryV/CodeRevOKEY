...
Процедура ПриНажатииДалееНаСтраницеВыборДействия(МастерДалее)
	...
	Если ИзменитьМобильныйТелефон И (НЕ ИспользоватьСуществующий ИЛИ ВключенЛокальныйСертификат) Тогда
...
Функция УстановленХотяБыОдинФлаг(Форма)
	...
	Возврат Форма.ПродлитьЛицензиюНа1СОтчетность 
		...
		ИЛИ Форма.ИзменитьМобильныйТелефон 
...
Процедура СохранитьЗаявление(СохранятьЗаявление = Истина)
	...
	Если НЕ (НовыйДокументЗаявление.ИзменившиесяРеквизитыВторичногоЗаявления.Найти(Перечисления.ПараметрыПодключенияК1СОтчетности.ТелефонМобильный, "ИзмененныйРеквизит") <> Неопределено
		...
		И Не ТелефонИзменен И ЭлектроннаяПочтаИзменена) ИЛИ НЕ ЭтоУчетнаяЗаписьВМоделиСервиса ИЛИ ИспользоватьСуществующий Тогда
...
&НаКлиенте
Функция ЭтоТолькоСменаТелефонаВОблаке()

	Отбор = Новый Структура();
	Отбор.Вставить("ИзмененныйРеквизит", ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ТелефонМобильный"));
	
	НайденныеСтроки        = ДокументЗаявление.ИзменившиесяРеквизитыВторичногоЗаявления.НайтиСтроки(Отбор);
	ЕстьСменаТелефона      = НайденныеСтроки.Количество() = 1;
	ЭтоТолькоСменаТелефона = ДокументЗаявление.ИзменившиесяРеквизитыВторичногоЗаявления.Количество() = 1;
		
	Возврат 
		ЕстьСменаТелефона
		И ЭтоТолькоСменаТелефона
		И НЕ ТелефонИзменен 
		И ЭлектроннаяПочтаИзменена 
...
Процедура ОпределитьСпособОтправки()
	...
	Если ЭтоТолькоСменаТелефонаВОблаке() Тогда
		
		Результат    = Новый Структура("ТолькоИзменениеТелефона");
		ДопПараметры = Новый Структура();
		 
		АнализРезультатовОтправки(Результат, ДопПараметры);
...
&НаКлиенте
Процедура ПодтвердитьПаролемСменуТелефона(Контекст)
...
Процедура ОтправитьЗаявлениеИзКонтейнера()
	...
	Если НужноПодтвердитьПаролемСменуТелефона() Тогда
		ПодтвердитьПаролемСменуТелефона(Контекст);
	Иначе
