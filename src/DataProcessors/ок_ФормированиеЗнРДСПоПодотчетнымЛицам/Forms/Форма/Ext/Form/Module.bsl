...
Функция СоздатьЗаявкиНаСервере(МассивСообщений)
	...
	//ОКЕЙ Завьялова Е.А.(ПервыйБИТ) Начало 2021-11-12 (#AT-2155948#)
	//ТекстПисьма = "";
	ТекстПисьма = "Добрый день!" + Символы.ПС + Символы.ПС   
					+ "Просьба произвести выплаты сотрудникам " + Объект.Организация.НаименованиеСокращенное 
					+ " согласно спискам." + Символы.ПС + Символы.ПС;
	//ОКЕЙ Завьялова Е.А.(ПервыйБИТ) Конец  2021-11-12 (#AT-2155948#)
	...
	Для Каждого Строка Из ВыбранныеСтроки Цикл
		Для Каждого ИмяКолонки Из МассивКолонокДляЗаполнения Цикл
			Если Не ЗначениеЗаполнено(Строка[ИмяКолонки]) Тогда
				ЕстьОшибки = Истина;
				МассивСообщенийОбОшибках.Добавить(НСтр("ru = 'В строке номер " + Строка.НомерСтроки + " не заполнено значение " + ИмяКолонки + ".'"));
			КонецЕсли;
		КонецЦикла;
		Если Не строка.УчаствуетВФормированииЗнРДС Тогда
			ЕстьОшибки = Истина;
			МассивСообщенийОбОшибках.Добавить(НСтр("ru = 'В строке номер " + Строка.НомерСтроки + " банковский счет не участвует в создании ЗнРДС.'"));
		КонецЕсли;
		Если ЗначениеЗаполнено(Строка.ЗаявкаНаРасходованиеДС) Тогда
			ЕстьОшибки = Истина;
			МассивСообщенийОбОшибках.Добавить(НСтр("ru = 'В строке номер " + Строка.НомерСтроки + " уже создана ЗнРДС " + Строка.ЗаявкаНаРасходованиеДС + ".'")); 
		КонецЕсли;
...
Процедура ПроверитьПраваНаПечать(ЕстьОшибки,МассивСообщенийОбОшибках)
	...
	Если Не ЕстьПраво Тогда
		ЕстьОшибки = Истина;
		МассивСообщенийОбОшибках.Добавить(НСтр("ru = 'Нет прав на печать ведомостей по формам банков. Формирование ЗнРДС невозможно!'"));
	КонецЕсли;
КонецПроцедуры
...
Функция СоздатьИЗаполнитьЗаявку(СтрокаЗаявки, ДанныеТабличнойЧастиЗаявки, НомерВедомости, МассивСообщений)
		...
		//ОКЕЙ Завьялова Е.А.(ПервыйБИТ) Начало 2021-11-11 (#AT-2155492#)
		//ДокЗаявка.УстановитьНовыйНомер();
		//ДокЗаявка.Организация = СтрокаЗаявки.Организация;
		ДокЗаявка.Организация							=	СтрокаЗаявки.Организация;
		ДокЗаявка.УстановитьНовыйНомер();
		//ОКЕЙ Завьялова Е.А.(ПервыйБИТ) Конец  2021-11-11 (#AT-2155492#)
		...
		ДокЗаявка.НазначениеПлатежа = "Выплата подотчет согласно ведомости № " + НомерВедомости + " от " + ДокЗаявка.ДатаРасхода + ", НДС не облагается";
