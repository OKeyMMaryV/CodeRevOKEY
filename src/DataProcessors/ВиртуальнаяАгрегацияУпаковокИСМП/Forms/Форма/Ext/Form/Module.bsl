...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЦветЗаголовкаГруппы   = ЦветаСтиля.ЦветАкцента;
...
Процедура ОстаткиПродукцииНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	ДанныеСтроки = Новый Структура("Номенклатура,Характеристика", ТекущиеДанные.Номенклатура, ТекущиеДанные.Характеристика);
...
Процедура СоставУпаковкиПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Иначе
		...
		ЗаполнитьЗначенияСвойств(СтрокаСостава, ПараметрыПеретаскивания.Значение, "Номенклатура,Характеристика");
	КонецЕсли;
...
Процедура ДобавитьВСоставУпаковки(Команда)
	...
	ТекущиеДанные = Элементы.ОстаткиПродукции.ТекущиеДанные;
...
Процедура УправлениеЭлементамиФормыПриСоздании()
	...
	Если ВидПродукции = Перечисления.ВидыПродукцииИС.Обувная Тогда
		Заголовок = НСтр("ru = 'Виртуальная агрегация обувных упаковок'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.Табачная Тогда
		Заголовок = НСтр("ru = 'Виртуальная агрегация табачных упаковок'");
	КонецЕсли;
...
Процедура СформироватьЗаголовокКоличествоУпаковок()
	...
	Если МаксимальноеКоличествоУпаковок > 0 Тогда
		СтрокаМаксимальноеКоличество = Формат(МаксимальноеКоличествоУпаковок, "ЧДЦ=0;");
		
		МассивПодстрок.Добавить(НСтр("ru = 'из'"));
		МассивПодстрок.Добавить(СтрокаМаксимальноеКоличество);
		МассивПодстрок.Добавить(ПросклонятьВозможных(СтрокаМаксимальноеКоличество));
	КонецЕсли;
...
Процедура ОбработатьИПроверитьПереданныеПараметры(Отказ)
	...
	Если Результат.Пустой() Тогда
		Если ЭтоЗаказНаЭмиссию Тогда
			ТекстОшибки = НСтр("ru = 'По заказу ""%1"" для %2 продукции эмиссия кодов еще не выполнена.'");
		Иначе
			ТекстОшибки = НСтр("ru = 'По документу ""%1"" для %2 продукции отсутствуют исполненные заказы на эмиссию кодов маркировки.'");
		КонецЕсли;
		
		Если ВидПродукции = Перечисления.ВидыПродукцииИС.Обувная Тогда
			ТекстОшибки = СтрШаблон(ТекстОшибки, Параметры.Документ, НСтр("ru = 'обувной'"));
		ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.Табачная Тогда
			ТекстОшибки = СтрШаблон(ТекстОшибки, Параметры.Документ, НСтр("ru = 'табачной'"));
		КонецЕсли;
...
Процедура ДобавитьСуществующиеУпаковкиВИтоговоеДеревоУпаковок(СуществующиеУпаковки)
	...
	Для Каждого СтрокаУпаковки Из СуществующиеУпаковки.Строки Цикл
		...
		НоваяАгрегация.Упаковка         = ПросклонятьУпаковку(СтрокаУпаковки.Количество);
		...
		Для Каждого СтрокаПродукции Из СтрокаУпаковки.Строки Цикл
			...
			НоваяПродукция.Упаковка         = ПросклонятьПару(НоваяПродукция.КоличествоИтог);
	...
	Для Каждого СтрокаСоставаУпаковки Из СоставУпаковки Цикл
		...
		Для Каждого СтрокаСоставаАгрегации Из СоставАгрегации Цикл
			Если СтрокаСоставаАгрегации.Номенклатура = СтрокаСоставаУпаковки.Номенклатура
				...
				И СтрокаСоставаАгрегации.Серия = СтрокаСоставаУпаковки.Серия Тогда
		...
		Если СтрокаПродукции = Неопределено Тогда
			...
			ЗаполнитьЗначенияСвойств(СтрокаПродукции, СтрокаСоставаУпаковки, "Номенклатура,Характеристика,Серия");
			СтрокаПродукции.Содержимое = ПредставлениеНоменклатуры(СтрокаПродукции.Номенклатура,
				СтрокаПродукции.Характеристика, СтрокаПродукции.Серия);
		...
		СтрокаПродукции.Упаковка       = ПросклонятьПару(СтрокаПродукции.КоличествоИтог);
	КонецЦикла;
...
&НаСервереБезКонтекста
Функция ПредставлениеНоменклатуры(Номенклатура, Характеристика, Серия)
	...
	Если ЗначениеЗаполнено(Серия) Тогда
		НаименованиеСерии = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Серия, "Наименование");
	Иначе
		НаименованиеСерии = "";
	КонецЕсли;
	
	Возврат ИнтеграцияИС.ПредставлениеНоменклатуры(НаименованиеНоменклатуры, НаименованиеХарактеристики,, НаименованиеСерии);
...
Функция НачатьЗаписьУпаковок()
	...
	Для Каждого СтрокаАгрегации Из ИтоговоеДеревоУпаковок.ПолучитьЭлементы() Цикл
		Если ПустаяСтрока(СтрокаАгрегации.ХешСуммаУпаковки) Тогда
			...
			ТаблицаСостава.Свернуть("Номенклатура,Характеристика,Серия", "Количество");
...
&НаКлиентеНаСервереБезКонтекста
Функция ПросклонятьВозможных(СтрокаКоличество)
	
	ПоследнийСимволКоличества     = Прав(СтрокаКоличество, 1);
	ДваПоследнихСимволаКоличества = Прав(СтрокаКоличество, 2);
	
	Если ДваПоследнихСимволаКоличества = "11" Тогда
		Возврат НСтр("ru = 'возможных'");
	ИначеЕсли ПоследнийСимволКоличества = "1" Тогда
		Возврат НСтр("ru = 'возможной'");
	Иначе
		Возврат НСтр("ru = 'возможных'");
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ПросклонятьПару(Количество)
	...
	Иначе
		...
		Если ДваПоследнихСимволаКоличества = "11"
		 ...
		 Или ДваПоследнихСимволаКоличества = "14" Тогда
			Возврат НСтр("ru = 'пар'");
		ИначеЕсли ПоследнийСимволКоличества = "1" Тогда
			Возврат НСтр("ru = 'пара'");
		ИначеЕсли СтрНайти("234", ПоследнийСимволКоличества) <> 0 Тогда
			Возврат НСтр("ru = 'пары'");
		Иначе
			Возврат НСтр("ru = 'пар'");
		КонецЕсли;
	КонецЕсли;
