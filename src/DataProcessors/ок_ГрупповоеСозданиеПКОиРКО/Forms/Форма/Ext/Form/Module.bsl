...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастройкаПриОткрытии = бит_ОтчетыСервер.НайтиНастройкуПоУмолчаниюДляОбъекта(ПолучитьИмяОбработки());
	
	Если Не НастройкаПриОткрытии = Неопределено Тогда 
		ПрименитьНастройкиНаСервере(НастройкаПриОткрытии);
	Иначе
...
Процедура Команда_СохранитьНастройки(Команда)
	...
	ПараметрыФормы.Вставить("НастраиваемыйОбъект", ПолучитьИмяОбработки());
...
&НаКлиенте
Процедура Команда_ВосстановитьНастройки(Команда)
...
Функция ПолучитьИмяОбработки()
	Возврат "Обработка.ок_ГрупповоеСозданиеПКОиРКО";
КонецФункции
...
Функция УпаковатьНастройкиВСтруктуру()
	...
	СтруктураНастроек.Вставить("ВидОперации", 	ВидОперации);
	СтруктураНастроек.Вставить("КорСчет", 		КорСчет);
	СтруктураНастроек.Вставить("Организация", 	Организация);
	СтруктураНастроек.Вставить("СтатьяДДС", 	СтатьяДДС);
	СтруктураНастроек.Вставить("Субконто1", 	Субконто1);
	СтруктураНастроек.Вставить("Субконто2", 	Субконто2);
	СтруктураНастроек.Вставить("Субконто3", 	Субконто3);
	СтруктураНастроек.Вставить("СчетУчета", 	СчетУчета);
	СтруктураНастроек.Вставить("ТипОперации", 	ТипОперации);
	
	Колонки	=	"Объект, СтатьяДДС, КорСчет, Субконто1, Субконто2, Субконто3, ПринятоВыдано, Основание, Приложение, Контрагент, ДоговорКонтрагента";
	СтруктураНастроек.Вставить("ТаблицаДокументов", бит_ОбщегоНазначения.УпаковатьТаблицуЗначений(ТаблицаДокументов.Выгрузить(, Колонки)));
...
Процедура ВосстановитьНастройкиОкончание(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатВыбора) Тогда
		ПрименитьНастройкиНаСервере(РезультатВыбора);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ПрименитьНастройкиНаСервере(ВыбНастройка = Неопределено)
	...
	Если ТипЗнч(ЗначениеНастроек) = Тип("Структура") Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтаФорма, ЗначениеНастроек, "ТипОперации, КорСчет");
		...
		ЗаполнитьЗначенияСвойств(ЭтаФорма, ЗначениеНастроек, , "ТипОперации, КорСчет, ТаблицаДокументов");
		
		ТаблицаДокументов.Загрузить(бит_ОбщегоНазначения.РаспаковатьТаблицуЗначений(ЗначениеНастроек.ТаблицаДокументов));
...
&НаКлиенте
Процедура ОбновитьКолонкиТаблицыДокументов(ИменаКолонок)
	
	Для Каждого СтрокаТаблицы Из ТаблицаДокументов Цикл
		ЗаполнитьЗначенияСвойств(СтрокаТаблицы, ЭтаФорма, ИменаКолонок);	
	КонецЦикла;
...
Процедура УправлениеФормой(Форма)
	...
	//Контрагент
	Если Форма.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ОплатаПокупателя") Тогда
	...
	Если Форма.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ПрочийПриход") 
		Или Форма.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРКО.ПрочийРасход") Тогда
		...
		ДанныеСчета	=	БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(Форма.КорСчет);
		Для Счетчик = 1 По 3 Цикл
			ТипЗначения	=	ДанныеСчета["ВидСубконто" + Счетчик + "ТипЗначения"];
			Если ТипЗначения = Неопределено Тогда
				Доступность	=	Ложь;
			Иначе
				Доступность	=	Не ТипЗначения.Типы().Найти(Тип("СправочникСсылка.ОбъектыСтроительства")) = Неопределено;		
			КонецЕсли;
				
			Форма.Элементы["ТаблицаДокументовСубконто" + Счетчик].Доступность	=	Доступность;
		КонецЦикла;
...
Процедура ПриИзмененииКорСчета()
	...
	ОбновитьКолонкиТаблицыДокументов("КорСчет, Субконто1, Субконто2, Субконто3");
