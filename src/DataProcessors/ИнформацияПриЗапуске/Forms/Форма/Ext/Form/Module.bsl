...
// бит_Финанс Процедура Переопределение типового обработчика события "ПередОткрытием" формы.
//
&НаСервере
Процедура бит_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	
	// для не русских интерфейсов обработку не нужно открывать, т.к. открытие приводит к ошибке
	...
	Если ЭтаФорма.бит_Обработчики.Форма.Свойство("бит_Типовой_ПриСозданииНаСервере") Тогда
		
		КодВыполнения = ЭтаФорма.бит_Обработчики.Форма.бит_Типовой_ПриСозданииНаСервере+"(Отказ, СтандартнаяОбработка)";
		Выполнить(КодВыполнения);
		
	КонецЕсли; 
		
КонецПроцедуры // бит_ПередОткрытием()

// бит_Финанс Процедура переопределяет типовые события формы.
//
&НаСервере
Процедура бит_УстановитьДействияФормы()
	
	// имя реквизита для хранения имен обработчиков
	ИмяРеквизитаХранилища = "бит_Обработчики";
	
	// массив перехватываемых событий
	События = Новый Массив;
	События.Добавить("ПриСозданииНаСервере");	
	
	// вызов перехватчика
	бит_РаботаСДиалогамиСервер.ВыполнитьПерехватСобытий(ЭтаФорма, События, ИмяРеквизитаХранилища);
	
КонецПроцедуры // бит_УстановитьДействияФормы()

#КонецОбласти

// Переопределение типовых обработчиков.
