...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек,
		"Счет", бит_БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПараметрыОтчета.Счет));
	...
	Для Каждого ПолеВыбраннойГруппировки Из ПараметрыОтчета.Группировка Цикл 
		Если ПолеВыбраннойГруппировки.Использование Тогда
			...
			Если СтрНайти(ПолеВыбраннойГруппировки.Поле, "КорСубконто1") <> 0
				...
				ИЛИ СтрНайти(ПолеВыбраннойГруппировки.Поле, "КорСубконто3") <> 0
				ИЛИ СтрНайти(ПолеВыбраннойГруппировки.Поле, "КорСубконто4") <> 0 Тогда
...
Процедура ПроверитьКорректностьИтогов(ПараметрыОтчета, Результат) Экспорт
	...
	Если КонтрольноеСоотношение <> 0 Тогда
		...
		Если бит_БухгалтерскиеОтчеты.ИспользуютсяОборотныеСубконто(ПараметрыОтчета.Счет, ЭлементыОтбора) Тогда
			Возврат;
		КонецЕсли;
...
// (см. БухгалтерскиеОтчетыКлиентСервер.НовыйУниверсальныеНастройки) для переданного экземпляра отчета.
//
// Параметры:
//  Настройки				 - Структура								 
...
Процедура ЗаполнитьПравилаРасшифровки(Правила) Экспорт
	...
	Правило.Отчет = "бит_ОтчетПоПроводкамМУ";
...
Функция КартаСубконто(СчетГруппа, НомераИспользуемыхСубконтоСчетаГруппы, Субсчета)
	...
	ВызватьИсключение "Рассказать как сюда попали.";
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.бит_Дополнительный_2.ВидыСубконто КАК ВидыСубконто1
	|		ПО ПланСчетов.Ссылка = ВидыСубконто1.Ссылка
	|			И (ВидыСубконто1.НомерСтроки = 1)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.бит_Дополнительный_2.ВидыСубконто КАК ВидыСубконто2
	|		ПО ПланСчетов.Ссылка = ВидыСубконто2.Ссылка
	|			И (ВидыСубконто2.НомерСтроки = 2)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.бит_Дополнительный_2.ВидыСубконто КАК ВидыСубконто3
	|		ПО ПланСчетов.Ссылка = ВидыСубконто3.Ссылка
	|			И (ВидыСубконто3.НомерСтроки = 3)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.бит_Дополнительный_2.ВидыСубконто КАК ВидыСубконто4
	|		ПО ПланСчетов.Ссылка = ВидыСубконто4.Ссылка
	|			И (ВидыСубконто4.НомерСтроки = 4)
	|ГДЕ
	|	ПланСчетов.Ссылка В (&Субсчета)";
	...
	СвойстваСчетаГруппы = бит_БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(СчетГруппа);
	...
	Для каждого СвойстваСубсчета Из СвойстваСчетов Цикл
		...
		Для НомерСубконтоСубсчета = 1 По 4 Цикл
...
Процедура ИзменитьСхемуДляОтборовПоСубконтоСубсчетов(ПараметрыОтчета, Схема, КомпоновщикНастроек)
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ПланСчетовВидыСубконто_СчетГруппа.НомерСтроки КАК НомерСубконто,
	|	ПланСчетовВидыСубконто_Субсчета.НомерСтроки КАК НомерСубконтоСубсчета,
	|	ПланСчетов_Субсчета.Ссылка КАК Субсчет
	|ИЗ
	|	ПланСчетов.бит_Дополнительный_2.ВидыСубконто КАК ПланСчетовВидыСубконто_СчетГруппа
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланСчетов.бит_Дополнительный_2 КАК ПланСчетов_Субсчета
	|		ПО (ПланСчетов_Субсчета.Ссылка В (&Субсчета))
	|			И (ПланСчетов_Субсчета.Ссылка <> &СчетГруппа)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.бит_Дополнительный_2.ВидыСубконто КАК ПланСчетовВидыСубконто_Субсчета
	|		ПО (ПланСчетов_Субсчета.Ссылка = ПланСчетовВидыСубконто_Субсчета.Ссылка)
	|			И ПланСчетовВидыСубконто_СчетГруппа.ВидСубконто = ПланСчетовВидыСубконто_Субсчета.ВидСубконто
	|ГДЕ
	|	ПланСчетовВидыСубконто_СчетГруппа.Ссылка = &СчетГруппа
	|	И ПланСчетовВидыСубконто_СчетГруппа.НомерСтроки В(&НомераИспользуемыхСубконтоСчетаГруппы)
	|	И ПланСчетовВидыСубконто_СчетГруппа.НомерСтроки <> ЕСТЬNULL(ПланСчетовВидыСубконто_Субсчета.НомерСтроки, 0)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Субсчет";
