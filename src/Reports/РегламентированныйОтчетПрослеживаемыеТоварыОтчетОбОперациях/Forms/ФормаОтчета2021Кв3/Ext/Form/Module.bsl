...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ПереопределитьТекстВЭкраннойИлиПечатнойФорме(ТабличныйДокумент);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	ВозможныеКодыОперации_ДолжныБытьЗаполненыСведенияОКонтрагенте_Строка = "08";
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если ЗначениеЗаполнено(КодОперации) Тогда
				...
				Если ДолжныБытьЗаполненыСведенияОКонтрагенте Тогда
				...
				Иначе
					...
					Если ЗаполненыСведенияОКонтрагенте Тогда
						
						Если ЗначениеЗаполнено(Контрагент_Наим) Тогда
							ИмяПок = "П1000007" + "_" + НомерСтрокиП10000_Строка;
						ИначеЕсли ЗначениеЗаполнено(Контрагент_ИНН) Тогда
							ИмяПок = "П1000008" + "_" + НомерСтрокиП10000_Строка;
						Иначе
							ИмяПок = "П1000009" + "_" + НомерСтрокиП10000_Строка;
						КонецЕсли;
						...
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " сведения о контрагенте не должны быть заполнены при коде операции " + КодОперации,
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
...
Процедура ВывестиНаПечатьОтчет(СтруктураПараметров, ВыбранныеСтраницы, СтрПоля = Неопределено)
	...
	ПереопределитьТекстВЭкраннойИлиПечатнойФорме(ПечФорма);
	
	ПодШапка       = Макет.Получитьобласть("ПодШапка");
	СтрокаДокумент = Макет.Получитьобласть("СтрокаДокумент");
	Подвал         = Макет.Получитьобласть("Подвал");
	
	ПереопределитьТекстВЭкраннойИлиПечатнойФорме(Подвал);
...
&НаСервере
Процедура ПереопределитьТекстВЭкраннойИлиПечатнойФорме(ТаблДокумент)
	
	Если ТекущаяДатаСеанса() < ДатаПримененияФормыВНовойРедакции() Тогда
		
		Обл = ТаблДокумент.Области.Найти("РеквизитыПриказа");
		Если Обл <> Неопределено Тогда
			Обл.Текст = "Приложение № 4
			|к письму ФНС России
			|от 14.04.2021
			|№ ЕА-4-15/5042@";
		КонецЕсли;
		
		Обл = ТаблДокумент.Области.Найти("ИННКПППравопреемника");
		Если Обл <> Неопределено Тогда
			Обл.Текст = "ИНН/КПП реорганизованной организации";
		КонецЕсли;
		
		Обл = ТаблДокумент.Области.Найти("ПодписантПризнаки");
		Если Обл <> Неопределено Тогда
			Обл.Текст = "1 - участник оборота товаров, подлежащих прослеживаемости
			|2 - представитель участника оборота товаров, подлежащих прослеживаемости";
		КонецЕсли;
		
		Обл = ТаблДокумент.Области.Найти("ПодписантНаимДок");
		Если Обл <> Неопределено Тогда
			Обл.Текст = "Наименование и реквизиты документа, подтверждающего полномочия"
			+ " представителя участника оборота товаров, подлежащих прослеживаемости";
		КонецЕсли;
		
		Обл = ТаблДокумент.Области.Найти("ПодписантНаимОрг");
		Если Обл <> Неопределено Тогда
			Обл.Текст = "Наименование организации-представителя участника оборота товаров, подлежащих прослеживаемости";
		КонецЕсли;
