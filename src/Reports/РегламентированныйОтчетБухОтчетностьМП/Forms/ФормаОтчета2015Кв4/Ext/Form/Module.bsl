...
Процедура ВывестиНаПечатьТитульныйЛистМЧБ(ПараметрыПечати, КодФормыПоКНД, ПериодОтчетности)
	...
	Если ИспользоватьФорму61н(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда
		ПечатнаяФорма = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаПечати2019Кв1_КНД" + КодФормыПоКНД);
...
Процедура ПроверкаКС_01_код(Настройки)
	...
	Настройки.ЭтоОшибка = НЕ Показатели.Ф1П1600.Значение = РасчетнаяСумма;
...
Процедура ПроверкаКС_02_код(Настройки)
	...
	Настройки.ЭтоОшибка = НЕ Показатели.Ф1П1600.Значение = Показатели.Ф1П1700.Значение;
...
Процедура ПроверкаКС_03_код(Настройки)
	...
	Настройки.ЭтоОшибка = НЕ Показатели.Ф1П1700.Значение = РасчетнаяСумма;
...
Процедура ПроверкаКС_04_код(Настройки)
	...
	Настройки.ЭтоОшибка = НЕ Показатели.Ф2П2400.Значение = РасчетнаяСумма;
...
Процедура ПроверкаКС_12_код(Настройки)
	...
	Настройки.ЭтоОшибка = НЕ Показатели.Ф3П0001.Значение = РасчетнаяСумма;
...
Процедура ПроверкаКС_14_код(Настройки)
	...
	Настройки.ЭтоОшибка = НЕ Показатели.Ф3П0001.Значение >= РасчетнаяСуммаМодуль;
...
Процедура ПроверкаКС_25_код(Настройки)
	...
	Настройки.ЭтоОшибка = НЕ Показатели.Ф4П0001.Значение = РасчетнаяСумма;
...
Процедура ПроверкаКС_28_код(Настройки)
	...
	Настройки.ЭтоОшибка = НЕ Показатели.Ф5П6400.Значение = РасчетнаяСумма;
...
Процедура СформироватьСтруктуруДанныхКС()
	...
	Для каждого Раздел Из СтруктураРазделов Цикл
		ДанныеКС.Вставить(Раздел.Ключ, ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ЭтаФорма[Раздел.Значение]));
	КонецЦикла;
...
Процедура ЗаполнитьПоказатель(Показатель, Источник, НомерСтроки = Неопределено)
	...
	Если Источник.Свойство(Показатель.ИмяПоказателя, ЗначениеВИсточнике) Тогда
		...
		Если ТипЗнч(ЗначениеВИсточнике) = Тип("Число") И Показатель.Округление Тогда
			Показатель.Значение = ОкруглитьЧислоПоФормату(ЭтаФорма, ЗначениеВИсточнике);
