...
&НаКлиенте
Перем мТекущийВидСравненияПоКорСчету; // Служит для передачи вида сравнения между обработчиками.

&НаКлиенте
Перем мТекущийВидСравненияКорСубконто; // Служит для передачи вида сравнения между обработчиками.
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	бит_БухгалтерскиеОтчетыСервер.ЗаполнитьСписокВыбораВидаСравненияДляСчета(Элементы.КорСчетВидСравнения.СписокВыбора);
	бит_БухгалтерскиеОтчетыСервер.ЗаполнитьСписокВыбораВидаСравненияДляСубконто(Элементы.ТаблицаКорСубконтоВидСравнения.СписокВыбора);
	
	бит_БухгалтерскиеОтчетыСервер.ЗаполнитьСчетПоУмолчанию_Бюджетирование(Отчет.КорСчет);
...
&НаКлиенте
Процедура КорСчетИспользованиеПриИзменении(Элемент)
	
	Если Не ЗначениеЗаполнено(Отчет.КорСчетВидСравнения) Тогда
		Отчет.КорСчетВидСравнения = ВидСравнения.Равно;	
	КонецЕсли;
	
КонецПроцедуры // КорСчетИспользованиеПриИзменении()

&НаКлиенте
Процедура КорСчетВидСравненияПриИзменении(Элемент)
	
	Отчет.КорСчетИспользование = Истина;
	
	ИзменениеВидаСравненияКорСчета(мТекущийВидСравненияПоКорСчету);
	
КонецПроцедуры // КорСчетВидСравненияПриИзменении()

&НаКлиенте
Процедура КорСчетВидСравненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	мТекущийВидСравненияПоКорСчету = Отчет.КорСчетВидСравнения;
...
КонецПроцедуры // КорСчетПриИзменении()
           
&НаКлиенте
Процедура КорСчетОчистка(Элемент, СтандартнаяОбработка)
	
	Если ТипЗнч(Отчет.КорСчет) = Тип("СписокЗначений") Тогда
	 	Отчет.КорСчет.Очистить();
	КонецЕсли;
			
КонецПроцедуры // КорСчетОчистка()
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыТаблицаКорСубконто

&НаКлиенте
Процедура ТаблицаКорСубконтоПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	
	Отказ = Истина;
		
	ОписаниеВС = Новый ОписаниеОповещения("ВыборПредставленияКорСубконтоЗавершение", ЭтотОбъект);
	ПоказатьВыборИзСписка(ОписаниеВС, фКэшЗначений.СписокВыбораПредставленияКорСубконто, Элемент);
		
КонецПроцедуры

// Процедура - завершение выбора представления кор. субконто.
// 
&НаКлиенте
Процедура ВыборПредставленияКорСубконтоЗавершение(ВыбранноеЗначение, ДопПараметры) Экспорт

	Если ВыбранноеЗначение = Неопределено Тогда
		Возврат;	
	КонецЕсли;	
	
	НоваяСтрока = ТаблицаКорСубконто.Добавить();
	НоваяСтрока.Имя 		  = ВыбранноеЗначение.Значение;
	НоваяСтрока.Представление = ВыбранноеЗначение.Представление;
	НоваяСтрока.ТипЗначения   = фКэшЗначений.ТипыКорСубконто;

КонецПроцедуры // ВыборПредставленияКорСубконтоЗавершение()

&НаКлиенте
Процедура ТаблицаКорСубконтоВидСравненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	ТекущиеДанные = Элементы.ТаблицаКорСубконто.ТекущиеДанные;
	мТекущийВидСравненияКорСубконто = ТекущиеДанные.ВидСравнения;
	
КонецПроцедуры // ТаблицаКорСубконтоВидСравненияНачалоВыбора()

&НаКлиенте
Процедура ТаблицаКорСубконтоВидСравненияПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.ТаблицаКорСубконто.ТекущиеДанные;
	ИзменениеВидаСравненияСервер(ТекущиеДанные.ПолучитьИдентификатор(), мТекущийВидСравненияКорСубконто, "ТаблицаКорСубконто");
	
КонецПроцедуры // ТаблицаКорСубконтоВидСравненияПриИзменении()

&НаКлиенте
Процедура ТаблицаКорСубконтоЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
КонецПроцедуры // ТаблицаКорСубконтоЗначениеНачалоВыбора()

&НаКлиенте
Процедура ТаблицаКорСубконтоЗначениеПриИзменении(Элемент)
...
КонецПроцедуры // ТаблицаКорСубконтоЗначениеПриИзменении()

&НаКлиенте
Процедура ТаблицаКорСубконтоЗначениеОчистка(Элемент, СтандартнаяОбработка)
	
	ТекущиеДанные = Элементы.ТаблицаКорСубконто.ТекущиеДанные;
	
	бит_БухгалтерскиеОтчетыКлиент.ОчиститьЗначениеОтбора(ТекущиеДанные.Значение);
	
КонецПроцедуры // ТаблицаКорСубконтоЗначениеОчистка()
...
Процедура ПравилаВыводаИтоговСчетПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Счет) Тогда
		ИзменениеСчетаТаблицыПравилВывода(ТекущиеДанные.ПолучитьИдентификатор());
	КонецЕсли;	
...
Процедура ПравилаВыводаИтоговПредставлениеРазворотПоСубконтоНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДопПараметры = Новый Структура("ИдТекущиеДанные", ТекущиеДанные.ПолучитьИдентификатор());
...
Процедура НастроитьРазворотПоСубконтоЗавершение(СписокСубконтоСчета, ПараметрыОповещения) Экспорт
	...
	ЗаполнитьРазворотПоСубконтоПоСписку(ПараметрыОповещения.ИдТекущиеДанные, СписокСубконтоСчета);
...
// Отрабатывает изменение вида сравнения для кор. счета.
// 
// Параметры:
// 	ПредВидСравнения.
// 
&НаСервере
Процедура ИзменениеВидаСравненияКорСчета(ПредВидСравнения)
	
	бит_БухгалтерскиеОтчетыСервер.ИзменениеВидаСравненияКорСчета(Отчет, ПредВидСравнения);
	бит_ОбщегоНазначенияКлиентСервер.УстановитьСостояниеПоляТабличногоДокумента(ЭтаФорма.Элементы.Результат, "НеАктуальностьОтчета");
...
// 
&НаСервере
Процедура ЗаполнитьРазворотПоСубконтоПоСписку(Ид, СписокСубконтоСчета)

	бит_БухгалтерскиеОтчетыСервер.ЗаполнитьРазворотПоСубконтоПоСписку(ЭтаФорма, Ид, СписокСубконтоСчета)
...
// 
&НаСервере
Процедура ИзменениеСчетаТаблицыПравилВывода(Ид)
	
	бит_БухгалтерскиеОтчетыСервер.ИзменениеСчетаТаблицыПравилВывода(ЭтаФорма, Ид);
