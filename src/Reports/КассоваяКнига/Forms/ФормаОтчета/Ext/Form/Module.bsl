...
Функция ПодготовитьПараметрыОтчета()
	...
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Начало 2021-09-22 (#ТП_БП11_ФР14)
	Если ЗначениеЗаполнено(ЭтаФорма.ок_Объект) Тогда
		ПараметрыОтчета.Вставить("ок_Объект",				ЭтаФорма.ок_Объект);	
	КонецЕсли;
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Конец 2021-09-22 (#ТП_БП11_ФР14)
...
Функция СформироватьОтчетНаСервере(Знач ГраницыПересчетаЛистов = Неопределено)
	...
	Если ПересчитатьНомераЛистов Тогда
		НомерЛистаНачало = ПолучитьНомерЛистаНачало(ПараметрыОтчета);
		ПараметрыОтчета.НомерЛистаНачало = НомерЛистаНачало;
	КонецЕсли;
