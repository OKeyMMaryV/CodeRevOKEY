...
Процедура СформироватьЛистыКассовойКниги(ПараметрыОтчета, ДатыЛистовСОшибками, ПоказатьОповещениеОНумерации)
	...
	Если ПараметрыОтчета.НачалоПериода > НачалоГода
		...
		Если Номера.Количество() > 0 И Номера[0].Период >= НачалоГода Тогда
			...
			Иначе
				ПараметрыОтчета.НомерЛистаНачало = Номера[0].НомерЛиста;
			КонецЕсли;
	...
	Если ПараметрыОтчета.Свойство("ок_Объект") Тогда 
		ЗапросПоИтогам.УстановитьПараметр("НеОтбиратьПоОбъектам",   Ложь);
		ЗапросПоИтогам.УстановитьПараметр("СписокОбъектов",   		ПараметрыОтчета.ок_Объект);
	Иначе
		ЗапросПоИтогам.УстановитьПараметр("НеОтбиратьПоОбъектам",   Истина);
		ЗапросПоИтогам.УстановитьПараметр("СписокОбъектов",   		Новый СписокЗначений);
	КонецЕсли;
	...
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Начало 2021-09-22 (#ТП_БП11_ФР14)
	|			И (&НеОтбиратьПоОбъектам ИЛИ Субконто2 В (&СписокОбъектов))
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Конец 2021-09-22 (#ТП_БП11_ФР14)
	...
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Начало 2021-09-22 (#ТП_БП11_ФР14)
	Если ПараметрыОтчета.Свойство("ок_Объект") Тогда 
		ЗапросПоДокументам.УстановитьПараметр("НеОтбиратьПоОбъектам",   Ложь);
		ЗапросПоДокументам.УстановитьПараметр("СписокОбъектов",   		ПараметрыОтчета.ок_Объект);
	Иначе
		ЗапросПоДокументам.УстановитьПараметр("НеОтбиратьПоОбъектам",   Истина);
		ЗапросПоДокументам.УстановитьПараметр("СписокОбъектов",   		Новый СписокЗначений);
	КонецЕсли;
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Конец 2021-09-22 (#ТП_БП11_ФР14)
	...
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Начало 2021-09-22 (#ТП_БП11_ФР14)
	|			И (&НеОтбиратьПоОбъектам ИЛИ ПриходныйКассовыйОрдер.ок_Объект В (&СписокОбъектов))
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Конец 2021-09-22 (#ТП_БП11_ФР14)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Начало 2021-09-22 (#ТП_БП11_ФР14)
	|			И (&НеОтбиратьПоОбъектам ИЛИ РасходныйКассовыйОрдер.ок_Объект В (&СписокОбъектов))
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Конец 2021-09-22 (#ТП_БП11_ФР14)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Документ
	...
	|УПОРЯДОЧИТЬ ПО
	|	День,
	//ОКЕЙ Михайлова М.В.(ПервыйБИТ) Начало 2021-10-26 (#ТП_БП11_ФР14_ДОП)
	//|	ДатаДок,
	|	НомерДок,
	//ОКЕЙ Михайлова М.В.(ПервыйБИТ) Конец  2021-10-26 (#ТП_БП11_ФР14_ДОП)
	|	Документ
	|ИТОГИ
	|	МАКСИМУМ(ДатаДок),
	|	МАКСИМУМ(НомерДок),
...
//		ЛистовЗаГод = 1 стр
		...
		Для каждого Документ Из ТаблицаДокументы Цикл
			...
			//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Начало 2021-10-11 (#ТП_БП11_ФР14)
			...
			//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Конец 2021-10-11 (#ТП_БП11_ФР14)
...
Процедура СформироватьОбложку(ТабличныйДокумент, ПараметрыОтчета, ДатаЛиста, ПредставлениеПодразделения)
	...
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Начало 2021-10-12 (#ТП_БП11_ФР14)
	Если ПараметрыОтчета.Свойство("ок_Объект") Тогда
		ОбластьОбложка.Параметры.НазваниеПодразделения = ПараметрыОтчета.ок_Объект;	
	КонецЕсли;
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Конец 2021-10-12 (#ТП_БП11_ФР14)
