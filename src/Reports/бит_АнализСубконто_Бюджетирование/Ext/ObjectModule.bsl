...
Функция СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	Иначе
		...
		Для каждого СтрГр Из ТаблицаГруппировки Цикл
			Если СтрГр.Использование Тогда	
				
				СписокГруппировок.Добавить(СтрГр.Имя, СтрГр.Представление, Найти(СтрГр.Имя, "Субконто") <> 0);
				ТекстГруппировокИтоги   = ТекстГруппировокИтоги + ", " + СтрГр.Имя + " КАК " + СтрГр.Имя + Символы.ПС;
				
				Если Найти(СтрГр.Имя, "Субконто") <> 0 Тогда
					Продолжить;			
				КонецЕсли;  				
				
				ТекстГруппировокВыбрать = ТекстГруппировокВыбрать + "БухОбороты." + СтрГр.Имя + " КАК " + СтрГр.Имя + "," + Символы.ПС;
				
			КонецЕсли;	
		КонецЦикла;
		
		// Для каждого ЭлСубконто Из МассивСубконто Цикл
		// 	ТекНомерСубконто = Число(СтрЗаменить(ЭлСубконто, "Субконто", ""))-1;
		// 	СписокГруппировок.Добавить(ЭлСубконто, Субконто.Получить(ТекНомерСубконто).ВидСубконто, Истина);
		// КонецЦикла;   
		
		Для каждого СтрСубконто Из Субконто Цикл
			
			ИмяСубконто = "Субконто" + СтрСубконто.НомерСтроки;
		...
		Для каждого Элемент Из ПостроительОтчета.Отбор Цикл
			...
			Если Врег(Лев(Элемент.ПутьКДанным, 4)) = "СЧЕТ" Тогда
				ТекстОтборСчетов = ТекстОтборСчетов + " И " + бит_БухгалтерскиеОтчетыСервер.ПолучитьСтрокуОтбора(Элемент.ВидСравнения
																				, "&Значение" + Сч
																				, Элемент.ПутьКДанным
																				, "&ЗначениеС" + Сч
																				, "&ЗначениеПо" + Сч
			...
			Иначе
				ТекстОтбор = ТекстОтбор + " И " + бит_БухгалтерскиеОтчетыСервер.ПолучитьСтрокуОтбора(Элемент.ВидСравнения
																				, "&Значение" + Сч
																				, Элемент.ПутьКДанным
																				, "&ЗначениеС" + Сч
																				, "&ЗначениеПо" + Сч
		...
		Текст = Текст + "
		|ИЗ
		|	РегистрБухгалтерии." + ИмяРегистраБухгалтерии + ".ОстаткиИОбороты(&ДатаНач, &ДатаКон, , , " + Сред(ТекстОтборСчетов, 3) + ", &ВидыСубконто, " + Сред(ТекстОтбор, 3) + ТекстДопОтбор +  ") КАК БухОбороты
		| ";
		...
		// Группировки
		Текст = Текст + ТекстГруппировокИтоги;		
...
Процедура ЗаполнитьПараметрыРасшифровки(Область, Выборка, ОтборСубконто = Неопределено, Измерение = Неопределено)
	...
	Если ТекГруппировка = "ЦФО" ИЛИ ТекГруппировка = "Организация" Тогда
