...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	бит_БухгалтерскиеОтчеты.ИзменитьСхемуДляОтборовПоДопСвойствамСубконто(ПараметрыОтчета, Схема, КомпоновщикНастроек);
	
	ЗначениеПараметра = бит_БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПараметрыОтчета.Счет);
	...
	Если Не ПараметрыОтчета.ПоказательМУ Тогда
		...
		Для Каждого ЭлементМассива Из ПараметрыОтчета.НаборПоказателей Цикл
			Если ЭлементМассива <> "МУ" И ПараметрыОтчета["Показатель" + ЭлементМассива] Тогда
	...
	//ТД.УстановитьТекст(ОбщегоНазначения.ЗначениеВСтрокуXML(ПараметрыОтчета.СхемаКомпоновкиДанных));
	//ТД.Записать("\\project-DS713\ProjectsData\БИТ.ФИНАНС\Наши_файлы\ALisitsin\Схема.xml");
	//
	//ТД.УстановитьТекст(ОбщегоНазначения.ЗначениеВСтрокуXML(КомпоновщикНастроек.ПолучитьНастройки()));
	//ТД.Записать("\\project-DS713\ProjectsData\БИТ.ФИНАНС\Наши_файлы\ALisitsin\Настройка.xml");
	//
КонецПроцедуры
...
Процедура ИзменитьСхемуДляОтборовПоСубконтоСубсчетов(ПараметрыОтчета, Схема, КомпоновщикНастроек)
	...
	|	ПланСчетовВидыСубконто_Субсчета.Ссылка КАК Субсчет
	|ИЗ
	...
	|		ПО ПланСчетовВидыСубконто_СчетГруппа.НомерСтроки <> ПланСчетовВидыСубконто_Субсчета.НомерСтроки
	...
	|	И ПланСчетовВидыСубконто_Субсчета.Ссылка В(&Субсчета)
	|	И ПланСчетовВидыСубконто_Субсчета.Ссылка <> &СчетГруппа
	|	И ПланСчетовВидыСубконто_СчетГруппа.НомерСтроки В(&СписокСубконто)";
	...
	ИсточникВидовСубконто = бит_БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(ПараметрыОтчета.Счет);
	НомерСубконто = 0;
	...
	ИменаОбрабатываемыхЗапросов.Добавить("ОстаткиИОбороты");
