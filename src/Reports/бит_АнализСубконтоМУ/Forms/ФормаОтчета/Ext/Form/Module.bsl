...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Отчет.СписокВидовСубконто.ТипЗначения = Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.бит_ВидыСубконтоДополнительные_2");
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(Настройки)
	...
	бит_БухгалтерскиеОтчеты.ЗаполнитьПризнакиУчета(ЭтотОбъект);
...
Процедура ПроверкаСпискаВидовСубконто(Знач СписокВидовСубконто, Отказ)
	...
	// Анализируем, что будет, если добавить еще одно.
...
Процедура СписокВидовСубконтоПриИзмененииСервер(ЭтоНастройкаЭпицентра)
	...
	Иначе
		...
		Если Не Отчет.РежимРасшифровки Тогда
			...
			Если ИзмененПорядокСубконто Тогда
				...
				бит_БухгалтерскиеОтчеты.СохранитьОписаниеНастроек(ЭтотОбъект, ВидСубконтоИсходноеЗначение);
	...
	Если ВидСубконто <> ВидСубконтоИсходноеЗначение И Не ИзмененПорядокСубконто Тогда // прочитать ранее сохраненные настройки
		бит_БухгалтерскиеОтчеты.ПрочитатьОписаниеНастроек(ЭтотОбъект, ВидСубконто);
	Иначе // обновить текущие настройки
		бит_БухгалтерскиеОтчеты.ПрочитатьОписаниеНастроек(ЭтотОбъект, ВидСубконто, Истина);
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ПоказательРегл.Доступность          = Форма.ЕстьСчета;
	Элементы.ПоказательУпр.Доступность           = Форма.ЕстьСчета;
	Элементы.РазвернутоеСальдо.Доступность       = Форма.ЕстьСчета;
	Элементы.ПоказательМУ.Доступность            = Форма.ЕстьСчета;
