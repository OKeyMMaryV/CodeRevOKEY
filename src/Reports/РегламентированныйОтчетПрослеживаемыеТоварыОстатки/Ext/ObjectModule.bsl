...
Процедура ПереопределитьТекстВЭкраннойИлиПечатнойФорме(ТаблДокумент) Экспорт
	...
	Если ТекущаяДатаСеанса() < ДатаПримененияФормыВНовойРедакции() Тогда
		...
		Если Обл <> Неопределено Тогда
			Обл.Текст = "1 - собственник товара, подлежащего прослеживаемости
		...
		Если Обл <> Неопределено Тогда
			...
			+ " представителя собственника товара, подлежащего прослеживаемости:";
		КонецЕсли;
		
		Обл = ТаблДокумент.Области.Найти("ПодписантНаимОрг");
		Если Обл <> Неопределено Тогда
			Обл.Текст = "Наименование организации - представителя собственника товара, подлежащего прослеживаемости";
		КонецЕсли;
