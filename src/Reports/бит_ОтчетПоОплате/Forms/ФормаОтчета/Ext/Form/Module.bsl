...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	пЗначениеПараметраЗапрета = Ложь;
	Если Параметры.Свойство("БлокироватьКмдСформировать", пЗначениеПараметраЗапрета) Тогда  
		
		фЗапретИнтерактивноСформировать = пЗначениеПараметраЗапрета;
	Иначе
		
		фЗапретИнтерактивноСформировать = Ложь;
	КонецЕсли;
	//} bit SVKushnirenko Bit 14.03.2017 #2745
	...
	//{ bit_SVKushnirenko 13.04.2017 #2745
	//ИсходныйКод{
	//бит_МодульЗащиты.ПроверитьВозможностьРаботы(ЭтаФорма, фПолноеИмяОтчета, фОтказ);
	//}ИсходныйКод
	//Удален старый код защиты, который не применяется в этом релизе решения бит.финанс
	//} bit_SVKushnirenko 13.04.2017 #2745
...
Процедура УправлениеВидимостьюДоступностью()
	...
	//{ bit SVKushnirenko Bit 14.03.2017 #2745
	Элементы.КомандаСформировать.Доступность = НЕ фЗапретИнтерактивноСформировать;
	//} bit SVKushnirenko Bit 14.03.2017 #2745
...
Процедура ОбновитьПанельСохраненныхНастроек(Очищать = Ложь, ТекКлючНастройки = Неопределено)
	...
	СтруктураДоступности = бит_ОтчетыСервер.ПроверитьДоступностьВариантовНастроек(фПолноеИмяОтчета, ложь);
