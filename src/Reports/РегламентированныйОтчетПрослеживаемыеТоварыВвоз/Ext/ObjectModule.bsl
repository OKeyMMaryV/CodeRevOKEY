...
Процедура ВывестиНаПечатьЛист(ИмяФормы, СтруктураПараметров, СтруктураРеквизитовФормы, ДанныеРазделов)
	...
	Для Ном = 9 По 15 Цикл
		...
		Если ИмяСекции = "Стр15" Тогда
			...
			ПереопределитьТекстВЭкраннойИлиПечатнойФорме(Секция);
...
Процедура ПереопределитьТекстВЭкраннойИлиПечатнойФорме(ТаблДокумент) Экспорт
	...
	Если ТекущаяДатаСеанса() < ДатаПримененияФормыВНовойРедакции() Тогда
		...
		Если Обл <> Неопределено Тогда
			Обл.Текст = "1 - покупатель
			|2 - представитель покупателя";
		КонецЕсли;
		
		Обл = ТаблДокумент.Области.Найти("ПодписантНаимДок");
		Если Обл <> Неопределено Тогда
			Обл.Текст = "Наименование и реквизиты документа, подтверждающего полномочия представителя покупателя";
		КонецЕсли;
		
		Обл = ТаблДокумент.Области.Найти("ПодписантНаимОрг");
		Если Обл <> Неопределено Тогда
			Обл.Текст = "Наименование организации-представителя покупателя";
		КонецЕсли;
