...
Функция ТекстЗапросаЗаписиРегистраНДСПродажи(СтруктураПараметров)
	...
	//ОКЕЙ Первухин В.С. (ПервыйБИТ) Начало 2021-10-08 (#НТП-МП-03)
	//|		ИНАЧЕ НДСЗаписиКнигиПродаж.СчетФактура
	|		ИНАЧЕ 
	|			ВЫБОР 
	|				КОГДА НДСЗаписиКнигиПродаж.Регистратор ССЫЛКА Документ.КорректировкаРеализации
	|					ТОГДА НДСЗаписиКнигиПродаж.Регистратор
	|				ИНАЧЕ НДСЗаписиКнигиПродаж.СчетФактура
	|			КОНЕЦ
	//ОКЕЙ Первухин В.С. (ПервыйБИТ) Конец 2021-10-08 (#НТП-МП-03)
...
Функция ТекстЗапросаРеквизитыСчетовФактурПостановление981()
	...
	|	СчетаФактуры.СчетФактура ЕСТЬ НЕ NULL 
	|	И ЗаявлениеОВвозеТоваров.Проведен
	//ОКЕЙ Лещенко Д.Ю. (ПервыйБИТ) Начало 2021-05-17 (#НТП-МП-03)
	|	ОБЪЕДИНИТЬ ВСЕ
	|	ВЫБРАТЬ
	|	КорректировкаРеализации.ДокументРеализации,
	|	КорректировкаРеализации.ок_НомерИсходногоСФ,
	|	КорректировкаРеализации.ок_ДатаИсходногоСФ,
	|	КорректировкаРеализации.ДокументРеализации,
	|	НЕОПРЕДЕЛЕНО,
	|	ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка),
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ СписокСчетовФактур КАК СчетаФактуры
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-09-09 (#4347)
	|		ПО КорректировкаРеализации.Ссылка = СчетаФактуры.СчетФактура
	|ГДЕ
	|	СчетаФактуры.СчетФактура ЕСТЬ НЕ NULL 
	|	И КорректировкаРеализации.Проведен
	|	И КорректировкаРеализации.ДокументРеализации ССЫЛКА Документ.бит_ок_ОперацияАксапты
	//ОКЕЙ Лещенко Д.Ю. (ПервыйБИТ) Конец 2021-05-17 (#НТП-МП-03)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетФактура
	|;
