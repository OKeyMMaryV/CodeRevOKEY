...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт 
	...
	Если ПараметрыОтчета.ЭлементРасшифровки <> "" Тогда
		// СофтБаланс Начало Кузнецова С. [16.01.2014] 
		ТекстПериод = "";
		ТекстПредлог = ?(ПараметрыОтчета.ОборотыЗаПериодДт Или ПараметрыОтчета.ОборотыЗаПериодКт Или ПараметрыОтчета.ОборотыСоСчетамиДт Или ПараметрыОтчета.ОборотыСоСчетамиКт, " за ", " на ");
		
		// СофтБаланс Начало Кузнецова С. [20.01.2014] 
		Если ПараметрыОтчета.ОборотыЗаПериодДт Или ПараметрыОтчета.ОборотыЗаПериодКт Или ПараметрыОтчета.ОборотыСоСчетамиДт Или ПараметрыОтчета.ОборотыСоСчетамиКт Тогда
		// СофтБаланс Окончание Кузнецова С. [20.01.2014]
		Если ЗначениеЗаполнено(ПараметрыОтчета.КонецПериода) Тогда 
			Если ПараметрыОтчета.КонецПериода >= ПараметрыОтчета.НачалоПериода Тогда
				ТекстПериод = ТекстПредлог + ПредставлениеПериода(НачалоДня(ПараметрыОтчета.НачалоПериода), КонецДня(ПараметрыОтчета.КонецПериода), "ФП = Истина");
			Иначе
				ТекстПериод = "";
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(ПараметрыОтчета.НачалоПериода) И Не ЗначениеЗаполнено(ПараметрыОтчета.КонецПериода) Тогда
			ТекстПериод = ТекстПредлог + ПредставлениеПериода(НачалоДня(ПараметрыОтчета.НачалоПериода), КонецДня(Дата(3999, 11, 11)), "ФП = Истина");
			ТекстПериод = СтрЗаменить(ТекстПериод, Сред(ТекстПериод, Найти(ТекстПериод, " - ")), " - ...");
		КонецЕсли;
		// СофтБаланс Начало Кузнецова С. [20.01.2014] 
		ИначеЕсли (ПараметрыОтчета.СальдоНаНачалоДт Или ПараметрыОтчета.СальдоНаНачалоКт) И Не (ПараметрыОтчета.СальдоНаКонецДт Или ПараметрыОтчета.СальдоНаКонецКт) Тогда
			ТекстПериод = ТекстПредлог + " " + Формат(ПараметрыОтчета.НачалоПериода, "ДФ=dd.MM.yyyy") + " г.";
		ИначеЕсли (ПараметрыОтчета.СальдоНаКонецДт Или ПараметрыОтчета.СальдоНаКонецКт) И Не (ПараметрыОтчета.СальдоНаНачалоДт Или ПараметрыОтчета.СальдоНаНачалоКт) Тогда
			ТекстПериод = ТекстПредлог + " " + Формат(ПараметрыОтчета.КонецПериода, "ДФ=dd.MM.yyyy") + " г.";
		Иначе // сальдо начальное + сальдо конечное
			Для Каждого Показатель Из ПараметрыОтчета.МассивПоказателейДляОтбора Цикл
				Если Лев(Показатель, 14) = "СальдоНаНачало" Тогда
					ТекстПериод = ТекстПредлог + " " + Формат(ПараметрыОтчета.НачалоПериода, "ДФ=dd.MM.yyyy") + " г.";
				ИначеЕсли Лев(Показатель, 13) = "СальдоНаКонец" Тогда
					ТекстПериод = ТекстПредлог + " " + Формат(ПараметрыОтчета.КонецПериода, "ДФ=dd.MM.yyyy") + " г.";
				Иначе
					ТекстПериод = ТекстПредлог + " " + Формат(ПараметрыОтчета.НачалоПериода, "ДФ=dd.MM.yyyy") + " г." + " - " + Формат(ПараметрыОтчета.КонецПериода, "ДФ=dd.MM.yyyy") + " г.";
				КонецЕсли;
			КонецЦикла;
		КонецЕсли; 
		// СофтБаланс Окончание Кузнецова С. [20.01.2014]
		
		ЗаголовокОтчета = ПараметрыОтчета.ЭлементРасшифровки + ТекстПериод;
		// СофтБаланс Окончание Кузнецова С. [16.01.2014]
	Иначе
	// СофтБаланс Окончание Кузнецова С. [15.01.2014]
	ЗаголовокОтчета = "Обороты счета " + ПараметрыОтчета.Счет + " (международный) " + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(ПараметрыОтчета.НачалоПериода, ПараметрыОтчета.КонецПериода);
	// СофтБаланс Начало Кузнецова С. [15.01.2014] 
	КонецЕсли;
	// СофтБаланс Окончание Кузнецова С. [15.01.2014]
...
Функция ПолучитьНаборПоказателей() Экспорт
	...
	НаборПоказателей.Добавить("СуммаРегл");
	НаборПоказателей.Добавить("СуммаУпр");
	НаборПоказателей.Добавить("СуммаМУ");
...
Процедура ДобавитьОтборПоВыводимымПолям(ВыводимыеПоля, Структура, ВыводитьОтбор = Истина)
	...
	ПропускатьКонтроль = ?(ВыводимыеПоля.Количество() > 0, НЕ Найти(ВыводимыеПоля[0], ".Контроль"), Истина);
