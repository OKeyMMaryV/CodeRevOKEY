...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.Вставить("КрупнейшийНалогоплательщик", ОрганизацияКрупнейшийНалогоплательщик());
...
Процедура ОбработкаВыбораИФНСЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено Тогда
		...
		//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2021-03-10 (#4068)
		Если СтруктураРеквизитовФормы.Свойство("КрупнейшийНалогоплательщик")
			И СтруктураРеквизитовФормы.КрупнейшийНалогоплательщик = Истина Тогда
...
Процедура Проверить(Команда)
	...
	|в редакции приказа ФНС России от 09.12.2020
	|№ КЧ-7-21/889@ будет реализована в будущих
	|версиях программы'"));
...
Процедура ЗаполнитьСведенияОбОрганизацииНаСервере(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		Если СтруктураРеквизитовФормы.Свойство("КрупнейшийНалогоплательщик")
			...
			Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.РегистрацияВНалОргане) Тогда
				ок_СписокРеквизитовНалОргане = "КПП";
				ок_СтруктураРеквизитовНалОргане = ОК_ОбщегоНазначения.ПолучитьЗначенияРеквизитов(СтруктураРеквизитовФормы.РегистрацияВНалОргане, ок_СписокРеквизитовНалОргане);
				ТабличныйДокумент.Области["КПП"].Значение = СокрЛП(ок_СтруктураРеквизитовНалОргане.КПП);
			КонецЕсли;
