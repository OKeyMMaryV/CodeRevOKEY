...
Функция СформироватьПустоеДеревоСтраниц(ВыбраннаяФорма)
	...
	Стр001.ИмяМакета = ?(ВыбраннаяФорма = "Форма2018_1", "Титульная_2018", "Титульная");
	Стр001.Многостраничность = Ложь;
	Стр001.Многострочность = Ложь;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "Титульная";
	Стр001.МакетыПФ = ?(ВыбраннаяФорма = "Форма2018_1", "Печать_Форма2018_1_Титульная", "Печать_Форма2017_1_Титульная");
	
	Стр001 = Дерево.Строки.Добавить();
	Стр001.Наименование = "Сведения о физическом"+символы.ПС+"лице, не являющимся"+символы.ПС+"индивидуальным предпринимателем";
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = ?(ВыбраннаяФорма = "Форма2018_1", "Лист002_2018", "Лист002");
	Стр001.Многостраничность = Ложь;
	Стр001.Многострочность = Ложь;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "Лист002";
	Стр001.МакетыПФ = ?(ВыбраннаяФорма = "Форма2018_1", "Печать_Форма2018_1_Лист002", "Печать_Форма2017_1_Лист002");
	
	Если ВыбраннаяФорма = "Форма2018_1" Тогда 
...
Процедура СоздатьЗаявлениеОЗачетеНалога(ВыборкаСтрока) Экспорт
	Попытка
		...
		Если ВыборкаСтрока.ВыбраннаяФорма <> "" И ВыборкаСтрока.ВыбраннаяФорма <> "" Тогда 
			Возврат;
		КонецЕсли;
		...
		СтруктураПараметров.Вставить("ДеревоСтраниц", СформироватьПустоеДеревоСтраниц(ВыборкаСтрока.ВыбраннаяФорма));
		...
		Если Уведомление.ИмяФормы = "Форма2018_1" Тогда
			ДанныеМногостраничныхРазделов = Новый Структура("Свед", Новый СписокЗначений);
			СтрокиСвед = СтруктураПараметров.ДеревоСтраниц.Строки[СтруктураПараметров.ДеревоСтраниц.Строки.Количество() - 1].Строки;
			Пока СтрокиСвед.Количество() < ДанныеОтчета.ДанныеМногостраничныхРазделов.Свед.Количество() Цикл 
				СтрокиСвед.Добавить();
			КонецЦикла;
			
			Инд = 0;
			Для Каждого СтрСвед Из ДанныеОтчета.ДанныеМногостраничныхРазделов.Свед Цикл 
				СтрСведСтр = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтрСвед);
				СтрСведСтр.Вставить("УИД", Новый УникальныйИдентификатор);
				СтрокиСвед[Инд].УИД = СтрСведСтр.УИД;
				Инд = Инд + 1;
			КонецЦикла;
