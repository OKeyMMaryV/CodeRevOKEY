...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	//СофтЛаб Конаков Ю.М. Начало 13.11.2020 
	ОтчетТабличныйДокументОбработкаРасшифровкиДополнительная(Расшифровка, СтандартнаяОбработка);
	//СофтЛаб Конаков Ю.М. Конец 13.11.2020 
КонецПроцедуры
...
//СофтЛаб Конаков Ю.М. Начало 13.11.2020 
&НаКлиенте
Процедура ОтчетТабличныйДокументОбработкаРасшифровкиДополнительная(Расшифровка, СтандартнаяОбработка);
	
	Если ТипЗнч(Расшифровка) = Тип("ИдентификаторРасшифровкиКомпоновкиДанных") Тогда 
		
		ДанныеДополнительнойРасшифровки = ПолучитьДанныеДополнительнойРасшифровки(Расшифровка);
		
		Если ЗначениеЗаполнено(ДанныеДополнительнойРасшифровки.ВидРасшифровки) Тогда 
			
			СтандартнаяОбработка = Ложь;
			
			РезультатОбработкаРасшифровки(ДанныеДополнительнойРасшифровки);
...
&НаСервере
Функция ПолучитьДанныеДополнительнойРасшифровки(Расшифровка)
	...
	ПолеОсновноеСредство = Неопределено;
	
	ДанныеРасшифровки = ПолучитьИзВременногоХранилища(ОтчетДанныеРасшифровки);
	
	ПараметрПериода = ДанныеРасшифровки.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Период"));
	
	Если Не ПараметрПериода = Неопределено Тогда 
	...
	Группировки = ДанныеРасшифровки.Элементы.Получить(Расшифровка).ПолучитьРодителей();
	...
	Для Каждого Элемент Из Группировки Цикл
		...
		ПоляГруппировки = Элемент.ПолучитьПоля();
		
		ПолеОсновноеСредство = ПоляГруппировки.Найти("ОсновноеСредство");
		
		Если Не ПолеОсновноеСредство = Неопределено Тогда 
			
			ОС = ПолеОсновноеСредство.Значение;
			
		КонецЕсли;
		
		
		ПолеОрганизация = ПоляГруппировки.Найти("Организация");
		
		Если Не ПолеОрганизация = Неопределено Тогда 
			
			Организация = ПолеОрганизация.Значение;
	...
	Если Не Поля.Найти("УвеличениеПервоначальнойСтоимости") = Неопределено Тогда 
		
		ВидРасшифровки = "УвеличениеПервоначальнойСтоимости";
		
	ИначеЕсли Не Поля.Найти("ИзменениеВосстановительнойСтоимости") = Неопределено Тогда
		
		ВидРасшифровки = "ИзменениеВосстановительнойСтоимости";
		
	ИначеЕсли Не Поля.Найти("АмортизацияНачисленная") = Неопределено Тогда
		
		ВидРасшифровки = "АмортизацияНачисленная";
		
	ИначеЕсли Не Поля.Найти("ВыбытиеПоПервоначальнойСтоимости") = Неопределено Тогда
		
		ВидРасшифровки = "ВыбытиеПоПервоначальнойСтоимости";
		
	ИначеЕсли Не Поля.Найти("ВыбытиеПоВосстановительнойСтоимости") = Неопределено Тогда
		
		ВидРасшифровки = "ВыбытиеПоВосстановительнойСтоимости";
		
	ИначеЕсли Не Поля.Найти("АмортизацияПоВыбывшимОбъектам") = Неопределено Тогда
		
		ВидРасшифровки = "АмортизацияПоВыбывшимОбъектам";
		
	ИначеЕсли Не Поля.Найти("ПрочееДвижениеПервоначальнаяСтоимость") = Неопределено Тогда
		
		ВидРасшифровки = "ПрочееДвижениеПервоначальнаяСтоимость";
		
	ИначеЕсли Не Поля.Найти("ПрочееДвижениеАмортизация") = Неопределено Тогда
		
		ВидРасшифровки = "ПрочееДвижениеАмортизация";
		
	КонецЕсли;
	
	Если ПолеОсновноеСредство = Неопределено Тогда 
		ВидРасшифровки = Неопределено;
	КонецЕсли;
	
	Возврат Новый Структура("НачалоПериода, КонецПериода, ОсновноеСредство, ОтборОрганизация, ВидРасшифровки", НачалоПериода, КонецПериода, ОС, Организация, ВидРасшифровки);
	
КонецФункции

&НаКлиенте
Процедура СкопироватьОтборПоОрганизации(НовыйОтбор, СуществующийОтбор, ОтборовПоОрганизации, ЗначениеОрганизации)
	
	Для Каждого Элемент Из СуществующийОтбор Цикл
		
		Если ТипЗнч(Элемент) = Тип("ЭлементОтбораКомпоновкиДанных")
			И Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Организация") И Элемент.Использование Тогда 
			
			НовыйЭлементОтбора = НовыйОтбор.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			ЗаполнитьЗначенияСвойств(НовыйЭлементОтбора, Элемент, "ЛевоеЗначение, Использование, ВидСравнения, ПравоеЗначение");
			
			ОтборовПоОрганизации = ОтборовПоОрганизации + 1;
			
			Если Элемент.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Тогда 
				
				ЗначениеОрганизации = Элемент.ПравоеЗначение;
				
			КонецЕсли;			
			
		ИначеЕсли ТипЗнч(Элемент) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда 
			
			НоваяГруппа = НовыйОтбор.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
			
			ЗаполнитьЗначенияСвойств(НоваяГруппа, Элемент);
			
			СкопироватьОтборПоОрганизации(НоваяГруппа.Элементы, Элемент.Элементы, ОтборовПоОрганизации, ЗначениеОрганизации);
			
			Если НоваяГруппа.Элементы.Количество() = 0 Тогда 
				
				НовыйОтбор.Удалить(НоваяГруппа);
...
&НаКлиенте
Процедура РезультатОбработкаРасшифровки(ДанныеРасшифровки)
	
	ПараметрыФормыРасшифровки = Новый Структура("СформироватьПриОткрытии", Ложь);
	
	ФормаРасшифровки = ПолучитьФорму("Отчет.СБ_ОтчетПоПроводкамМеждународный.Форма", ПараметрыФормыРасшифровки, , Новый УникальныйИдентификатор);
	
	КомпоновщикРасшифровки = ФормаРасшифровки.Отчет.КомпоновщикНастроек;
	
	ФормаРасшифровки.Отчет.ПоказательСуммаУпр = Истина;
	ФормаРасшифровки.ИспользуетсяНесколькоОрганизаций = Истина;
	
	ФормаРасшифровки.Отчет.Организация = ДанныеРасшифровки.ОтборОрганизация;
	
	ЗаполнитьЗначенияСвойств(ФормаРасшифровки.Отчет, ДанныеРасшифровки, "НачалоПериода, КонецПериода");
	
	ПользовательскиеНастройкиРасшифровки = ПолучитьИзВременногоХранилища(ФормаРасшифровки.ПользовательскиеНастройки);
	
	Для Каждого Элемент Из ПользовательскиеНастройкиРасшифровки.Элементы Цикл
		
		Если Элемент.ИдентификаторПользовательскойНастройки = "Отбор" Тогда 
			
			Отбор = Элемент;
			
			Отбор.Элементы.Очистить();
						
			//СкопироватьОтборПоОрганизации(Отбор.Элементы, ДанныеРасшифровки.ОтборОрганизация.Элементы);
			
			
			Если ДанныеРасшифровки.ВидРасшифровки = "УвеличениеПервоначальнойСтоимости" Тогда 
				
				//Счет Дт в группе 111 И Счет Дт не равно 11120 И Счет Кт в группе из списка: 122, 121,123.
				//ИЛИ
				//Счет Дт равно 11120
				//ИЛИ
				//Счет Дт равно ПК И Счет Кт в иерархии 111
				...
				Условие = ГруппаИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетДт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.ВИерархии;
				Условие.ПравоеЗначение = ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.ОС");
				
				
				Условие = ГруппаИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетДт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
				Условие.ПравоеЗначение = ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.ОсновныеСредстваДо40");
				
				
				СписокКт = Новый СписокЗначений;
				СписокКт.Добавить(ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.НезавершенноеСтроительствоЗемля"));
				СписокКт.Добавить(ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.НезавершенноеСтроительствоЗдания"));
				СписокКт.Добавить(ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.НезавершенноеСтроительствоОборудование"));
				
				Условие = ГруппаИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетКт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии;
				Условие.ПравоеЗначение = СписокКт;
				
				
				Условие = ГруппаИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СубконтоДт1");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
				Условие.ПравоеЗначение = ДанныеРасшифровки.ОсновноеСредство;
				...
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетДт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
				Условие.ПравоеЗначение = ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.ПромежуточныйСчетДляКонсолидации");
				...
				Условие = ГруппаИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СубконтоКт1");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
				Условие.ПравоеЗначение = ДанныеРасшифровки.ОсновноеСредство;
				
			ИначеЕсли ДанныеРасшифровки.ВидРасшифровки = "ИзменениеВосстановительнойСтоимости" Тогда 
				
				//Счет Дт в группе 113 И Счет Кт не равно 11199
				//ИЛИ
				//Счет Кт в группе 114 И Счет Дт не равно 11199
				//ИЛИ
				//Счет Кт в группе 113 И Счет Дт не равно 11199
				//ИЛИ
				//Счет Дт в группе 114 И Счет Кт не равно 11199
				ГруппаИли = Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
				ГруппаИли.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
				...
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетДт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.ВИерархии;
				Условие.ПравоеЗначение = ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.ИнвестиционнаяСобственность");
				
				Условие = ГруппаИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетКт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
				Условие.ПравоеЗначение = ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.ОСПредназначенныеДляПродажи");
				...
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетКт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.ВИерархии;
				Условие.ПравоеЗначение = ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.ОбесценениеОС");
			...
			ИначеЕсли ДанныеРасшифровки.ВидРасшифровки = "АмортизацияНачисленная" Тогда 
				
				//Счет Дт в группе 722 И Счет Кт в группе 112
				//ИЛИ
				//Счет Дт в группе 112 И Счет Кт равно ПК
				ГруппаИли = Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
				ГруппаИли.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
				...
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетДт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.ВИерархии;
				Условие.ПравоеЗначение = ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.АмортизацияОСиНМА");
				
				Условие = ГруппаИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетКт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.ВИерархии;
				Условие.ПравоеЗначение = ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.АмортизацияОС");
				...
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетКт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
				Условие.ПравоеЗначение = ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.ОсновныеСредстваДо40НакопленнаяАмортизация");
			...
			ИначеЕсли ДанныеРасшифровки.ВидРасшифровки = "ВыбытиеПоПервоначальнойСтоимости" Тогда 
				
				//Счет Дт равно 11199 И Счет Кт в группе 111
				Условие = Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетДт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
				Условие.ПравоеЗначение = ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.ОСПредназначенныеДляПродажи");
			...
			ИначеЕсли ДанныеРасшифровки.ВидРасшифровки = "ВыбытиеПоВосстановительнойСтоимости" Тогда 
				
				
				//Счет Дт в группе 114 и Счет Кт равно 11199 
				//ИЛИ
				//Счет Кт в группе 113 и счет Дт равно 11199
				ГруппаИли = Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
				ГруппаИли.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
			...
			ИначеЕсли ДанныеРасшифровки.ВидРасшифровки = "АмортизацияПоВыбывшимОбъектам" Тогда 
				
				//Счет Дт в группе 112 И Счет Кт равно11199
				Условие = Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетДт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.ВИерархии;
				Условие.ПравоеЗначение = ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.АмортизацияОС");
			...
			ИначеЕсли ДанныеРасшифровки.ВидРасшифровки = "ПрочееДвижениеПервоначальнаяСтоимость" Тогда 
				
				//(Счет Дт в группе 111 И Счет Дт не равно 11120 И Счет Кт не в группе из списка: 122, 121, 123)
				//ИЛИ
				//(Счет Дт не равно 11199 И Счет Кт в группе 111)
				ГруппаИли = Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
				ГруппаИли.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
				...
				Условие = ГруппаИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетКт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии;
				Условие.ПравоеЗначение = СписокКт;
				...
				СписокДт.Добавить(ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.ОСПредназначенныеДляПродажи"));
				СписокДт.Добавить(ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.ПромежуточныйСчетДляКонсолидации"));
				
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСписке;
				Условие.ПравоеЗначение = СписокДт;
			...
			ИначеЕсли ДанныеРасшифровки.ВидРасшифровки = "ПрочееДвижениеАмортизация" Тогда 
				
				//(Счет Дт в группе 112 И Счет Кт не равно 11199)
				//ИЛИ
				//(Счет Дт не в группе 722 И Счет Кт в группе 112)
				ГруппаИли = Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
				ГруппаИли.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
				...
				СписокКт = Новый СписокЗначений;
				СписокКт.Добавить(ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.ОСПредназначенныеДляПродажи"));
				СписокКт.Добавить(ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.ПромежуточныйСчетДляКонсолидации"));
				...
				Условие = ГруппаИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				Условие.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СчетДт");
				Условие.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВИерархии;
				Условие.ПравоеЗначение = ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.АмортизацияОСиНМА");
	...
	ФормаРасшифровки.ПользовательскиеНастройки = ПоместитьВоВременноеХранилище(ПользовательскиеНастройкиРасшифровки, ФормаРасшифровки.УникальныйИдентификатор);
