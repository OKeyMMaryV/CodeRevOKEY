...
Процедура ЗаполнитьПериодОтчета(Форма)
	...
	Если ПоМесту = "124" Тогда
		...
		КонецЕсли;
	Иначе
		Если (ЗначениеЗаполнено(КодФормыРеорганизации) И КодФормыРеорганизации <> "9")
			ИЛИ (ПоМесту = "240" И МесяцОкончания < 12) Тогда
...
Процедура ОчиститьТекущуюСтраницуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1Прил9" Тогда
			// Особая обработка текста в строке 020 Приложения 9 к Разделу 1.
			ТабличныйДокумент.Области.П00019М102001_1.Значение = "001";
