...
// BIT AMerkulov 20-10-2015 ++
Перем СуммаКонКт Экспорт;
Перем СуммаКонДт Экспорт;
// BIT AMerkulov 20-10-2015--
...
Функция СформироватьОтчет(ДокументРезультат,  ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	Иначе
		...
		|	Таблица.КорСчет.Представление КАК КорСчетПредставление," + ТекстКорСубконто + "";
		...
		Если   ТипЗнч(Организация) = Тип("СправочникСсылка.бит_му_ПериметрыКонсолидации") и
			...
			Для Каждого Строка из ТаблицаОрганизаций цикл
				...
				//ТекстОтбор = ТекстОтбор + " И Организация В (&Организация)";
				ТекстОтбор = СтрЗаменить(ТекстОтбор,"Организация В(&Организация )","Организация = &Организация"+Сч);
				Ном = Сч -1;
				ТекстОтбор = СтрЗаменить(ТекстОтбор,"Организация = &Организация"+Ном ,"Организация = &Организация"+Сч);	
...
Процедура ВыводСубконто(Знач ВыборкаОбороты, Знач ВыборкаОстатки, Знач Инд, СтруктураПараметров, ОтборДляРасшифровки)
	...
	Если Инд < СтруктураПараметров.МассивСубконто.Количество() Тогда
		...
		Пока Истина Цикл
			...
			// BIT AMerkulov 20-10-2015 ++
			Если  СуммаКонКт <> Неопределено ИЛИ СуммаКонДт <> Неопределено  Тогда
				ВыводимаяОбласть.Параметры.СуммаНачКт = СуммаКонКт;
				ВыводимаяОбласть.Параметры.СуммаНачДт = СуммаКонДт;
			КонецЕсли;
			// BIT AMerkulov 20-10-2015 --
	...
	Если ЗначениеЗаполнено(Периодичность) Тогда
		...
		Пока Истина Цикл
            ...
            Если ОстатокИспользован Тогда                 
                ...
                бит_БухгалтерскиеОтчетыСервер.ЗаполнитьЗначениеПараметровОбластиПоМассивуРесурсов(ВыводимаяОбласть, ВыборкаОстатки, СтруктураПараметров.МассивРесурсов, "КонДт, КонКт");
				// BIT AMerkulov 20-10-2015 ++
				СуммаКонКт = ВыводимаяОбласть.Параметры.СуммаКонКт;
				СуммаКонДт = ВыводимаяОбласть.Параметры.СуммаКонДт;
				// BIT AMerkulov 20-10-2015 --
            КонецЕсли;
...
Процедура ЗаполнитьПараметрыРасшифровки(Область, Выборка, ОтборСубконто = Неопределено)
    ...
    ПараметрыРасшифровки.Вставить("РегистрБухгалтерии", РегистрБухгалтерии);
	
	Если мЕстьРесурсСуммаМУ Тогда
		ПараметрыРасшифровки.Вставить("ВыводитьСуммуМУ", ВыводитьСуммуМУ);
	КонецЕсли;
...
// Процедура очищает параметры области.
// 
// Параметры:
//  Область
// 
