...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт 
	...
	Возврат "Плановые и фактические ДДС";
	
КонецФункции
//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2019-01-24 (#3062, #3197)
//Процедура ДоработатьНастройкиКомпоновкиДляРасшифровки(КомпоновщикНастроек, МассивРасшифровки)
//Заменено на:
Процедура ДоработатьНастройкиКомпоновкиДляРасшифровки(КомпоновщикНастроек, МассивРасшифровки, Группировка)
//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2019-01-24 (#3062, #3197)
	...
	//Заменено на:
	Для Каждого ЭлГруппировка Из Группировка Цикл 
		Если ЭлГруппировка.Использование Тогда 
			ДобавитьГруппировку(КомпоновщикНастроек, ЭлГруппировка.Поле);
		КонецЕсли;
	КонецЦикла;	
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2019-01-24 (#3062, #3197)	
	...
	Для каждого ЭлементРасшифровки Из МассивПолейРасшифровки Цикл
		...
		Если НЕ ЗначениеЗаполнено(Значение) Тогда
			БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(КомпоновщикНастроек, Поле, Значение, ВидСравненияКомпоновкиДанных.НеЗаполнено);
		Иначе
...
Функция ДобавитьГруппировку(КомпоновщикНастроек, Знач Поле = Неопределено, Строки = Истина)
	...
	Если Поле <> Неопределено Тогда
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2019-01-24 (#3062, #3197)
		Если ТипЗнч(Поле) = Тип("Строка") Тогда 
			мПолей = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Поле,",");
			Для Каждого ЭлМассива Из мПолей Цикл 
				ПолеГруппировки = НоваяГруппировка.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
				ПолеГруппировки.Поле = Новый ПолеКомпоновкиДанных(ЭлМассива);
			КонецЦикла;
		Иначе
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2019-01-24 (#3062, #3197)	
			ПолеГруппировки = НоваяГруппировка.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Если ПараметрыОтчета.Свойство("МассивПолейРасшифровки") И ПараметрыОтчета.МассивПолейРасшифровки <> Неопределено Тогда
		...
		//Заменено на:
		Если ПараметрыОтчета.НазначениеПлатежа Тогда 
			ЭлДокумент = ПараметрыОтчета.Группировка.Найти("Документ");
			Если ЭлДокумент<>Неопределено Тогда 
				ЭлДокумент.Поле = "Документ,НазначениеПлатежа";
			КонецЕсли;			
		КонецЕсли;
		ДоработатьНастройкиКомпоновкиДляРасшифровки(КомпоновщикНастроек, ПараметрыОтчета.МассивПолейРасшифровки, ПараметрыОтчета.Группировка);	
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2019-01-24 (#3062, #3197)		
	КонецЕсли;
	...
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2019-02-19 (#3237)
	...
	//Заменено на:
	мСценарии = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("ОперативныйСценарийПланирования,ок_ОперационныйСценарийПланирования,ок_ИнвестиционныйСценарийПланирования",",");	
	пСписокСценариев = Новый СписокЗначений;
	Для Каждого пИмяНастройкиСценария Из мСценарии Цикл 		
		пСценарий = СБ_КазначействоПовтИсп.ПолучитьЗначениеКонстанты(пИмяНастройкиСценария);
		Если ЗначениеЗаполнено(пСценарий) Тогда 
			пСписокСценариев.Добавить(пСценарий);
		КонецЕсли;
	КонецЦикла;	
	ПараметрыДанных.УстановитьЗначениеПараметра("Сценарий", 				пСписокСценариев);
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2019-02-19 (#3237)
	...
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "Группировка", ВариантГруппировки);
	...
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2019-01-24 (#3062, #3197)
	Алгоритмы = ПрочитатьАлгоритмыПоказателей(ПараметрыОтчета.ВидОтчета = 0);
	Алгоритмы.Свернуть("Алгоритм");
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "АлгоритмыПроверки", Алгоритмы.ВыгрузитьКолонку("Алгоритм"));
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2019-01-24 (#3062, #3197)
		
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2019-02-19 (#3237)
	//Если ВидОтчета = 0 Тогда
	//	//Это план ДДС
	//	//ОКЕЙ Наумов А.Н.(СофтЛаб) Начало 2019-01-24 (#3062, #3197)
	//	//ПроверитьРеестрыПлатежей(Период, Организация);
	//	ПроверитьРеестрыПлатежей(Период, Организация, ПараметрыОтчета.РеестрПлатежей);
	//	//ОКЕЙ Наумов А.Н.(СофтЛаб) Конец 2019-01-24 (#3062, #3197)
	//КонецЕсли; 
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2019-02-19 (#3237)
	...
	ВнешниеНаборыДанных.Вставить("Алгоритмы", 				ПрочитатьАлгоритмыПоказателей(ВидОтчета = 0));
	ВнешниеНаборыДанных.Вставить("ОстаткиПоОвердрафту", 	ПрочитатьОстаткиПоОвердафту(ПараметрыОтчета.ОстаткиПоОвердрафту));
	
	////ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2019-01-24 (#3062, #3197)


	//Для Каждого ЭлементОбъединения Из МакетКомпоновки.НаборыДанных.Данные.Элементы Цикл  
	//	Если ЭлементОбъединения.Имя = "План_Поступления" ИЛИ
	//		 ЭлементОбъединения.Имя = "План_Выплаты" ИЛИ
	//		 ЭлементОбъединения.Имя = "Факт_Обороты"
	//	Тогда 
	//	
	//		ЭлементОбъединения.Запрос = СтрЗаменить(ЭлементОбъединения.Запрос,""""" КАК Алгоритм","АлгоритмыДляПроверки.Алгоритм КАК Алгоритм");
	//		ЭлементОбъединения.Запрос = СтрЗаменить(ЭлементОбъединения.Запрос,"ПОМЕСТИТЬ АлгоритмыДляПроверки",
	//																		  "ПОМЕСТИТЬ АлгоритмыДляПроверки
	//																		  |ИЗ
	//																		  |		&АлгоритмыПроверки КАК АлгоритмыДляПроверки");		
	//		парАлгоритмы = ЭлементОбъединения.ЗначенияПараметров.Добавить();
	//		парАлгоритмы.Имя = "АлгоритмыПроверки";
	//		парАлгоритмы.Значение = Алгоритмы;
	//		
	//	КонецЕсли; 		
	//КонецЦикла;	
	////ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2019-01-24 (#3062, #3197)	
	...
	//ОКЕЙ Наумов А.Н.(СофтЛаб) Начало 2019-01-24 (#3062, #3197)
	// Добавляем отбор по реестру.
	Если ЗначениеЗаполнено(ПараметрыОтчета.РеестрПлатежей) Тогда
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(КомпоновщикНастроек, "РеестрПлатежей", ПараметрыОтчета.РеестрПлатежей);
		//ОтборПоРееструПлатежей = КомпоновщикНастроек.Настройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		//ОтборПоРееструПлатежей.ЛевоеЗначение 		= Новый ПолеКомпоновкиДанных("РеестрПлатежей");
		//ОтборПоРееструПлатежей.ВидСравнения 		= ВидСравненияКомпоновкиДанных.Равно;
		//ОтборПоРееструПлатежей.ПравоеЗначение 		= РеестрПлатежей;
		//ОтборПоРееструПлатежей.Использование 		= Истина;
		//ОтборПоРееструПлатежей.РежимОтображения 	= РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
	КонецЕсли;	
	//ОКЕЙ Наумов А.Н.(СофтЛаб) Конец 2019-01-24 (#3062, #3197)
...
Процедура ПослеВыводаРезультата(ПараметрыОтчета, Результат) Экспорт
	...
	ОбъединитьЯчейкиПоЯкорю(Результат);
	УдалитьПустуюКолонку(Результат);
	
КонецПроцедуры

Процедура НастроитьВариантыОтчета(Настройки, ОписаниеОтчета) Экспорт
	
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "СБ_ПланИФактДДС").Размещение.Вставить(Метаданные.Подсистемы.Руководителю.Подсистемы.ДенежныеСредства, "");
...
Функция ВариантыНастроек() Экспорт
	...
	Массив.Добавить(Новый Структура("Имя, Представление","СБ_ПланИФактДДС", "Плановые и фактические ДДС"));
...
Функция ПрочитатьАлгоритмыПоказателей(ПлановыеДанные)
	...
	Алгоритмы.Колонки.Добавить("Депозит");
	...
	Для НомерПрохода = 0 По 4 Цикл
		...
		Пока Истина Цикл
			...
			Депозит			= Макет.Область(НомерСтроки, 10).Текст;
			...
			Если ПустаяСтрока(Алгоритм) И ТекущийПриоритет <> "" Тогда
				...
				Для каждого АлгоритмПоказателяСтрока Из НайденныеАлгоритмы Цикл
					...
					АлгоритмСтрока.Депозит				= Депозит;
...
Процедура ПроверитьРеестрыПлатежей(Период, Организация, Реестр = Неопределено)
//ОКЕЙ Наумов А.Н.(СофтЛаб) Конец 2019-01-24 (#3062, #3197)
	Запрос = Новый Запрос;
	//ОКЕЙ Наумов А.Н.(СофтЛаб) Начало 2019-01-24 (#3062, #3197)
	//Запрос.Текст = 
	...
	Если ЗначениеЗаполнено(Реестр) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//ОтборПоДокументу", "И бит_РеестрПлатежей.Ссылка = &Реестр");
