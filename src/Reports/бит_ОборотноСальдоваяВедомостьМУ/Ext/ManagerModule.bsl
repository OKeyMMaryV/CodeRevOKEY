...
Процедура ПроверитьКорректностьИтогов(ПараметрыОтчета, Результат) Экспорт
	...
	Если КонтрольноеСоотношение = 0
	   И ПараметрыОтчета.ПоказательМУ Тогда
...
// Заполняет настройки расшифровки для переданного экземпляра отчета
//  (см. БухгалтерскиеОтчетыКлиентСервер.НовыйУниверсальныеНастройки).
//
// Параметры:
//  Настройки				 - Структура								
//     - Настройки расшифровки отчета, которые нужно заполнить (см. БухгалтерскиеОтчетыКлиентСервер.НовыйУниверсальныеНастройки).
...
Процедура ЗаполнитьНастройкиРасшифровки(Настройки, Объект, ДанныеРасшифровки, ИдентификаторРасшифровки, РеквизитыРасшифровки) Экспорт
	...
	Если Настройки.Свойство("Счет", Счет) Тогда
		...
		Если СтрокаГруппировкиПоСчету <> Неопределено Тогда
			...
			бит_БухгалтерскиеОтчетыРасшифровка.ПоменятьНумерациюСубконтоВОтборе(Настройки, Счет, КартаВидовСубконто);
...
Процедура ЗаполнитьПравилаРасшифровки(Правила) Экспорт
	...
	Правило.Отчет = "бит_ОборотноСальдоваяВедомостьПоСчетуМУ";
	...
	Правило = Правила.Добавить();
	Правило.Отчет = "бит_АнализСчетаМУ";
	БухгалтерскиеОтчетыРасшифровка.ДобавитьТребуемыйРеквизитРасшифровки(Правило.ТребуемыеРеквизиты, "Счет", "Счет");
	Правило.ШаблонПредставления = НСтр("ru = 'Анализ счета [Счет]'");
	
	Правило = Правила.Добавить();
	Правило.Отчет = "бит_ОборотыСчетаМУ";
	БухгалтерскиеОтчетыРасшифровка.ДобавитьТребуемыйРеквизитРасшифровки(Правило.ТребуемыеРеквизиты, "Счет", "Счет");
...
Процедура СформироватьСтруктуруОтчета(КомпоновщикНастроек, ПараметрыОтчета, ПараметрыАлгоритма)
	...
	// -- NBБухгалтерскиеОтчетыВызовСервера.ДобавитьОтборДляПоказателяКонтроль(ПараметрыОтчета, КомпоновщикНастроек);
	...
	//Поле.Поле = Новый ПолеКомпоновкиДанных("Показатели.МУ");
	//Поле = УсловноеОформление.Поля.Элементы.Добавить();
	//Поле.Поле = Новый ПолеКомпоновкиДанных("Показатели.Упр");
	//Поле = УсловноеОформление.Поля.Элементы.Добавить();
...
Функция СчетаССовпадающимНаборомСубконто(СчетаГруппы, СубконтоДетализацииПоСчетам);
	...
	|	ПланСчетовВидыСубконто.Ссылка
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(ПланСчетовВидыСубконто.Ссылка) = &[КоличествоСубконто]";
	...
	Для Каждого ИсключенныйСчет Из СчетаГруппы Цикл
		...
		Если бит_БухгалтерскийУчетПовтИсп.СчетаВИерархии(Счет).Количество() > 1 Тогда
	...
	СтруктураПараметров = Новый Структура("ВидСубконто1, ВидСубконто2, ВидСубконто3, ВидСубконто4", "ВидСубконто1", "ВидСубконто2", "ВидСубконто3", "ВидСубконто4");
	...
	Запрос.УстановитьПараметр("ВидСубконто4", Неопределено);
...
Функция СчетаВИерархии(СчетГруппа)
	...
	// В отличие от функции бит_БухгалтерскийУчет.СформироватьМассивСубсчетов() в данном случае для нас важно отсечь счета,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.бит_Дополнительный_2 КАК Дополнительный_Субсчета
	|		ПО ПланСчетов.Ссылка = Дополнительный_Субсчета.Родитель
	|ГДЕ
	|	ПланСчетов.Ссылка В ИЕРАРХИИ(&СчетГруппа)
	|	И Дополнительный_Субсчета.Родитель ЕСТЬ NULL";
	...
	Для Каждого ПроверяемыйСчет Из Счета Цикл
		Для Каждого Счет Из Счета Цикл
			Если ПроверяемыйСчет.ПринадлежитЭлементу(Счет) Тогда
				Иерархия.Вставить(Счет, бит_БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПроверяемыйСчет));
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Процедура ДоработатьНаборДанных_ПоСубконто(ПараметрыОтчета, ПараметрыАлгоритма)
	...
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("Счет",                 Новый ОписаниеТипов("ПланСчетовСсылка.бит_Дополнительный_2"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоДетализации",  Новый ОписаниеТипов("СписокЗначений"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоДетализации1", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.бит_ВидыСубконтоДополнительные_2"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоДетализации2", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.бит_ВидыСубконтоДополнительные_2"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоДетализации3", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.бит_ВидыСубконтоДополнительные_2"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоДетализации4", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.бит_ВидыСубконтоДополнительные_2"));
	...
	НастройкаГруппировки = бит_БухгалтерскиеОтчеты.УникальныеНастройкиОтчетаПоСчетам(ПараметрыОтчета.Группировка);
	...
	Для Каждого СтрокаТаблицы Из НастройкаГруппировки Цикл
		...
		ДанныеСчета = бит_БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(СтрокаТаблицы.Счет);
	...
	Колонки = "ПоСубсчетам,СубконтоДетализации1,СубконтоДетализации2,СубконтоДетализации3,СубконтоДетализации4";
	...
	Для Каждого ОписаниеПодзапроса Из ЗапросыДляФормирования Цикл
		...
		Если ОписаниеПодзапроса.ПоСубсчетам ИЛИ ПараметрыОтчета.ПоСубсчетам Тогда
			...
			Для Каждого Счет Из бит_БухгалтерскийУчет.СформироватьМассивСубсчетов(СчетаПодзапроса) Цикл
		...
		СчетаГруппировки = бит_БухгалтерскийУчет.СформироватьМассивСубсчетов(СчетаПодзапроса);
...
Процедура ДоработатьНаборДанных_ПоСубконтоРазвернутое(ПараметрыОтчета, ПараметрыАлгоритма)
	...
	// Оставим уникальные строки со счетами.
	НастройкаРазвернутоеСальдо = бит_БухгалтерскиеОтчеты.УникальныеНастройкиОтчетаПоСчетам(ПараметрыОтчета.РазвернутоеСальдо);
	...
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоРазвернутоеСальдо1",             Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.бит_ВидыСубконтоДополнительные_2"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоРазвернутоеСальдо2",             Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.бит_ВидыСубконтоДополнительные_2"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоРазвернутоеСальдо3",             Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.бит_ВидыСубконтоДополнительные_2"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоРазвернутоеСальдо4",             Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.бит_ВидыСубконтоДополнительные_2"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("ПоСубсчетам",                            Новый ОписаниеТипов("Булево"));
	...
	// Сворачиваем копию таблицы запросов, получая тем самым минимальный набор запросов, которые надо выполнить.
	Колонки = "ПоСубсчетам,ЕстьРазворотПоСубконтоТекущегоСчета,КоличествоСубконтоДетализируемыхПоСчету,СубконтоРазвернутоеСальдо1,СубконтоРазвернутоеСальдо2,СубконтоРазвернутоеСальдо3,СубконтоРазвернутоеСальдо4";
	ЗапросыДляФормирования = ПараметрыФормируемыхЗапросов.Скопировать(, Колонки);
	ЗапросыДляФормирования.Свернуть(Колонки);	
	...
	Для Каждого ОписаниеПодзапроса Из ЗапросыДляФормирования Цикл
		...
		СчетаРазвернутоеСальдо = бит_БухгалтерскийУчет.СформироватьМассивСубсчетов(СчетаПодзапроса);
