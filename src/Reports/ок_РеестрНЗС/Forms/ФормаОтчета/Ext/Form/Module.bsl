...
Функция ПолучитьДанныеДополнительнойРасшифровки(Расшифровка)
	...
	Счет						= Неопределено;
	ОбъектНЗС					= Неопределено;
	Номенклатура				= Неопределено;
	НомерСубконтоОбъект			= 0;
	...
	Для Каждого Элемент Из Группировки Цикл
		...
		ПолеСчет = ПоляГруппировки.Найти("Счет");
		
		Если Не ПолеСчет = Неопределено Тогда 
			
			Счет = ПолеСчет.Значение;
			
		КонецЕсли;
		
		ПолеНомерСубконтоНоменклатура = ПоляГруппировки.Найти("НомерСубконтоНоменклатура");
		
		Если Не ПолеНомерСубконтоНоменклатура = Неопределено Тогда 
			
			НомерСубконтоНоменклатура = ПолеНомерСубконтоНоменклатура.Значение;
			
		КонецЕсли;
		
		ПолеНомерСубконтоОбъект = ПоляГруппировки.Найти("НомерСубконтоОбъект");
		
		Если Не ПолеНомерСубконтоОбъект = Неопределено Тогда 
			
			НомерСубконтоОбъект = ПолеНомерСубконтоОбъект.Значение;
	...
	ПолеОбъектыНЗС = Поля.Найти("СписокОбъектовПоГруппировке");
	
	Если Не ПолеОбъектыНЗС = Неопределено Тогда 
		
		ОбъектНЗС = ПолеОбъектыНЗС.Значение;
		
	КонецЕсли;
	
	
	ПолеНоменклатура = Поля.Найти("СписокНоменклатурыПоГруппировке");
	
	Если Не ПолеНоменклатура = Неопределено Тогда 
		
		Номенклатура = ПолеНоменклатура.Значение;
		
	КонецЕсли; 
			
		
	Если Не Поля.Найти("УвеличениеСтоимости") = Неопределено Тогда 
		
		ВидРасшифровки = "УвеличениеСтоимости";
		
	ИначеЕсли Не Поля.Найти("ПереносНаСчетОС") = Неопределено Тогда
		
		ВидРасшифровки = "ПереносНаСчетОС";
		
	ИначеЕсли Не Поля.Найти("ВыбытиеНЗС") = Неопределено Тогда
		
		ВидРасшифровки = "ВыбытиеНЗС";
		
	КонецЕсли;
	
	СтруктураРасшифровки = Новый Структура;
	СтруктураРасшифровки.Вставить("НачалоПериода",				НачалоПериода);
	СтруктураРасшифровки.Вставить("КонецПериода",				КонецПериода);
	СтруктураРасшифровки.Вставить("Организация",				Организация);
	СтруктураРасшифровки.Вставить("Счет",						Счет);
	СтруктураРасшифровки.Вставить("ОбъектНЗС",					ОбъектНЗС);
	СтруктураРасшифровки.Вставить("Номенклатура",				Номенклатура);
	СтруктураРасшифровки.Вставить("НомерСубконтоОбъект",		НомерСубконтоОбъект);
	СтруктураРасшифровки.Вставить("НомерСубконтоНоменклатура",	НомерСубконтоНоменклатура);
	СтруктураРасшифровки.Вставить("ВидРасшифровки",				ВидРасшифровки);
...
Процедура РезультатОбработкаРасшифровки(ДанныеРасшифровки)
	...
	ФормаРасшифровки.Отчет.Организация = ДанныеРасшифровки.Организация;
	...
	Для Каждого Элемент Из ПользовательскиеНастройкиРасшифровки.Элементы Цикл
		...
		Если Элемент.ИдентификаторПользовательскойНастройки = "Отбор" Тогда 
			...
			Если ДанныеРасшифровки.ВидРасшифровки = "УвеличениеСтоимости" Тогда
				...
				Условие.ПравоеЗначение	= ДанныеРасшифровки.Счет;
				
				
				//Условие = Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				//
				//Условие.ЛевоеЗначение	= Новый ПолеКомпоновкиДанных("Организация");
				//Условие.ВидСравнения	= ВидСравненияКомпоновкиДанных.Равно;
				//Условие.ПравоеЗначение	= ДанныеРасшифровки.Организация;
				
				
				Если ДанныеРасшифровки.НомерСубконтоОбъект > 0 Тогда 
					
					СписокОбъектыНЗС = Новый СписокЗначений;
					СписокОбъектыНЗС.ЗагрузитьЗначения(ДанныеРасшифровки.ОбъектНЗС);
					
					Условие = Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
					
					Условие.ЛевоеЗначение	= Новый ПолеКомпоновкиДанных("СубконтоДт"+ДанныеРасшифровки.НомерСубконтоОбъект);
					Условие.ВидСравнения	= ВидСравненияКомпоновкиДанных.ВСписке;
					Условие.ПравоеЗначение	= СписокОбъектыНЗС;	
					
				КонецЕсли;
				
				
				Если ДанныеРасшифровки.НомерСубконтоНоменклатура > 0 Тогда 
					
					СписокНоменклатура = Новый СписокЗначений;
					СписокНоменклатура.ЗагрузитьЗначения(ДанныеРасшифровки.Номенклатура);
					
					Условие = Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
					
					Условие.ЛевоеЗначение	= Новый ПолеКомпоновкиДанных("СубконтоДт"+ДанныеРасшифровки.НомерСубконтоНоменклатура);
					Условие.ВидСравнения	= ВидСравненияКомпоновкиДанных.ВСписке;
					Условие.ПравоеЗначение	= СписокНоменклатура;	
					
				КонецЕсли;
				
			ИначеЕсли ДанныеРасшифровки.ВидРасшифровки = "ПереносНаСчетОС" Тогда 
				
				Условие = Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				
				Условие.ЛевоеЗначение	= Новый ПолеКомпоновкиДанных("СчетДт");
				Условие.ВидСравнения	= ВидСравненияКомпоновкиДанных.ВИерархии;
				Условие.ПравоеЗначение	= ПредопределенноеЗначение("ПланСчетов.бит_Дополнительный_2.ОСиИнвСобственность");
				...
				Если ДанныеРасшифровки.НомерСубконтоОбъект > 0 Тогда 
					...
					Условие.ЛевоеЗначение	= Новый ПолеКомпоновкиДанных("СубконтоКт"+ДанныеРасшифровки.НомерСубконтоОбъект);
					Условие.ВидСравнения	= ВидСравненияКомпоновкиДанных.ВСписке;
					Условие.ПравоеЗначение	= СписокОбъектыНЗС;	
				...
				Если ДанныеРасшифровки.НомерСубконтоНоменклатура > 0 Тогда 
					...
					Условие.ЛевоеЗначение	= Новый ПолеКомпоновкиДанных("СубконтоКт"+ДанныеРасшифровки.НомерСубконтоНоменклатура);
					Условие.ВидСравнения	= ВидСравненияКомпоновкиДанных.ВСписке;
					Условие.ПравоеЗначение	= СписокНоменклатура;	
					
				КонецЕсли;
				
			ИначеЕсли ДанныеРасшифровки.ВидРасшифровки = "ВыбытиеНЗС" Тогда 
