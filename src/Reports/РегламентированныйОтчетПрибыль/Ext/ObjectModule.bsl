...
Функция ПроверитьВозможностьВыгрузки(ИмяФормы,
	...
	//ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2020-12-28 (#4004)
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2019Кв4") И ТекущаяДатаСеанса() >= '20210101' Тогда
		
		ГодОтчета = Год(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания);
		
		Если ГодОтчета > 2020 Тогда
			
			СообщитьВТС(ИмяФормы, РезультатВыгрузки,
			"Декларация по форме, утвержденной приказом ФНС России от 23.09.2019 № ММВ-7-3/475@,"
			+ " не представляется за периоды " + Формат(ГодОтчета, "ЧГ=") + " года",
			"Титульный", "ОтчетГод");
			
			Возврат Ложь;
			
		ИначеЕсли ПараметрыВыгрузки.НомКорр = "0" Тогда
			
			ОтчетЗа2020Год = КонецКвартала(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) = КонецГода('20201231');
			
			Если ОтчетЗа2020Год Тогда
				
				СообщитьВТС(ИмяФормы, РезультатВыгрузки,
				"С 01.01.2021 не представляется первичная декларация по форме, утвержденной"
				+ " приказом ФНС России от 23.09.2019 № ММВ-7-3/475@, за налоговый период 2020 год",
			...
			ИначеЕсли ПризнакНП = "03" Тогда
				...
				Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2020Кв4") Тогда
					...
					Если ЗначениеЗаполнено(Лист02.Данные[ИмяПок])
						И НЕ НомерДокументаСоответствуетТребованиям(Лист02.Данные[ИмяПок], ТекстСообщения) Тогда
						СообщитьВТС(ИмяФормы, РезультатВыгрузки, "Лист 02 -> Страница " + НомСтраницы_Лист02
				...
				ИначеЕсли ПризнакНП = "03" Тогда
					...
					Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2020Кв4") Тогда
						...
						Если ЗначениеЗаполнено(Лист02_4.Данные[ИмяПок])
							И НЕ НомерДокументаСоответствуетТребованиям(Лист02_4.Данные[ИмяПок], ТекстСообщения) Тогда
				...
				ИначеЕсли ПризнакНП = "03" Тогда
					...
					Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2020Кв4") Тогда
						...
						Если ЗначениеЗаполнено(Лист02_5.Данные[ИмяПок])
							И НЕ НомерДокументаСоответствуетТребованиям(Лист02_5.Данные[ИмяПок], ТекстСообщения) Тогда
...
//ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2020-12-28 (#4004)
Функция НомерДокументаСоответствуетТребованиям(Знач НомерДокумента, ТекстСообщения)
	
	НомерДок = СокрЛП(НомерДокумента);
	
	НомерДокументаСоответствуетТребованиям
	= СтрДлина(НомерДок) >= 3 И СтрНайти("1234567", Лев(НомерДок, 1)) > 0 И Сред(НомерДок, 2, 1) = "/";
	
	Если НЕ НомерДокументаСоответствуетТребованиям Тогда
		
		ТекстСообщения = "Значение должно соответствовать шаблону ""К/Н"", где ""К"" - код типа особых "
		+ "(свободных) экономических зон (цифра от 1 до 7), ""Н"" - строка длиной 1-19 символов";
