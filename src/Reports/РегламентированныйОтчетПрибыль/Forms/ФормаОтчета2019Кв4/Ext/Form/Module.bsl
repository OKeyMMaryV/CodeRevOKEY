			...
			Для каждого Лист02 Из ДеревоДанных.Строки Цикл
				...
				Для каждого Лист02_6бБ Из Листы02_6бБ Цикл
					...
					Если ЗначениеЗаполнено(Данные["ПризнакВключения"]) Тогда
						...
						//ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2020-12-28 (#4004)
						//Данные.Вставить("П0026б0026003", ОкрЧФ(Форма, Данные["П0026б0021003"])
...
Процедура СобратьДанные()
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1_2" Тогда
		...
		//Сообщение = Новый СообщениеПользователю;
		//Сообщение.Текст = НСтр(
		//"ru='Заполнение Подраздела 1.2 в форме отчета, утвержденной приказом ФНС России от 23.09.2019 № ММВ-7-3/475@, будет реализовано в будущих версиях программы'");
		//Сообщение.Сообщить();
		ПоМесту = СокрЛП(СтруктураДанныхТитульный.ПоМесту);
			...
			Для НомЛиста = 1 По КолЛистовРаздела2 Цикл
				...
				Для ИндСтрокиМнЧ = 1 По СтрокМнЧ2НаЛисте Цикл
					...
					Если КолСтрокМнЧ2 >= ИндСПриращением Тогда
						...
						//ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2020-12-28 (#4004)
						//ДанныеМнЧ2ДляПечати.Вставить(НовИмяП00008М200006,
						//	ДанныеМнЧ2.Строки[ИндСПриращением - 1].Данные[ИмяП00008М200006]);
						//ДанныеМнЧ2ДляПечати.Вставить(НовИмяП00008М200007,
						//	ДанныеМнЧ2.Строки[ИндСПриращением - 1].Данные[ИмяП00008М200007]);
						ДанныеМнЧ2ДляПечати.Вставить(НовИмяП00008М200006,
...
Процедура ПроверкаКС_1_17(СтруктураРезультата)
	...
	Заголовки.Вставить("Условие"			, НСтр("ru=''"));
	Заголовки.Вставить("Комментарий"		, НСтр("ru=''"));
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru='Проверка Раздела 1.2 (КС 1.17)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='п.2 ст.286 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
...
Процедура ПроверкаКС_1_01_доп(СтруктураРезультата)
	...
	//ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2020-12-28 (#4004)
	//Заголовки.Вставить("Условие"			, НСтр("ru='2017 - 2020'"));
