...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ИмяПоказателя = "П000020000200" ИЛИ ИмяПоказателя = "П000240000200" ИЛИ ИмяПоказателя = "П000250000200" Тогда
		...
		Иначе
			Область.Значение = "";
			Сообщение = Новый СообщениеПользователю;
			ТекстСообщения = "Номер документа должен соответствовать шаблону ""К/Н"", где ""К"" - код типа особых "
			+ "(свободных) экономических зон (цифра от 1 до 7), ""Н"" - строка длиной 1-19 символов. Значение очищено";
		...
		Для каждого Ветка Из ДеревоДанныхРаздела.Строки Цикл
			...
			Если СтраницаВключенаВСоставОтчета(Ветка) Тогда
				...
				Для каждого Пок Из Ветка.Данные Цикл
					...
					Если Обл <> Неопределено Тогда
						...
						//Если Обл.Имя = "П000020000200" И НЕ ЗначениеЗаполнено(Обл.Значение) Тогда
						//	Обл.Значение = "-/-------------------";
						//КонецЕсли;
			...
			Если СтраницаВключенаВСоставОтчета(Ветка_1) Тогда
				...
				Для каждого Пок Из Ветка_1.Данные Цикл
					...
					Если Обл <> Неопределено Тогда
						...
						//Если Обл.Имя = "П000210000200" И НЕ ЗначениеЗаполнено(Обл.Значение) Тогда
						//	Обл.Значение = "-/-------------------";
						//КонецЕсли;
			...
			Если СтраницаВключенаВСоставОтчета(Ветка_2) Тогда
				...
				Для каждого Пок Из Ветка_2.Данные Цикл
					...
					Если Обл <> Неопределено Тогда
						...
						//Если Обл.Имя = "П000220000200" И НЕ ЗначениеЗаполнено(Обл.Значение) Тогда
						//	Обл.Значение = "-/-------------------";
						//КонецЕсли;
			...
			Если СтраницаВключенаВСоставОтчета(Ветка_3) Тогда
				...
				Для каждого Пок Из Ветка_3.Данные Цикл
					...
					Если Обл <> Неопределено Тогда
						...
						//Если Обл.Имя = "П000230000200" И НЕ ЗначениеЗаполнено(Обл.Значение) Тогда
						//	Обл.Значение = "-/-------------------";
						//КонецЕсли;
			...
			Для каждого Ветка_4 Из Ветка.Строки[3].Строки Цикл
				...
				Для НомЛиста = 1 По КолЛистовРаздела1 Цикл
					...
					//Обл = ТабличныйДокумент.Области.Найти("П000240000200");
			...
			Для каждого Ветка_5 Из Ветка.Строки[4].Строки Цикл
				...
				Для каждого Пок Из Ветка_5.Данные Цикл
					...
					Если Обл <> Неопределено Тогда
						...
						//Если Обл.Имя = "П000250000200" И НЕ ЗначениеЗаполнено(Обл.Значение) Тогда
						//	Обл.Значение = "-/-------------------";
						//КонецЕсли;
...
Процедура Проверить(Команда)
	...
	|в редакции приказа ФНС России от 11.09.2020
	|№ ЕД-7-3/655@ будет реализована в будущих
	|версиях программы'"));
