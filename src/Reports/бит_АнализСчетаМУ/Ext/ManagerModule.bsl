...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ВидСубконто4ТолькоОбороты", СвойстваСчета.ВидСубконто4ТолькоОбороты);
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	ПозицияКорсчета = СтрНайти(ИсходныйТекстЗапроса, ", ) КАК Обороты");
	...
	ГруппировкаКор  = бит_БухгалтерскиеОтчеты.УникальныеНастройкиОтчетаПоСчетам(ПараметрыОтчета.ГруппировкаКор);
	...
	Для Каждого СтрокаТаблицы Из ГруппировкаКор Цикл 
		...
		Для ИндексКор = НачальныйИндексКор + 1 По 4 Цикл
			НачалоСтроки = СтрНайти(ПромежуточныйТекстЗапроса, "Обороты.КорСубконто" + ИндексКор);
	...
	ПостояннаяЧастьЗапроса = СтрЗаменить(ПостояннаяЧастьЗапроса, "Обороты.ПодразделениеКор КАК", "NULL КАК");
	Для НомерСубконто = 1 По 4 Цикл
		ПостояннаяЧастьЗапроса = СтрЗаменить(ПостояннаяЧастьЗапроса, "Обороты.КорСубконто" + НомерСубконто + " КАК", "NULL КАК");
	КонецЦикла;
...
Процедура ДобавитьУсловиеПоВыбраннымРесурсам(ПараметрыОтчета, Схема)
	...
	Для Каждого ИмяПоказателя Из ПолучитьНаборПоказателей() Цикл
		...
		Если ПараметрыОтчета["Показатель" + ИмяПоказателя] Тогда
			...
			Если ИмяПоказателя = "МУ" Тогда
				
				ИмяПоказателяДляПодстановки = "СуммаМУ";
			...
			ТекстУсловияДт = ТекстУсловияДт +" ОстаткиИОбороты." + ИмяПоказателяДляПодстановки + "ОборотДт -";
			
			ТекстУсловияКт = ТекстУсловияКт +" ОстаткиИОбороты." + ИмяПоказателяДляПодстановки + "ОборотКт -";
...
Функция СчетаВИерархии(СчетГруппа)
	...
	|	И ПланСчетов.ЗапретитьИспользоватьВПроводках = ЛОЖЬ";
