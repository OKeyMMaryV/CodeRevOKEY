...
Процедура ПереопределитьТекстВЭкраннойИлиПечатнойФорме(ТаблДокумент) Экспорт
	
	Если ТекущаяДатаСеанса() < ДатаПримененияФормыВНовойРедакции() Тогда
		
		Обл = ТаблДокумент.Области.Найти("РеквизитыПриказа");
		Если Обл <> Неопределено Тогда
			Обл.Текст = "Приложение № 1
			|к письму ФНС России
			|от 14.04.2021
			|№ ЕА-4-15/5042@";
		КонецЕсли;
		
		Обл = ТаблДокумент.Области.Найти("УведомлениеНаим");
		Если Обл <> Неопределено Тогда
			Обл.Текст = "Уведомление о перемещении товаров, подлежащих прослеживаемости, с территории"
			+ " Российской Федерации на территорию другого государства - члена Евразийского экономического союза";
		КонецЕсли;
		...
		Если Обл <> Неопределено Тогда
			Обл.Текст = "1 - продавец
			|2 - представитель продавца";
		КонецЕсли;
		
		Обл = ТаблДокумент.Области.Найти("ПодписантНаимДок");
		Если Обл <> Неопределено Тогда
			Обл.Текст = "Наименование и реквизиты документа, подтверждающего полномочия представителя продавца";
		КонецЕсли;
		
		Обл = ТаблДокумент.Области.Найти("ПодписантНаимОрг");
		Если Обл <> Неопределено Тогда
			Обл.Текст = "Наименование организации-представителя продавца";
		КонецЕсли;
