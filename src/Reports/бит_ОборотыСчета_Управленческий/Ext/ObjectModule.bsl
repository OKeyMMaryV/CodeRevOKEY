...
Процедура СформироватьЗначениеПеременной_мТекстРесурсыДляВывода() Экспорт
	...
	Если мЕстьРесурсСуммаМУ И ВыводитьСуммуМУ Тогда 
		мТекстРесурсыДляВывода = мТекстРесурсыДляВывода + ?(ПустаяСтрока(мТекстРесурсыДляВывода), "", ", ") + "СуммаМУ";
    КонецЕсли;
...
Процедура СформироватьТекстЗапросов(МассивРесурсов, ЗапросОбороты, ЗапросОстатки)
	...
	Если   ТипЗнч(Организация) = Тип("СправочникСсылка.бит_му_ПериметрыКонсолидации") и
		...
		Для Каждого Строка из ТаблицаОрганизаций цикл
			...
			|ВЫБРАТЬ " + ТекстСчет + ТекстСубконто + "
			|	Таблица.КорСчет.Ссылка КАК КорСчет, 
			|	Таблица.КорСчет.Порядок КАК КорСчетПорядок, 
			|	Таблица.КорСчет.Представление КАК КорСчетПредставление,";
			...
			Для каждого ИмяРесурса Из МассивРесурсов Цикл
				...
				Если Строка.Период > Строка.НачалоПериод Тогда 
					...
					//			   
					//ТекстКонец =   ""   + ИмяРесурса + "ОборотДт КАК " + ИмяРесурса + "КонДт,
					...
					ТекстКонец =   "
					|   Таблица."   + ИмяРесурса + "ОборотДт КАК " + ИмяРесурса + "КонДт,
					|   Таблица."   + ИмяРесурса + "ОборотКт КАК " + ИмяРесурса + "КонКт,";
					// Ок Видяйкин В.И. 20170220 -
				...
				ТекстОстатки = ТекстОстатки + ТекстНачало+ТекстКонец + "
				|	Таблица." + ИмяРесурса + "ОборотДт КАК " + ИмяРесурса + "ЕстьОборотДт,
				|	Таблица." + ИмяРесурса + "ОборотКт КАК " + ИмяРесурса + "ЕстьОборотКт,";
			...
			Если Строка.ДатаОкончания > Период.ДатаНачала и
				...
				ЗапросОбороты.УстановитьПараметр("ДатаКон"+Сч, КонецДня(Строка.ДатаОкончания));
				ЗапросОстатки.УстановитьПараметр("ДатаКон"+Сч, КонецДня(Строка.ДатаОкончания));
			...
			Если Строка.Период > Период.ДатаНачала Тогда 
				...
				ЗапросОбороты.УстановитьПараметр("ДатаНач"+Сч,Строка.Период);
				ЗапросОстатки.УстановитьПараметр("ДатаНач"+Сч,Строка.Период);
				
			Иначе 
				ДатаНачало = "&ДатаНач";
			КонецЕсли;
			
			ЗапросОстатки.УстановитьПараметр("Организация"+Сч,Строка.Организация);
			ЗапросОбороты.УстановитьПараметр("Организация"+Сч,Строка.Организация);
			...
			ОграниченияПоПостроителюОтчета = бит_БухгалтерскиеОтчетыСервер.ПолучитьТекстОграниченийПоПостроителюОтчета(ПостроительОтчета, ЗапросОстатки);
			...
			ОграниченияПоРеквизитам =  СтрокаОграниченийПоРеквизитам + ОграниченияПоПостроителюОтчета;
			
			ОграниченияПоПостроителюОтчета = бит_БухгалтерскиеОтчетыСервер.ПолучитьТекстОграниченийПоПостроителюОтчета(ПостроительОтчета, ЗапросОбороты);
			...
			ТекстОборотыИЗ = "
			|ИЗ
			|	РегистрБухгалтерии." + ИмяРегистраБухгалтерии + ".Обороты("+ДатаНачало+","+ ДатаКонец+", " + Периодичность + 
			", Счет В ИЕРАРХИИ (&Счет), " + ОтборПоВидамСубконто + " , " + ТекстУсловияОбороты + ТекстОтбор + " , " + ТекстКорСчет + ") КАК Таблица
			| ";  	
			
			ТекстОстаткиИЗ = "
			|	РегистрБухгалтерии." + ИмяРегистраБухгалтерии + ".ОстаткиИОбороты("+ДатаНачало+","+ ДатаКонец+", " + Периодичность + 
			", ДвиженияИГраницыПериода, счет В ИЕРАРХИИ (&счет), " + ОтборПоВидамСубконто + " , " + ТекстУсловия + ТекстОтбор + ") КАК Таблица
			|";
	...
	Для каждого Измерение Из ПостроительОтчета.ИзмеренияСтроки Цикл
		// Изменение кода. Начало. 14.10.2014{{
		ТекстПорядок = ТекстПорядок + ", Таблица." + Измерение.ПутьКДанным;
		// Изменение кода. Конец. 14.10.2014}}
	КонецЦикла;
...
// Параметры:
//  Запрос
//  АвтоматическиРассчитатьКонечнуюДатуОтчета.
// 
...
Процедура ВывестиКорСчета(ВыборкаОборотыПоКорСчетам, СтруктураПараметров, ВидОборота, Ресурсы, ВыводимаяОбласть, 
	...
	Для каждого ЭлементСчет Из СписокСчетов Цикл
		...
		КонецЦикла;    
			...
			Расшифровка.Вставить("ТаблицаКорСубконто", бит_ОбщегоНазначения.УпаковатьТаблицуЗначений(ТаблицаКорСубконто));
...
Процедура ЗаполнитьПараметрыРасшифровки(Область, Выборка, Знач СтруктураПараметров)
	...
	Если Лев(Выборка.Группировка(), 8) = "Субконто" 
		ИЛИ Лев(Выборка.Группировка(), 12) = "Организация" Тогда
	...
	// СписокРасшифровки.Добавить(ТаблицаКорСубконто, бит_ОбщегоНазначения.УпаковатьТаблицуЗначений(ТаблицаКорСубконто));
