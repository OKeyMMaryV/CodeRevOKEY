...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Подпукнт = Сред(Параметры.РеквизитыДляРедактирования, 13, 4);
	Абзац = Сред(Параметры.РеквизитыДляРедактирования, 17, 4);
	Иное = Сред(Параметры.РеквизитыДляРедактирования, 21, 4);
	Для Каждого Элт Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("Абзац,Иное,Подпукнт,Пункт,Статья,Часть", ",") Цикл 
...
Процедура Ок(Команда)
	Результат = Прав("0000" + СокрЛП(Статья), 4) + Прав("0000" + СокрЛП(Часть), 4) + Прав("0000" + СокрЛП(Пункт), 4) 
			+ Прав("0000" + СокрЛП(Подпукнт), 4) + Прав("0000" + СокрЛП(Абзац), 4) + Прав("0000" + СокрЛП(Иное), 4);
