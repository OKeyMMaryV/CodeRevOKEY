...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	УстановитьДоступныеЗначенияДляПолей();
КонецПроцедуры
...
Функция РезультатФормированияОтчета(Знач ФормированиеПриОткрытии, Непосредственно)
	...
	бит_БК_ТекущийИнициатор = ПараметрыСеанса.бит_БК_ТекущийИнициатор;
	Если НЕ ЗначениеЗаполнено(бит_БК_ТекущийИнициатор) Тогда
	...
	Если Непосредственно Тогда
		ПараметрыФормирования.Объект.бит_БК_ТекущийИнициатор = бит_БК_ТекущийИнициатор;
	Иначе
		ПараметрыФормирования.НастройкиКД.ПараметрыДанных.УстановитьЗначениеПараметра("ИнициаторФВБ", бит_БК_ТекущийИнициатор);
	КонецЕсли;
...
&НаСервере
Процедура УстановитьДоступныеЗначенияДляПолей()
	
	ОтчетОбъект = РеквизитФормыВЗначение("Отчет");
	МакетОтборов = ОтчетОбъект.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных");
	...
	Пока Истина Цикл
		...
		МассивЗначений_ЦФО = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ЦФО");
		
		Если МассивЗначений_ЦФО.Количество() > 0 Тогда 
			
			СписокЗначений_ЦФО = Новый СписокЗначений;
			СписокЗначений_ЦФО.ЗагрузитьЗначения(МассивЗначений_ЦФО);
			
			ПолеНД_ЦФО = МакетОтборов.НаборыДанных.НаборДанных1.Поля.Найти("ЦФО");
			ПолеНД_ЦФО.УстановитьДоступныеЗначения(СписокЗначений_ЦФО);
		...
		Если ТаблицаРезультат.Количество() > 0 Тогда 
			...
			//Статья оборотов
			МассивЗначений_СтатьяОборотов = ТаблицаРезультат.ВыгрузитьКолонку("СтатьяОборотов");
			
			СписокЗначений_СтатьяОборотов = Новый СписокЗначений;
			СписокЗначений_СтатьяОборотов.ЗагрузитьЗначения(МассивЗначений_СтатьяОборотов);
			
			ПолеНД_СтатьяОборотов = МакетОтборов.НаборыДанных.НаборДанных1.Поля.Найти("СтатьяОборотов");
			ПолеНД_СтатьяОборотов.УстановитьДоступныеЗначения(СписокЗначений_СтатьяОборотов);
			
			//Объект
			МассивЗначений_Объект = ТаблицаРезультат.ВыгрузитьКолонку("Объект");
			
			СписокЗначений_Объект = Новый СписокЗначений;
			СписокЗначений_Объект.ЗагрузитьЗначения(МассивЗначений_Объект);
			
			ПолеНД_Объект = МакетОтборов.НаборыДанных.НаборДанных1.Поля.Найти("Объект");
			ПолеНД_Объект.УстановитьДоступныеЗначения(СписокЗначений_СтатьяОборотов);
			
			//ЦФО
			МассивЗначений_ЦФО = ТаблицаРезультат.ВыгрузитьКолонку("ЦФО");
    ...
    Отчет.КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(АдресСхемы));
	Отчет.КомпоновщикНастроек.ЗагрузитьНастройки(НастройкиОтборов);
