...
Функция ТекстЗапросаРеквизитыСчетовФактурПостановление981() Экспорт
	...
	|	КорректировкаПоступления.ок_НомерИсходногоСФ,
	|	КорректировкаПоступления.ок_ДатаИсходногоСФ,
	|	КорректировкаПоступления.ДокументПоступления,
	|	НЕОПРЕДЕЛЕНО,
	|	ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка),
	|	ЛОЖЬ,
	|	КорректировкаПоступления.Контрагент,
	|	КорректировкаПоступления.ок_ВидСчетаФактуры,
	|	ЛОЖЬ,
	|	0,
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ДокументыОснования КАК СчетФактураВыданныйДокументыОснования
	|			ПО (СчетФактураВыданныйДокументыОснования.Ссылка.Проведен)
	|				И КорректировкаПоступления.Ссылка = СчетФактураВыданныйДокументыОснования.ДокументОснование
	|				И (СчетФактураВыданныйДокументыОснования.Ссылка.ВидСчетаФактуры <> ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.НаАванс))
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|			ПО КорректировкаПоступления.Ссылка = ДанныеПервичныхДокументов.Документ
	|		ПО СписокДокументовОснований.ДокументОснованиеСчетаФактуры = КорректировкаПоступления.Ссылка
	|	ГДЕ
	|	СчетФактураВыданныйДокументыОснования.ДокументОснование ЕСТЬ NULL
	|	И КорректировкаПоступления.Проведен
	|	И &Постановление1137_2019
	|	И КорректировкаПоступления.ДокументПоступления ССЫЛКА Документ.бит_ок_ОперацияАксапты
	...
	//ОКЕЙ Сафронов А.А. (СофтЛаб) Конец 2020-12-23 (#3946)
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-05-17 (#НТП-МП-03)
	|		КОГДА СчетаФактурыДокументы.СчетФактураДокумент ССЫЛКА Документ.бит_ок_ОперацияАксапты
	|	ТОГДА ""01""	
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Конец 2021-05-17 (#НТП-МП-03)
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК КодВидаОперации,
	...
	|	ЖурналУчетаСчетовФактур.СчетФактура.ДокументОснование.ДокументПоступления,
	|	ЖурналУчетаСчетовФактур.СчетФактура.ДокументОснование.ок_НомерИсходногоСФ,
	|	ЖурналУчетаСчетовФактур.СчетФактура.ДокументОснование.ок_ДатаИсходногоСФ,
	|	НЕОПРЕДЕЛЕНО,
	|	ДАТАВРЕМЯ(1,1,1),
	|	НЕОПРЕДЕЛЕНО,
	|	ДАТАВРЕМЯ(1,1,1),
	...
	|	ЖурналУчетаСчетовФактур.СчетФактура.ДокументОснование.ок_СуммаПоСчетуФактуре,
	|	ЖурналУчетаСчетовФактур.Валюта.Код,
	|	ЖурналУчетаСчетовФактур.СчетФактура.ДокументОснование.ДокументПоступления.ок_КодВидаОперации,
	|	ЖурналУчетаСчетовФактур.СчетФактура.ДокументОснование.ДокументПоступления.ок_ИННКонтрагентаДоИзменения,
	...
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|	И ЖурналУчетаСчетовФактур.СчетФактура.ДокументОснование ССЫЛКА Документ.КорректировкаПоступления
	|			И ЖурналУчетаСчетовФактур.СчетФактура.ДокументОснование.ДокументПоступления ССЫЛКА Документ.бит_ок_ОперацияАксапты
	|
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Конец 2021-05-17 (#НТП-МП-03)
	|
	|ИНДЕКСИРОВАТЬ ПО
...
Функция ТекстЗапросаЗаписиРегистраНДСПокупки(СтруктураПараметров)
	...
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-05-17 (#НТП-МП-03)
	|		КОГДА НДСЗаписиКнигиПокупок.СчетФактура ССЫЛКА Документ.бит_ок_ОперацияАксапты
