		...
		Если ПредопределеннаяГруппировка[0].ПоляГруппировки.Элементы.Количество() > 0 Тогда
			...
			Если ПредопределеннаяГруппировка[0].РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный и ПредопределеннаяГруппировка[0].Состояние = СостояниеЭлементаНастройкиКомпоновкиДанных.Включен Тогда
				...
				Для Каждого ПолеПредопределеннойГруппировки Из ПредопределеннаяГруппировка[0].ПоляГруппировки.Элементы Цикл
					// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-09-06 (#4346)
					Если КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Свойство("НеВыводитьРегистратор")
						И КомпоновщикНастроек.Настройки.ДополнительныеСвойства.НеВыводитьРегистратор
						И ТипЗнч(ПолеПредопределеннойГруппировки) = Тип("ПолеГруппировкиКомпоновкиДанных")
						И (ПолеПредопределеннойГруппировки.Поле = Новый ПолеКомпоновкиДанных("Регистратор")
							Или ПолеПредопределеннойГруппировки.Поле = Новый ПолеКомпоновкиДанных("ДатаОперации")
							Или ПолеПредопределеннойГруппировки.Поле = Новый ПолеКомпоновкиДанных("Содержание")) Тогда
						Продолжить;
					КонецЕсли;
					// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-09-06 (#4346)
