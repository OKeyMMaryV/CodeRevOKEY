...
Функция ЗагрузитьПриходныйКассовыйОрдер(Выборка, Параметры)
	...
	Попытка
		...
		//Если Не СтрокаРасшифровки.СчетУчетаРасчетовПоАвансам = ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученным Тогда
		//	СтрокаРасшифровки.СчетУчетаРасчетовПоАвансам = ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученным;
		//КонецЕсли;
...
Функция ЗагрузитьРеализациюТоваровУслуг(Выборка, Параметры)
	...
	Попытка
		...
		//Если Не ДокументОбъект.СчетУчетаРасчетовПоАвансам = ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученным Тогда
		//	ДокументОбъект.СчетУчетаРасчетовПоАвансам = ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученным;
		//КонецЕсли;
		//
		//Если Не ДокументОбъект.СпособЗачетаАвансов = Перечисления.СпособыЗачетаАвансов.Автоматически Тогда
...
Функция ПолучитьПараметры(ИспользоватьЛогирование, ЗаписыватьСообщенияПользователю, ПараметрыЛогирования)
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Запрос.УстановитьПараметр("СчетУчетаРасчетов",	ПланыСчетов.Хозрасчетный.ПереводыВПути);
		Запрос.Текст	=
		"ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	И ВидыОплатОрганизаций.СчетУчетаРасчетов = &СчетУчетаРасчетов
	...
	Если Не ЗначениеЗаполнено(ВидОплаты) Тогда
		ок_ЛогированиеОпераций.ЗаписатьИнформациюОбОшибке("Вид оплаты для организации не найден: " + ИмяНастройкиОрганизация, Параметры);
	КонецЕсли;
	#КонецОбласти
	
	Параметры.Вставить("ВидОплаты",	ВидОплаты);
	
	Запрос	=	Новый Запрос;
	Запрос.УстановитьПараметр("Группа",			"ЗагрузкаДанныхИзУТ");
	Запрос.УстановитьПараметр("ИмяНастройки",	ПолучитьИмяНастройкиНоменклатураДоставки());
	Запрос.Текст	=
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВЫРАЗИТЬ(бит_ок_НастройкиМеханизмаИмпортаДанных.Значение КАК Справочник.Номенклатура) КАК Значение
	|ИЗ
	...
	Если Не Результат.Пустой() Тогда
		Выборка	=	Результат.Выбрать();
		Если Выборка.Следующий() Тогда
			Номенклатура	=	Выборка.Значение;
		КонецЕсли;
	КонецЕсли;
	
	#Область Логирование
	Если Не ЗначениеЗаполнено(Номенклатура) Тогда
		ок_ЛогированиеОпераций.ЗаписатьИнформациюОбОшибке("Номенклатура не найдена: " + ПолучитьИмяНастройкиНоменклатураДоставки(), Параметры);
	КонецЕсли;
	#КонецОбласти
