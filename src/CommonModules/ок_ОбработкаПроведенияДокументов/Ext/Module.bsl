...
Процедура СформироватьАрендныеОборотыПоБюджетамДляПТУ(Источник, Отказ, РежимПроведения)
	...
	СчетаУчета = ОК_ОбщегоНазначения.ПолучитьЗначениеУниверсальнойНастройки("Аренда_ФСБУ25", "Счета учета (Арендные обязательства)");
	
	Если ТипЗнч(СчетаУчета) <> Тип("Массив") Тогда
		
		// Если не указано то счет 76.37.1 или 76.07.1
		СчетаУчета = Новый Массив;
		СчетаУчета.Добавить(ПланыСчетов.Хозрасчетный.АрендныеОбязательства);
		СчетаУчета.Добавить(ПланыСчетов.Хозрасчетный.АрендныеОбязательстваУЕ);
	...
	Для каждого Строка Из Источник.Услуги Цикл
		...
		Движение.СуммаРегл 			= бит_КурсыВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(Строка.Сумма,
																			Источник.ВалютаДокумента,
																			...
																			1);
		Движение.СуммаУпр 			= бит_КурсыВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(Строка.Сумма,
																			Источник.ВалютаДокумента,
																			...
																			Источник.КратностьВзаиморасчетов,
																			1);
		Движение.СуммаСценарий		= Строка.Сумма;
...
Процедура СформироватьАрендныеОборотыПоБюджетамДляКорректировкиПТУ(Источник, Отказ, РежимПроведения)
	...
	Если ТипЗнч(СчетаУчета) <> Тип("Массив") Тогда
		...
		// Если не указано то счет 76.37.1 и 76.07.1
		СчетаУчета = Новый Массив;
		СчетаУчета.Добавить(ПланыСчетов.Хозрасчетный.АрендныеОбязательства);
		СчетаУчета.Добавить(ПланыСчетов.Хозрасчетный.АрендныеОбязательстваУЕ);
	...
	Для каждого Строка Из Источник.Услуги Цикл
		...
		Движение.Аналитика_4		= Источник.ОК_ID_Разноска;
		Движение.Количество 		= Строка.Количество;
		Движение.Сумма 				= Строка.Сумма - Строка.СуммаДоИзменения;
