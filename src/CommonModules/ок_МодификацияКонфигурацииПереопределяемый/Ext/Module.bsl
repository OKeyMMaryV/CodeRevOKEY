...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Форма.ИмяФормы= "Документ.ПоступлениеДенежныхДокументов.Форма.ФормаДокумента" Тогда
		...
		КомандаСоздатьНаОснованииВыдачаДД.Действие = "ок_СоздатьНаОснованииВыдачаДД";
		КомандаСоздатьНаОснованииВыдачаДД.Заголовок = НСтр("ru = 'Создать выдачу ден.документов'");
		
		ЭлементСоздатьНаОснованииВыдачаДД = Форма.Элементы.Добавить("ок_СоздатьНаОснованииВыдачаДД", Тип("КнопкаФормы"), Форма.КоманднаяПанель.ПодчиненныеЭлементы.ГруппаКнопкиКоманднойПанели.ПодчиненныеЭлементы.ГруппаВажныеКоманды);
	...
	Если Форма.ИмяФормы= "Документ.бит_ЗаявкаНаРасходованиеСредств.Форма.ФормаДокумента" Тогда
		ПолеНеФормироватьПлатежноеПоручение = Форма.Элементы.Вставить("ОК_НеФормироватьПлатежноеПоручение", Тип("ПолеФормы"), Форма.Элементы.ГруппаДополнительно, Форма.Элементы.НазначениеПлатежа_Список);	      
	...
	КонецЕсли;
	//ОКЕЙ Завьялова Е.А. (ПервыйБИТ) Конец 2021-03-14 (#ТП_БП05_ФР08, #ТП_БП05_ФР08)
	...
	Если  Форма.ИмяФормы = "Справочник.ФизическиеЛица.Форма.ФормаСписка" Тогда
		...
		ПолеОрганизация.Заголовок = "Организация";
		...
		Форма.ок_ДанныеФизическихЛицБОСС.ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		КомандаЗагрузитьИзБОСС.Действие = "ок_ЗагрузитьИзБОСС";
	...
	Если Форма.ИмяФормы = "Документ.бит_ок_ОперацияАксапты12.Форма.ФормаДокумента" Тогда
		Если Форма.Объект.ТипДокумента = Справочники.бит_ок_ТипыОперацийАксапты.НайтиПоКоду("210") Тогда
			...
			ГруппаСтраницыТГ.Заголовок = "Данные по товарным группам"; 		
			
			ТаблицаТГ = Форма.Элементы.Добавить("ТаблицаТГ", Тип("ТаблицаФормы"),ГруппаСтраницыТГ);
			ТаблицаТГ.ПутьКДанным = "Объект.ок_ДанныеПоТГ";
			
			РеквизитКонтрагент = Форма.Элементы.Добавить("Контрагент", Тип("ПолеФормы"), ТаблицаТГ);
			РеквизитКонтрагент.Заголовок   = "Контрагент";
			РеквизитКонтрагент.Вид         = ВидПоляФормы.ПолеВвода;
			РеквизитКонтрагент.ПутьКДанным = "Объект.ок_ДанныеПоТГ.Контрагент";
			
			РеквизитДоговор = Форма.Элементы.Добавить("Договор", Тип("ПолеФормы"), ТаблицаТГ);
			РеквизитДоговор.Заголовок   = "Договор";
			РеквизитДоговор.Вид         = ВидПоляФормы.ПолеВвода;
			РеквизитДоговор.ПутьКДанным = "Объект.ок_ДанныеПоТГ.Договор";
			РеквизитДоговор.СвязьПоТипу = Новый СвязьПоТипу("Объект.ок_ДанныеПоТГ.Контрагент");
			
			РеквизитТоварнаяГруппа = Форма.Элементы.Добавить("ТоварнаяГруппа", Тип("ПолеФормы"), ТаблицаТГ);
			РеквизитТоварнаяГруппа.Заголовок   = "Товарная группа";
			...
			РеквизитСрокИсполнения.Заголовок   = "Срок исполнения";
			...
			РеквизитСтавкаНДС.Заголовок   = "Ставка НДС";
			...
			РеквизитСуммаНДС.Заголовок   = "Сумма НДС";
			РеквизитСуммаНДС.Вид         = ВидПоляФормы.ПолеВвода;
			РеквизитСуммаНДС.ПутьКДанным = "Объект.ок_ДанныеПоТГ.СуммаНДС";
			
			РеквизитСуммаВзаиморасчетов = Форма.Элементы.Добавить("СуммаВзаиморасчетов", Тип("ПолеФормы"), ТаблицаТГ);
			РеквизитСуммаВзаиморасчетов.Заголовок   = "Сумма взаиморасчетов";
			РеквизитСуммаВзаиморасчетов.Вид         = ВидПоляФормы.ПолеВвода;
			РеквизитСуммаВзаиморасчетов.ПутьКДанным = "Объект.ок_ДанныеПоТГ.СуммаВзаиморасчетов";
		КонецЕсли;
	КонецЕсли;	
