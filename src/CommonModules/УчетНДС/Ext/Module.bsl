...
Функция ПодготовитьПараметрыРеализацияТоваровУслуг(ТаблицаРеализованныеТоварыУслуги, ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты)
	...
	Если ТипЗнч(ТаблицаРеализованныеТоварыУслуги) = Тип("ТаблицаЗначений") И ТаблицаРеализованныеТоварыУслуги.Колонки.Найти("Субконто3") <> Неопределено Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ",Субконто3"
	КонецЕсли;
	//izhtc-Kir 18.08.2015 )
	//izhtc alena 18.08.2015 (
	Если ТипЗнч(ТаблицаРеализованныеТоварыУслуги) = Тип("ТаблицаЗначений") И ТаблицаРеализованныеТоварыУслуги.Колонки.Найти("Субконто2") <> Неопределено Тогда
...
Функция ПодготовитьДанныеДвиженийПоступлениеТоваровУслугОтПодотчетногоЛица(Товары, Услуги, Реквизиты)
	...
	// +СБ. Широков Николай. 2016-05-23. #2466 Добавлены субконто
	//ДанныеДвижений = Товары.Скопировать(,
	ДанныеДвижений = Услуги.Скопировать(,
	// -СБ. Широков Николай
	...
	// +СБ. Чеплин Денис. 2016-08-22. #2610 Изменена структура таблицы товаров (в таблице нет реквизитов субконто), и заполнены добавленные реквизиты
	Если ТипЗнч(Реквизиты.Регистратор) = Тип("ДокументСсылка.АвансовыйОтчет") Тогда
		
		КолонкаСубконто1 = Услуги.Колонки.Субконто1;
		КолонкаСубконто2 = Услуги.Колонки.Субконто2;
		КолонкаСубконто3 = Услуги.Колонки.Субконто3;
		
		Товары.Колонки.Добавить(КолонкаСубконто1.Имя, КолонкаСубконто1.ТипЗначения, КолонкаСубконто1.Заголовок, КолонкаСубконто1.Ширина);
		Товары.Колонки.Добавить(КолонкаСубконто2.Имя, КолонкаСубконто2.ТипЗначения, КолонкаСубконто2.Заголовок, КолонкаСубконто2.Ширина);
		Товары.Колонки.Добавить(КолонкаСубконто3.Имя, КолонкаСубконто3.ТипЗначения, КолонкаСубконто3.Заголовок, КолонкаСубконто3.Ширина);
		
		Если Услуги.Количество() > 0 Тогда
			Для каждого ТекСтрТовары Из Товары Цикл
				ЗаполнитьЗначенияСвойств(ТекСтрТовары, Услуги[0], "Субконто1,Субконто2,Субконто3");	
			КонецЦикла;
		Иначе
			Для каждого ТекСтрТовары Из Товары Цикл
				ТекСтрТовары.Субконто2 = Реквизиты.Регистратор.Склад.Объект;	
			КонецЦикла;
		КонецЕсли;
	...
	// -СБ. Чеплин Денис
	//ОбщегоНазначенияБПВызовСервера.ЗагрузитьВТаблицуЗначений(Услуги, ДанныеДвижений);
	ОбщегоНазначенияБПВызовСервера.ЗагрузитьВТаблицуЗначений(Товары, ДанныеДвижений);
	ДанныеДвижений.Свернуть(
		// +СБ. Широков Николай. 2016-05-23. #2466 Добавлены субконто
		"Субконто1,Субконто2,Субконто3,"+
		// -СБ. Широков Николай
...
Процедура СформироватьПроводкиИспользованиеБилетов(ДанныеДвижений, Движения, Отказ)
	...
	СчетКт = ПланыСчетов.Хозрасчетный.Билеты;  // 76.14
...
Процедура СформироватьДвиженияОшибочныеРеквизитыКонтрагентов(ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для Каждого ЗаписьОшибочныеРеквизиты Из Параметры.ТаблицаОшибочныеРеквизитыКонтрагентов Цикл
		...
		Если Не ЗначениеЗаполнено(ЗаписьОшибочныеРеквизиты.СчетФактура) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СформироватьПроводкиРеализацияТоваровУслуг(ТоварыУслуги, Реквизиты, Движения, Отказ)
	...
	ЭтоРеализацияТоваровУслуг = Типзнч(Регистратор) = Тип("ДокументСсылка.РеализацияТоваровУслуг");
	Если ЭтоРеализацияТоваровУслуг Тогда 
		ТаблицаВыручки = ТоварыУслуги.Скопировать(,
			"Контрагент,Номенклатура,Содержание,СчетУчетаНДСПоРеализации,СтавкаНДС,Субконто,Субконто2,Субконто3,Подразделение,СуммаНДСРуб");
    //ОК+ Аверьянова #2538 от 21.06.16 //заполнение функции(3-е с/к) в 91 счете   
	ИначеЕсли ТипЗнч(Регистратор) = Тип("ДокументСсылка.ПередачаОС")  или Типзнч(Регистратор) = Тип("ДокументСсылка.СписаниеОС") Тогда
		ТаблицаВыручки = ТоварыУслуги.Скопировать(,"Контрагент,Номенклатура,Содержание,СчетУчетаНДСПоРеализации,СтавкаНДС,Субконто,Субконто3,Подразделение,СуммаНДСРуб");
	//ОК-
	Иначе
	//izhtc alena 18.08.2015 )
		ТаблицаВыручки = ТоварыУслуги.Скопировать(,
	...
	//ОК+ Аверьянова #2538 от 21.06.16 //заполнение функции(3-е с/к) в 91 счете 
	ИначеЕсли ТипЗнч(Регистратор) = Тип("ДокументСсылка.ПередачаОС")  или Типзнч(Регистратор) = Тип("ДокументСсылка.СписаниеОС") Тогда
		ТаблицаВыручки.Свернуть("Контрагент,Номенклатура,Содержание,СчетУчетаНДСПоРеализации,СтавкаНДС,Субконто,Субконто3,Подразделение", "СуммаНДСРуб");
	//ОК-
	Иначе
	//izhtc alena 18.08.2015 )
		ТаблицаВыручки.Свернуть(
	...
	Для каждого СтрокаВыручки Из ТаблицаВыручки Цикл
		...
		Попытка
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт,
				3, СтрокаВыручки.Субконто3); // Для счетов 90.03 и 91.02
		Исключение
		КонецПопытки;
		//ОК Калинин М. 
		//izhtc-Kir 18.08.2015 )			
		...
		Если ЭтоРеализацияТоваровУслуг Тогда 
			//ОК Калинин М. 260313
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 3, СтрокаВыручки.Субконто3);
			//ОК Калинин М.
			
			// БИТ AMerkulov 09012014 ++ /Возможность выбора второго субконто
			Если ЗначениеЗаполнено(СтрокаВыручки.Субконто2) Тогда
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 2,           СтрокаВыручки.Субконто2);
			КонецЕсли;
			// БИТ AMerkulov --	
		КонецЕсли;
		//izhtc alena 18.08.2015)
...
Процедура СформироватьПроводкиСписаниеНДС(Реквизиты, ТаблицаДвижений, Движения, Отказ)
	...
	// +СБ. Широков Николай. 2016-05-23. #2466
	ЭтоАвансовыйОтчет = (ТипЗнч(Движения.Хозрасчетный.Отбор.Регистратор.Значение) = Тип("ДокументСсылка.АвансовыйОтчет"));
	// -СБ. Широков Николай 
...
Функция ПечатьСчетовФактур534(МассивОбъектов, ОбъектыПечати, ТекстЗапросаПоСчетамФактурам, ПараметрыПечати) Экспорт
...
Функция ПечатьСчетовФактур534Прослеж(МассивОбъектов, ОбъектыПечати, ТекстЗапросаПоСчетамФактурам, ПараметрыПечати) Экспорт
	...
	ТабДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_СчетФактураВыданный_СчетФактура534Прослеж";
	
	УстановкаМинимальныхПолейДляПечати(ТабДокумент);
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_СчетФактура534Прослеж");
...
Функция ПечатьКорректировочныхСчетовФактур534(МассивОбъектов, ОбъектыПечати, ТекстЗапросаПоСчетамФактурам, ПараметрыПечати) Экспорт
	...
	ТабДокумент.АвтоМасштаб = Истина;
	ТабДокумент.ОриентацияСтраницы = ОриентацияСтраницы.Ландшафт;
	ТабДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_СчетФактураВыданный_КорректировочныйСчетФактура534";
...
Функция ПечатьКорректировочныхСчетовФактур534Прослеж(МассивОбъектов, ОбъектыПечати, ТекстЗапросаПоСчетамФактурам, ПараметрыПечати) Экспорт
	...
	ТабДокумент.АвтоМасштаб = Истина;
	ТабДокумент.ОриентацияСтраницы = ОриентацияСтраницы.Ландшафт;
	ТабДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_СчетФактураВыданный_КорректировочныйСчетФактура534Прослеж";
	
	УстановкаМинимальныхПолейДляПечати(ТабДокумент);
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_КорректировочныйСчетФактура534Прослеж");
	Возврат ТабличныйДокументКорректировочногоСчетаФактуры(
		Макет, МассивОбъектов, ОбъектыПечати, ТабДокумент, ТекстЗапросаПоСчетамФактурам, ПараметрыПечати);
		
КонецФункции
// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-08 (#4206)
...
Процедура ВывестиСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ВыборкаСФ, ЭтоУниверсальныйПередаточныйДокумент = Ложь)
	...
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-06-21 (#4208)
	// При печати УПД используем постановление 4, после переноса функционала печати УПД необходимо удалить этот код 
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-30 (#4214)
	//#Область ВременноеРешение
	//
	//Если ЭтоУниверсальныйПередаточныйДокумент И ВерсияПостановления1137 = 5 Тогда
	//	ВерсияПостановления1137 = 4;
	//КонецЕсли;
	//
	//#КонецОбласти
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-30 (#4214) 
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-21 (#4208)
	...
	//		ОбластьМакета.Параметры.ДокументыОбОтгрузке       = ДанныеШапки.ДокументыОбОтгрузке;
...
Процедура ВывестиКорректировочныйСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ВыборкаСФ, ЭтоУниверсальныйКорректировочныйДокумент = Ложь)
	...
	Если ЭтоУниверсальныйКорректировочныйДокумент Тогда
		...
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-06-16 (#4208)
		//ОбластьМакета.Параметры.Валюта = ДанныеШапки.ВалютаНаименованиеУКД;
		ОбластьМакета.Параметры.Валюта = ДанныеШапки.ВалютаНаименованиеБезЗаголовка;
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-16 (#4208)
	...
	Если ВедетсяУчетНДСПоФЗ56 Тогда 
		...
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-06-09 (#4208)
		//Если ЭтоУниверсальныйКорректировочныйДокумент Тогда 
		Если ЭтоУниверсальныйКорректировочныйДокумент
			ИЛИ ВерсияПостановления1137 >= 5 Тогда
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-09 (#4208)
			ОбластьМакета.Параметры.ИдентификаторГосКонтракта = ДанныеШапки.ИдентификаторГосКонтракта;
		Иначе
	...
	Для каждого СтрокаДокумента Из ТаблицаДокумента Цикл
		...
		Если ВерсияПостановления1137 >= 5 Тогда
			Если ПараметрСуществует(ОбластьМакетаСтрокаДо, "ПредставлениеГТДДо") Тогда
				Если ЗначениеЗаполнено(СтрокаДокумента.РегистрационныйНомерТДДо) Тогда
					ОбластьМакетаСтрокаДо.Параметры.ПредставлениеГТДДо = СтрокаДокумента.РегистрационныйНомерТДДо;
				Иначе
			...
			Если ПараметрСуществует(ОбластьМакетаСтрокаДо, "ПредставлениеГТДПосле") Тогда
				Если ЗначениеЗаполнено(СтрокаДокумента.РегистрационныйНомерТДПосле) Тогда
					ОбластьМакетаСтрокаДо.Параметры.ПредставлениеГТДПосле = СтрокаДокумента.РегистрационныйНомерТДПосле;
				Иначе
			...
			КонецЕсли;
		КонецЕсли;
		ДозаполнитьСтрокуКорректировочногоСчетаФактуры1137(ОбластьМакетаСтрокаДо, НаАванс);
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-09 (#4208)
		...
		Если ВыводитьРНПТ Тогда
			...
			Если ЗначениеЗаполнено(СтрокаДокумента.СведенияОПрослеживаемости) Тогда
				Для ИндексСтроки = 0 По ТаблицаРНПТ.Количество() - 1 Цикл
					...
					КонецЕсли;
					Если ДанныеШапки.НДСИсчисляетсяНалоговымАгентом Тогда
						ДозаполнитьПараметрыНДСИсчисляетНалоговыйАгент(ОбластьМакетаСтрокаДо);
					КонецЕсли;
					ПроверитьВыводКорректировочногоСчетаФактурыНаСтраницу(
						ТабДокумент, ОбластьМакетаСтрокаДо, Ложь, Макет, НомерЛиста, НомерСчетаФактуры, ДатаСчетаФактуры, ЭтоУниверсальныйКорректировочныйДокумент);
			...
			Иначе
				...
				Если ЗначениеЗаполнено(СтрокаДокумента.РегистрационныйНомерТДДо) Тогда
					ОбластьМакетаСтрокаДо.Параметры.ПредставлениеГТДДо       = СтрокаДокумента.РегистрационныйНомерТДДо;
					ОбластьМакетаСтрокаДо.Параметры.ПредставлениеСтраныДо    = СтрокаДокумента.ПредставлениеСтраныДо;
					ОбластьМакетаСтрокаДо.Параметры.СтранаПроисхожденияКодДо = СтрокаДокумента.СтранаПроисхожденияКодДо;
				Иначе
					ОбластьМакетаСтрокаДо.Параметры.ПредставлениеГТДДо       = "--";
					ОбластьМакетаСтрокаДо.Параметры.ПредставлениеСтраныДо    = "--";
					ОбластьМакетаСтрокаДо.Параметры.СтранаПроисхожденияКодДо = "--";
			...
			Если ЗначениеЗаполнено(СтрокаДокумента.СведенияОПрослеживаемости) Тогда
				Для ИндексСтроки = 0 По ТаблицаРНПТ.Количество() - 1 Цикл
					Если Не ПерваяСтрока Тогда
						...
						ОбластьЗаписьПосле.ГраницаСверху = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии);
					Иначе
						ДозаполнитьСтрокуКорректировочногоСчетаФактуры1137(ОбластьМакетаСтрокаПосле, НаАванс);
					КонецЕсли;
					...
					КонецЕсли;
					Если ДанныеШапки.НДСИсчисляетсяНалоговымАгентом Тогда
						ДозаполнитьПараметрыНДСИсчисляетНалоговыйАгент(ОбластьМакетаСтрокаПосле);
					КонецЕсли;
					ПроверитьВыводКорректировочногоСчетаФактурыНаСтраницу(
						ТабДокумент, ОбластьМакетаСтрокаПосле, Ложь, Макет, НомерЛиста, НомерСчетаФактуры, ДатаСчетаФактуры, ЭтоУниверсальныйКорректировочныйДокумент);
			...
			Иначе
				// Непрослеживаемый товар.
				Если ЭтоУниверсальныйКорректировочныйДокумент 
					И ДанныеШапки.СтатусУКД = 2 Тогда
					ПроставитьПрочеркиУКДСтатус2(ОбластьМакетаСтрокаПосле);
				КонецЕсли;
				Если ЗначениеЗаполнено(СтрокаДокумента.РегистрационныйНомерТДПосле) Тогда
					ОбластьМакетаСтрокаПосле.Параметры.ПредставлениеГТДПосле       = СтрокаДокумента.РегистрационныйНомерТДПосле;
					ОбластьМакетаСтрокаПосле.Параметры.ПредставлениеСтраныПосле    = СтрокаДокумента.ПредставлениеСтраныПосле;
					ОбластьМакетаСтрокаПосле.Параметры.СтранаПроисхожденияКодПосле = СтрокаДокумента.СтранаПроисхожденияКодПосле;
				Иначе
			...
			Если ДанныеШапки.НДСИсчисляетсяНалоговымАгентом Тогда
				ДозаполнитьПараметрыНДСИсчисляетНалоговыйАгент(ОбластьМакетаСтрокаУвеличение);
			КонецЕсли;
			Если ЭтоУниверсальныйКорректировочныйДокумент 
				И ДанныеШапки.СтатусУКД = 2 Тогда
				ПроставитьПрочеркиУКДСтатус2(ОбластьМакетаСтрокаУвеличение);
			КонецЕсли;
			ПроверитьВыводКорректировочногоСчетаФактурыНаСтраницу(
				ТабДокумент, ОбластьМакетаСтрокаУвеличение, Ложь, Макет, НомерЛиста, НомерСчетаФактуры, ДатаСчетаФактуры, ЭтоУниверсальныйКорректировочныйДокумент);
			...
			Если ДанныеШапки.НДСИсчисляетсяНалоговымАгентом Тогда
				ДозаполнитьПараметрыНДСИсчисляетНалоговыйАгент(ОбластьМакетаСтрокаУменьшение);
			КонецЕсли;
			Если ЭтоУниверсальныйКорректировочныйДокумент 
				И ДанныеШапки.СтатусУКД = 2 Тогда
				ПроставитьПрочеркиУКДСтатус2(ОбластьМакетаСтрокаУменьшение);
			КонецЕсли;
			ПроверитьВыводКорректировочногоСчетаФактурыНаСтраницу(
				ТабДокумент, ОбластьМакетаСтрокаУменьшение, (НомерСтроки = КоличествоСтрок), Макет, НомерЛиста, НомерСчетаФактуры, ДатаСчетаФактуры, ЭтоУниверсальныйКорректировочныйДокумент);
		...
		Иначе
			...
			КонецЕсли;
			ПроверитьВыводКорректировочногоСчетаФактурыНаСтраницу(
				ТабДокумент, ОбластьМакетаСтрокаДо, (НомерСтроки = КоличествоСтрок), Макет, НомерЛиста, НомерСчетаФактуры, ДатаСчетаФактуры, ЭтоУниверсальныйКорректировочныйДокумент);
			ТабДокумент.Вывести(ОбластьМакетаСтрокаДо);
		КонецЕсли;
...
Процедура ДозаполнитьСтрокуКорректировочногоСчетаФактуры1137(ОбластьМакета, НаАванс)
	...
	Если НЕ НаАванс 
		И ПараметрСуществует(ОбластьМакета, "СтавкаНДС")
...
Функция ПодготовитьДанныеДляПечатиСчетовФактур(ДокументОснование, ДанныеСчетаФактуры) Экспорт
	...
	// Прослеживаемость
	ЕстьСведенияОПрослеживаемости = Ложь;
	Если НомераТаблиц.Свойство("СведенияОПрослеживаемости") Тогда
		СведенияОПрослеживаемости = Результат[НомераТаблиц.СведенияОПрослеживаемости].Выгрузить();
		ЕстьСведенияОПрослеживаемости = Истина;
		
		//СведенияОПрослеживаемости.Колонки.НомерТовара.ТипЗначения = Новый ОписаниеТипов("Строка",,,, Новый КвалификаторыСтроки(30,ДопустимаяДлина.Переменная));
		Для Каждого СтрокаТаблицы Из СведенияОПрослеживаемости Цикл
			СтрокаТаблицы.НомерТовара = СокрЛП(СтрокаТаблицы.НомерТовара);	
		КонецЦикла; 
	...
	Если НуженПересчетВРубли Тогда
		...
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-15 (#4214)
		// Необходимо пересчитать цену и в таблице "СведенияОПрослеживаемости" для связи товарных позиций с РНПТ.
		// Для позиций с совпадающей ценой и количеством порядок пересчета будет одинаковым.
		Если ЕстьСведенияОПрослеживаемости Тогда
			СведенияОПрослеживаемости.Индексы.Добавить("Цена, КоличествоУчетное");
		КонецЕсли;
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-15 (#4214) 
	КонецЕсли;
	...
	Для каждого СтрокаДокумента Из ТаблицаДокумента Цикл
		...
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-15 (#4214)
		СтрокиПрослеживаемостиДляПересчетаЦены = Новый Массив;
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-15 (#4214) 
		Если НуженПересчетВРубли Тогда
			//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-15 (#4214)
			Если ЕстьСведенияОПрослеживаемости Тогда
				Отбор = Новый Структура("Цена, КоличествоУчетное", СтрокаДокумента.Цена, СтрокаДокумента.Количество);
				СтрокиПрослеживаемостиДляПересчетаЦены = СведенияОПрослеживаемости.НайтиСтроки(Отбор);
			КонецЕсли;
			//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-15 (#4214) 
		...
		ИначеЕсли СтрокаДокумента.Цена = 0 Тогда
			...
			//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-15 (#4214)
			Для Каждого СтрокаПрослеживаемости ИЗ СтрокиПрослеживаемостиДляПересчетаЦены Цикл
...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц)
	...
	|	ТаблицаДокумента.ПериодичностьУслуги КАК ПериодичностьУслуги,
	|	ЕСТЬNULL(ДанныеПервичныхДокументовКорректировка.Дата, ДанныеПервичныхДокументов.Дата) КАК ДатаОтгрузочногоДокумента,
	|	ЕСТЬNULL(ДанныеПервичныхДокументовКорректировка.Номер, ДанныеПервичныхДокументов.Номер) КАК НомерОтгрузочногоДокумента
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-09 (#4208)
	|ИЗ
	|	ТаблицаДокумента КАК ТаблицаДокумента
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументовКорректировка
	|			ПО КорректировкаРеализации.ИсправляемыйДокументРеализации = ДанныеПервичныхДокументовКорректировка.Документ
	|		ПО ТаблицаДокумента.Ссылка = КорректировкаРеализации.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ТаблицаДокумента.Ссылка = ДанныеПервичныхДокументов.Документ
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-09 (#4208)
	|ГДЕ
	|	ВЫБОР
	|			КОГДА &НеподтверждениеНулевойСтавки
...
Функция ТекстЗапросаДанныеДляПечатиКорректировочныхСчетовФактур(НомераТаблиц)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СтранаПроисхожденияДо = ЗНАЧЕНИЕ(Справочник.СтраныМира.РОССИЯ)
	|			ТОГДА """"
	...
	|	ТаблицаДокумента.РегистрационныйНомерТДДо КАК РегистрационныйНомерТДДо,
	|	ТаблицаДокумента.РегистрационныйНомерТДПосле КАК РегистрационныйНомерТДПосле
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-09 (#4208)
	|ИЗ
	|	ТаблицаДокумента КАК ТаблицаДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторЕдиницИзмерения КАК КлассификаторЕдиницИзмерения
	...
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-06-09 (#4208)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтраныМира КАК СтраныМираПосле
	|		ПО ТаблицаДокумента.СтранаПроисхождения = СтраныМираПосле.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтраныМира КАК СтраныМираДо
...
// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-09 (#4208)	
	...
	Пока ВыборкаСФ.Следующий() Цикл
		...
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-06-09 (#4208)
		Если ЗначениеЗаполнено(СведенияОПрослеживаемости) Тогда
			ДополнитьСведениямиОПрослеживаемости(СведенияОПрослеживаемости, ТаблицаДокумента);
		КонецЕсли;
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-09 (#4208)
...
Процедура ДозаполнитьПараметрыНДСИсчисляетНалоговыйАгент(ОбластьМакета)
...
Процедура ДополнитьДанныеШапкиСведениямиОДокументеОтгрузки(ДанныеШапки, ТаблицаДокумента, ЭтоУПД = Ложь)
	...
	Для ИндексСтроки = 0 По ТаблицаДокумента.Количество() - 1 Цикл
		...
		Если ТаблицаДокумента[ИндексСтроки].Ссылка <> ПредыдущийДокумент Тогда
			...
			Если (ПоследняяСтрокаПоДокументуОтгрузки - ПерваяСтрокаСтрокаПоДокументуОтгрузки) > 0 Тогда
				ДиапазонСтрок = "" + ПерваяСтрокаСтрокаПоДокументуОтгрузки + "-" + ПоследняяСтрокаПоДокументуОтгрузки;
			Иначе
				ДиапазонСтрок = ПерваяСтрокаСтрокаПоДокументуОтгрузки;
			КонецЕсли;
			ДокументОтгрузки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПодстрокиДокументаОбОтгрузке,
				ДиапазонСтрок,
				ТаблицаДокумента[ИндексСтроки-1].НомерОтгрузочногоДокумента,
			...
			ПерваяСтрокаСтрокаПоДокументуОтгрузки = ТекущийНомерСтроки;
			ПоследняяСтрокаПоДокументуОтгрузки    = ТекущийНомерСтроки;
			ПредыдущийДокумент = ТаблицаДокумента[ИндексСтроки].Ссылка;
		КонецЕсли;
		...
		Если ЭтоПоследняяСтрока Тогда
			...
			ДокументОтгрузки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПодстрокиДокументаОбОтгрузке,
				ДиапазонСтрок,
				ТаблицаДокумента[ИндексСтроки].НомерОтгрузочногоДокумента,
...
Процедура ДополнитьСведениямиОПрослеживаемости(СведенияОПрослеживаемости, ТаблицаДокумента)
	...
	ИдентифицирующиеРеквизитыСтроки = КолонкиГруппировкиТаблицыСчетаФактуры(Истина);
	КолонкиГруппировок = ИдентифицирующиеРеквизитыСтроки
		+ ","
		+ "НомерТовара,"
		+ "ЕдиницаИзмеренияПрослеживаемостиКод,"
		+ "ЕдиницаИзмеренияПрослеживаемостиНаименование";
	КолонкиСуммирования = ""
		+ "Количество,"
		+ "КоличествоУчетное";
	СведенияОПрослеживаемости.Свернуть(КолонкиГруппировок, КолонкиСуммирования);
	ТаблицаДокумента.Колонки.Добавить("СведенияОПрослеживаемости");
	МассивИдентифицирующихРеквизитов = 
	СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ИдентифицирующиеРеквизитыСтроки);
	Для Каждого СтрокаТаблицыДокумента Из ТаблицаДокумента Цикл
		...
		Для Каждого НайденнаяСтрока Из СтрокиСРНПТ Цикл
			...
			СтрокаСведенийОПрослеживаемости.ЕдиницаИзмеренияКод = НайденнаяСтрока.ЕдиницаИзмеренияПрослеживаемостиКод;
			СтрокаСведенийОПрослеживаемости.ЕдиницаИзмеренияНаименование = 
			НайденнаяСтрока.ЕдиницаИзмеренияПрослеживаемостиНаименование;
		КонецЦикла;
...
Процедура ДополнитьСведениямиОПрослеживаемостиПоКорректировке(СведенияОПрослеживаемости, ТаблицаДокумента)
	
	ИдентифицирующиеРеквизитыСтроки = КолонкиГруппировкиТаблицыКорректировочногоСчетаФактуры(Истина);
	КолонкиГруппировок = ИдентифицирующиеРеквизитыСтроки
		+ ","
		+ "НомерТовара,"
		+ "ЕдиницаИзмеренияПрослеживаемостиКод,"
	...
	МассивИдентифицирующихРеквизитов = 
		СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ИдентифицирующиеРеквизитыСтроки);
	Для Каждого СтрокаТаблицыДокумента Из ТаблицаДокумента Цикл
		ТаблицаСведенийОПрослеживаемости = НовыйТаблицаСведенийОПрослеживаемостиПоКорректировке();
		ПараметрыОтбора = ПараметрыОтбораСтрокДляРНПТ(МассивИдентифицирующихРеквизитов, СтрокаТаблицыДокумента);
		СтрокиСРНПТ = СведенияОПрослеживаемости.НайтиСтроки(ПараметрыОтбора);
		Для Каждого НайденнаяСтрока Из СтрокиСРНПТ Цикл
			СтрокаСведенийОПрослеживаемости = ТаблицаСведенийОПрослеживаемости.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаСведенийОПрослеживаемости, НайденнаяСтрока);
			//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-09-02 (#4281)
			СтрокаСведенийОПрослеживаемости.НомерТовара = СокрЛП(НайденнаяСтрока.НомерТовара);
			//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-09-02 (#4281) 
			СтрокаСведенийОПрослеживаемости.ЕдиницаИзмеренияКод = НайденнаяСтрока.ЕдиницаИзмеренияПрослеживаемостиКод;
			СтрокаСведенийОПрослеживаемости.ЕдиницаИзмеренияНаименование = 
				НайденнаяСтрока.ЕдиницаИзмеренияПрослеживаемостиНаименование;
		КонецЦикла;
...
Функция НовыйТаблицаСведенийОПрослеживаемостиПоКорректировке()
...
Функция КолонкиГруппировкиТаблицыКорректировочногоСчетаФактуры(ДляИдентификацииСтроки = Ложь)
...
Процедура ПроставитьПрочеркиУКДСтатус2(ОбластьМакета)
...
Функция ПолучитьДанныеДляПечатиКорректировочныхСчетовФактур1137(МассивОбъектов, ТекстЗапросаПоСчетамФактурам, ДополнитьДаннымиУПД = Ложь) Экспорт
	...
	Пока ВыборкаСФ.Следующий() Цикл
		...
		//+"СтавкаНДСПослеИзменения,"
		//+"Акциз";
		КолонкиГруппировок = КолонкиГруппировкиТаблицыКорректировочногоСчетаФактуры();
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-09 (#4208)
		...
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-06-09 (#4208)
		Если ЗначениеЗаполнено(СведенияОПрослеживаемости) Тогда
			ДополнитьСведениямиОПрослеживаемостиПоКорректировке(СведенияОПрослеживаемости, ТаблицаДокумента);
		КонецЕсли;
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-09 (#4208)
...
Процедура ПечатьКомплектаСчетовФактур(КоллекцияПечатныхФорм, МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода = Неопределено) Экспорт
	...
	Для Каждого МакетСчетаФактуры Из МакетыСчетовФактур Цикл
		...
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-06-08 (#4208)
		ИначеЕсли МакетСчетаФактуры.Ключ = "СчетФактура534" Тогда
			СинонимМакета = ?(КоличествоЗначащихМакетов > 1, НСтр("ru = 'Счет-фактура 1137'"), НСтр("ru = 'Счет-фактура'"));
			ТабДокумент = ПечатьСчетовФактур534(
			МассивОбъектовПечатьСчетовФактур,
			ОбъектыПечатиСчетаФактуры,
			Документы.СчетФактураВыданный.ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура534"]),
			ПараметрыПечати);
		ИначеЕсли МакетСчетаФактуры.Ключ = "СчетФактура534Прослеж" Тогда
			СинонимМакета = ?(КоличествоЗначащихМакетов > 1, НСтр("ru = 'Счет-фактура 1137'"), НСтр("ru = 'Счет-фактура'"));
			ТабДокумент = ПечатьСчетовФактур534Прослеж(
			МассивОбъектовПечатьСчетовФактур,
			ОбъектыПечатиСчетаФактуры,
			Документы.СчетФактураВыданный.ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура534Прослеж"],Ложь, Истина),
			ПараметрыПечати);	
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-08 (#4208)
...
// Формирует табличный документ для вывода на печать УПД.
//
// Параметры:
// СтруктураПараметровПечати - см. НовыйСтруктураПараметровПечатиУПД_УКД()
//
// Возвращаемое значение:
// ТабДокумент - ТабличныйДокумент печати УПД.
//
Функция ПечатьУниверсальныхПередаточныхДокументов(СтруктураПараметровПечати) Экспорт
	
	ТабДокумент = СтруктураПараметровПечати.ТабДокумент;
	Если ТабДокумент = Неопределено Тогда
		ТабДокумент = Новый ТабличныйДокумент;
	Иначе
		ТабДокумент.ВывестиГоризонтальныйРазделительСтраниц();
	...
	ТабДокумент.КлючПараметровПечати = СтруктураПараметровПечати.КлючПараметровПечати;
	
	УстановкаМинимальныхПолейДляПечати(ТабДокумент);
	
	Макет = УправлениеПечатью.МакетПечатнойФормы(СтруктураПараметровПечати.ПолныйПутьКМакету);
	
	Возврат ТабличныйДокументУПД(
		Макет, 
		СтруктураПараметровПечати.МассивОбъектов,
		СтруктураПараметровПечати.ОбъектыПечати, 
		ТабДокумент, 
		СтруктураПараметровПечати.ТекстЗапросаДокументам, 
		СтруктураПараметровПечати.ТолькоПередаточныйДокумент, 
		СтруктураПараметровПечати.ПараметрыПечати);
...
Функция ПолучитьДанныеДляПечатиУниверсальногоПередаточногоДокумента(МассивОбъектов, ТекстЗапросаДокументам, ФормированиеЭД = Ложь) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("МассивОбъектов", МассивОбъектов);
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-27 (#4214)
	Запрос.УстановитьПараметр("НачалоПримененияПостановления1137",
		УчетНДСБП.ПолучитьДатуНачалаДействияПостановления1137());
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-27 (#4214) 
	...
	Пока ВыборкаПоОснованиям.Следующий() Цикл
		...
		ДополнитьДанныеШапкиСведениямиОДокументеОтгрузки(ДанныеШапки, ТаблицаДокумента, Истина);
...
Функция ПодготовитьДанныеДляПечатиУниверсальныхПередаточныхДокументов(ДокументОснование, ДанныеСчетаФактуры, ФормированиеЭД = Ложь)
	...
	Если ДанныеДляПечати.ТаблицаДокумента <> Неопределено Тогда
		СведенияОПрослеживаемости = Неопределено;
		ДанныеДляПечати.Свойство("СведенияОПрослеживаемости", СведенияОПрослеживаемости);
		Если ЗначениеЗаполнено(СведенияОПрослеживаемости) Тогда
			ДополнитьСведениямиОПрослеживаемости(СведенияОПрослеживаемости, ДанныеДляПечати.ТаблицаДокумента);
		КонецЕсли;
	КонецЕсли;
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-15 (#4214) 
...
Функция ПечатьКомплектаУПД(КоллекцияПечатныхФорм, МассивОбъектов, ОбъектыПечати, ИменаФайлов, ПараметрыПечати, ПараметрыВывода = Неопределено) Экспорт
	...
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-20 (#4214)
	ТаблицаМакетовУПД = ТаблицаМакетовУПД();
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-20 (#4214) 
	...
	Для Каждого МакетУПД Из МакетыУПД Цикл
		...
		//		ТабДокумент = ПечатьУниверсальныхПередаточныхДокументов981(МассивОбъектовПечатьУПД, ОбъектыПечатиУПД, 
		//			ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов, Истина, ТабДокумент, ПараметрыПечати);
		СтрокаПараметровМакета = ТаблицаМакетовУПД.Найти(МакетУПД.Ключ, "ИДМакета");
		Если Не ЗначениеЗаполнено(СтрокаПараметровМакета) Тогда
			// Пустой табличный документ
		ИначеЕсли СтрокаПараметровМакета.Статус = "1" Тогда
			
			ТекстЗапроса = Документы.СчетФактураВыданный.ТекстЗапросаПечатьСчетовФактур(
				СтрокаПараметровМакета.ВерсияПостановленияНДС1137, Истина, СтрокаПараметровМакета.ПрослеживаемыеТовары);
			
			СтруктураПараметровПечати = НовыйСтруктураПараметровПечатиУПД_УКД();
			СтруктураПараметровПечати.МассивОбъектов              = МассивОбъектовПечатьУПД;
			СтруктураПараметровПечати.ОбъектыПечати               = ОбъектыПечатиУПД;
			СтруктураПараметровПечати.ТекстЗапросаДокументам      = ТекстЗапроса;
			СтруктураПараметровПечати.ТолькоПередаточныйДокумент  = Ложь;
			СтруктураПараметровПечати.ТабДокумент                 = ТабДокумент;
			СтруктураПараметровПечати.ПараметрыПечати             = ПараметрыПечати;
			СтруктураПараметровПечати.КлючПараметровПечати        = СтрокаПараметровМакета.КлючПараметровПечати;
			СтруктураПараметровПечати.ПолныйПутьКМакету           = СтрокаПараметровМакета.ПолныйПутьКМакету;
			
			ТабДокумент = ПечатьУниверсальныхПередаточныхДокументов(СтруктураПараметровПечати);
			
		ИначеЕсли СтрокаПараметровМакета.Статус = "2" Тогда
			ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов = "";
			Для каждого Объект Из МассивОбъектовПечатьУПД Цикл
				Если ТипЗнч(Объект) <> Тип("ДокументСсылка.СчетФактураВыданный") Тогда
					ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов = 
						Документы[Объект.Метаданные().Имя].ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов(
						СтрокаПараметровМакета.ВерсияПостановленияНДС1137,СтрокаПараметровМакета.ПрослеживаемыеТовары);
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если НЕ ПустаяСтрока(ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов) Тогда
...
//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-08-29 (#4214) 
...
//		Макет, МассивОбъектов, ОбъектыПечати, ТабДокумент, ТекстЗапросаДокументам, ТолькоПередаточныйДокумент, ПараметрыПечати);
Функция ПечатьУниверсальныхКорректировочныхДокументов(СтруктураПараметровПечати) Экспорт
	...
	Возврат ТабличныйДокументУКД(
		...
		СтруктураПараметровПечати.ТолькоПередаточныйДокумент, 
		СтруктураПараметровПечати.ПараметрыПечати);
//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-08-29 (#4214) 		
КонецФункции
...
Функция ПолучитьДанныеДляПечатиУниверсальногоКорректировочногоДокумента(МассивОбъектов, ТекстЗапросаДокументам) Экспорт
	...
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-09-06 (#4281) 
	Запрос.УстановитьПараметр("НачалоПримененияПостановления1137", УчетНДСБП.ПолучитьДатуНачалаДействияПостановления1137());
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-09-06 (#4281) 
	...
	Пока ВыборкаПоОснованиям.Следующий() Цикл
		...
		// Прослеживаемость
		СведенияОПрослеживаемости = Неопределено;
		ПараметрыОснования.Свойство("СведенияОПрослеживаемости", СведенияОПрослеживаемости);
		Если ЗначениеЗаполнено(СведенияОПрослеживаемости) Тогда
			ДополнитьСведениямиОПрослеживаемостиПоКорректировке(СведенияОПрослеживаемости, ТаблицаДокумента);
		КонецЕсли;
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-20 (#4214)
...
Функция ТабличныйДокументУКД(Макет, МассивОбъектов, ОбъектыПечати, ТабДокумент, ТекстЗапросаДокументам, ТолькоПередаточныйДокумент, ПараметрыПечати)
	...
	Для Каждого ВыборкаУКД ИЗ ДанныеУниверсальныхКорректировочныхДокументов Цикл
		...
		ВывестиКорректировочныйСчетФактуруВТабличныйДокумент2020(ТабДокумент, Макет, ВыборкаУКД, Истина);
...
Процедура ВывестиШапкуДопЛиста(ТабличныйДокумент, Макет, СтруктураПараметров, НомерДополнительногоЛиста) Экспорт
	...
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-09-12 (#4347)
	...
	Если ВерсияПостановленияНДС1137 >= 5 Тогда
		...
		КонецЕсли;
	КонецЕсли;
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-09-12 (#4347)
...
// Формирует таблицу макетов всех печатных форм УКД для разный версий постановления 1137.
//
// Возвращаемое значение:
//  ТаблицаМакетов - ТаблицаЗначений - см.тело функции
//
Функция ТаблицаМакетовУКД() Экспорт
	...
	// УКД.
	НовыйМакет = ТаблицаМакетов.Добавить();
	НовыйМакет.ИДМакета                   = "УКДСтатус1";
	НовыйМакет.СинонимМакета              = "УКД (статус 1)";
	НовыйМакет.Статус                     = "1";
	НовыйМакет.ВерсияПостановленияНДС1137 = ПорядокПримененияФормСФ[НовыйМакет.ИДМакета];
	НовыйМакет.ПолныйПутьКМакету          = "Обработка.ПечатьУКД.ПФ_MXL_УниверсальныйКорректировочныйДокумент";
	НовыйМакет.ПрослеживаемыеТовары       = Ложь;
	НовыйМакет.КлючПараметровПечати       = "ПАРАМЕТРЫ_ПЕЧАТИ_УниверсальныйКорректировочныйДокумент";
	
	НовыйМакет = ТаблицаМакетов.Добавить();
	НовыйМакет.ИДМакета                   = "УКДСтатус2";
	НовыйМакет.СинонимМакета              = "УКД (статус 2)";
	НовыйМакет.Статус                     = "2";
	НовыйМакет.ВерсияПостановленияНДС1137 = ПорядокПримененияФормСФ[НовыйМакет.ИДМакета];
	НовыйМакет.ПолныйПутьКМакету          = "Обработка.ПечатьУКД.ПФ_MXL_УниверсальныйКорректировочныйДокумент";
	НовыйМакет.ПрослеживаемыеТовары       = Ложь;
	...
	// УКД 981.
	НовыйМакет = ТаблицаМакетов.Добавить();
	НовыйМакет.ИДМакета                   = "УКДСтатус1_981";
	НовыйМакет.СинонимМакета              = "УКД (статус 1)";
	НовыйМакет.Статус                     = "1";
	НовыйМакет.ВерсияПостановленияНДС1137 = ПорядокПримененияФормСФ[НовыйМакет.ИДМакета];
	НовыйМакет.ПолныйПутьКМакету          = "Обработка.ПечатьУКД.ПФ_MXL_УниверсальныйКорректировочныйДокумент981";
	НовыйМакет.ПрослеживаемыеТовары       = Ложь;
	НовыйМакет.КлючПараметровПечати       = "ПАРАМЕТРЫ_ПЕЧАТИ_УниверсальныйКорректировочныйДокумент_981";
	
	НовыйМакет = ТаблицаМакетов.Добавить();
	НовыйМакет.ИДМакета                   = "УКДСтатус2_981";
	НовыйМакет.СинонимМакета              = "УКД (статус 2)";
	НовыйМакет.Статус                     = "2";
	НовыйМакет.ВерсияПостановленияНДС1137 = ПорядокПримененияФормСФ[НовыйМакет.ИДМакета];
	НовыйМакет.ПолныйПутьКМакету          = "Обработка.ПечатьУКД.ПФ_MXL_УниверсальныйКорректировочныйДокумент981";
	...
	// без прослеживаемого товара.
	НовыйМакет = ТаблицаМакетов.Добавить();
	НовыйМакет.ИДМакета                   = "УКДСтатус1_534";
	НовыйМакет.СинонимМакета              = "УКД (статус 1)";
	НовыйМакет.Статус                     = "1";
	НовыйМакет.ВерсияПостановленияНДС1137 = ПорядокПримененияФормСФ[НовыйМакет.ИДМакета];
	НовыйМакет.ПолныйПутьКМакету          = "Обработка.ПечатьУКД.ПФ_MXL_УниверсальныйКорректировочныйДокумент534";
	НовыйМакет.ПрослеживаемыеТовары       = Ложь;
	НовыйМакет.КлючПараметровПечати       = "ПАРАМЕТРЫ_ПЕЧАТИ_УниверсальныйКорректировочныйДокумент_534";
	
	НовыйМакет = ТаблицаМакетов.Добавить();
	НовыйМакет.ИДМакета                   = "УКДСтатус2_534";
	НовыйМакет.СинонимМакета              = "УКД (статус 2)";
	НовыйМакет.Статус                     = "2";
	НовыйМакет.ВерсияПостановленияНДС1137 = ПорядокПримененияФормСФ[НовыйМакет.ИДМакета];
	НовыйМакет.ПолныйПутьКМакету          = "Обработка.ПечатьУКД.ПФ_MXL_УниверсальныйКорректировочныйДокумент534";
	...
	// с прослеживаемым товаром.
	НовыйМакет = ТаблицаМакетов.Добавить();
	НовыйМакет.ИДМакета                   = "УКДСтатус1_534Прослеж";
	НовыйМакет.СинонимМакета              = "УКД (статус 1)";
	НовыйМакет.Статус                     = "1";
	НовыйМакет.ВерсияПостановленияНДС1137 = ПорядокПримененияФормСФ[НовыйМакет.ИДМакета];
	НовыйМакет.ПолныйПутьКМакету          = "Обработка.ПечатьУКД.ПФ_MXL_УниверсальныйКорректировочныйДокумент534Прослеж";
	НовыйМакет.ПрослеживаемыеТовары       = Истина;
	НовыйМакет.КлючПараметровПечати       = "ПАРАМЕТРЫ_ПЕЧАТИ_УниверсальныйКорректировочныйДокумент_534Прослеж";
	
	НовыйМакет = ТаблицаМакетов.Добавить();
	НовыйМакет.ИДМакета                   = "УКДСтатус2_534Прослеж";
	НовыйМакет.СинонимМакета              = "УКД (статус 2)";
	НовыйМакет.Статус                     = "2";
	НовыйМакет.ВерсияПостановленияНДС1137 = ПорядокПримененияФормСФ[НовыйМакет.ИДМакета];
	НовыйМакет.ПолныйПутьКМакету          = "Обработка.ПечатьУКД.ПФ_MXL_УниверсальныйКорректировочныйДокумент534Прослеж";
...
//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-09-06 (#4281)
Процедура ВывестиКорректировочныйСчетФактуруВТабличныйДокумент2020(ТабДокумент, Макет, ВыборкаСФ, ЭтоУниверсальныйКорректировочныйДокумент = Ложь)
	...
	Для каждого СтрокаДокумента Из ТаблицаДокумента Цикл
		...
		Если ЭтоУниверсальныйКорректировочныйДокумент Тогда
			...
			Если ДанныеШапки.СтатусУКД = 2 Тогда
				ПроставитьПрочеркиУКДСтатус2(ОбластьМакетаСтрокаДо);
			ИначеЕсли ВерсияПостановления1137 >= 4 Тогда
				СтрокаДокумента.Акциз = "без
				|акциза";
			Иначе
