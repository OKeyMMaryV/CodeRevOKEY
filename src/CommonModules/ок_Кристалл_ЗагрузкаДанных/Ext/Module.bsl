...
Функция ПолучитьПараметры(ИспользоватьЛогирование, ЗаписыватьСообщенияПользователю, ПараметрыЛогирования)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(Выборка.Значение) Тогда
			ок_ЛогированиеОпераций.ЗаписатьИнформациюОбОшибке("Настройка не заполнена: """ + Выборка.Группа + ": " + Выборка.Идентификатор + """", Параметры);
		КонецЕсли;
		#КонецОбласти
...
Функция ПолучитьТекстЗапросаИнтеграционнаяТаблица()
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНастроекЗапроса Цикл
		...
		|				ТОГДА Таблица.Объект = &ОбъектСтрока
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|	И ВЫБОР
...
Функция Мэппинг(ИнтеграционнаяТаблица, Параметры)
	...
	|		ПО (РезультирующаяТаблица.ВидОперации = &ВидОперацииОплатаТовара
	|				ИЛИ РезультирующаяТаблица.ВидОперации = &ВидОперацииОплатаСертификата
	|				ИЛИ РезультирующаяТаблица.ВидОперации = &ВидОперацииОплатаСТОЛОТО)
	|			И (КОНЕЦПЕРИОДА(РезультирующаяТаблица.Дата, ДЕНЬ) = КОНЕЦПЕРИОДА(ПриходныйКассовыйОрдер.Дата, ДЕНЬ))
	...
	|				ИЛИ РезультирующаяТаблица.ВидОперации = &ВидОперацииОплатаСертификата
	|				ИЛИ РезультирующаяТаблица.ВидОперации = &ВидОперацииОплатаСТОЛОТО)
	|			И (КОНЕЦПЕРИОДА(РезультирующаяТаблица.Дата, ДЕНЬ) = КОНЕЦПЕРИОДА(ОплатаПлатежнойКартой.Дата, ДЕНЬ))
	...
	|		ПО (РезультирующаяТаблица.ВидОперации = &ВидОперацииОплатаТовара
	|				ИЛИ РезультирующаяТаблица.ВидОперации = &ВидОперацииОплатаСТОЛОТО)
	|			И (КОНЕЦПЕРИОДА(РезультирующаяТаблица.Дата, ДЕНЬ) = КОНЕЦПЕРИОДА(КорректировкаДолга.Дата, ДЕНЬ))
	|			И (КорректировкаДолга.Комментарий ПОДОБНО РезультирующаяТаблица.Комментарий + ""%"")
	|			И РезультирующаяТаблица.ДоговорСсылка.Владелец = КорректировкаДолга.КонтрагентДебитор
	|
	|СГРУППИРОВАТЬ ПО
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	РезультирующаяТаблица.ВидОплаты";
