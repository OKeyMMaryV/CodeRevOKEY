...
Функция ЗагрузитьПриходныйКассовыйОрдер(Выборка, Параметры)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Выборка.ПриходныйКассовыйОрдерСсылка) Тогда
			...
			Если Выборка.ПриходныйКассовыйОрдерНеЗагружатьПовторно Тогда
				#Область Логирование
				ок_ЛогированиеОпераций.ЗаписатьСообщенияПользователю("Документ запрещено загружать повторно: " 
...
Функция ЗагрузитьОплатуПлатежнойКартой(Выборка, Параметры)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Выборка.ОплатаПлатежнойКартойСсылка) Тогда
			...
			Если Выборка.ОплатаПлатежнойКартойНеЗагружатьПовторно Тогда
				#Область Логирование
				ок_ЛогированиеОпераций.ЗаписатьСообщенияПользователю("Документ запрещено загружать повторно: " 
...
Функция ЗагрузитьКорректировкуДолга(Выборка, Параметры)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Выборка.КорректировкаДолгаСсылка) Тогда
			...
			Если Выборка.КорректировкаДолгаНеЗагружатьПовторно Тогда
				#Область Логирование
				ок_ЛогированиеОпераций.ЗаписатьСообщенияПользователю("Документ запрещено загружать повторно: " 
...
Функция ПолучитьПараметры(ИспользоватьЛогирование, ЗаписыватьСообщенияПользователю, ПараметрыЛогирования)
	...
	НоваяСтрока.Группа			=	"ЗагрузкаДанныхИзКристалл";
	...
	НоваяСтрока	=	ТаблицаЗначений.Добавить();
	НоваяСтрока.Группа			=	"ЗагрузкаДанныхИзКристалл";
	НоваяСтрока.ИмяНастройки	=	ок_ОбщегоНазначенияФинансы21.ПолучитьИмяНастройкиНеЗагружатьПовторно();
	НоваяСтрока.Идентификатор	=	ок_ОбщегоНазначенияФинансы21.ПолучитьИдентификаторНастройкиНеЗагружатьПовторно();
...
Функция ПолучитьСвойствоНеЗагружатьПовторно(Параметры)
	
	ИмяСвойства		=	Параметры[ок_ОбщегоНазначенияФинансы21.ПолучитьИдентификаторНастройкиНеЗагружатьПовторно()];
...
Функция Мэппинг(ИнтеграционнаяТаблица, Параметры)
	...
	СвойствоНеЗагружатьПовторно	=	ПолучитьСвойствоНеЗагружатьПовторно(Параметры);
	...
	Запрос.УстановитьПараметр("СвойствоНеЗагружатьПовторно",	СвойствоНеЗагружатьПовторно);
	...
	|	ЕСТЬNULL(ДополнительныеСведенияКорректировкаДолга.Значение, ЛОЖЬ) КАК КорректировкаДолгаНеЗагружатьПовторно,
	|	ЕСТЬNULL(ДополнительныеСведенияОплатаПлатежнойКартой.Значение, ЛОЖЬ) КАК ОплатаПлатежнойКартойНеЗагружатьПовторно,
	|	ЕСТЬNULL(ДополнительныеСведенияПриходныйКассовыйОрдер.Значение, ЛОЖЬ) КАК ПриходныйКассовыйОрдерНеЗагружатьПовторно
	|ИЗ
	...
	|			И (ДополнительныеСведенияПриходныйКассовыйОрдер.Свойство = &СвойствоНеЗагружатьПовторно)
	...
	|			И (ДополнительныеСведенияОплатаПлатежнойКартой.Свойство = &СвойствоНеЗагружатьПовторно)
	...
	|			И (ДополнительныеСведенияКорректировкаДолга.Свойство = &СвойствоНеЗагружатьПовторно)";
