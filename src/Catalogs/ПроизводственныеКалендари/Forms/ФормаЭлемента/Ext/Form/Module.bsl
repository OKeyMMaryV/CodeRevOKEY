...
Процедура ЗаполнитьПоУмолчанию(Команда)
	...
	Если Объект.Код = "ДР" Тогда
		
		СписокЗначений = Новый СписокЗначений;
		СписокЗначений.Добавить(1, "Понедельник");
		СписокЗначений.Добавить(2, "Вторник");
		СписокЗначений.Добавить(3, "Среда");
		СписокЗначений.Добавить(4, "Четверг");
		СписокЗначений.Добавить(5, "Пятница");
		СписокЗначений.Добавить(6, "Суббота");
		СписокЗначений.Добавить(7, "Воскресенье");

		Оповещение = Новый ОписаниеОповещения("ПослеОтметкиРабочихДнейНедели", ЭтаФорма);
		СписокЗначений.ПоказатьОтметкуЭлементов(Оповещение, НСтр("ru = 'Для производственного календаря с кодом ""ДР"" необходимо выбрать день оплаты.'"));
...
//СофтЛаб Начало 2019-08-11 #3421
&НаКлиенте
Процедура ПослеОтметкиРабочихДнейНедели(Список, Параметры) Экспорт
	
	Если Список = Неопределено Тогда
        Возврат;
    Иначе
		
		РабочиеДниНедели = Новый Массив();
		Для каждого Элемент из Список Цикл
			
			Если Элемент.Пометка Тогда
				
				РабочиеДниНедели.Добавить(Элемент.Значение);
				
			КонецЕсли;
			
		КонецЦикла;
		
		ЗаполнитьДаннымиПоУмолчанию(РабочиеДниНедели);
		Элементы.Календарь.Обновить();
...
Процедура ЗаполнитьДаннымиПоУмолчанию(РабочиеДниНедели = Неопределено)
//СофтЛаб Конец 2019-08-11 #3421
	...
	ДанныеПоУмолчанию = Справочники.ПроизводственныеКалендари.РезультатЗаполненияПроизводственногоКалендаряПоУмолчанию(
		Объект.Код, НомерТекущегоГода, КодБазовогоКалендаря, РабочиеДниНедели);
	//СофтЛаб Конец 2019-08-11 #3421
