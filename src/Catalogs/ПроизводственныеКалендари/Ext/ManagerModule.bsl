...
//
//СофтЛаб Начало 2019-08-11 #3421
...
Функция РезультатЗаполненияПроизводственногоКалендаряПоУмолчанию(КодКалендаря, НомерГода, Знач КодБазовогоКалендаря = Неопределено, РабочиеДниНедели = Неопределено) Экспорт
//СофтЛаб Конец 2019-08-11 #3421
	...
	//СофтЛаб Начало 2019-08-11 #3421
	Если ТипЗнч(РабочиеДниНедели) = Тип("Массив")
		И РабочиеДниНедели.Количество() > 0 Тогда
		
		ДанныеПроизводственногоКалендаря.Сортировать("Дата Возр");
		
		Для каждого СтрокаТЗ Из ДанныеПроизводственногоКалендаря Цикл
		
			Если СтрокаТЗ.ВидДня = ПредопределенноеЗначение("Перечисление.ВидыДнейПроизводственногоКалендаря.Рабочий") 
				ИЛИ СтрокаТЗ.ВидДня = ПредопределенноеЗначение("Перечисление.ВидыДнейПроизводственногоКалендаря.Предпраздничный") Тогда
			
				ДеньНедели = ДеньНедели(СтрокаТЗ.Дата);
				Если РабочиеДниНедели.Найти(ДеньНедели) = Неопределено Тогда
					
					ДеньНеделиДатыПереноса = ДеньНедели(СтрокаТЗ.ДатаПереноса);
					Если ЗначениеЗаполнено(СтрокаТЗ.ДатаПереноса)
						И РабочиеДниНедели.Найти(ДеньНеделиДатыПереноса) <> Неопределено Тогда
					
						Продолжить;
					
					КонецЕсли; 
					
					СтрокаТЗ.ВидДня = ПредопределенноеЗначение("Перечисление.ВидыДнейПроизводственногоКалендаря.Нерабочий");
				
				КонецЕсли; 
				
			ИначеЕсли СтрокаТЗ.ВидДня = ПредопределенноеЗначение("Перечисление.ВидыДнейПроизводственногоКалендаря.Праздник")
				ИЛИ СтрокаТЗ.ВидДня = ПредопределенноеЗначение("Перечисление.ВидыДнейПроизводственногоКалендаря.Нерабочий")
				ИЛИ СтрокаТЗ.ВидДня = ПредопределенноеЗначение("Перечисление.ВидыДнейПроизводственногоКалендаря.Суббота")
				ИЛИ СтрокаТЗ.ВидДня = ПредопределенноеЗначение("Перечисление.ВидыДнейПроизводственногоКалендаря.Воскресенье") Тогда
				
				ДеньНедели = ДеньНедели(СтрокаТЗ.Дата);
				Если РабочиеДниНедели.Найти(ДеньНедели) <> Неопределено Тогда
					
					СледующийДень = НачалоДня(СтрокаТЗ.Дата + 60*60*24);
					
					ПоискДатыПереноса = Истина;
					Пока ПоискДатыПереноса = Истина Цикл
					
						НайденныеСтроки = ДанныеПроизводственногоКалендаря.НайтиСтроки(Новый Структура("Дата", СледующийДень));
						Если НайденныеСтроки.Количество() > 0 Тогда

							НайденнаяСтрока = НайденныеСтроки[0];
							Если НайденнаяСтрока.ВидДня = ПредопределенноеЗначение("Перечисление.ВидыДнейПроизводственногоКалендаря.Рабочий") 
								ИЛИ НайденнаяСтрока.ВидДня = ПредопределенноеЗначение("Перечисление.ВидыДнейПроизводственногоКалендаря.Предпраздничный") Тогда
							
								НайденнаяСтрока.ДатаПереноса = СтрокаТЗ.Дата;
								ПоискДатыПереноса = Ложь;
