...
//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-12-09 (#МДМ)
Функция ПолучитьОчереднойНомДокИмпорт(Контрагент, Договор = Неопределено) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	НомДокИмпорт = Контрагент.НомДокИмпорт;
	
	Если НЕ ЗначениеЗаполнено(НомДокИмпорт) Тогда
		
		МассивВидовДоговора = Новый Массив();
		МассивВидовДоговора.Добавить(ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.Прочее"));
		МассивВидовДоговора.Добавить(ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.СПоставщиком"));
		
		СтруктураПараметров = Новый Структура();
		СтруктураПараметров.Вставить("Организация", 	бит_БК_Общий.ПолучитьЗначениеНастройкиМеханизмаИмпортаДанных("Организации", "Организация ОКЕЙ"));
		СтруктураПараметров.Вставить("ВидДоговора", 	МассивВидовДоговора);
		СтруктураПараметров.Вставить("ПометкаУдаления", Ложь);
		
		Если Договор <> Неопределено Тогда
			СтруктураПараметров.Вставить("Ссылка", Договор);
		КонецЕсли; 
		
		Если бит_Общий.КонтрагентИмеетДоговорПоПараметрам(Контрагент.Ссылка, СтруктураПараметров) Тогда
			
			НумераторУспешноПолучен = Ложь;
			Попытка
			
				НачатьТранзакцию(РежимУправленияБлокировкойДанных.Управляемый);
				
				Блокировка = Новый БлокировкаДанных;
				ЭлементБлокировки = Блокировка.Добавить("Константа.ок_Нумератор_НомДокИмпорт");
				ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
				Блокировка.Заблокировать();
				
				Нумератор_НомДокИмпорт = Константы.ок_Нумератор_НомДокИмпорт.Получить();
				Если Нумератор_НомДокИмпорт = 0 Тогда
					Нумератор_НомДокИмпорт = 1;
				КонецЕсли;
				Константы.ок_Нумератор_НомДокИмпорт.Установить(Нумератор_НомДокИмпорт + 1);
				
				ЗафиксироватьТранзакцию();
				
				НумераторУспешноПолучен = Истина;
			...
			НомДокИмпорт = "П" + бит_МеханизмХозяйственныхОперацийСервер.ДобавитьЛидирующиеНули(Нумератор_НомДокИмпорт, 6);
			
		КонецЕсли;
	
	КонецЕсли; 
	
	Возврат НомДокИмпорт;
	
КонецФункции

Функция ПолучитьОчереднойНомДокИмпорт2(Контрагент, Договор = Неопределено) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	НомДокИмпорт2 = Контрагент.НомДокИмпорт2;
	
	Если НЕ ЗначениеЗаполнено(НомДокИмпорт2)
		И Контрагент.бит_ГруппаКлиентов <> ПредопределенноеЗначение("Перечисление.бит_ГруппаКлиентов.РОЗНИЦА") Тогда
		...
		Если бит_Общий.КонтрагентИмеетДоговорПоПараметрам(Контрагент.Ссылка, СтруктураПараметров) Тогда
			...
			Попытка
				...
				Блокировка = Новый БлокировкаДанных;
				ЭлементБлокировки = Блокировка.Добавить("Константа.ок_Нумератор_НомДокИмпорт2");
				ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
				Блокировка.Заблокировать();
				
				Нумератор_НомДокИмпорт2 = Константы.ок_Нумератор_НомДокИмпорт2.Получить();
				Если Нумератор_НомДокИмпорт2 = 0 Тогда
					Нумератор_НомДокИмпорт2 = 1;
				КонецЕсли;
				Константы.ок_Нумератор_НомДокИмпорт2.Установить(Нумератор_НомДокИмпорт2 + 1);
			...
			НомДокИмпорт2 = "КЛ" + бит_МеханизмХозяйственныхОперацийСервер.ДобавитьЛидирующиеНули(Нумератор_НомДокИмпорт2, 6);
			
		КонецЕсли;
	
	КонецЕсли;
	
	Возврат НомДокИмпорт2;
	
КонецФункции
//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2020-12-09 (#МДМ)
...
// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-07-05 (#4147)

//Функция возвращает информацию об истории статуса контрагента на переданную дату
// Параметры:
...
Функция ПолучитьДанныеИсторииСтатусаКонтрагента(КонтрагентСсылка, Дата = Неопределено) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	Контрагентыок_ИсторияСтатусов.Период КАК Период,
		|	Контрагентыок_ИсторияСтатусов.Статус КАК Статус,
		|	Контрагентыок_ИсторияСтатусов.Ответственный КАК Ответственный,
		|	Контрагентыок_ИсторияСтатусов.Описание КАК Описание
		|ИЗ
		|	Справочник.Контрагенты.ок_ИсторияСтатусов КАК Контрагентыок_ИсторияСтатусов
		|ГДЕ
		|	Контрагентыок_ИсторияСтатусов.Ссылка = &КонтрагентСсылка
		|	И Контрагентыок_ИсторияСтатусов.Период <= &Дата
		|
		|УПОРЯДОЧИТЬ ПО
		|	Период УБЫВ";
...
КонецФункции
// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-07-05 (#4147)
