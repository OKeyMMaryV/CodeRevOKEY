...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ Ссылка.Пустая() Тогда
		...
		Если НЕ бит_Общий.КонтрагентИмеетДоговорПоПараметрам(Ссылка, СтруктураПараметров) Тогда
		
			//МассивНепроверяемыхРеквизитов.Добавить("ок_СчетНа");
			//МассивНепроверяемыхРеквизитов.Добавить("ок_ЗагрузкаЗакупочныхЦенДляАссортиментаПоДоговору");
			//МассивНепроверяемыхРеквизитов.Добавить("ок_ИнтеграцияСФГИСМеркурий");
			//МассивНепроверяемыхРеквизитов.Добавить("ок_ПодключениеКЭДО");
			МассивНепроверяемыхРеквизитов.Добавить("ок_АльтернативныеАдреса.НаименованиеПоставщика");
			//МассивНепроверяемыхРеквизитов.Добавить("ок_АльтернативныеАдреса.КПППоставщика");
			МассивНепроверяемыхРеквизитов.Добавить("ок_АльтернативныеАдреса.ТипАдреса");
			//МассивНепроверяемыхРеквизитов.Добавить("ок_АльтернативныеАдреса.Город");
			//МассивНепроверяемыхРеквизитов.Добавить("ок_АльтернативныеАдреса.ПредставлениеАдреса");
			
		КонецЕсли;
		
	КонецЕсли;
	
	ок_ВыгрузкаВАксапту.ПроверитьКонтрагентаПриИзмененииСчетНа(Ссылка, ок_СчетНа,, Отказ);
...
Процедура ПередЗаписью(Отказ)
	...
	//Если бит_му_СвязаннаяСторона И Не ЗначениеЗаполнено(СБ_ТипКонтрагентаДляСвязанныхСторон) Тогда
	//	ОбщегоНазначения.СообщитьОбОшибке("Для контрагента с установленным признаком ""Связанная сторона"" необходимо указать тип контрагента (для связанных сторон)!", Отказ);
	//	Возврат;
	//КонецЕсли;
	// -СБ. Кузнецова С.
	...
	Если бит_му_СвязаннаяСторона И СБ_ПериметрыКонсолидации.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Для контрагента с установленным признаком ""Связанная сторона"" необходимо заполнить табличную часть ""Периметры консолидации""!", , , , Отказ);
		Возврат;
	КонецЕсли;
	// -СБ. Кузнецова С.
	
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2021-02-16 (#МДМ)
	Для каждого ТекущаяСтрока Из ок_АльтернативныеАдреса Цикл
	...
	КонецЦикла;
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2021-02-16 (#МДМ)
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	//Ванюков К. + //При копировании некоторые реквизиты не должены копироваться
	НомДокИмпорт 	= "";
	Код77 			= "";
	//Ванюков К. -
	
	//СофтЛаб Начало 2019-08-10 #3486
	бит_Статус = Неопределено;
	ок_ИсторияСтатусов.Очистить();
	//СофтЛаб Конец 2019-08-10 #3486
	
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-11-30 (#МДМ)
	ок_ПодключениеКЭДО 									= Ложь;
	ок_СчетНа 											= Неопределено;
	ок_ЗагрузкаЗакупочныхЦенДляАссортиментаПоДоговору 	= 2;
	ок_ИнтеграцияСФГИСМеркурий 							= Ложь;
	ок_КонтрагентВторсырья 								= Ложь;
	
	НомДокИмпорт 										= "";
	НомДокИмпорт2 										= "";
	
	ок_ПлательщикНДС 									= Истина;
	ок_ИДПоставщика 									= "";
	ок_КодПровайдера 									= "";
	ок_ДатаНачалаДействияПровайдера 					= Неопределено;
	ок_ДатаОкончанияДействияПровайдера 					= Неопределено;
	ок_GLN 												= "";
	ок_RECADV 											= Ложь;
	ок_РазрешитьМенятьПризнакОтправкиПоECOD 			= Ложь;
	ок_ВерсияАксаптаEDI 								= "";
	ок_ВерсияАксаптаУПД 								= "";
	ок_ПремииШтрафы 									= Ложь;
	ок_Взаимозачеты 									= Ложь;
	
	ок_АльтернативныеАдреса.Очистить();
