...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//1c-izhtc А.В. Пушин (
	//// +СБ. Широков Николай. 2014-09-10. ТЗ "Казначейство" 2.2.3.3.
	//	СБ_КазначействоКлиент.БанковскиеСчетаФормаЭлементаПриОтркытии(ЭтаФорма);
	ИжТиСи_СВД_Сервер.ОК_ВывестиРеквизиты(ЭтаФорма, "Справочник.БанковскиеСчета.ФормаЭлемента");
	//// -СБ. Широков Николай
	//1c-izhtc А.В. Пушин )
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	мЭтоНовый = Не ЗначениеЗаполнено(Объект.Ссылка);
...
// ++ BIT AMerkulov 14/01/2014 
&НаСервере
Функция ВыполнитьПроверкуРеквизитов_ВызовСервера1()
	Возврат Объект.Ссылка.Банк.Наименование <> Объект.Банк.Наименование 
		ИЛИ Объект.Ссылка.Банк.КоррСчет <> Объект.Банк.КоррСчет  
		ИЛИ Объект.Ссылка.Банк.Код <> Объект.Банк.Код  
		ИЛИ Объект.Ссылка.НомерСчета <> Объект.НомерСчета 
	
КонецФункции

&НаСервере
Процедура ВыполнитьПроверкуРеквизитов_ВызовСервера2(Отказ, ДатаНачалаДействия)
	Запись = РегистрыСведений.бит_ИсторияИзмененияРеквизитовБанковскихСчетов.СоздатьМенеджерЗаписи();
	
	Запись.БанковскийСчет = Объект.Ссылка;
	Запись.Наименование = Объект.Банк.Наименование;
	Запись.КоррСчет = Объект.Банк.КоррСчет;
	Запись.БИК 		= Объект.Банк.Код;
	Запись.НомерСчета 			= Объект.НомерСчета;
...
Процедура ВыполнитьПроверкуРеквизитов(Отказ)
	...
	Если ВыполнитьПроверкуРеквизитов_ВызовСервера1() Тогда
		...
		Если ЗначениеЗаполнено(ДатаНачалаДействия) Тогда
			ВыполнитьПроверкуРеквизитов_ВызовСервера2(Отказ, ДатаНачалаДействия);
		Иначе 
			Отказ = Истина;
		КонецЕсли;
