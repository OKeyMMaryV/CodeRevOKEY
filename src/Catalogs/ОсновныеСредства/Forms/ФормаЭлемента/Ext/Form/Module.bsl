...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.КодТНВЭД.Видимость = Объект.ПрослеживаемыйТовар;
	...
	//++ СВВ {[+](фрагмент добавлен), Сапожников Вадим 20.07.2015 10:43:55
	ИжТиСи_СВД_Сервер.ОК_ВывестиРеквизиты(ЭтаФорма, "Справочник.ОсновныеСредства.ФормаЭлемента");
	//-- СВВ}Сапожников Вадим 20.07.2015 10:43:55
	...
	Если Не Объект.ПрослеживаемыйТовар Тогда
		Элементы.ПрослеживаемыйТовар.Видимость = ВедетсяУчетПрослеживаемыхТоваров
		И СписокГруппУчетаБезПрослеживаемости.НайтиПоЗначению(Объект.ГруппаОС) = Неопределено;
	КонецЕсли;
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-10-21 (#4405)
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	// +СБ Кузнецова С. 2016-10-07 Redmine 2651
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = "Если это ОС получено от своей организации, не забудьте заполнить поле «Связанное ОС»";
	Сообщение.Сообщить(); 
	// -СБ Кузнецова С.
...
Процедура ГруппаОСПриИзменении(Элемент)
	...
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-10-21 (#4405)
	Если Объект.ПрослеживаемыйТовар = Ложь Тогда
		Элементы.ПрослеживаемыйТовар.Видимость = ВедетсяУчетПрослеживаемыхТоваров
		И СписокГруппУчетаБезПрослеживаемости.НайтиПоЗначению(Объект.ГруппаОС) = Неопределено;
	КонецЕсли;
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-10-21 (#4405)
...
//ОК Ванюков К. +// Перенесены настройки // 2012-04-11
&НаКлиенте
Процедура ВЛизингеПриИзменении(Элемент)
	Элементы.КонтрагентЛизинга.Доступность = Объект.ВЛизинге;
	Элементы.ДоговорЛизинга.Доступность = Объект.ВЛизинге;
КонецПроцедуры

&НаКлиенте
Процедура ВЗалогеПриИзменении(Элемент)
	Элементы.КонтрагентЗалога.Доступность = Объект.ВЗалоге;
	Элементы.ДоговорЗалога.Доступность = Объект.ВЗалоге;
КонецПроцедуры

&НаКлиенте
Процедура ВАрендеПриИзменении(Элемент)
	Элементы.КонтрагентАренды.Доступность = Объект.ВАренде;
	Элементы.ДоговорАренды.Доступность = Объект.ВАренде;
КонецПроцедуры

&НаКлиенте
Процедура ЗастрахованоПриИзменении(Элемент)
	Элементы.СтраховаяКомпания.Доступность = Объект.Застраховано;
	Элементы.ДоговорСтрахования.Доступность = Объект.Застраховано;
КонецПроцедуры
// Конец СтандартныеПодсистемы.Печать

// +СБ Кузнецова С. 2016-10-06 Redmine 2651
&НаКлиенте
Процедура СБ_СвязанноеОСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ПараметрыФормы = Новый Структура;
	Форма = ПолучитьФорму("Справочник.ОсновныеСредства.Форма.СБ_ФормаПодбораСвязанногоОС", ПараметрыФормы, Элемент);
	Форма.Открыть();	
	
КонецПроцедуры

&НаКлиенте
Процедура СБ_СвязанноеОСОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	Объект.СБ_СвязанноеОС = ВыбранноеЗначение;
