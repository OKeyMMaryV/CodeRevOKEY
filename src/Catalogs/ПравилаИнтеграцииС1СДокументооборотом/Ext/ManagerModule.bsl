...
Функция ПолучитьРеквизитыОбъектаДО(ТипОбъектаДО, Параметры = Неопределено) Экспорт 
	...
	ИначеЕсли ТипОбъектаДО = "DMInternalDocument" Тогда 
		...
		Если НастройкиВида = Неопределено Или НастройкиВида.durationEnabled = Истина Тогда 
			ДобавитьРеквизит(Реквизиты, "beginDate", НСтр("ru = 'Дата начала действия'"), "Дата");
			ДобавитьРеквизит(Реквизиты, "endDate", НСтр("ru = 'Дата окончания действия'"), "Дата");
			ДобавитьРеквизит(Реквизиты, "openEnded", НСтр("ru = 'Бессрочный'"), "Булево");
			ДобавитьРеквизит(Реквизиты, "prolongationProcedure", НСтр("ru = 'Порядок продления'"), "DMProlongationProcedure");
		КонецЕсли;
		...
		Если НастройкиВида <> Неопределено
			...
			ДобавитьРеквизитТаблицы(Реквизиты, "productRows.rows", "product", НСтр("ru = 'Товар, услуга'"), "DMProduct");
			ДобавитьРеквизитТаблицы(Реквизиты, "productRows.rows", "price", НСтр("ru = 'Цена'"), "Число");
			ДобавитьРеквизитТаблицы(Реквизиты, "productRows.rows", "quantity", НСтр("ru = 'Количество'"), "Число");
			ДобавитьРеквизитТаблицы(Реквизиты, "productRows.rows", "measurementUnit", НСтр("ru = 'Единица измерения'"), "DMMeasurementUnit");
			ДобавитьРеквизитТаблицы(Реквизиты, "productRows.rows", "VATRate", НСтр("ru = 'Ставка НДС'"), "DMVATRate");
			ДобавитьРеквизитТаблицы(Реквизиты, "productRows.rows", "VAT", НСтр("ru = 'Сумма НДС'"), "Число");
			ДобавитьРеквизитТаблицы(Реквизиты, "productRows.rows", "total", НСтр("ru = 'Сумма'"), "Число");
	...
	ИначеЕсли ТипОбъектаДО = "DMCorrespondent" Тогда 
		...
		Если ИнтеграцияС1СДокументооборотПовтИсп.ИспользоватьТерминКорреспонденты() Тогда
			ДобавитьРеквизит(Реквизиты, "legalPrivatePerson", НСтр("ru = 'Вид корреспондента'"), "DMLegalPrivatePerson", Истина);
		Иначе
			ДобавитьРеквизит(Реквизиты, "legalPrivatePerson", НСтр("ru = 'Вид контрагента'"), "DMLegalPrivatePerson", Истина);
		КонецЕсли;
	...
	Если ТипОбъектаДО = "DMIncomingDocument"
		...
		Или ТипОбъектаДО = "DMInternalDocument" Тогда
	...
	Пока Повторять Цикл
		...
		Для Каждого Реквизит Из Реквизиты Цикл
			...
			Для Каждого СвязьПараметровВыбора Из Реквизит.СвязиПараметровВыбора Цикл
				РазделеннаяСтрока = СтрРазделить(СвязьПараметровВыбора.ПутьКДанным, ".");
				...
				КонецЕсли;
				Зависимости = Реквизиты.НайтиСтроки(СтруктураПоиска);
...
Функция СоответствиеСвойствXDTOиРеквизитовФормыОбъектаДО(ТипОбъекта) Экспорт 
	...
	Если Найти(ТипОбъекта, "Document") <> 0 Тогда
		...
		Результат.Вставить("statusConfirmation", "СостояниеРассмотрение");
		Результат.Вставить("statusApproval", "СостояниеСогласование");
		Результат.Вставить("statusConsideration", "СостояниеУтверждение");
