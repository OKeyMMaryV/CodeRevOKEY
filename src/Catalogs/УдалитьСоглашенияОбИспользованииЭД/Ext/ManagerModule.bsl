...
Процедура ОбработатьДанные_НоваяАрхитектураНастроекЭДО(Объект, Реквизиты, СвойстваПрофилейЭДО, КонверторФорматов, НастройкиПолученияВручную, Записать)
	...
	Если ЗначениеЗаполнено(Объект.ПрофильНастроекЭДО.ок_СпособОбменаЭД) Тогда
		СпособыОбменаНовойАрхитектуры.Добавить(Перечисления.СпособыОбменаЭД.БыстрыйОбмен);
	КонецЕсли;
	//1С-ИжТиСи, Кондратьев, 03.2020, обновление ЭДО )
		...
		Для каждого СтрокаНастроек Из Настройки Цикл
			...
			Если Не ЗначениеЗаполнено(Набор) Тогда
				...
				Если СвойстваПрофиляЭДО <> Неопределено Тогда
					...
					ИначеЕсли СтрокаНастроек.СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезFTP 
						ИЛИ ЗначениеЗаполнено(СвойстваПрофиляЭДО.ПрофильНастроекЭДО.ок_СпособОбменаЭД) Тогда
					//1С-ИжТиСи, Кондратьев, 03.2020, обновление ЭДО )
				...
				//1С-ИжТиСи, Кондратьев, 03.2020, обновление ЭДО (
				Запись.ок_ЗагружаемыеДокументы = Новый ХранилищеЗначения(СвойстваПрофиляЭДО.ПрофильНастроекЭДО.ок_ЗагружаемыеДокументы.Выгрузить());
				Запись.ок_СпособОбменаЭД = СвойстваПрофиляЭДО.ПрофильНастроекЭДО.ок_СпособОбменаЭД;
				Запись.ок_СпособОбменаОператораЭДО = СвойстваПрофиляЭДО.ПрофильНастроекЭДО.ок_СпособОбменаОператораЭДО;
				Запись.ок_ИдентификаторУчастникаОбмена = СвойстваПрофиляЭДО.ПрофильНастроекЭДО.ок_ИдентификаторУчастникаОбмена;
				Запись.ок_СпособАутентификацииОператораЭДО = СвойстваПрофиляЭДО.ПрофильНастроекЭДО.ок_СпособАутентификацииОператораЭДО;
				//1С-ИжТиСи, Кондратьев, 03.2020, обновление ЭДО )
	...
	Если Объект.ИспользуетсяДляОтправки 
		...
		Если Не ЗначениеЗаполнено(Набор) 
			...
			Запись.ок_ПериодыДействияВходящие = Новый ХранилищеЗначения(Объект.ок_ПериодыДействияВходящие.Выгрузить());
			Запись.ок_ПериодыДействияИсходящие = Новый ХранилищеЗначения(Объект.ок_ПериодыДействияИсходящие.Выгрузить());
			Запись.ок_АвтоматическиСоздаватьДокументПоступленияПослеСогласования = Объект.ок_АвтоматическиСоздаватьДокументПоступленияПослеСогласования;
			Запись.ок_ИдентификаторУчастникаОбмена = Объект.ок_ИдентификаторУчастникаОбмена;
			//1С-ИжТиСи, Кондратьев, 03.2020, обновление )
