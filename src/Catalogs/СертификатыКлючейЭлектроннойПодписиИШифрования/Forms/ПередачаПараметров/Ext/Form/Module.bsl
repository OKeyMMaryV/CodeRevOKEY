...
&НаКлиенте
Функция ПользовательУказалПризнакЗапомнитьПарольСертификата(Сертификат) Экспорт
	
	Если ТипЗнч(ОбщиеВнутренниеДанные) <> Тип("Соответствие") Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если ТипЗнч(ХранилищеПаролей) <> Тип("Соответствие") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ДанныеПароля = ХранилищеПаролей.Получить(Сертификат);
