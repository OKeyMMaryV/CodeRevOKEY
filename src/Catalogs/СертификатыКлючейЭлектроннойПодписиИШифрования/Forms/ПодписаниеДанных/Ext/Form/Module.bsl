...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// бит_Финанс изменение кода. Начало. 22.10.2015{{
	бит_СоздатьРеквизитПодписьБИТ();
	
    ПодписьБИТ = Ложь;	
	Если ТипЗнч(Параметры.ПараметрыДополнительныхДействий) = Тип("Строка") Тогда
		Если Параметры.ПараметрыДополнительныхДействий = "ПодписьБИТ" Тогда

			ТекПользовательБИТ = бит_ОбщиеПеременныеСервер.ЗначениеПеременной("ТекущийПользователь");
			ПодписьБИТ = Истина;
			
			Элементы.Сертификат.РежимВыбораИзСписка = Истина;
			Элементы["Сертификат"].СписокВыбора.ЗагрузитьЗначения(СертификатСписокВыбора.ВыгрузитьЗначения());

			Если Сертификат.Пользователь <> ТекПользовательБИТ Тогда
				Если СертификатСписокВыбора.Количество() > 0 Тогда
					Сертификат =  СертификатСписокВыбора[0].Значение;
				Иначе
					Сертификат = Справочники.СертификатыКлючейЭлектроннойПодписиИШифрования.ПустаяСсылка();
				КонецЕсли;
			КонецЕсли;
			
		КонецЕсли;
	КонецЕсли;
	// бит_Финанс изменение кода. Конец. 22.10.2015}}
...
Процедура ПриОткрытии(Отказ)
	...
	//ругается на нормальное поведение
	Возврат;
	//ОК(СофтЛаб) Вдовиченко Г.В. 20181101 №3103
	...
	// бит_Финанс изменение кода. Начало: 21.10.2015.
	Если СертификатСписокВыбора.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru = 'Не установлено ни одного личного сертификата для выполнения операции.
	                  |Установить личный сертификат можно в персональных настройках ЭП.'");

		ПоказатьПредупреждение(, ТекстПредупреждения);
		Отказ = Истина;
	КонецЕсли;
	// бит_Финанс изменение кода. Конец: 21.10.2015.
...
Процедура СертификатНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// бит_Финанс изменение кода. Начало. 22.10.2015{{
	Если ЭтаФорма.ПодписьБИТ Тогда
		Возврат;
	КонецЕсли;
	// бит_Финанс изменение кода. Конец. 22.10
...
Процедура ПродолжитьОткрытиеПослеНачала(Результат, Контекст) Экспорт
	...
	//ОК(СофтЛаб) Вдовиченко Г.В. 20181101 №3103
	Если ЗначениеЗаполнено(Сертификат) И СертификатСписокВыбора.Количество() = 1 
		И ДополнительныеПараметры.Свойство("ПарольУказан") И Не ДополнительныеПараметры.ПарольУказан  Тогда
		...
		Если ПарольСертификата <> Неопределено Тогда
			...
			СвойстваПароля.Вставить("Значение", ПарольСертификата);
			СвойстваПароля.Вставить("ОбработкаПоясненияПароля", Неопределено);
			СвойстваПароля.Вставить("ПарольПроверен", Истина);
...
#Область ПроцедурыБИТ

// бит_Финанс Создает реквизит ПодписьБИТ. 
//
// Параметры:
//
&НаСервере
Процедура бит_СоздатьРеквизитПодписьБИТ()

	ДобавляемыеРеквизиты = Новый Массив;
	МассивТипов = новый Массив;
	МассивТипов.Добавить(Тип("Булево"));
	Реквизит = Новый РеквизитФормы("ПодписьБИТ",новый ОписаниеТипов(МассивТипов),, "ПодписьБИТ", Истина);
