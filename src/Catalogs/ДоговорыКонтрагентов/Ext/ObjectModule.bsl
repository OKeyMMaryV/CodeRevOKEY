...
Процедура ПриКопировании(ОбъектКопирования)
	...
	//ОК Ванюков К. +// Необходимо доработать справочник договоров таким образом, чтобы при копировании существующего элемента очищались поля в новой записи: Номер АХ, REC ID, Комментарий // 2012-08-09
	бит_НомерАХ	= "";
	бит_RECID	= "";
	Комментарий	= "";
	//ОК Ванюков К. -
	...
	бит_СтадияДоговора = ПредопределенноеЗначение("Перечисление.бит_ВидыСтадийДоговоров.ПустаяСсылка");
	// ОКЕЙ Наумов А.Н.(СофтЛаб) Конец 2018-06-27 (#2965)
	
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-08-07 (#3065)
	Ок_ПересчетКурса = Ложь;
	...
	ВидОбеспеченияОбязательствИПлатежей = Неопределено;
	ОбеспечениеПредоставил 				= Неопределено;
	ОбеспечениеПолучил 					= Неопределено;
	ОбеспечениеВыданоЗа 				= Неопределено;
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2021-04-24 (#4138)
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2021-06-28 (#4245)
	ок_ИнтеграцияСAX12					= Ложь;
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2021-06-28 (#4245)
...
Процедура ПередЗаписью(Отказ)
	...
	ок_ВыгрузкаВАксапту.ПроверитьСозданиеДоговора(ЭтотОбъект, Отказ);
