...
Функция ПолучитьСтатус(Объект) Экспорт

	Если Объект.ок_ИсторияСтатусов.Количество() > 0 Тогда
	
		ИсторияСтатусов = Объект.ок_ИсторияСтатусов.Выгрузить();
		ИсторияСтатусов.Сортировать("Период Убыв");

		СтруктураРезультата = Новый Структура();
		Для каждого ТекущаяКолонка Из ИсторияСтатусов.Колонки Цикл
			СтруктураРезультата.Вставить(ТекущаяКолонка.Имя);
		КонецЦикла; 
		
		ЗаполнитьЗначенияСвойств(СтруктураРезультата, ИсторияСтатусов[0]);
...
//Функция ПолучитьБанковскийСчетКонтрагента(Договор = Неопределено, Контрагент = Неопределено) Экспорт
Функция ПолучитьБанковскийСчетКонтрагента(Договор = Неопределено, Контрагент = Неопределено, ПроверятьБанковскийСчетКонтрагента = Ложь) Экспорт
// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-07-06 (#4209)
	...
	//Поиск банковского счета по договору
	Если ЗначениеЗаполнено(Договор) Тогда
	
		ОсновнойСчетДоговора = Договор.ок_БанковскиеСчета.Найти(Истина, "Основной");
		Если ОсновнойСчетДоговора <> Неопределено
			И ЗначениеЗаполнено(ОсновнойСчетДоговора.БанковскийСчет) Тогда
			Возврат ОсновнойСчетДоговора.БанковскийСчет;
		КонецЕсли; 
		
		МассивБанковскихСчетов = Новый Массив;
		Для каждого СтрокаТЧ  Из Договор.ок_БанковскиеСчета Цикл
			Если ЗначениеЗаполнено(СтрокаТЧ.БанковскийСчет)
				И МассивБанковскихСчетов.Найти(СтрокаТЧ.БанковскийСчет) = Неопределено Тогда
				МассивБанковскихСчетов.Добавить(СтрокаТЧ.БанковскийСчет);
			КонецЕсли; 
		КонецЦикла; 
		...
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-07-06 (#4209)
		//ИначеЕсли МассивБанковскихСчетов.Количество() > 1 Тогда
		ИначеЕсли МассивБанковскихСчетов.Количество() > 1 И Не ПроверятьБанковскийСчетКонтрагента Тогда
		// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-07-06 (#4209)
			Возврат ПредопределенноеЗначение("Справочник.ДоговорыКонтрагентов.ПустаяСсылка");
		КонецЕсли; 
	...
	//Поиск банковского счета по контрагенту
	Если ЗначениеЗаполнено(Контрагент)
		И ЗначениеЗаполнено(Контрагент.ОсновнойБанковскийСчет) Тогда
...
Функция ОпределитьСпособПогашенияЗадолженностиПоДоговору(ДоговорКонтрагента) Экспорт
	
	СпособПогашенияЗадолженности = Неопределено;
	
	Если ЗначениеЗаполнено(ДоговорКонтрагента)
		И ДоговорКонтрагента.ок_СложныйДоговор Тогда
		СпособПогашенияЗадолженности = ПредопределенноеЗначение("Перечисление.СпособыПогашенияЗадолженности.НеПогашать");
	Иначе
		СпособПогашенияЗадолженности = ПредопределенноеЗначение("Перечисление.СпособыПогашенияЗадолженности.Автоматически");
	КонецЕсли; 
...
Функция ОпределитьСпособЗачетаАвансовПоДоговору(ДоговорКонтрагента) Экспорт
	
	СпособЗачетаАвансов = Неопределено;
	
	Если ЗначениеЗаполнено(ДоговорКонтрагента)
		И ДоговорКонтрагента.ок_СложныйДоговор Тогда
		СпособЗачетаАвансов = ПредопределенноеЗначение("Перечисление.СпособыЗачетаАвансов.НеЗачитывать");
	Иначе
		СпособЗачетаАвансов = ПредопределенноеЗначение("Перечисление.СпособыЗачетаАвансов.Автоматически");
	КонецЕсли; 
	
	Возврат СпособЗачетаАвансов;
	
КонецФункции
//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2020-06-04 (#3704)

//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-07-15 (#3762)
Функция ИсключенияДляИзмененийСтатусовПоДоговорам(Договор, СтадияДоговора = Неопределено) Экспорт
	
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-12-28 (#МДМ)
	...
	Если Договор.Организация = Организация_Окей Тогда
		Возврат Ложь;
	КонецЕсли; 
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2020-12-28 (#МДМ)
	
	ОрганизацияВходитВНастройкиМеханизмаИмпортаДанных = бит_Общий.ОрганизацияВходитВНастройкиМеханизмаИмпортаДанных(Договор.Организация);
	
	Если ТипЗНЧ(Договор) = ТИП("СправочникОбъект.ДоговорыКонтрагентов") Тогда
		бит_СтадияДоговора = Договор.бит_СтадияДоговора;
	Иначе
		бит_СтадияДоговора = СтадияДоговора;
	КонецЕсли;
	
	Если ОрганизацияВходитВНастройкиМеханизмаИмпортаДанных
		И (бит_СтадияДоговора = ПредопределенноеЗначение("Перечисление.бит_ВидыСтадийДоговоров.Замечания")
			ИЛИ бит_СтадияДоговора = ПредопределенноеЗначение("Перечисление.бит_ВидыСтадийДоговоров.Закрыт"))
			И НЕ Договор.Ссылка.бит_СтадияДоговора = ПредопределенноеЗначение("Перечисление.бит_ВидыСтадийДоговоров.Замечания")
		И  (Договор.Владелец.бит_ГруппаПоставщиков = ПредопределенноеЗначение("Перечисление.бит_ГруппаПоставщиков.Факторинг") 
			Или Договор.Владелец.бит_ГруппаПоставщиков = ПредопределенноеЗначение("Перечисление.бит_ГруппаПоставщиков.ПТов")) Тогда
...
//ОКЕЙ Горбачева А.Г. (СофтЛаб) Начало 2020-08-27 (#3852)
Функция ЭтоДоговорАренды(ДоговорКонтрагента) Экспорт  
	
	ОтключитьФункционалАренды = ОК_ОбщегоНазначения.ПолучитьЗначениеУниверсальнойНастройки("Аренда", "ОтключитьФункционалАренды", Неопределено);
	
	Если ОтключитьФункционалАренды <> Неопределено
		И ОтключитьФункционалАренды = Истина Тогда
		Возврат Ложь;
	КонецЕсли;	
	
	ОсновнаяСтатьяОборотовПоАренде = ОК_ОбщегоНазначения.ПолучитьЗначениеУниверсальнойНастройки("Аренда", "ОсновнаяСтатьяОборотовПоАренде", Неопределено);
	
	Если ОсновнаяСтатьяОборотовПоАренде = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли; 
	
	НайденнаяСтрока = ДоговорКонтрагента.бит_СтатьиОборотов.Найти(ОсновнаяСтатьяОборотовПоАренде, "СтатьяОборотов");
	
	Возврат НайденнаяСтрока <> Неопределено;
	
КонецФункции
//ОКЕЙ Горбачева А.Г. (СофтЛаб) Конец 2020-08-27 (#3852)
...
#КонецОбласти
// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-07-06 (#4147)

//Функция возвращает информацию об истории статуса договора на переданную дату
// Параметры:
// КонтрагентСсылка - Проверяемый контрагент, Тип - СправочникСсылка.Контрагенты
// Дата - Дата на которую необходимо получить информацию, Тип - Дата
Функция ПолучитьДанныеИсторииСтатусаДоговора(ДоговорСсылка, Дата = Неопределено) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ДоговорыКонтрагентовок_ИсторияСтатусов.Период КАК Период,
		|	ДоговорыКонтрагентовок_ИсторияСтатусов.Статус КАК Статус,
		|	ДоговорыКонтрагентовок_ИсторияСтатусов.Ответственный КАК Ответственный,
		|	ДоговорыКонтрагентовок_ИсторияСтатусов.Описание КАК Описание
		|ИЗ
		|	Справочник.ДоговорыКонтрагентов.ок_ИсторияСтатусов КАК ДоговорыКонтрагентовок_ИсторияСтатусов
		|ГДЕ
		|	ДоговорыКонтрагентовок_ИсторияСтатусов.Ссылка = &ДоговорСсылка
		|	И ДоговорыКонтрагентовок_ИсторияСтатусов.Период <= &Дата
		|
		|УПОРЯДОЧИТЬ ПО
		|	Период УБЫВ";
	...
	Возврат ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(ВыборкаДетальныеЗаписи);
	
КонецФункции
// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-07-06 (#4147)
#КонецЕсли
