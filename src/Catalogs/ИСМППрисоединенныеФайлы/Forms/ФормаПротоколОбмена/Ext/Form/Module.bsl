...
Процедура ЗаполнитьПоОснованиюДокументаИСМП(МетаданныеДокументаИСМП, ДокументОснование, ДобавитьГиперссылку = Истина)
	...
	Если СтатусыОформления[МетаданныеДокументаИСМП.Имя].Статус <> Перечисления.СтатусыОформленияДокументовГосИС.Оформлено
...
Процедура ЗаполнитьПоДокументу(ТаблицаДокументы, ОтображатьСИерархией = Ложь)
	...
	|	ЕСТЬNULL(Сообщения.ДатаСоздания, ДАТАВРЕМЯ(1, 1, 1))           КАК ДатаСоздания,
		...
		Пока ВыборкаПоФайлам.Следующий() Цикл
			...
			Если ТипЗнч(ВыборкаПоФайлам.Ссылка) = Тип("Строка") Тогда
				СтрокаВторогоУровня.Представление  = ПредставлениеОперации(
					...
					ВыборкаПоФайлам.РеквизитыИсходящегоСообщения.Получить(),
...
Процедура ЗаполнитьДеревоФайлов()
	...
	ИначеЕсли Метаданные.ОпределяемыеТипы.ОснованиеМаркировкаТоваровИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаМаркировкаТоваровИСМП(Документ);
		
	ИначеЕсли Метаданные.ОпределяемыеТипы.ОснованиеЗаказНаЭмиссиюКодовМаркировкиИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаЗаказНаЭмиссиюКодовМаркировкиСУЗ(Документ);
		
	ИначеЕсли Метаданные.ОпределяемыеТипы.ОснованиеВыводаИзОборотаИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаВыводИзОборотаИСМП(Документ);
		
	ИначеЕсли Метаданные.ОпределяемыеТипы.ОснованиеСписаниеКодовМаркировкиИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаСписаниеКодовМаркировкиИСМП(Документ);
		
	ИначеЕсли Метаданные.ОпределяемыеТипы.ОснованиеПеремаркировкиТоваровИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
