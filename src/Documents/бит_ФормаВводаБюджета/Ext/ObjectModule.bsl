...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2021-10-19 (#4390) для Заявки У если дата заполнения проектов еще не наступила, то проверять на заполнение не требуется
	Если бит_СписокИзмеренийКонтроляБюджета.НайтиПоЗначению(Перечисления.бит_БК_ИзмеренияКонтроляБюджета.Проект) <> Неопределено Тогда
		Если Сценарий = Справочники.СценарииПланирования.Заявка_У
			И Не ОК_ОбщегоНазначения.ТребуетсяЗаполнениеПроектаПоСценарию(Дата, Сценарий, Истина) Тогда 
			бит_СписокИзмеренийКонтроляБюджета.Удалить(бит_СписокИзмеренийКонтроляБюджета.НайтиПоЗначению(Перечисления.бит_БК_ИзмеренияКонтроляБюджета.Проект));
		ИначеЕсли ВидОперации = Перечисления.бит_БК_ВидыОперацийФормаВводаБюджета.ФинансовыйПлан 
			И Сценарий = Справочники.СценарииПланирования.Бюджет_У Тогда
			бит_СписокИзмеренийКонтроляБюджета.Удалить(бит_СписокИзмеренийКонтроляБюджета.НайтиПоЗначению(Перечисления.бит_БК_ИзмеренияКонтроляБюджета.Проект));
		КонецЕсли;	
	КонецЕсли;	
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Конец 2021-10-19 (#4390)
	...
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2021-10-19 (#4390)
	Если ВидОперации = Перечисления.бит_БК_ВидыОперацийФормаВводаБюджета.ФинансовыйПлан Тогда
		Если Сценарий = Справочники.СценарииПланирования.Бюджет_У
			И ОК_ОбщегоНазначения.ТребуетсяЗаполнениеПроектаПоСценарию(Дата, Сценарий) Тогда 
			ПроверяемыеРеквизиты.Добавить("БДДС.Проект");
		КонецЕсли;
	ИначеЕсли ВидОперации = Перечисления.бит_БК_ВидыОперацийФормаВводаБюджета.Заявка_Операционный
		Или ВидОперации = Перечисления.бит_БК_ВидыОперацийФормаВводаБюджета.Заявка_Инвестиционный 
		Или ВидОперации = Перечисления.бит_БК_ВидыОперацийФормаВводаБюджета.Заявка_Универсальный Тогда 
		ПроверитьЗаполнениеПроектаПоВидуОперацииЗаявка(Отказ);
	КонецЕсли;	
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Конец 2021-10-19 (#4390)
...
// ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2021-10-19 (#4390)
Процедура ПроверитьЗаполнениеПроектаПоВидуОперацииЗаявка(Отказ)
	ШаблонСообщенияНеверныйПроект = НСтр("ru = 'В строке № %1 неверно указан проект. Укажите проект %2'");
	ШаблонСообщенияПроектНеУказан = НСтр("ru = '«Проект по умолчанию» для периода %1 не найден. Строка №%2. Обратитесь к менеджеру по контроллингу'");
	Для Каждого СтрокаБДДС Из БДДС Цикл
		Если ЗначениеЗаполнено(СтрокаБДДС.Проект) Тогда 
			РеквизитыПроекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаБДДС.Проект, "ок_ПроектПоУмолчанию, ок_Период");
			Если РеквизитыПроекта.ок_ПроектПоУмолчанию
				И Не НачалоГода(СтрокаБДДС.Период) = РеквизитыПроекта.ок_Период Тогда
				ПроектПоУмолчанию = ОК_ОбщегоНазначения.ПроектПоУмолчаниюЗаПериод(СтрокаБДДС.Период);
				Если ЗначениеЗаполнено(ПроектПоУмолчанию) Тогда
					ТекстСообщения = СтрШаблон(ШаблонСообщенияНеверныйПроект, СтрокаБДДС.НомерСтроки, ПроектПоУмолчанию);;
				Иначе
					ТекстСообщения = СтрШаблон(ШаблонСообщенияПроектНеУказан, Формат(СтрокаБДДС.Период, "ДФ=гггг"), СтрокаБДДС.НомерСтроки);;
				КонецЕсли;	
