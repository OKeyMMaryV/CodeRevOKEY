...
Процедура БДДСПроектПолеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если СтандартнаяОбработка = Истина
		И ЗначениеЗаполнено(Элементы.БДДС.ТекущиеДанные.Период) Тогда
		СтандартнаяОбработка = Ложь;
		ОткрытьФорму("Справочник.Проекты.ФормаВыбора", Новый Структура("ДатаПроекта", Элементы.БДДС.ТекущиеДанные.Период),Элементы.БДДСПроектПоле);
	КонецЕсли;
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Конец 2021-10-19 (#4390)
