...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = ПредопределенноеЗначение("РежимЗаписиДокумента.Проведение")
		И (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.бит_БК_ВидыОперацийФормаВводаБюджета.Заявка_Операционный")
			Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.бит_БК_ВидыОперацийФормаВводаБюджета.Заявка_Инвестиционный") 
		...
		НомераСтрокГдеНужноЗаполнитьПроект = ПолучитьНомераСтрокГдеНужноЗаполнитьПроект();
		Если НомераСтрокГдеНужноЗаполнитьПроект.Количество() > 0 Тогда 
			
			ПослеВопросаОЗаполненияПроекта = Новый ОписаниеОповещения("ПослеВопросаОЗаполненияПроекта", ЭтотОбъект, Новый Структура("НомераСтрок", НомераСтрокГдеНужноЗаполнитьПроект));
			
			ШаблонСообщения = НСтр("ru = 'Не заполнен «Проект» в строках: %1. Расход относится к определенному Проекту?'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, СтрСоединить(НомераСтрокГдеНужноЗаполнитьПроект,","));
			
			СписокКнопок = Новый СписокЗначений();
			СписокКнопок.Добавить(КодВозвратаДиалога.Да, "Расход не относится к Проекту");
			СписокКнопок.Добавить(КодВозвратаДиалога.Нет, "Да, заполнить Проект вручную");
			
			ПоказатьВопрос(ПослеВопросаОЗаполненияПроекта,
							ТекстСообщения,
							СписокКнопок,
							, 								// таймаут в секундах
...
Процедура Подключаемый_АналитикаСтрокиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2021-10-19 (#4390)
	ИначеЕсли ИмяПоля = "Проект"
		И ЗначениеЗаполнено(Элементы.БДДС.ТекущиеДанные.Период)
		И (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.бит_БК_ВидыОперацийФормаВводаБюджета.Заявка_Операционный")
			Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.бит_БК_ВидыОперацийФормаВводаБюджета.Заявка_Инвестиционный") 
			Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.бит_БК_ВидыОперацийФормаВводаБюджета.Заявка_Универсальный")) Тогда 
		СтандартнаяОбработка = Ложь;
		ОткрытьФорму("Справочник.Проекты.ФормаВыбора", Новый Структура("ДатаПроекта", Элементы.БДДС.ТекущиеДанные.Период),Элементы.БДДСПроект);
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Конец 2021-10-19 (#4390)
	Иначе	
...
// ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2021-10-19 (#4390)
&НаСервереБезКонтекста
Функция ТребуетсяЗаполнениеПроектаПоСценарию(Дата, Сценарий, ПроверятьДатуПринудительно)
	
	Возврат ОК_ОбщегоНазначения.ТребуетсяЗаполнениеПроектаПоСценарию(Дата, Сценарий, ПроверятьДатуПринудительно);
	
КонецФункции	
	
&НаСервере
Функция ПолучитьНомераСтрокГдеНужноЗаполнитьПроект()
	
	СписокСтрок = Новый Массив;
	Для Каждого СтрокаДДС Из Объект.БДДС Цикл
		
		Если Не ЗначениеЗаполнено(СтрокаДДС.Проект) Тогда
			
			ПроверятьДатуПринудительно = (СтрокаДДС.Сценарий = Справочники.СценарииПланирования.Заявка_У);
			Если ТребуетсяЗаполнениеПроектаПоСценарию(Объект.Дата, СтрокаДДС.Сценарий, ПроверятьДатуПринудительно) Тогда
				
				СписокСтрок.Добавить(СтрокаДДС.НомерСтроки);
...
&НаКлиенте
Процедура ПослеВопросаОЗаполненияПроекта(Результат, ДопПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли;

	ЗаполнитьПроектПоУмолчаниюВСтрокахБДДС(ДопПараметры.НомераСтрок);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПроектПоУмолчаниюВСтрокахБДДС(НомераСтрок)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Проекты.Ссылка КАК Проект,
		|	Проекты.ок_Период КАК Период
		|ИЗ
	...
	ШаблонСообщения = НСтр("ru = '«Проект по умолчанию» для периода %1 не найден. Строка №%2. Обратитесь к менеджеру по контроллингу'");
	
	Для Каждого СтрокаБДДС Из Объект.БДДС Цикл
		
		Если НомераСтрок.Найти(СтрокаБДДС.НомерСтроки) <> Неопределено
			И Не ЗначениеЗаполнено(СтрокаБДДС.Проект) Тогда
			
			НайденнаяСтрока = РезультатЗапроса.Найти(НачалоГода(СтрокаБДДС.Период),"Период");
			Если НайденнаяСтрока = Неопределено Тогда
				ТекстСообщения = СтрШаблон(ШаблонСообщения, Формат(СтрокаБДДС.Период, "ДФ=гггг"), СтрокаБДДС.НомерСтроки);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Объект.Ссылка);
			Иначе
				СтрокаБДДС.Проект = НайденнаяСтрока.Проект;
			КонецЕсли;
