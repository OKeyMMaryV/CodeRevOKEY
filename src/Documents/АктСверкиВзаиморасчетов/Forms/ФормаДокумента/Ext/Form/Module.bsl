...
Процедура ПриОткрытии(Отказ)
	...
	//ОК Довбешка Т. 16.12.2014
	Элементы.ГруппаСписокКонтрагентов.Видимость = Объект.ОК_Флаг_СписокКонтрагентов;
...
Функция ЗаполнитьПоДаннымБухгалтерскогоУчета(ЗаполнятьДанныеКонтрагента)
	...
	ЕстьСписокДоговоров = Ложь;
	Если Объект.ОК_Флаг_СписокКонтрагентов Тогда
		ФильтрСписокКонтрагентов = Новый массив();
		ФильтрСписокДоговоровКонтрагентов = Новый массив();
		Для каждого СтрокаКотрагент Из Объект.ОК_СписокКонтрагентов Цикл
			ФильтрСписокКонтрагентов.Добавить(СтрокаКотрагент.ОК_Контрагенты);
			Если ЗначениеЗаполнено(СтрокаКотрагент.ОК_ДоговорыКонтрагентов) Тогда
				ФильтрСписокДоговоровКонтрагентов.Добавить(СтрокаКотрагент.ОК_ДоговорыКонтрагентов);
					ЕстьСписокДоговоров = Истина;
			КонецЕсли; 
		КонецЦикла;
	КонецЕсли; 
	//ОК Довбешка Т.	
	...
	СтруктураПараметров.Вставить("ЕстьСписокДоговоров", ЕстьСписокДоговоров);
	СтруктураПараметров.Вставить("Контрагент", ?(Объект.ОК_Флаг_СписокКонтрагентов,ФильтрСписокКонтрагентов,Объект.Контрагент));
	//ОК Довбешка Т.
	//ижтиси, шадрин, 17.08.2015(
	СтруктураПараметров.Вставить("ОК_Флаг_СписокКонтрагентов", Объект.ОК_Флаг_СписокКонтрагентов);
	СтруктураПараметров.Вставить("ФильтрСписокДоговоровКонтрагентов", ФильтрСписокДоговоровКонтрагентов);
	СтруктураПараметров.Вставить("ВыводитьВнНомераРеализаций", Объект.ВыводитьВнНомераРеализаций);
	//ижтиси, шадрин, 17.08.2015)
...
&НаСервере
Процедура ЗаполнитьСписокКонтрагентовПоИНННаСервере()
	
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ИНН = Объект.Контрагент.ИНН;
		Запрос = Новый Запрос;
		...
		Пока Выборка.Следующий() Цикл
			Строка = Объект.ОК_СписокКонтрагентов.Добавить();
			Строка.ОК_Контрагенты = Выборка.Ссылка;
			Если ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда
				Если Выборка.Ссылка = Объект.Контрагент Тогда
					Строка.ОК_ДоговорыКонтрагентов = Объект.ДоговорКонтрагента;
				Иначе
					Если ЗначениеЗаполнено(Выборка.Ссылка.ОсновнойДоговорКонтрагента) Тогда
						Строка.ОК_ДоговорыКонтрагентов = Выборка.Ссылка.ОсновнойДоговорКонтрагента;
					Иначе	
						НовыйДоговор = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
						СписокВидовДоговора = Неопределено;
						СтруктураПараметров = Неопределено;
						ВладелецДоговора = Выборка.Ссылка;
						...
						ГоловнаяОрганизацияДоговора = ОбщегоНазначенияБПВызовСервераПовтИсп.ГоловнаяОрганизация(Объект.Организация);
...
&НаКлиенте
Процедура ЗаполнитьСписокКонтрагентовПоИНН(Команда)
	ЗаполнитьСписокКонтрагентовПоИНННаСервере();
КонецПроцедуры
//ОК Довбешка Т.
