...
Функция ПодготовитьПараметрыТаблицаПоступленияПоСчетуГТД(ТаблицаПоступленияПоСчетуГТД, ТаблицаРеквизиты)
	...
	//Параметры.Вставить("Реквизиты",
	//	ОбщегоНазначенияБПВызовСервера.ПолучитьТаблицуПараметровПроведения(ТаблицаРеквизиты, СписокОбязательныхКолонок));
	НаборРеквизитов = ОбщегоНазначенияБПВызовСервера.ПолучитьТаблицуПараметровПроведения(ТаблицаРеквизиты, СписокОбязательныхКолонок);
	Если ТипЗНЧ(НаборРеквизитов.Колонки.Найти("Контрагент")) = Тип("Неопределено") Тогда
	    НаборРеквизитов.Колонки.Добавить("Контрагент", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
	КонецЕсли; 
	Параметры.Вставить("Реквизиты", НаборРеквизитов);
	//1С-ИжТиСи, Кондратьев, 03.2020, обновление, некомментированный исправленный код )
