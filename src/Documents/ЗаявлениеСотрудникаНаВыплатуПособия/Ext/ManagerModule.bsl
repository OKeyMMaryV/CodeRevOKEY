...
Процедура ЗаполнитьДанныеЗаявленияВМакете_2017(Макет, Заявление)
	...
	Если ПособиеЗависитОтСреднего(Заявление.ВидПособия) Тогда
...
Процедура ЗаполнитьВМакетеВыплатуЧерезПочту_2017(Макет, Заявление)
	...
	СтруктураАдреса = СтруктураАдресаИзXML(Заявление.АдресПроживания, ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.АдресМестаПроживанияФизическиеЛица"));	
	Если СтруктураАдреса <> Неопределено Тогда
		Город           = ПолеАдресаВВерхнемРегистре(СтруктураАдреса, "Город");
		НаселенныйПункт = ПолеАдресаВВерхнемРегистре(СтруктураАдреса, "НаселенныйПункт");
		ЗарплатаКадры.ВывестиДанныеПоБуквенно(ПолеАдресаВВерхнемРегистре(СтруктураАдреса, "Индекс"), Макет, "Адрес_", 6);
...
Процедура ЗаполнитьВМакетеСведенияОМестеЖительства_2017(Макет, Заявление)
	...
	СтруктураАдреса = СтруктураАдресаИзXML(Заявление.АдресРегистрации, ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.АдресПоПропискеФизическиеЛица"));	
	Если СтруктураАдреса <> Неопределено Тогда
		ЗарплатаКадры.ВывестиДанныеПоБуквенно(ПолеАдресаВВерхнемРегистре(СтруктураАдреса, "Индекс"), Макет, "АдресРегистрации_", 6);
...
Функция СтруктураАдресаИзXML(Адрес, ВидАдреса = Неопределено)
	...
	Попытка
		ДопСведенияОбАдресе = СтруктураАдресаВМашиночитаемомФормате_2012(Адрес);
		СтруктураАдреса.Вставить("Улица", ДопСведенияОбАдресе.Улица);
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаголовокОшибки = НСтр("ru = 'Не удалось подготовить адрес в машиночитаемом формате по причине:'");
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Прямые выплаты ФСС'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			ЗаголовокОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке) + Символы.ПС + Символы.ПС + Адрес);
		...
		Возврат ЗарплатаКадры.СтруктураАдресаИзXML(Адрес, ВидАдреса);
	КонецПопытки;
...
Функция СтруктураАдресаВМашиночитаемомФормате_2012(Адрес)
	...
	Если СведенияОбАдресе.Свойство("Территория") И ЗначениеЗаполнено(СведенияОбАдресе.Территория) Тогда
		...
		КонецЕсли;
		// Порядок проверок важен: В приоритете - пустой населенныйй пункт, затем пустая улица.
