...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	бит_Казначейство.ОбработкаЗаполненияПлатежноеПоручение(ЭтотОбъект, ДанныеЗаполнения);
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	// бит_Финанс изменения кода. Начало.
	бит_Казначейство.ПриКопированииПлатежноеПоручение(ЭтотОбъект, ОбъектКопирования);
	// бит_Финанс изменения кода. Конец. 
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// +СБ. Широков Николай. 2017-03-23. Доработка #2797      //*Дремов
...
Процедура ПриЗаписи(Отказ)
	...
	//СофтЛаб Начало 2019-08-17 #3424
	Если НЕ ЭтотОбъект.ПометкаУдаления Тогда
	
		УстановитьСтатус();
	
	КонецЕсли;
	//СофтЛаб Конец 2019-08-17 #3424
...
Процедура УстановитьСтатус()
	...
	Если НЕ ЗначениеЗаполнено(мАлгоритмИзмененияСтатуса) Тогда
		...
		НовыйСтатус = Справочники.бит_СтатусыОбъектов.Заявка_СозданоПП; 
		
		Для каждого СтрокаТЧ Из ЭтотОбъект.бит_ПлатежныеПозиции Цикл
		
			Если НЕ (ЗначениеЗаполнено(СтрокаТЧ.ПлатежнаяПозиция)
					И ЗначениеЗаполнено(СтрокаТЧ.ПлатежнаяПозиция.ДокументОснование)) Тогда
			
				Продолжить;
			
			КонецЕсли; 
			
			ЗнРДС = СтрокаТЧ.ПлатежнаяПозиция.ДокументОснование;
			
			СтруктураСтатусаОбъекта = бит_Визирование.ПолучитьСтатусОбъекта(ЗнРДС, Перечисления.бит_ВидыСтатусовОбъектов.Статус);
			ПредыдущийСтатус = СтруктураСтатусаОбъекта.Статус;
			
			//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-04-14 (#3681)
			//Если ПредыдущийСтатус = Справочники.бит_СтатусыОбъектов.Заявка_Закрыта Тогда
			Если ПредыдущийСтатус = Справочники.бит_СтатусыОбъектов.Заявка_Закрыта 
				ИЛИ ПредыдущийСтатус = Справочники.бит_СтатусыОбъектов.Заявка_Оплачена 
				Тогда
			...
			Если НовыйСтатус <> ПредыдущийСтатус Тогда
				...
				ДействиеВыполнено = бит_Визирование.УстановитьСтатусОбъекта(ЗнРДС
	...
	Иначе	
		...
		бит_уп_Сервер.ВыполнитьАлгоритм(мАлгоритмИзмененияСтатуса,СтруктураКонтекст);
