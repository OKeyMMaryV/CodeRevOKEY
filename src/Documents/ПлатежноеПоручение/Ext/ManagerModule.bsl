...
Процедура СформироватьНазначениеПлатежа(Объект, АвтоЗначенияРеквизитов, НазначениеПлатежаБылоИзмененоВручную, ТолькоСумму = Ложь, ШаблонНазначенияПлатежаВБюджет = "") Экспорт
	...
	Если ПозицияСумма = 0 Тогда // для совместимости, при редактировании старых платежек
		...
		Если ПозицияСумма = 0 Тогда
			...
			//ОК Ванюков К. + //При ставке БезНДС, при каждом изменении добавлял новое окончание, не удаляя старого
			Если ПозицияСумма = 0 Тогда
				ПозицияСумма = СтрНайти(ТекстНазначение, ", без налога (НДС)");
			КонецЕсли;
			Если ПозицияСумма = 0 Тогда
				ПозицияСумма = СтрНайти(ТекстНазначение, ",без налога (НДС)");
			КонецЕсли;
    ...
    //ОК Ванюков К. +// Не нужна лишняя пустая строка // 2012-04-27
	ТекстСуммаНазначения = ТекстСумма + ТекстНДС; 
    //ОК Ванюков К. -	
...
Функция ПечатьПлатежногоПоручения(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Для каждого ПлатежноеПоручение Из ДанныеДляПечати Цикл
		...
		//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-07-09 (#3795)
		Обл.Параметры.ок_КодНазначенияПлатежа = ПлатежноеПоручение.ок_КодНазначенияПлатежа;
		//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2020-07-09 (#3795)
