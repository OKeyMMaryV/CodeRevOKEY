...
// +СБ. Широков Николай. 2014-10-22. Доработка #1094 
	СБ_КазначействоСервер.ПлатежноеПоручениеФормаДокументаПриОткрытии(ЭтаФорма, НЕ ЗначениеЗаполнено(Объект.Ссылка));	
// -СБ. Широков Николай 		
...
Процедура бит_ПриСозданииНаСервереЗавершение()
	...
	Если НЕ ПравоДоступа("Просмотр", Метаданные.Документы.бит_ПлатежнаяПозиция) Тогда
		Возврат;	
	КонецЕсли; 
	
	РеквизитНадписьПлатежныеПозиции = Новый РеквизитФормы("бит_НадписьПлатежныеПозиции", Новый ОписаниеТипов("Строка"), , НСтр("ru = 'Платежные позиции'"));
	НовыеРеквизиты = Новый Массив(); 
	НовыеРеквизиты.Добавить(РеквизитНадписьПлатежныеПозиции);
	ИзменитьРеквизиты(НовыеРеквизиты);
	
	ЭтотОбъект["бит_НадписьПлатежныеПозиции"] = "Платежные позиции";
	
	НадписьПлатежныеПозиции = Элементы.Добавить("бит_НадписьПлатежныеПозиции", Тип("ПолеФормы"), Элементы.ГруппаСостояниеДокументСписания); 
	НадписьПлатежныеПозиции.Вид = ВидПоляФормы.ПолеНадписи;
	НадписьПлатежныеПозиции.ПутьКДанным = "бит_НадписьПлатежныеПозиции";
	НадписьПлатежныеПозиции.Гиперссылка = Истина;
	НадписьПлатежныеПозиции.ЦветТекста = ЦветаСтиля.ЦветГиперссылки;
	НадписьПлатежныеПозиции.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	НадписьПлатежныеПозиции.УстановитьДействие("Нажатие", "бит_ПодключаемыйНадписьПлатежныеПозицииНажатие");
	НадписьПлатежныеПозиции.РастягиватьПоГоризонтали = Истина;
	Элементы.Переместить(НадписьПлатежныеПозиции, Элементы.ГруппаСостояниеДокументСписания, Элементы.НадписьДокументСписания);

	бит_ОбновитьНадписьПлатежныеПозиции(ЭтотОбъект);
	
КонецПроцедуры

// Подключаемый из кода обработчик "Нажатие" элемента формы "бит_НадписьПлатежныеПозиции" 
//
// Параметры:
...
//
&НаКлиенте
Процедура бит_ПодключаемыйНадписьПлатежныеПозицииНажатие(Элемент, СтандартнаяОбработка)

	СтандартнаяОбработка = Ложь;
	
	АдресТаблицы = "";
	бит_ПодключаемыйПлатежныеПозицииНаСервере("Поместить", АдресТаблицы);
	
	Оповещение = Новый ОписаниеОповещения("бит_ПодключаемыйПриЗавершенииРедактированияПлатежныхПозиций", ЭтотОбъект); 
 	ПараметрыПередачи = Новый Структура(); 
	ПараметрыПередачи.Вставить("Организация",	Объект.Организация);
	ПараметрыПередачи.Вставить("Контрагент",	Объект.Контрагент);
	ПараметрыПередачи.Вставить("АдресТаблицы", 	АдресТаблицы);
  	ОткрытьФорму("ОбщаяФорма.бит_ПлатежныеПозицииПлатежногоПоручения", ПараметрыПередачи, ЭтотОбъект, ЭтотОбъект,,,Оповещение);

КонецПроцедуры

// Подключаемый из кода функционал редактирования ТЧ "бит_ПлатежныеПозиции".
//
// Параметры:
//  Действие	 - Строка - может принмать значения "Получить", "Поместить".
//  АдресТаблицы - Строка - адрес таблицы во временном хранилище.
//
&НаСервере
Процедура бит_ПодключаемыйПлатежныеПозицииНаСервере(Действие, АдресТаблицы)

	Если Действие = "Получить" Тогда
		Объект.бит_ПлатежныеПозиции.Загрузить(ПолучитьИзВременногоХранилища(АдресТаблицы));
		Модифицированность = Истина;
	Иначе	
		АдресТаблицы = ПоместитьВоВременноеХранилище(Объект.бит_ПлатежныеПозиции.Выгрузить(), 
							УникальныйИдентификатор);
	КонецЕсли; 

КонецПроцедуры

// Обработчик оповещения закрытия формы "ОбщаяФорма.бит_ПлатежныеПозицииПлатежногоПоручения".
//
// Параметры:
...
//
&НаКлиенте
Процедура бит_ПодключаемыйПриЗавершенииРедактированияПлатежныхПозиций(Результат, Дополнительно) Экспорт

	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		бит_ПодключаемыйПлатежныеПозицииНаСервере("Получить", Результат.АдресТаблицы);
	КонецЕсли; 
...
&НаКлиентеНаСервереБезКонтекста
Процедура бит_ОбновитьНадписьПлатежныеПозиции(Форма)

	Объект 	 = Форма.Объект;
	Элементы = Форма.Элементы;
	
	ЧислоСтрок = Объект.бит_ПлатежныеПозиции.Количество();
	
	Если ЧислоСтрок = 0 Тогда
		ТекстСообщения = Нстр("ru = 'Платежные позиции'");
	Иначе	
		ТекстСообщения = СтрШаблон(Нстр("ru = 'Платежные позиции (%1)'"), Формат(ЧислоСтрок, "ЧФ='Ч поз.'"));
	КонецЕсли; 
	
	Форма["бит_НадписьПлатежныеПозиции"] = ТекстСообщения;
	
КонецПроцедуры

// +СБ. Широков Николай. 2014-10-22. Доработка #1094
Процедура СБ_ОрганизацияНачалоВыбора(Элемент, СтандартнаяОбработка) Экспорт
	СБ_КазначействоСервер.ОрганизацияНачалоВыбора(ЭтаФорма, Элемент, СтандартнаяОбработка, Истина);
