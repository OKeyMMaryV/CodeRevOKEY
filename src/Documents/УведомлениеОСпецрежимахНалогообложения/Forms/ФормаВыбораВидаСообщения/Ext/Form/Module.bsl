...
&НаСервере
Процедура ДобавитьВеткуУчастиеВзаиморасчетыСНалоговой(КорневойУровень, ФлагиУчета)
	Если ЗначениеЗаполнено(ФлагиУчета) И ФлагиУчета.СервисЭлектронныхТрудовыхКнижек Тогда
		Возврат;
	КонецЕсли;
...
Процедура СформироватьДерево()
	...
	ДобавитьВеткуУчастиеВзаиморасчетыСНалоговой(КорневойУровень, ФлагиУчета);
