...
Функция ОбработатьДанныеШтрихкода(Форма, ДанныеШтрихкода, ПараметрыСканирования, ВложенныеШтрихкоды = Неопределено) Экспорт
	...
	ИначеЕсли ДанныеШтрихкода.ВидыПродукции.Найти(Перечисления.ВидыПродукцииИС.Обувная) <> Неопределено Тогда
...
Процедура ЗаполнитьСчетКасса(Параметры) Экспорт
	...
	"ВЫБРАТЬ ПЕРВЫЕ 1000
	|	РозничнаяПродажа.Ссылка КАК Ссылка,
	|	ДокументОтчетОРозничныхПродажах.СчетКасса КАК СчетКасса
	...
	|	ВЫБОР
	|		КОГДА НЕ ВТ_СодержаниеДокумента.СчетКасса ЕСТЬ NULL
