...
Процедура ОбработатьПроведениеПоОперацииАксапты(ПараметрыПроведения)
	...
	Если Типзнч(ПараметрыПроведения.РеализацияРеквизиты) = Тип("ТаблицаЗначений") И ПараметрыПроведения.РеализацияРеквизиты.Количество() > 0 Тогда
	    Реквизиты = ПараметрыПроведения.РеализацияРеквизиты[0];
		Основание = Реквизиты.ДокументРеализации;
		ПрошлыйПериод = Реквизиты.ПрошлыйПериод;
		
		Если Типзнч(Основание) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		    Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеОшибки") Тогда
				Для Каждого СтрокаУслуга из Реквизиты.Регистратор.Услуги Цикл
					Если СтрокаУслуга.СуммаДоИзменения <> СтрокаУслуга.Сумма Тогда
						НовСтрока = ПараметрыПроведения.НДСКнигаПродаж.Добавить();
						НовСтрока.Период 						= Реквизиты.Период;	
						НовСтрока.Регистратор	            	= Реквизиты.Регистратор;
						НовСтрока.Организация	            	= Реквизиты.Организация;
						НовСтрока.Покупатель	               	= Реквизиты.Регистратор.Контрагент;
						НовСтрока.СчетФактура	            	= Реквизиты.ДокументРеализации;
						...
						НовСтрока.Событие	                	= Перечисления.СобытияПоНДСПродажи.Реализация;
						НовСтрока.ДатаСобытия	            	= Реквизиты.Период;
						НовСтрока.ЗаписьДополнительногоЛиста 	= Истина;	
						НовСтрока.СторнирующаяЗаписьДопЛиста 	= ПрошлыйПериод;	
						НовСтрока.КорректируемыйПериод	        = НачалоКвартала(Реквизиты.Регистратор.ок_ДатаИсходногоСФ);
						НовСтрока.СуммаБезНДС	                = -СтрокаУслуга.СуммаДоИзменения;
						НовСтрока.НДС	                        = -СтрокаУслуга.СуммаНДСДоИзменения;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			СчетДт = ок_СчетУчетаРасчетовСКонтрагентом;
			
			Для Каждого СтрокаТЧ из ПараметрыПроведения.РеализацияТаблицаДокумента Цикл
				СтрокаТЧ.КорСчет = СчетДт;
			КонецЦикла;
			
			Для Каждого СтрокаТЧ из ПараметрыПроведения.СторноРеализацияТаблицаДокумента Цикл
				СтрокаТЧ.КорСчет = СчетДт;
			КонецЦикла;
