...
Функция ТекстЗапросаДанныеДляПечатиЧека(ИменаТаблиц) Экспорт
	...
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.НомерТабличнойЧасти КАК НомерТабличнойЧасти,
	...
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.НомерТабличнойЧасти
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерТабличнойЧасти,
	|	НомерСтроки
...
Функция ДоступнаКорректировкаБУиНУ(ДокументРеализации) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументРеализации) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		Возврат Истина;
...
Процедура УстановитьСчетаУчетаРасчетов(ДокументКорректировка, ДокументРеализации) Экспорт
	...
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-05-17 (#НТП-МП-03)	
	Исключения = Новый ОписаниеТипов(Исключения, "ДокументСсылка.бит_ок_ОперацияАксапты");  
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Конец 2021-05-17 (#НТП-МП-03)
...
Функция ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|		ПО КорректировкаРеализацииТовары.НомерСтроки = ТаблицаТовары.НомерСтроки
	|ГДЕ
	|	КорректировкаРеализацииТовары.Ссылка = &Ссылка
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_РНПТДокументРеализации КАК ВТ_РНПТДокументРеализации
...
// Возвращает массив документов с прослеживаемыми товарами.
//
// Параметры:
