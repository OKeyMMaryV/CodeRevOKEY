...
Функция ВыводИзОборотаJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	ПараметрыСканирования = ШтрихкодированиеИС.ПараметрыСканирования(ДокументСсылка, Неопределено, Перечисления.ВидыПродукцииИС.Обувная);
	...
	Если Шапка.ВидПервичногоДокумента = Перечисления.ВидыПервичныхДокументовВыводаИзОборотаИСМП.КассовыйЧек Тогда
		ВидДокумента = "RECEIPT";
	ИначеЕсли Шапка.ВидПервичногоДокумента = Перечисления.ВидыПервичныхДокументовВыводаИзОборотаИСМП.ТоварныйЧек Тогда
		ВидДокумента = "SALES_RECEIPT";
	ИначеЕсли Шапка.ВидПервичногоДокумента = Перечисления.ВидыПервичныхДокументовВыводаИзОборотаИСМП.ТоварнаяНакладная Тогда
		ВидДокумента = "CONSIGNMENT_NOTE";
	ИначеЕсли Шапка.ВидПервичногоДокумента = Перечисления.ВидыПервичныхДокументовВыводаИзОборотаИСМП.ТаможеннаяДекларация Тогда
		ВидДокумента = "CUSTOMS_DECLARATION";
	ИначеЕсли Шапка.ВидПервичногоДокумента = Перечисления.ВидыПервичныхДокументовВыводаИзОборотаИСМП.УПД Тогда
		ВидДокумента = "UTD";
	ИначеЕсли Шапка.ВидПервичногоДокумента = Перечисления.ВидыПервичныхДокументовВыводаИзОборотаИСМП.АктУничтожения Тогда
		ВидДокумента = "DESTRUCTION_ACT";
	ИначеЕсли Шапка.ВидПервичногоДокумента = Перечисления.ВидыПервичныхДокументовВыводаИзОборотаИСМП.Прочее Тогда
		ВидДокумента = "OTHER";
	Иначе
		ИнтеграцияИСКлиентСервер.ДобавитьТекстОшибки(
			СообщениеJSON,
			СтрШаблон(
				НСтр("ru = 'Не известный вид первичного документа: %1'"), Шапка.ВидПервичногоДокумента));
	КонецЕсли;
	...
	ТелоЗапроса.Вставить("document_type", ВидДокумента);
	Если Шапка.ВидПервичногоДокумента = Перечисления.ВидыПервичныхДокументовВыводаИзОборотаИСМП.Прочее Тогда
	...
	Для Каждого СтрокаТЧМаркированныеТовары Из РезультатПоиска.МаркированныеТовары Цикл
		...
		Если НайденныеСтроки.Количество() = 1 Тогда
			...
			СтрокаТЧ.Вставить("cis", ШтрихкодированиеИСМПКлиентСервер.КодМаркировкиБезСкобок(СтрокаТЧМаркированныеТовары.Штрихкод));
			...
			Если Не Шапка.Операция = ПредопределенноеЗначение(
				"Перечисление.ВидыОперацийИСМП.ВыводИзОборотаЭкспортЗаПределыСтранЕАЭС") Тогда
				СтрокаТЧ.Вставить("product_cost", СтрокаТЧТовары.Цена);
				СтрокаТЧ.Вставить("product_tax",  СтрокаТЧТовары.СуммаНДС);
			КонецЕсли;
...
Функция ТекстЗапросаПанелиОбменаИСМПОтработайте() Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВыводИзОборотаИСМП КАК ДокументИСМП
...
// 
// Возвращаемое значение:
//  (Cм. ИнтеграцияИСПереопределяемый.ЗаполнитьПараметрыУказанияСерий) - параметры указания серий
//
...
Процедура ЗаполнитьДанныеОбъектаФормыПоДеревуУпаковок(Форма, ДеревоУпаковок, ПараметрыЗаполнения, УпаковкаВерхнегоУровня = Неопределено) Экспорт
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		Если СтрокаДерева.ВидПродукции <> Неопределено
			...
			ОбработатьДанныеШтрихкодаПротребительскойУпаковки(Форма, СтрокаДерева, Неопределено, ПараметрыЗаполнения);
