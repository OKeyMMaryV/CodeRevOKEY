...
Процедура ЗаполнитьУчредителей(Объект, Знач Руководитель = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("Субсчета80",  
	...
	Пока Выборка.Следующий() Цикл
		...
		// Пользователи в своих базах на счетах 80 могут вести сводный учет.
		// Добавляем в заявку, только если учредитель указан явно.
		Если НЕ ЗначениеЗаполнено(Выборка.Учредитель) Тогда
			// Если конкретное значение учредителя не указано, но остаток по нему есть,
			// то по умолчанию установим признак наличия среди учредителей и физлиц и юрлиц.
			// Если список останется пустым, то перед отправкой заявки будем ругаться,
			// пока пользователь либо не выберет другое значение переключателя, либо не заполнит учредителей.
			ЕстьУчредителиФизическиеЛица  = Истина;
			ЕстьУчредителиЮридическиеЛица = Истина;
			Продолжить;
		КонецЕсли;
		
		Если ТипЗнч(Выборка.Учредитель) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
			НоваяСтрока = Объект.Учредители.Добавить();
			НоваяСтрока.Учредитель = Выборка.Учредитель;
...
Процедура ПрочитатьУсловияКредитованияБанка(СведенияОЗаемщике, ДатаЗаявки, Банк, ХранилищеXML, СведенияОБанках)
	...
	Если НЕ ПроверитьСоставУчредителейЗаемщикаЮрЛица(СведенияОЗаемщике, ДействующийКредитныйПродукт) Тогда
...
Процедура ПроверитьЗаполнениеУчредители(ОбъектИлиФорма, РеквизитыОрганизации, ИмяСтраницы, Отказ)
	...
	Если НЕ Объект.ТребуютсяДанныеУчредителей Тогда
		// Выбранным банкам не требуются данные об учредителях, не проверяем их заполнение.
		Возврат;
	КонецЕсли;
	...
	Для каждого СтрокаТаблицы Из Учредители Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ДатаРождения) Тогда
			ПутьКДанным    = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
				"Учредители", СтрокаТаблицы.НомерСтроки, "ДатаРождения");
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ИНН) Тогда
			ПутьКДанным    = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
				"Учредители", СтрокаТаблицы.НомерСтроки, "ИНН");
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.Адрес) Тогда
			ПутьКДанным    = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
				"Учредители", СтрокаТаблицы.НомерСтроки, "Адрес");
		...
		// Паспортные данные
		ПутьКДанным = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
			"Учредители", СтрокаТаблицы.НомерСтроки, "ПредставлениеДокумента");
...
Функция ПодготовитьФайлыДляОтправки_ЗаявкаНаКредит(ЗаявкаНаКредит, Банки, ЭтоЭлектроннаяПодписьВМоделиСервиса)
	...
	ИнформацияОБанкахДляСогласий = ЗаявкиНаКредит.ПодготовитьИнформациюОБанкахДляСогласий(Банки, ПараметрыСогласия);
	ТекстыСогласий               = ЗаявкиНаКредит.ТекстСогласия(
									ПараметрыСогласия, ИнформацияОБанкахДляСогласий, "ПолноеНаименованиеБанка", Ложь);
