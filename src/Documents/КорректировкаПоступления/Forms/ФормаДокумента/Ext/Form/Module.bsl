...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//СБ Ванюков К. -
	ИжТиСи_СВД_Сервер.ОК_ВывестиРеквизиты(ЭтаФорма, "Документ.КорректировкаПоступления.ФормаДокумента");
	//ОК Ванюков К. -
	...
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-21 (#3050)
	...
	КонецЦикла;                                              	
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-21 (#3050)	
	...
	ок_ДобавитьРеквизитыСФ(ЭтаФорма);
...
Процедура ПриОткрытии(Отказ)
	...
	//ОКЕЙ Рычаков А.С. (СофтЛаб) Начало 2019-11-17 (#3526)
	УстановитьВидимостьок_КорректировкаТекущимГодом(Ложь);
	//ОКЕЙ Рычаков А.С. (СофтЛаб) Конец 2019-11-17 (#3526)
	
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-07-13 (#НТП-МП-03)	
	ок_УправлениеЭлементамиФормыСФ();
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Конец 2021-077-13 (#НТП-МП-03)	
...
Процедура ДокументПоступленияПриИзменении(Элемент)
	...
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-05-17 (#НТП-МП-03)
	ок_ПриИзмененииДокументаПоступления();
	ок_ЗаполнитьРеквизитыСФ(ЭтаФорма);
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Конец 2021-05-17 (#НТП-МП-03)
...
Процедура ДокументПоступленияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.СчетФактураПолученный") Тогда
		...
		КонецЕсли;
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-06-08 (#НТП-МП-03)
	ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		Возврат;
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Конец 2021-06-08 (#НТП-МП-03)
...
&НаКлиенте
Процедура ок_КорректировкаТекущимГодомПриИзменении(Элемент)
	
	ок_КорректировкаТекущимГодомПриИзмененииНаСервере();
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ДанныеСтрокиТаблицы = Новый Структура(
		...
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-30 (#4214)
		|ПрослеживаемыйТовар, РНПТ, ПодсказкаРНПТ, ИдентификаторСтроки,
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-30 (#4214) 
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ТоварыРНПТ" Тогда
		...
		Если ТекущиеДанные.ПрослеживаемыйТовар Тогда
			ПрослеживаемостьФормыКлиентБП.ОткрытьФормуПодбораРНПТ(ЭтаФорма, "Товары", Истина);
			СтандартнаяОбработка = Ложь;
		КонецЕсли;
	ИначеЕсли Поле.Имя = "ТоварыРНПТДо" Тогда
		ПрослеживаемостьФормыКлиентБП.ОткрытьФормуПросмотраРНПТДоКорректировки(ЭтаФорма, "Товары");
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
...
Процедура ЗарегистрироватьСчетФактуру(Команда)
	...
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-05-17 (#НТП-МП-03)
	Если Типзнч(Объект.ДокументПоступления) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		Отказ = Ложь;
	КонецЕсли;
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Конец 2021-05-17 (#НТП-МП-03)
	Если Отказ Тогда
...
Процедура УправлениеФормой(ЭтотОбъект)
	...
	Если НЕ ЭтотОбъект.ВедетсяУчетПрослеживаемыхТоваров
		ИЛИ ЭтотОбъект.ПоставщикРезидентТаможенногоСоюза Тогда
	...
	Если ЗначениеЗаполнено(Объект.ДокументПоступления)
		...
		//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2019-11-17 (#3526)
		И НЕ Объект.ок_КорректировкаТекущимГодом
		//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2019-11-17 (#3526)		
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.ДокументПоступления)
		...
		И Объект.ок_КорректировкаТекущимГодом
		И Объект.КорректироватьБУиНУ Тогда
...
Процедура ПодготовитьФормуНаСервере()
	...
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-30 (#4214)
	ПрослеживаемостьФормыБП.ЗаполнитьРНПТДоКорректировки(ЭтотОбъект);		
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-30 (#4214) 
...
//ОКЕЙ Рычаков А.С. (СофтЛаб) Начало 2019-11-17 (#3526)
&НаКлиенте
Процедура УстановитьВидимостьок_КорректировкаТекущимГодом(ЭтоИзменениеАналитики)
		
	Если ЗначениеЗаполнено(Объект.ДокументПоступления)
		И Год(ДатаДокументаПоступления) < Год(ТекущаяДата()) Тогда
		
		Элементы.ок_КорректировкаТекущимГодом.Видимость = Истина;
		
	Иначе 
		
		Элементы.ок_КорректировкаТекущимГодом.Видимость = Ложь;
		
		Если ЭтоИзменениеАналитики Тогда
			Объект.ок_КорректировкаТекущимГодом = Ложь;
		КонецЕсли;	
...
Процедура ДатаПриИзмененииСервер()
	...
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-30 (#4214)
	// Признак прослеживаемости устанавливаем на дату сходного документа поступления.
	РаботаСНоменклатурой.ОбновитьПризнакПрослеживаемости(Объект.Товары, ВедетсяУчетПрослеживаемыхТоваров);
	ЗаполнитьДобавленныеКолонкиТаблиц();
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-30 (#4214) 
...
Процедура ДокументПоступленияПриИзмененииКлиент()
	...
	//ОКЕЙ Рычаков А.С. (СофтЛаб) Начало 2019-11-17 (#3526)
	УстановитьВидимостьок_КорректировкаТекущимГодом(Истина);
	//ОКЕЙ Рычаков А.С. (СофтЛаб) Конец 2019-11-17 (#3526)
...
Функция СведенияОбИсправляемомДокументе(ВыбранноеЗначение, СсылкаНаТекущийДокумент)
	...
	Иначе
		...
		//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-05-17 (#НТП-МП-03)	
		Если ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
...
//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2019-11-17 (#3526)
&НаСервере
Процедура ок_КорректировкаТекущимГодомПриИзмененииНаСервере()
...
//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-05-17 (#НТП-МП-03)
#Область ок_ДанныеИсходнойСчетФактуры

&НаСервереБезКонтекста
Процедура ок_ДобавитьРеквизитыСФ(Форма);
	
	ок_ЗаполнитьРеквизитыСФ(Форма);
	
	НовыйЭлемент = Форма.Элементы.Добавить("ок_СтраницаДанныеСФ",Тип("ГруппаФормы"),Форма.Элементы.Страницы);
    НовыйЭлемент.Вид = ВидГруппыФормы.Страница;
    НовыйЭлемент.Заголовок = "Данные исходной с\ф";
	
	НовыйЭлемент = Форма.Элементы.Добавить("ок_НомерИсходногоСФ", Тип("ПолеФормы"), Форма.Элементы.ок_СтраницаДанныеСФ);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.ок_НомерИсходногоСФ";
	НовыйЭлемент.Заголовок = "Номер с\ф";
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "ДанныеСФПриИзмененииДанных");	
	
	НовыйЭлемент = Форма.Элементы.Добавить("ок_ДатаИсходногоСФ", Тип("ПолеФормы"), Форма.Элементы.ок_СтраницаДанныеСФ);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.ок_ДатаИсходногоСФ";
	НовыйЭлемент.Заголовок = "Дата с\ф";
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "ДанныеСФПриИзмененииДанных");
	
	НовыйЭлемент = Форма.Элементы.Добавить("ок_ВидСчетаФактуры", Тип("ПолеФормы"), Форма.Элементы.ок_СтраницаДанныеСФ);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.ок_ВидСчетаФактуры";
	НовыйЭлемент.Заголовок = "Вид счета-фактуры";
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "ДанныеСФПриИзмененииДанных");
	
	НовыйЭлемент = Форма.Элементы.Добавить("ок_КодВидаОперации", Тип("ПолеФормы"), Форма.Элементы.ок_СтраницаДанныеСФ);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.ок_КодВидаОперации";
	НовыйЭлемент.Заголовок = "Код вида операции";
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "ДанныеСФПриИзмененииДанных");
	
	НовыйЭлемент = Форма.Элементы.Добавить("ок_КодВидаОперацииНаУменьшение", Тип("ПолеФормы"), Форма.Элементы.ок_СтраницаДанныеСФ);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.ок_КодВидаОперацииНаУменьшение";
	НовыйЭлемент.Заголовок = "Код вида операции (уменьшение)";
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "ДанныеСФПриИзмененииДанных");
	
	НовыйЭлемент = Форма.Элементы.Добавить("ок_ИННКонтрагентаДоИзменения", Тип("ПолеФормы"), Форма.Элементы.ок_СтраницаДанныеСФ);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.ок_ИННКонтрагентаДоИзменения";
	НовыйЭлемент.Заголовок = "ИНН контрагента";
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "ДанныеСФПриИзмененииДанных");
	
	НовыйЭлемент = Форма.Элементы.Добавить("ок_КППКонтрагентаДоИзменения", Тип("ПолеФормы"), Форма.Элементы.ок_СтраницаДанныеСФ);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.ок_КППКонтрагентаДоИзменения";
	НовыйЭлемент.Заголовок = "КПП контрагента";
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "ДанныеСФПриИзмененииДанных");
	
	НовыйЭлемент = Форма.Элементы.Добавить("ок_СчетУчетаРасчетовСКонтрагентом", Тип("ПолеФормы"), Форма.Элементы.ок_СтраницаДанныеСФ);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.ок_СчетУчетаРасчетовСКонтрагентом";
	НовыйЭлемент.Заголовок = "Счет учета расчетов с контрагентом";
	
	НовыйЭлемент = Форма.Элементы.Добавить("ок_СчетУчетаРасчетовПоАвансам", Тип("ПолеФормы"), Форма.Элементы.ок_СтраницаДанныеСФ);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.ок_СчетУчетаРасчетовПоАвансам";
	НовыйЭлемент.Заголовок = "Счет учета расчетов по авансам";
	
	НовыйЭлемент = Форма.Элементы.Добавить("ПодразделениеОрганизации", Тип("ПолеФормы"), Форма.Элементы.ок_СтраницаДанныеСФ);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.ПодразделениеОрганизации";
	НовыйЭлемент.Заголовок = "Подразделение";
	
	Форма.Элементы.ИННКонтрагента.УстановитьДействие("ПриИзменении", "ок_ИННКонтрагентаПриИзменении");
		
КонецПроцедуры	

&НаКлиентеНаСервереБезКонтекста
Процедура ок_ЗаполнитьРеквизитыСФ(Форма)
	
	Если ТипЗнч(Форма.Объект.ДокументПоступления) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		Форма.НомерСчетаФактурыДоИзменения 							= Форма.Объект.ок_НомерИсходногоСФ;
		Форма.ДатаСчетаФактурыДоИзменения 							= Форма.Объект.ок_ДатаИсходногоСФ;
		Форма.КодВидаОперацииСчетаФактурыДоИзменения				= Форма.Объект.ок_КодВидаОперации;
		Форма.КодВидаОперацииСчетаФактурыНаУменьшениеДоИзменения 	= Форма.Объект.ок_КодВидаОперацииНаУменьшение;
		Форма.ИННКонтрагентаДоИзменения 							= Форма.Объект.ок_ИННКонтрагентаДоИзменения;
		Форма.КППКонтрагентаДоИзменения 							= Форма.Объект.ок_КППКонтрагентаДоИзменения;	
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ок_ПриИзмененииДокументаПоступления()
	...
	Если ТипЗнч(Объект.ДокументПоступления) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		...
		Объект.ок_НомерИсходногоСФ					= "";
		Объект.ок_ДатаИсходногоСФ					= Дата(1,1,1);
		Объект.ок_ВидСчетаФактуры					= ПредопределенноеЗначение("Перечисление.ВидСчетаФактурыПолученного.ПустаяСсылка");
		Объект.ок_КодВидаОперации					= "";
		Объект.ок_КодВидаОперацииНаУменьшение		= "";
		Объект.ок_ИННКонтрагентаДоИзменения			= "";
		Объект.ок_КППКонтрагентаДоИзменения			= "";
		Объект.ок_СчетУчетаРасчетовСКонтрагентом	= ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПустаяСсылка");
		Объект.ок_СчетУчетаРасчетовПоАвансам		= ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПустаяСсылка");
		Объект.ок_СуммаПоСчетуФактуре				= 0;
		Корректировать 								= 1;
				
		//ОКЕЙ Завьялова Е.А. (ПервыйБИТ) Начало 2021-06-17 (#НТП-МП-10)
		ок_ЗаполнитьПоОперацииАксапты();	
		//ОКЕЙ Завьялова Е.А. (ПервыйБИТ) Конец 2021-06-17 (#НТП-МП-10)
	КонецЕсли;	
	
КонецПроцедуры

&НаКлиенте
Процедура ок_УправлениеЭлементамиФормыСФ()
	
	Если ТипЗнч(Объект.ДокументПоступления) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		Элементы.ок_СтраницаДанныеСФ.Видимость = Истина;
		Элементы.Контрагент.ТолькоПросмотр = Ложь;
		Элементы.ДоговорКонтрагента.ТолькоПросмотр = Ложь;	
	Иначе
		Элементы.ок_СтраницаДанныеСФ.Видимость = Ложь;
...
//ОКЕЙ Завьялова Е.А. (ПервыйБИТ) Начало 2021-06-17 (#НТП-МП-10)
&НаСервере
Процедура ок_ЗаполнитьПоОперацииАксапты()
	
	Если ЗначениеЗаполнено(Объект.ДокументПоступления) Тогда
		Объект.ОК_ID_Разноска = Объект.ДокументПоступления.ОК_ID_Разноска;
	КонецЕсли;
...
&НаКлиенте
Процедура ДанныеСФПриИзмененииДанных(Элемент)
	
	Если Элемент.Имя = "ок_НомерИсходногоСФ" Тогда
		НомерСчетаФактурыДоИзменения = Объект.ок_НомерИсходногоСФ;
	ИначеЕсли Элемент.Имя = "ок_ДатаИсходногоСФ" Тогда
		ДатаСчетаФактурыДоИзменения = Объект.ок_ДатаИсходногоСФ;
	ИначеЕсли Элемент.Имя = "ок_КодВидаОперации" Тогда
		КодВидаОперацииСчетаФактурыДоИзменения = Объект.ок_КодВидаОперации;
	ИначеЕсли Элемент.Имя = "ок_КодВидаОперацииНаУменьшение" Тогда
		КодВидаОперацииСчетаФактурыНаУменьшениеДоИзменения = Объект.ок_КодВидаОперацииНаУменьшение;
	ИначеЕсли Элемент.Имя = "ок_ИННКонтрагентаДоИзменения" Тогда
		ИННКонтрагентаДоИзменения = Объект.ок_ИННКонтрагентаДоИзменения;
	ИначеЕсли Элемент.Имя = "ок_КППКонтрагентаДоИзменения" Тогда
		КППКонтрагентаДоИзменения = Объект.ок_КППКонтрагентаДоИзменения;
	КонецЕсли;	
	
КонецПроцедуры

&НаКлиенте
Процедура ок_ИННКонтрагентаПриИзменении(Элемент)
	
	УправлениеВидимостьюРеквизитовИсправляемогоСчетаФактуры(ЭтаФорма);
	
КонецПроцедуры

&НаКлиенте
Процедура ок_ОбновитьДанныеСФ()
	
	Объект.ок_СуммаПоСчетуФактуре = Объект.Услуги.Итог("СуммаДоИзменения");
...
Процедура ТоварыПриАктивизацииЯчейки(Элемент)
	...
	ИначеЕсли ТекущаяКолонкаТаблицы.Имя = "ТоварыРНПТДо" Тогда
		ПрослеживаемостьФормыКлиентБП.ОткрытьФормуПросмотраРНПТДоКорректировки(ЭтаФорма, "Товары");
	КонецЕсли;
