...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	Если Реквизиты.СчетКасса = ПланыСчетов.Хозрасчетный.ПереводыВПути Тогда
		Запрос.УстановитьПараметр("Эквайер", ДокументСсылка.ок_ВидыПереводов);
		Запрос.УстановитьПараметр("ДоговорЭквайринга", ДокументСсылка.ок_СтатьяДвиженияДенежныхСредств);
	КонецЕсли;
