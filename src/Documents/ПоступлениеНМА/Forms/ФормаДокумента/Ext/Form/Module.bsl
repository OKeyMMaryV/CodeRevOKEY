...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-12-20 (#3052)
	ЗаполнитьКэшЗначений();
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-12-20 (#3052)	
	...
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-18 (#3052)
	...
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-18 (#3052)
...
Процедура НематериальныеАктивыНематериальныйАктивПриИзменении(Элемент)
	...
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-27 (#3052)
	//ДанныеСтроки = Новый Структура("НематериальныйАктив, СтавкаНДС, СчетУчета, СчетУчетаНДС, СпособУчетаНДС");
	//Заменено на:
	ДанныеСтроки = Новый Структура("НематериальныйАктив, СтавкаНДС, СчетУчета, СчетУчетаНДС, СпособУчетаНДС, ок_Аналитика_2");
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-27 (#3052)	
	ЗаполнитьЗначенияСвойств(ДанныеСтроки, ТекущиеДанные);

	НематериальныйАктивПриИзмененииНаСервере(ДанныеСтроки, Объект.Дата);
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-12-10 (#3052)
	//ЗаполнитьЗначенияСвойств(ТекущиеДанные, ДанныеСтроки);
	//Заменено на:
	ЗаполнитьЗначенияСвойств(ТекущиеДанные, ДанныеСтроки,,"СтавкаНДС");
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-12-10 (#3052)
...
Процедура НематериальныйАктивПриИзмененииНаСервере(СтрокаТаблицы, Период)
	...
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-19 (#3052)
	Если ЗначениеЗаполнено(СтрокаТаблицы.НематериальныйАктив) Тогда 
		СтрокаТаблицы.ок_Аналитика_2 = СтрокаТаблицы.НематериальныйАктив.бит_ОбъектСтроительства;
	КонецЕсли;
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-19 (#3052)
...
//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-12-20 (#3052)
&НаСервереБезКонтекста
Процедура УстановитьУОНМА(Форма)
		
	Если ЗначениеЗаполнено(Форма.ок_ФВБ) Тогда 
		...
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.НематериальныеАктивы.ок_Период");
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
		ЭлементОтбора.Использование = Истина;
	
			
		КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НематериальныеАктивыок_Период");
		...
		ЭлементОтбора = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.НематериальныеАктивы.ок_ЦФО");
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
		ЭлементОтбора.Использование = Истина;
				
		КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НематериальныеАктивыок_ЦФО");
		...
		ЭлементОтбора = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.НематериальныеАктивы.ок_СтатьяОборотов");
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
		ЭлементОтбора.Использование = Истина;
			
		КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НематериальныеАктивыок_СтатьяОборотов");
		...
		ЭлементОтбора = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.НематериальныеАктивы.ок_Проект");
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
		ЭлементОтбора.Использование = Истина;
			
		КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НематериальныеАктивыок_Проект");
		...
		ЭлементОтбора = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.НематериальныеАктивы.ок_Аналитика_2");
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
		ЭлементОтбора.Использование = Истина;
				
		КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НематериальныеАктивыАналитика_2");
...
Процедура ОтветПоЗаполнениюПериодаВТЧ(Результат, ДопПараметры) Экспорт
	...
	Для Каждого стрТЧ Из Объект["НематериальныеАктивы"] Цикл 
...
&НаКлиенте
Процедура ЗаполнитьПоУправленческимАналитикамНМА(Команда)
	бит_БК_ОбщийКлиент.ЗаполнитьТЧПоУправленческимАналитикам(ЭтотОбъект, "НематериальныеАктивы");
КонецПроцедуры

&НаКлиенте
Процедура ДозаполнитьПоУправленческимАналитикамНМА(Команда)
	бит_БК_ОбщийКлиент.ЗаполнитьТЧПоУправленческимАналитикам(ЭтотОбъект, "НематериальныеАктивы",Ложь);
КонецПроцедуры

//ОКЕЙ Василенко В.В. (СофтЛаб) Начало 2021-10-28 (#4415)
&НаКлиенте
Процедура ДозаполнитьПоУправленческимАналитикамНМАСтрока(Команда)
...
//ОКЕЙ Василенко В.В. (СофтЛаб) Конец 2021-10-28 (#4415)
	...
	Если ТипЗнч(пДанныеЗаполнения) <> Тип("ТаблицаЗначений") Тогда 
		Возврат;	
	КонецЕсли;
...
//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-07-15 (#3758)
&НаСервере
Процедура ОчиститьIDРазноски()
	
	Если Объект.ОК_ID_Разноска <> "" Тогда
