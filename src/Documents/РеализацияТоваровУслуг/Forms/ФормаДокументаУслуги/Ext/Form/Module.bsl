...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИжТиСи_СВД_Сервер.ОК_ВывестиРеквизиты(ЭтаФорма, "Документ.РеализацияТоваровУслуг.ФормаДокументаУслуги");
	//izhtc alena 17.08.2015)		
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Документ.РеализацияТоваровУслуг.Форма.ФормаРеквизитыОрганизацииКонтрагентаУслуги" Тогда
		...
		УстановитьТипСубконто(3,ТекущиеДанные);
		//ОК Калинин М.
		
		ДанныеСчета = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(Элементы.Услуги.ТекущиеДанные.СчетДоходов);
		...
		Если НЕ (ДанныеСчета.КоличествоСубконто > 1 
			...
			УстановитьТипСубконто(2,ТекущиеДанные);
		Иначе
...
Процедура ДекорацияРеквизитыОрганизацииКонтрагентаНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("ДатаПередачиДокументов",		Объект.ДатаПередачиДокументов);
	//izhtc alena 19.08.2015)	
...
Процедура ЗаполнитьАналитикуУчета(СтрокаТабличнойЧасти, ПараметрыОбъекта)
	...
	ДанныеЗаполнения.Вставить("Субконто2");
	ДанныеЗаполнения.Вставить("Субконто3");
	//izhtc alena 18.08.2015)
...
Процедура УслугиСубконто2НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	КонецЕсли;
	ДанныеСчета = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(Элементы.Услуги.ТекущиеДанные.СчетДоходов);
	Если НЕ ЗначениеЗаполнено(Элементы.Услуги.ТекущиеДанные.Субконто2) Тогда 
	...
	Иначе 
		Массив = Новый Массив;
		Массив.Добавить(ТипЗнч(Элементы.Услуги.ТекущиеДанные.Субконто2));
		ОписаниеТиповСубконто2 = Новый ОписаниеТипов(Массив);
		Элемент.ОграничениеТипа = ОписаниеТиповСубконто2;
	КонецЕсли;
КонецПроцедуры
...
Процедура УслугиСубконто3НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Если Элементы.Услуги.ТекущиеДанные = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	Если ЗначениеЗаполнено(Элементы.Услуги.ТекущиеДанные.Субконто3) Тогда 		
		Массив = Новый Массив;		
		Массив.Добавить(ТипЗнч(Элементы.Услуги.ТекущиеДанные.Субконто3));
...
//ОК Калинин М. 260313
&НаКлиенте 
Процедура УстановитьТипСубконто(номер,СтрокаТЧ)
	ИмяТЧ = "Услуги";
	...
	Если ДанныеСчета.КоличествоСубконто >= номер Тогда
		СтрокаТЧ[СубконтоИмя] = ДанныеСчета["ВидСубконто" + номер + "ТипЗначения"].ПривестиЗначение();
		ТипЗначенияСубконто = ДанныеСчета["ВидСубконто" + номер + "ТипЗначения"];
		Элементы[ИмяТЧ + СубконтоИмя].ОграничениеТипа = ТипЗначенияСубконто;
	Иначе
		СтрокаТЧ[СубконтоИмя] = Неопределено;
		Элементы[ИмяТЧ + СубконтоИмя].ОграничениеТипа = Новый ОписаниеТипов("Неопределено");
	КонецЕсли;

	Элементы[ИмяТЧ +СубконтоИмя].Доступность = НЕ (НЕ ЗначениеЗаполнено(Счет) ИЛИ (ДанныеСчета.КоличествоСубконто < номер));
КонецПроцедуры
//izhtc alena 18.08.2015)

//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-05-11 (#4139)
&НаСервере
Процедура ЗаполнитьСодержаниеУслугНаСервере()
	
	ПользовательскаяФункция = ок_ОбщегоНазначения.ПолучитьЗначениеУниверсальнойНастройки("ПользовательскиеФункции", "РеализацияТоваровУслуг_ПередЗаписью");
	Если ЗначениеЗаполнено(ПользовательскаяФункция) Тогда
		Результат = бит_ОбщегоНазначения.ВыполнитьПользовательскуюФункцию(ПользовательскаяФункция, Объект,,,,Элементы.Услуги.ВыделенныеСтроки);
	КонецЕсли;
	                    
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьСодержаниеУслуг()
	ЗаполнитьСодержаниеУслугНаСервере();
КонецПроцедуры
//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-05-11 (#4139)
