...
Процедура СкопироватьТабличныеЧастиСчетаНаОплату(ИменаТабличныхЧастей, Счет)
	...
	Если СкопироватьТовары Или СкопироватьУслуги Тогда
		...
		ТоварыУслугиКРеализации = Документы.РеализацияТоваровУслуг.ТоварыУслугиКРеализацииПоСчету(
				Счет, РеализацииПоСчету, ЗаполнятьПризнакПрослеживаемости);
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ок_СуммаДокументаПрисоединенныйФайлРасчета = 0;
	//ОК(СофтЛаб) Вдовиченко Г.В. 20181101 №3103
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.Услуги Тогда
		...
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2019-01-14 (#3156) (#AT-1782026#)
		//Если Справочники.Склады.ИспользуетсяНесколькоСкладов() Тогда
		//	Склад = Справочники.Склады.ПустаяСсылка();
		//КонецЕсли;
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2019-01-14 (#3156) (#AT-1782026#)		
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ПрослеживаемостьБП.СформироватьДвиженияРеализацияТоваров(
		...
		Если ПараметрыПроведения.Реквизиты[0].ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером 
			И ПараметрыПроведения.ПрослеживаемыеТовары <> Неопределено Тогда
		ТаблицаПрослеживаемыеТоварыПереданныеКомиссионеру = ПараметрыПроведения.ПрослеживаемыеТовары;
		ТаблицаПрослеживаемыеТоварыПереданныеКомиссионеру.ЗаполнитьЗначения(
			ПараметрыПроведения.Реквизиты[0].Контрагент, "Комиссионер");
		ПрослеживаемостьБП.СформироватьДвиженияПоступлениеТоваров(
		ТаблицаПрослеживаемыеТоварыПереданныеКомиссионеру,
		Неопределено,
		Неопределено,
		ПараметрыПроведения.Реквизиты,
		Движения);
	...
	ПрослеживаемостьПереопределяемый.СформироватьДвиженияПрослеживаемыхТоваровВЕАЭС(
		ПараметрыПроведения.ПрослеживаемыеТоварыРеализованные, ПараметрыПроведения.РеализацияТаблицаДокумента,
		ПараметрыПроведения.Реквизиты,
		Движения);
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-21 (#4214) 
