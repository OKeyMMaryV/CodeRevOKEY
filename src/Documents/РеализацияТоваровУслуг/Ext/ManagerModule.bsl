...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫРАЗИТЬ(РеализацияТоваровУслугСведенияПрослеживаемости.РНПТ.Код КАК СТРОКА(30)) КАК НомерТовара,
...
Функция ТекстЗапросаРеализация(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	//izhtc alena 18.08.2015(
	// БИТ AMerkulov 09012014 ++ /Возможность выбора второго субконто
	|	ВременнаяТаблицаРеализация.Субконто2  КАК Субконто2,
	// БИТ AMerkulov --	
	//ОК Калинин М. 260313
	|	ВременнаяТаблицаРеализация.Субконто3  КАК Субконто3,
	//ОК Калинин М.
	//izhtc alena 18.08.2015)	
...
Функция ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ВЫБОР
	|		КОГДА &ЭтоДоговорСКомиссионером
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ПередачаТовараКомитентомКомиссионеру)
	|		ИНАЧЕ ВЫБОР
	|				КОГДА &ПрослеживаемыеОперацииОтображаютсяВДекларацииНДС
	...
	|		ПО РеализацияТоваровУслугТовары.НомерСтроки = ТаблицаТовары.НомерСтроки
	|ГДЕ
	|	РеализацияТоваровУслугТовары.Ссылка = &Ссылка
