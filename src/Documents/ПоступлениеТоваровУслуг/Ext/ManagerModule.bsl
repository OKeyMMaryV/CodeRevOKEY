...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц) Экспорт
	...
	|	ТаблицаОборудование.Номенклатура.КодТНВЭД КАК НоменклатураКодТНВЭД
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-30 (#4214)
	|ИЗ
	|	Документ.ПоступлениеТоваровУслуг.Оборудование КАК ТаблицаОборудование
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведенийРублевыеСуммыДокументовВВалюте КАК РегистрСведенийРублевыеСуммыДокументовВВалюте
	...
	|	"""",
	|	"""",
	|	ТаблицаТопливо.Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	...
	|	ЛОЖЬ КАК ЭтоУслуга,
	|	ТаблицаТопливо.Ссылка.СуммаВключаетНДС КАК СуммаВключаетНДС,
	|	ТаблицаТопливо.Ссылка КАК Ссылка,
	|	ЛОЖЬ КАК ЭтоКомиссия,
	|	ТаблицаТопливо.Сумма КАК ВсегоРуб,
	|	ТаблицаТопливо.Сумма - ТаблицаТопливо.СуммаНДС КАК СуммаБезНДСРуб,
	|	ТаблицаТопливо.СуммаНДС КАК НДСРуб,
	|	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) КАК КонтрагентСводныйСФ,
	|	НЕОПРЕДЕЛЕНО КАК ПериодичностьУслуги,
	|	ЗНАЧЕНИЕ(Справочник.КлассификаторТНВЭД.ПустаяСсылка) КАК ТоварКодТНВЭД
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-30 (#4214) 
...
Функция ПолучитьСоответствиеВидовОперацийФормам() Экспорт
	...
	ФормыПоступлениеТоваровУслуг.Вставить(Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Товары, "ФормаДокументаОбщая");
	ФормыПоступлениеТоваровУслуг.Вставить(Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Услуги, "ФормаДокументаОбщая");
	// BIT AMerkulov 30-12-2015 --
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ПрослеживаемыеОперацииОтображаютсяВДекларацииНДС", ПлательщикНДС И Реквизиты.ЕстьНДС);
	...
	Запрос.Текст = ТекстЗапросаЗачетАвансов(НомераТаблиц, ПараметрыПроведения, Реквизиты)
		...
		Запрос.Текст = Запрос.Текст + ТекстЗапросаПрослеживаемыеОС(НомераТаблиц, ПараметрыПроведения, Реквизиты);
...
Функция ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц, Реквизиты)
	...
	Если Реквизиты.ЕстьТовары Тогда
		...
		|	,ТаблицаТовары.ок_Период КАК Период,		
		|	ТаблицаТовары.ок_СтатьяОборотов КАК СтатьяОборотов, 
		|	ТаблицаТовары.ок_СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
		|	ТаблицаТовары.ок_ЦФО КАК ЦФО,
		|	"""" КАК ОК_ID_Разноска, 
		|	ТаблицаТовары.ок_Проект КАК Проект,
		|	ТаблицаТовары.ок_Аналитика_2 КАК Аналитика_2		
	...
	Если Реквизиты.ЕстьУслуги Тогда
		...
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-09 (#3048)
		|	,ТаблицаУслуги.ок_Период КАК Период,
		|	ТаблицаУслуги.ок_СтатьяОборотов КАК СтатьяОборотов, 		
		|	ТаблицаУслуги.ок_СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
		|	ТаблицаУслуги.ок_ЦФО КАК ЦФО,		
		|	ТаблицаУслуги.ОК_ID_Разноска КАК ОК_ID_Разноска, 
		|	ТаблицаУслуги.ок_Проект КАК Проект,
		|	ТаблицаУслуги.ок_Аналитика_2 КАК Аналитика_2		
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)		
	...
	Если Реквизиты.ЕстьАгентскиеУслуги Тогда
		...
		|	ТаблицаАгентскиеУслуги.СтавкаНДС
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-09 (#3048)
		|	,ТаблицаАгентскиеУслуги.ок_Период КАК Период,
		|	ТаблицаАгентскиеУслуги.ок_СтатьяОборотов КАК СтатьяОборотов, 
		|	ТаблицаАгентскиеУслуги.ок_СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
		|	ТаблицаАгентскиеУслуги.ок_ЦФО КАК ЦФО,
		|	"""" КАК ОК_ID_Разноска, 
		|	ТаблицаАгентскиеУслуги.ок_Проект КАК Проект,
		|	ТаблицаАгентскиеУслуги.ок_Аналитика_2 КАК Аналитика_2		
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)		
	...
	Если Реквизиты.ЕстьОборудование Тогда
		...
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-09 (#3048)
		|	,ТаблицаОборудование.ок_Период КАК Период,
		|	ТаблицаОборудование.ок_СтатьяОборотов КАК СтатьяОборотов, 
		|	ТаблицаОборудование.ок_СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
		|	ТаблицаОборудование.ок_ЦФО КАК ЦФО,
		|	"""" КАК ОК_ID_Разноска, 
		|	ТаблицаОборудование.ок_Проект КАК Проект,
		|	ТаблицаОборудование.ок_Аналитика_2 КАК Аналитика_2		
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)		
	...
	Если Реквизиты.ЕстьОбъектыСтроительства Тогда
		...
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-09 (#3048)
		|	,ТаблицаОбъектыСтроительства.ок_Период КАК Период,
		|	ТаблицаОбъектыСтроительства.ок_СтатьяОборотов КАК СтатьяОборотов, 
		|	ТаблицаОбъектыСтроительства.ок_СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
		|	ТаблицаОбъектыСтроительства.ок_ЦФО КАК ЦФО,
		|	"""" КАК ОК_ID_Разноска, 
		|	ТаблицаОбъектыСтроительства.ок_Проект КАК Проект,
		|	ТаблицаОбъектыСтроительства.ок_Аналитика_2 КАК Аналитика_2		
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)		
...
Функция ТекстЗапросаВременныеТаблицыДокументаРасчетыВВалюте(НомераТаблиц, Реквизиты)
	...
	Если Реквизиты.ЕстьТовары Тогда
		...
		|	ВременнаяТаблицаТовары.СтатьяОборотов КАК СтатьяОборотов, 
		|	ВременнаяТаблицаТовары.ВидСтатьи КАК ВидСтатьи,
		|	ВременнаяТаблицаТовары.ЦФО КАК ЦФО,
		|	"""" КАК ОК_ID_Разноска, 
		|	ВременнаяТаблицаТовары.Проект КАК Проект,
		|	ВременнаяТаблицаТовары.Аналитика_2 КАК Аналитика_2		
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)		
		|ИЗ
	...
	Если Реквизиты.ЕстьУслуги Тогда
		...
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-09 (#3048)
		|	,ВременнаяТаблицаУслуги.Период КАК Период,
		|	ВременнаяТаблицаУслуги.СтатьяОборотов КАК СтатьяОборотов, 
		|	ВременнаяТаблицаУслуги.ВидСтатьи КАК ВидСтатьи,
		|	ВременнаяТаблицаУслуги.ЦФО КАК ЦФО,		
		|	ВременнаяТаблицаУслуги.ОК_ID_Разноска КАК ОК_ID_Разноска, 
		|	ВременнаяТаблицаУслуги.Проект КАК Проект,
		|	ВременнаяТаблицаУслуги.Аналитика_2 КАК Аналитика_2		
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)		
		|ИЗ
	...
	Если Реквизиты.ЕстьАгентскиеУслуги Тогда
		...
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-09 (#3048)
		|	,ВременнаяТаблицаАгентскиеУслуги.Период КАК Период,
		|	ВременнаяТаблицаАгентскиеУслуги.СтатьяОборотов КАК СтатьяОборотов, 
		|	ВременнаяТаблицаАгентскиеУслуги.ВидСтатьи КАК ВидСтатьи,
		|	ВременнаяТаблицаАгентскиеУслуги.ЦФО КАК ЦФО,
		|	"""" КАК ОК_ID_Разноска, 
		|	ВременнаяТаблицаАгентскиеУслуги.Проект КАК Проект,
		|	ВременнаяТаблицаАгентскиеУслуги.Аналитика_2 КАК Аналитика_2		
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)		
		|ИЗ
	...
	Если Реквизиты.ЕстьОборудование Тогда
		...
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-09 (#3048)
		|	,ВременнаяТаблицаОборудование.Период КАК Период,
		|	ВременнаяТаблицаОборудование.СтатьяОборотов КАК СтатьяОборотов, 
		|	ВременнаяТаблицаОборудование.ВидСтатьи КАК ВидСтатьи,
		|	ВременнаяТаблицаОборудование.ЦФО КАК ЦФО,
		|	"""" КАК ОК_ID_Разноска, 
		|	ВременнаяТаблицаОборудование.Проект КАК Проект,
		|	ВременнаяТаблицаОборудование.Аналитика_2 КАК Аналитика_2		
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)		
		|ИЗ
	...
	Если Реквизиты.ЕстьОбъектыСтроительства Тогда
		...
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-09 (#3048)
		|	,ВременнаяТаблицаОбъектыСтроительства.Период КАК Период,
		|	ВременнаяТаблицаОбъектыСтроительства.СтатьяОборотов КАК СтатьяОборотов, 
		|	ВременнаяТаблицаОбъектыСтроительства.ВидСтатьи КАК ВидСтатьи,
		|	ВременнаяТаблицаОбъектыСтроительства.ЦФО КАК ЦФО,
		|	"""" КАК ОК_ID_Разноска, 
		|	ВременнаяТаблицаОбъектыСтроительства.Проект КАК Проект,
		|	ВременнаяТаблицаОбъектыСтроительства.Аналитика_2 КАК Аналитика_2		
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)		
		|ИЗ
...
Функция ТекстЗапросаРасчетыВВалютеТовары()
	...
	|	,ВременнаяТаблицаТовары.Период КАК Период,
	|	ВременнаяТаблицаТовары.СтатьяОборотов КАК СтатьяОборотов, 
	|	ВременнаяТаблицаТовары.СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
	|	ВременнаяТаблицаТовары.ЦФО КАК ЦФО,
	|	"""" КАК ОК_ID_Разноска, 
	|	ВременнаяТаблицаТовары.Проект КАК Проект,
	|	ВременнаяТаблицаТовары.Аналитика_2 КАК Аналитика_2		
...
Функция ТекстЗапросаРасчетыВВалютеУслуги()
	...
	|	,ВременнаяТаблицаУслуги.Период КАК Период,
	|	ВременнаяТаблицаУслуги.СтатьяОборотов КАК СтатьяОборотов, 
	|	ВременнаяТаблицаУслуги.СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
	|	ВременнаяТаблицаУслуги.ЦФО КАК ЦФО,
	|	ВременнаяТаблицаУслуги.ОК_ID_Разноска КАК ОК_ID_Разноска, 
	|	ВременнаяТаблицаУслуги.Проект КАК Проект,
	|	ВременнаяТаблицаУслуги.Аналитика_2 КАК Аналитика_2		
...
Функция ТекстЗапросаРасчетыВВалютеОборудование()
	...
	|	,ВременнаяТаблицаОборудование.Период КАК Период,
	|	ВременнаяТаблицаОборудование.СтатьяОборотов КАК СтатьяОборотов, 
	|	ВременнаяТаблицаОборудование.СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
	|	ВременнаяТаблицаОборудование.ЦФО КАК ЦФО,
	|	"""" КАК ОК_ID_Разноска, 
	|	ВременнаяТаблицаОборудование.Проект КАК Проект,
	|	ВременнаяТаблицаОборудование.Аналитика_2 КАК Аналитика_2		
...
Функция ТекстЗапросаРасчетыВВалютеОбъектыСтроительства()
	...
	|	,ВременнаяТаблицаОбъектыСтроительства.Период КАК Период,
	|	ВременнаяТаблицаОбъектыСтроительства.СтатьяОборотов КАК СтатьяОборотов, 
	|	ВременнаяТаблицаОбъектыСтроительства.СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
	|	ВременнаяТаблицаОбъектыСтроительства.ЦФО КАК ЦФО,
	|	"""" КАК ОК_ID_Разноска, 
	|	ВременнаяТаблицаОбъектыСтроительства.Проект КАК Проект,
	|	ВременнаяТаблицаОбъектыСтроительства.Аналитика_2 КАК Аналитика_2		
...
Функция ТекстЗапросаПоступление(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Иначе
		...
		|	ТаблицаТовары.СтатьяОборотов КАК СтатьяОборотов, 
		|	ТаблицаТовары.ВидСтатьи КАК ВидСтатьи,
		|	ТаблицаТовары.ЦФО КАК ЦФО,
		|	"""" КАК ОК_ID_Разноска, 
		|	ТаблицаТовары.Проект КАК Проект,
		|	ТаблицаТовары.Аналитика_2 КАК Аналитика_2		
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)		
		|ИЗ
		|	ТаблицаТовары КАК ТаблицаТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ Реквизиты КАК Реквизиты
	...
	Иначе
		...
		|	ТаблицаУслуги.ПодразделениеЗатрат
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-09 (#3048)
		|	,ТаблицаУслуги.Период КАК Период,
		|	ТаблицаУслуги.СтатьяОборотов КАК СтатьяОборотов, 
		|	ТаблицаУслуги.ВидСтатьи КАК ВидСтатьи,
		|	ТаблицаУслуги.ЦФО КАК ЦФО,
		|	ТаблицаУслуги.ОК_ID_Разноска КАК ОК_ID_Разноска, 
		|	ТаблицаУслуги.Проект КАК Проект,
		|	ТаблицаУслуги.Аналитика_2 КАК Аналитика_2		
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)		
		|ИЗ
		|	ТаблицаУслуги КАК ТаблицаУслуги
		|		ЛЕВОЕ СОЕДИНЕНИЕ Реквизиты КАК Реквизиты
	...
	Иначе
		...
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-09 (#3048)
		|	,Оборудование.Период КАК Период,
		|	Оборудование.СтатьяОборотов КАК СтатьяОборотов, 
		|	Оборудование.ВидСтатьи КАК ВидСтатьи,
		|	Оборудование.ЦФО КАК ЦФО,
		|	"""" КАК ОК_ID_Разноска, 
		|	Оборудование.Проект КАК Проект,
		|	Оборудование.Аналитика_2 КАК Аналитика_2		
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)		
		|ИЗ
	...
	Иначе
		...
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-09 (#3048)
		|	,ОбъектыСтроительства.Период КАК Период,
		|	ОбъектыСтроительства.СтатьяОборотов КАК СтатьяОборотов, 
		|	ОбъектыСтроительства.ВидСтатьи КАК ВидСтатьи,
		|	ОбъектыСтроительства.ЦФО КАК ЦФО,
		|	"""" КАК ОК_ID_Разноска, 
		|	ОбъектыСтроительства.Проект КАК Проект,
		|	ОбъектыСтроительства.Аналитика_2 КАК Аналитика_2		
		//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)		
		|ИЗ
	...
	Иначе
		...
		Если Реквизиты.ЕстьАгентскиеУслуги Тогда
			...
			|	Реквизиты.Ссылка КАК ДокументРасчетовПоставщика
			//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-09 (#3048)
			|	,АгентскиеУслуги.Период КАК Период,
			|	АгентскиеУслуги.СтатьяОборотов КАК СтатьяОборотов, 
			|	АгентскиеУслуги.СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
			|	АгентскиеУслуги.ЦФО КАК ЦФО,
			|	"""" КАК ОК_ID_Разноска, 
			|	АгентскиеУслуги.Проект КАК Проект,
			|	АгентскиеУслуги.Аналитика_2 КАК Аналитика_2		
			//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-09 (#3048)			
			|ИЗ
...
Функция ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов(ВерсияПостановленияНДС1137 = Неопределено, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	...
	|ГДЕ
	|	ПоступлениеТоваровУслуг.Ссылка В(&МассивОбъектов)
	|	И ПоступлениеТоваровУслуг.Дата > &УсловиеПоДате
...
Функция ТоварыУслугиКПоступлениюПоСчету(СчетНаОплату, ПоступленияПоСчету, ЗаполнятьПризнакПрослеживаемости = Ложь) Экспорт
//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-21 (#4214) 
...
Функция ТекстЗапросаРеквизитыСчета(ИменаТаблиц)
	...
	|			ТОГДА ТоварыУслуги.Номенклатура.ПрослеживаемыйТовар
	|		ИНАЧЕ ЛОЖЬ
...
//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-07-27 (#3048)
Функция ПолучитьСтатьюРеглПоСтатьеОборотов(СтатьяОборотов, фКэшЗначений, СтатьяПДиР = Ложь) Экспорт
	
	Если фКэшЗначений.Свойство("СоответствиеСтатейОборотовИРегл") И 
		 фКэшЗначений.СоответствиеСтатейОборотовИРегл.Свойство(?(СтатьяПДиР, "СтПДиР", "СтЗатрат")) 
	Тогда 
	
		пНайденнаяСтатья = фКэшЗначений.СоответствиеСтатейОборотовИРегл[?(СтатьяПДиР, "СтПДиР", "СтЗатрат")].Получить(СтатьяОборотов);
		
		Если пНайденнаяСтатья<>Неопределено Тогда 
			Если ЗначениеЗаполнено(пНайденнаяСтатья) Тогда 
				Возврат пНайденнаяСтатья;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	|ГДЕ
	|	&СтатьяЗатрат
	|	И бит_СтатьиОборотов_СтатьиРегл.СтатьяРегл ССЫЛКА Справочник.СтатьиЗатрат
	...
	|ГДЕ
	|	&СтатьяПДиР
	|	И бит_СтатьиОборотов_СтатьиРегл.СтатьяРегл ССЫЛКА Справочник.ПрочиеДоходыИРасходы
	|	И бит_СтатьиОборотов_СтатьиРегл.СтатьяОборотов = &Статья
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-05-22 (#3713)
	|УПОРЯДОЧИТЬ ПО
	...
	Запрос.УстановитьПараметр("Статья"		 , СтатьяОборотов);
	Запрос.УстановитьПараметр("СтатьяПДиР"   , СтатьяПДиР);
	Запрос.УстановитьПараметр("СтатьяЗатрат" , Не СтатьяПДиР);
	
	пРезультат = Запрос.Выполнить();
	...
	Если Выборка.Следующий() Тогда 
		...
		//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-05-22 (#3713)
		//Если НЕ фКэшЗначений.Свойство("СоответствиеСтатейОборотовИРегл") Тогда 
		//	фКэшЗначений.Вставить("СоответствиеСтатейОборотовИРегл", Новый Структура);
		//КонецЕсли;
		//
		//Если НЕ фКэшЗначений.СоответствиеСтатейОборотовИРегл.Свойство(?(СтатьяПДиР, "СтПДиР", "СтЗатрат")) Тогда 
		//	фКэшЗначений.СоответствиеСтатейОборотовИРегл.Вставить(?(СтатьяПДиР, "СтПДиР", "СтЗатрат"), Новый Соответствие) ;
		//КонецЕсли;
		//
		//фКэшЗначений.СоответствиеСтатейОборотовИРегл[?(СтатьяПДиР, "СтПДиР", "СтЗатрат")].Вставить(СтатьяОборотов, Выборка.СтатьяРегл);
		//
		//Если Выборка.Количество()>1 Тогда                                                                           			
		//	пТекстОшибки = НСтр("ru = 'Найдено более одной статьи %1 по соответствию с указанной статьей оборотов %2!'");
		//	пТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(пТекстОшибки, ?(СтатьяПДиР,"прочих доходов-расходов","затрат"),
		//																		  СтатьяОборотов);
		//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(пТекстОшибки);
		//КонецЕсли;
		//
		//Возврат Выборка.СтатьяРегл;
		
		Если Выборка.Количество() > 1 
			И НЕ Выборка.ок_ОсновноеСоответствие Тогда
		
			ТекстСообшенияОбОшибках = "Для статьи оборотов «%1» найдено несколько сопоставлений статье %2.";
			ТекстСообшенияОбОшибках = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообшенияОбОшибках, СтатьяОборотов, ?(СтатьяПДиР,"прочих доходов-расходов","затрат"));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообшенияОбОшибках);
			
			Если СтатьяПДиР Тогда
				Возврат ПредопределенноеЗначение("Справочник.ПрочиеДоходыИРасходы.ПустаяСсылка");
			Иначе
				Возврат ПредопределенноеЗначение("Справочник.СтатьиЗатрат.ПустаяСсылка");
			КонецЕсли;
...
КонецФункции
//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-07-27 (#3048)
...
Функция ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ЕстьТовары Тогда
		...
		|		ПО ПоступлениеТоваровУслугТовары.НомерСтроки = ТаблицаТовары.НомерСтроки
		|ГДЕ
