...
Процедура ОсновныеСредстваПриИзменении(Элемент)
	...
	УстановитьДоступностьКнопокКопированияКолонокТЧ("ОсновныеСредства", Элементы.ОсновныеСредства);
...
//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2021-09-08 (#4338)
&НаКлиенте
Процедура ОсновныеСредстваПриАктивизацииЯчейки(Элемент)
...
Процедура ОткрытьСписокРНПТДляРедактирования(ТекущаяСтрока)
	...
	Если Не ЗначениеЗаполнено(ТекущаяСтрока.Сумма) Тогда 
		...
		"Колонка", "Заполнение", НСтр("ru = 'Сумма'"), ТекущаяСтрока.НомерСтроки, НСтр("ru = 'Основные средства'"));
...
&НаКлиенте
Процедура ок_ЗаполнитьКолонку_ОсновныеСредства(Команда)

	ЗаполнитьКолонкуТЧ(ЭтаФорма, Элементы.ОсновныеСредства);
...
Процедура ПослеЗакрытияВопросаЗаполнитьКолонкуТЧ(Результат, Параметры) Экспорт
	...
	Для Каждого СтрокаТЧ Из Объект[ИмяТЧ] Цикл
		...
		Если ИмяТЧ = НРег("ОсновныеСредства") Тогда
		
			Если ИмяКолонки = НРег("СтавкаНДС") Тогда
				
				ОсновныеСредстваСтавкаНДСПриИзменении(Неопределено);
				
			КонецЕсли; 
			
			ОсновныеСредстваПриИзменении(Неопределено);
