...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПроверитьСоответствиеРБПТаблицыУслуги(Ложь);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда 
		ПроверитьСоответствиеРБПТаблицыУслуги(Истина, Отказ);
	КонецЕсли;
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Конец 2021-10-27 (#4382)	
...
Процедура УслугиПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Не ТекущиеДанные = Неопределено Тогда 
		ПроверитьСоответствиеРБПТаблицыУслуги(Ложь,,ТекущиеДанные.НомерСтроки);
	КонецЕсли;
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Конец 2021-10-27 (#4382)
...
Процедура УслугиСчетЗатратПриИзменении(Элемент)
	...
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2021-10-27 (#4382)
	УстановитьДоступностьКомандыСоздатьПерезаполнитьРБП();
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Конец 2021-10-27 (#4382)
...
Процедура УслугиСубконто1ПриИзменении(Элемент)
	...
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2021-10-27 (#4382)
	УстановитьДоступностьКомандыСоздатьПерезаполнитьРБП();
	ПроверитьСоответствиеРБПТаблицыУслуги(Ложь, , Элементы.Услуги.ТекущиеДанные.НомерСтроки);
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Конец 2021-10-27 (#4382)
...
// ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2021-10-27 (#4382)
&НаКлиенте
Процедура ок_УслугиПриАктивизацииСтроки(Элемент)
	УстановитьДоступностьКомандыСоздатьПерезаполнитьРБП();
КонецПроцедуры
// ОКЕЙ Сафронов А.А. (СофтЛаб) Конец 2021-10-27 (#4382)
...
// ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2021-10-27 (#4382)
&НаКлиенте
Процедура ок_СоздатьРБП(Команда)	
	Если Модифицированность Тогда 
		ТекстВопроса = "ru = 'Для создания карточки РБП необходимо записать документ. Записать?'";
		ПоказатьВопрос(Новый ОписаниеОповещения("ок_СоздатьРБПЗаписьЗавершение", ЭтотОбъект)
							,НСтр(ТекстВопроса), РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет);
	Иначе	
		ок_СоздатьРБПОткрытьФорму();
	КонецЕсли;	
КонецПроцедуры

&НаКлиенте
Процедура ок_ПерезаполнитьРБП(Команда)
	ВыделенныеСтроки = Элементы.Услуги.ВыделенныеСтроки;
	МассивРБП = Новый Массив;
	Для Каждого СтрокаУслуги Из ВыделенныеСтроки Цикл
		ТекДанныеУслуги = Объект.Услуги.НайтиПоИдентификатору(СтрокаУслуги);
		Если ТекДанныеУслуги <> Неопределено
			И ЗначениеЗаполнено(ТекДанныеУслуги.Субконто1)
			И ТипЗнч(ТекДанныеУслуги.Субконто1) = Тип("СправочникСсылка.РасходыБудущихПериодов") Тогда 
			РеквизитыРБП = ПолучитьЗначенияРеквизитовРБППриСозданииИзДокументаПТУ(ТекДанныеУслуги); 
			МассивРБП.Добавить(Новый Структура("НомерСтроки, РБП, РеквизитыРБП", ТекДанныеУслуги.НомерСтроки, ТекДанныеУслуги.Субконто1, РеквизитыРБП));
		КонецЕсли;
	КонецЦикла;
	ПерезаполнитьРБПНаСервере(МассивРБП);
КонецПроцедуры
// ОКЕЙ Сафронов А.А. (СофтЛаб) Конец 2021-10-27 (#4382)
...
Процедура УстановитьУсловноеОформлениеУслуги() Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Услуги.НесоответствиеРБП", ВидСравненияКомпоновкиДанных.Равно, Истина);
...
&НаКлиенте
Процедура ДозаполнитьПоУправленческимАналитикамАгентскиеУслугиСтрока(Команда)
	ИмяТЧ = "АгентскиеУслуги";
	ОчиститьТЧ = Ложь;
	ОднаСтрока = Истина;
	бит_БК_ОбщийКлиент.ЗаполнитьТЧПоУправленческимАналитикам(ЭтотОбъект, ИмяТЧ, ОчиститьТЧ, ОднаСтрока);
КонецПроцедуры

&НаКлиенте
Процедура ДозаполнитьПоУправленческимАналитикамОборудованиеСтрока(Команда)
	ИмяТЧ = "Оборудование";
	ОчиститьТЧ = Ложь;
	ОднаСтрока = Истина;
	бит_БК_ОбщийКлиент.ЗаполнитьТЧПоУправленческимАналитикам(ЭтотОбъект, ИмяТЧ, ОчиститьТЧ, ОднаСтрока);
...
&НаКлиенте
Процедура ДозаполнитьПоУправленческимАналитикамОбъектыСтроительстваСтрока(Команда)
...
&НаКлиенте
Процедура ДозаполнитьПоУправленческимАналитикамТоварыСтрока(Команда)
	ИмяТЧ = "Товары";
	ОчиститьТЧ = Ложь;
	ОднаСтрока = Истина;
	бит_БК_ОбщийКлиент.ЗаполнитьТЧПоУправленческимАналитикам(ЭтотОбъект, ИмяТЧ, ОчиститьТЧ, ОднаСтрока);
...
&НаКлиенте
Процедура ДозаполнитьПоУправленческимАналитикамУслугиСтрока(Команда)
	ИмяТЧ = "Услуги";
	ОчиститьТЧ = Ложь;
	ОднаСтрока = Истина;
	бит_БК_ОбщийКлиент.ЗаполнитьТЧПоУправленческимАналитикам(ЭтотОбъект, ИмяТЧ, ОчиститьТЧ, ОднаСтрока);
...
Процедура ЗаполнитьТЧПоУправленческимАналитикам(Результат, ДопПараметры) Экспорт
	...
	Если ДопПараметры.Свойство("ОднаСтрока") Тогда
		ЗаполнитьТЧПоУправленческимАналитикамНаСервере(Результат, ДопПараметры.ИмяТЧ, ДопПараметры.Заявка, ДопПараметры.ОднаСтрока);
	Иначе
		ЗаполнитьТЧПоУправленческимАналитикамНаСервере(Результат, ДопПараметры.ИмяТЧ, ДопПараметры.Заявка);
	КонецЕсли;
	//ОКЕЙ Василенко В.В. (СофтЛаб) Конец 2021-10-28 (#4415)
...
&НаСервере
Процедура ЗаполнитьТЧПоУправленческимАналитикамНаСервере(АдресДанныхЗаполнения, ИмяТЧ, Заявка, ОднаСтрока = Ложь)
//ОКЕЙ Василенко В.В. (СофтЛаб) Конец 2021-10-28 (#4415)
	...
	//ОКЕЙ Василенко В.В. (СофтЛаб) Начало 2021-10-28 (#4415)
	//пИндекс = 0;
	Если ОднаСтрока И ЗначениеЗаполнено(Элементы[ИмяТЧ].ТекущаяСтрока) Тогда
		пИндекс = Объект[ИмяТЧ].НайтиПоИдентификатору(Элементы[ИмяТЧ].ТекущаяСтрока).НомерСтроки - 1;
	Иначе
		пИндекс = 0;
	КонецЕсли; 
	//ОКЕЙ Василенко В.В. (СофтЛаб) Конец 2021-10-28 (#4415)
...
// ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2021-10-27 (#4382)
#Область Принятие_к_учету_РБП

&НаСервере
Процедура ПроверитьСоответствиеРБПТаблицыУслуги(СообщитьОбОшибке, Отказ = Ложь, ТекНомерСтроки = 0)
	СчетаВидимостиКомандыСоздатьРБП = ЭтаФорма.СчетаВидимостиКомандыСоздатьРБП;
	МассивСообщенийОбОшибке = Новый Массив;
	
	Если ТекНомерСтроки = 0 Тогда 
		СтрокиДляПроверки = Объект.Услуги;
	Иначе
		СтрокиДляПроверки = Объект.Услуги.НайтиСтроки(Новый Структура("НомерСтроки", ТекНомерСтроки));	
	КонецЕсли;	
	
	Для Каждого СтрокаУслуги Из СтрокиДляПроверки Цикл
		СтрокаУслуги.НесоответствиеРБП = Ложь;
		Если СчетаВидимостиКомандыСоздатьРБП.НайтиПоЗначению(СтрокаУслуги.СчетЗатрат) = Неопределено
			Или Не ЗначениеЗаполнено(СтрокаУслуги.Субконто1)
			Или Не ТипЗнч(СтрокаУслуги.Субконто1) = Тип("СправочникСсылка.РасходыБудущихПериодов") Тогда 
			Продолжить;
		КонецЕсли;	
		ПроверитьСоответствиеРБПТаблицыУслугиПоСтроке(МассивСообщенийОбОшибке, СтрокаУслуги);
	КонецЦикла;
	Если СообщитьОбОшибке Тогда 
		Для Каждого ТекстСообщения Из МассивСообщенийОбОшибке Цикл
...
&НаСервере
Процедура ПроверитьСоответствиеРБПТаблицыУслугиПоСтроке(МассивСообщенийОбОшибке, СтрокаУслуги)
	ШаблонСообщенияОбОшибке = "В карточке РБП %1 неверно %2 %3. Строка №%4"; 
	
	РеквизитыРБП = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаУслуги.Субконто1, "Контрагент, ДоговорКонтрагента, Сумма, Ок_СтавкаНДС, Объект, ОК_Функция, СубконтоЗатрат1, СубконтоЗатрат2, СубконтоЗатрат3, ок_Проект");
	Если РеквизитыРБП.Контрагент <> Объект.Контрагент Тогда 
		ДобавитьСообщениеОНесоответствииВМассивСообщений(МассивСообщенийОбОшибке, СтрШаблон(ШаблонСообщенияОбОшибке, СтрокаУслуги.Субконто1, "указан", "контрагент", СтрокаУслуги.НомерСтроки), СтрокаУслуги);
	КонецЕсли;
	Если РеквизитыРБП.ДоговорКонтрагента <> Объект.ДоговорКонтрагента Тогда 
		ДобавитьСообщениеОНесоответствииВМассивСообщений(МассивСообщенийОбОшибке, СтрШаблон(ШаблонСообщенияОбОшибке, СтрокаУслуги.Субконто1, "указан", "договор контрагента", СтрокаУслуги.НомерСтроки), СтрокаУслуги);
	КонецЕсли;
	СуммаДляПроверки = ?(Объект.СуммаВключаетНДС, СтрокаУслуги.Сумма - СтрокаУслуги.СуммаНДС, СтрокаУслуги.Сумма);
	Если СуммаДляПроверки <> РеквизитыРБП.Сумма Тогда 
		ДобавитьСообщениеОНесоответствииВМассивСообщений(МассивСообщенийОбОшибке, СтрШаблон(ШаблонСообщенияОбОшибке, СтрокаУслуги.Субконто1, "указана", "сумма", СтрокаУслуги.НомерСтроки), СтрокаУслуги);
	КонецЕсли;	
	Если РеквизитыРБП.Ок_СтавкаНДС <> СтрокаУслуги.СтавкаНДС Тогда 
		ДобавитьСообщениеОНесоответствииВМассивСообщений(МассивСообщенийОбОшибке, СтрШаблон(ШаблонСообщенияОбОшибке, СтрокаУслуги.Субконто1, "указана", "ставка НДС", СтрокаУслуги.НомерСтроки), СтрокаУслуги);
	КонецЕсли;
	Если РеквизитыРБП.Объект <> СтрокаУслуги.ок_Аналитика_2 Тогда 
		ДобавитьСообщениеОНесоответствииВМассивСообщений(МассивСообщенийОбОшибке, СтрШаблон(ШаблонСообщенияОбОшибке, СтрокаУслуги.Субконто1, "указан", "объект", СтрокаУслуги.НомерСтроки), СтрокаУслуги);
	КонецЕсли;
	Если РеквизитыРБП.ОК_Функция <> СтрокаУслуги.ок_ЦФО Тогда 
		ДобавитьСообщениеОНесоответствииВМассивСообщений(МассивСообщенийОбОшибке, СтрШаблон(ШаблонСообщенияОбОшибке, СтрокаУслуги.Субконто1, "указана", "функция", СтрокаУслуги.НомерСтроки), СтрокаУслуги);
	КонецЕсли;
	Если ЗначениеЗаполнено(СтрокаУслуги.ок_СтатьяОборотов) Тогда 
		СтатьиРегл = РегистрыСведений.бит_СтатьиОборотов_СтатьиРегл.ПолучитьСписокСтатейРеглСоответствующихСтатьеОборотов(СтрокаУслуги.ок_СтатьяОборотов);
		СтатьяЗатратРБП = Неопределено;
		Если ТипЗнч(РеквизитыРБП.СубконтоЗатрат1) = Тип("СправочникСсылка.СтатьиЗатрат") Тогда 
			СтатьяЗатратРБП = РеквизитыРБП.СубконтоЗатрат1;
		ИначеЕсли ТипЗнч(РеквизитыРБП.СубконтоЗатрат2) = Тип("СправочникСсылка.СтатьиЗатрат") Тогда 
			СтатьяЗатратРБП = РеквизитыРБП.СубконтоЗатрат2;
		ИначеЕсли ТипЗнч(РеквизитыРБП.СубконтоЗатрат3) = Тип("СправочникСсылка.СтатьиЗатрат") Тогда 
			СтатьяЗатратРБП = РеквизитыРБП.СубконтоЗатрат3;
		КонецЕсли;	
		Если СтатьяЗатратРБП <> Неопределено
			И СтатьиРегл.НайтиПоЗначению(СтатьяЗатратРБП) = Неопределено Тогда 
			ДобавитьСообщениеОНесоответствииВМассивСообщений(МассивСообщенийОбОшибке, СтрШаблон(ШаблонСообщенияОбОшибке, СтрокаУслуги.Субконто1, "указана", "статья затрат", СтрокаУслуги.НомерСтроки), СтрокаУслуги);
		КонецЕсли;
	КонецЕсли;	
	Если РеквизитыРБП.ок_Проект <> СтрокаУслуги.ок_Проект Тогда 
		ДобавитьСообщениеОНесоответствииВМассивСообщений(МассивСообщенийОбОшибке, СтрШаблон(ШаблонСообщенияОбОшибке, СтрокаУслуги.Субконто1, "указан", "проект", СтрокаУслуги.НомерСтроки), СтрокаУслуги);
	КонецЕсли;
КонецПроцедуры	

&НаСервере
Процедура ДобавитьСообщениеОНесоответствииВМассивСообщений(МассивСообщенийОбОшибке, ТекстСообщенияОбОшибке, СтрокаУслуги)
	СтрокаУслуги.НесоответствиеРБП = Истина;
	МассивСообщенийОбОшибке.Добавить(ТекстСообщенияОбОшибке);
КонецПроцедуры	
	
&НаКлиенте
Процедура УстановитьДоступностьКомандыСоздатьПерезаполнитьРБП()
	ТекДанныеУслуги = Элементы.Услуги.ТекущиеДанные;
	Если ТекДанныеУслуги <> Неопределено Тогда 
		Если ЭтаФорма.СчетаВидимостиКомандыСоздатьРБП.НайтиПоЗначению(ТекДанныеУслуги.СчетЗатрат) <> Неопределено
			И Не ЗначениеЗаполнено(ТекДанныеУслуги.Субконто1) Тогда 	
	...
	Элементы.ок_ПерезаполнитьРБП.Доступность = Ложь;
	ВыделенныеСтроки = Элементы.Услуги.ВыделенныеСтроки;
	Для Каждого СтрокаУслуги Из ВыделенныеСтроки Цикл
		ТекДанныеУслуги = Объект.Услуги.НайтиПоИдентификатору(СтрокаУслуги);
		Если ТекДанныеУслуги <> Неопределено
			И ЭтаФорма.СчетаВидимостиКомандыСоздатьРБП.НайтиПоЗначению(ТекДанныеУслуги.СчетЗатрат) <> Неопределено
			И ЗначениеЗаполнено(ТекДанныеУслуги.Субконто1)
			И ТипЗнч(ТекДанныеУслуги.Субконто1) = Тип("СправочникСсылка.РасходыБудущихПериодов") Тогда 
			Элементы.ок_ПерезаполнитьРБП.Доступность = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры	
...
&НаКлиенте
Процедура ок_СоздатьРБПЗаписьЗавершение(Результат, ДопПараметры) Экспорт
	Если Результат <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли; 
...
&НаКлиенте
Процедура ок_СоздатьРБПОткрытьФорму()
	...
	ПутьКДаннымСтроки = СтрШаблон("Объект.Услуги[%1].",Элементы.Услуги.ТекущаяСтрока);
	
	Если Не ЗначениеЗаполнено(Объект.Контрагент) Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Укажите контрагента",, "Объект.Контрагент", , НайденыОшибки);	
	КонецЕсли;	
	Если Не ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Укажите договор контрагента", , "Объект.ДоговорКонтрагента", , НайденыОшибки);	
	КонецЕсли;	
	Если Не ЗначениеЗаполнено(ТекДанныеУслуги.Сумма) Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Укажите сумму", , ПутьКДаннымСтроки+"Сумма", , НайденыОшибки);	
	КонецЕсли;	
	Если Не ЗначениеЗаполнено(ТекДанныеУслуги.СтавкаНДС) Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Укажите ставку НДС", , ПутьКДаннымСтроки+"СтавкаНДС", , НайденыОшибки);	
	КонецЕсли;	
	Если Не ЗначениеЗаполнено(ТекДанныеУслуги.ок_Аналитика_2) Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Укажите объект", , ПутьКДаннымСтроки+"ок_Аналитика_2", , НайденыОшибки);	
	КонецЕсли;	
	Если Не ЗначениеЗаполнено(ТекДанныеУслуги.ок_ЦФО) Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Укажите ЦФО", , ПутьКДаннымСтроки+"ок_ЦФО", , НайденыОшибки);	
	КонецЕсли;	
	Если Не ЗначениеЗаполнено(ТекДанныеУслуги.ок_СтатьяОборотов) Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Укажите статью оборотов", , ПутьКДаннымСтроки+"ок_СтатьяОборотов", , НайденыОшибки);	
	КонецЕсли;	
	...
	СтруктураРеквизитов = ПолучитьЗначенияРеквизитовРБППриСозданииИзДокументаПТУ(Элементы.Услуги.ТекущиеДанные);
	СтруктураРеквизитов.Вставить("Событие",	"СозданиеРБПизПоступленияУслуг");
	ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ПослеЗаписиЭлементаРасходыБудущихПериодов", ЭтотОбъект); 
	ОткрытьФорму("Справочник.РасходыБудущихПериодов.Форма.ФормаЭлемента", Новый Структура("Основание", СтруктураРеквизитов), ЭтаФорма,,,,ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
	
&НаКлиенте
Процедура ПослеЗаписиЭлементаРасходыБудущихПериодов(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	Если ТипЗнч(РезультатЗакрытия) = Тип("СправочникСсылка.РасходыБудущихПериодов") Тогда 
		Элементы.Услуги.ТекущиеДанные.Субконто1 = РезультатЗакрытия;
		Элементы.Услуги.ТекущиеДанные.СубконтоНУ1 = РезультатЗакрытия;
		Модифицированность = Истина;
		ПроверитьСоответствиеРБПТаблицыУслуги(Ложь, , Элементы.Услуги.ТекущиеДанные.НомерСтроки); 
		УстановитьДоступностьКомандыСоздатьПерезаполнитьРБП();
	КонецЕсли;	
...
&НаКлиенте
Функция ПолучитьЗначенияРеквизитовРБППриСозданииИзДокументаПТУ(ТекСтрокаУслуги)
	СтруктураРеквизитов = Новый Структура;
	Если Объект.СуммаВключаетНДС Тогда 
		СтруктураРеквизитов.Вставить("Сумма", ТекСтрокаУслуги.Сумма - ТекСтрокаУслуги.СуммаНДС);
	Иначе
		СтруктураРеквизитов.Вставить("Сумма", ТекСтрокаУслуги.Сумма);
	КонецЕсли;	
	СтруктураРеквизитов.Вставить("Ок_СтавкаНДС",		ТекСтрокаУслуги.СтавкаНДС);
	СтруктураРеквизитов.Вставить("Объект",				ТекСтрокаУслуги.ок_Аналитика_2);
	...
	СтруктураРеквизитов.Вставить("ок_Проект",			ТекСтрокаУслуги.ок_Проект);
	СтруктураРеквизитов.Вставить("ок_СтатьяОборотов",	ТекСтрокаУслуги.ок_СтатьяОборотов);
	СтруктураРеквизитов.Вставить("СубконтоЗатрат2",		ТекСтрокаУслуги.ок_Аналитика_2);
	СтруктураРеквизитов.Вставить("СубконтоЗатрат3",		ТекСтрокаУслуги.ок_ЦФО);
	СтруктураРеквизитов.Вставить("ОК_Функция",			ТекСтрокаУслуги.ок_ЦФО);
	
	ПолучитьЗначенияРеквизитовРБППриСозданииИзДокументаПТУНаСервере(СтруктураРеквизитов);
	Возврат СтруктураРеквизитов;
КонецФункции	

&НаСервере
Процедура ПолучитьЗначенияРеквизитовРБППриСозданииИзДокументаПТУНаСервере(СтруктураРеквизитов)
	СтруктураРеквизитов.Вставить("Родитель",				ОК_ОбщегоНазначения.ПолучитьЗначениеУниверсальнойНастройки("РБП","ГруппаВерхнегоУровняРБП"));
	СтруктураРеквизитов.Вставить("ПринадлежностьРасчетов",	ОК_ОбщегоНазначения.ПолучитьЗначениеУниверсальнойНастройки("РБП","ПринадлежностьРасчетовРБП"));
	СтруктураРеквизитов.Вставить("СчетЗатрат",				ОК_ОбщегоНазначения.ПолучитьЗначениеУниверсальнойНастройки("РБП","СчетЗатратРБП"));
	
	СтатьиРегл = РегистрыСведений.бит_СтатьиОборотов_СтатьиРегл.ПолучитьСписокСтатейРеглСоответствующихСтатьеОборотов(СтруктураРеквизитов.ок_СтатьяОборотов);
	Если СтатьиРегл.Количество() > 0 Тогда 
		СтруктураРеквизитов.Вставить("СубконтоЗатрат1", СтатьиРегл.Получить(0).Значение);
	КонецЕсли;	
	СтруктураРеквизитов.Удалить("ок_СтатьяОборотов");
КонецПроцедуры	

&НаСервере
Процедура ПерезаполнитьРБПНаСервере(МассивРБП)
	
	Для Каждого СтрокаРБП Из МассивРБП Цикл
		
		РеквизитыРБПТекущие = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаРБП.РБП, "Контрагент, ДоговорКонтрагента, Сумма, Ок_СтавкаНДС, Объект, СубконтоЗатрат1, СубконтоЗатрат3, ок_Проект, ОК_Функция");
		НужныИзменения = Ложь;
		
		Для Каждого ЭлСтруктуры Из РеквизитыРБПТекущие Цикл
			
			Если СтрокаРБП.РеквизитыРБП[ЭлСтруктуры.Ключ] <> ЭлСтруктуры.Значение Тогда 
		...
		Если НужныИзменения Тогда
			...
			РБПОбъект = СтрокаРБП.РБП.ПолучитьОбъект();
			
			Для Каждого ЭлСтруктуры Из СтрокаРБП.РеквизитыРБП Цикл
				РБПОбъект[ЭлСтруктуры.Ключ] = ЭлСтруктуры.Значение;
			КонецЦикла;
			
			Если РБПОбъект.СубконтоЗатрат1 <> РеквизитыРБПТекущие.СубконтоЗатрат1 Тогда 
				РБПОбъект.ПерезаполнитьСвязанныеРеквизитыПоСтатьеЗатрат(Новый Структура("Событие", "СозданиеРБПизПоступленияУслуг"));	
			КонецЕсли;
			
			РБПОбъект.Записать();
			
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон("Строка %1. Перезаполнено %2",СтрокаРБП.НомерСтроки, СтрокаРБП.РБП), Объект.Ссылка);
			Объект.Услуги.Получить(СтрокаРБП.НомерСтроки-1).НесоответствиеРБП = Ложь;
			
		Иначе
			
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон("Строка %1. Перезаполнение не требуется %2",СтрокаРБП.НомерСтроки, СтрокаРБП.РБП), Объект.Ссылка);
