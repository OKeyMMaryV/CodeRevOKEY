...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СБ_КазначействоСервер.СписаниеСРасчетногоСчетаФормаДокументаПриОткрытии(ЭтаФорма);
	// -СБ. Широков Николай 	

	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2019-02-27 (#3240)
	ЭлементыСтатейДляПереименования = "СтатьяДвиженияДенежныхСредств,СтатьяДвиженияДенежныхСредствДепонентыВедомость,
									  |СтатьяДвиженияДенежныхСредствДивиденды,СтатьяДвиженияДенежныхСредствЗарплатаПоВедомостям,
									  |СтатьяДвиженияДенежныхСредствОднаНаСтранице,СтатьяДвиженияДенежныхСредствПрочие,
									  |СтатьяДвиженияДенежныхСредствСнятиеНаличных,СтатьяДвиженияДенежныхСредствЗарплатаРаботнику";
	
	мЭлементовПереименования = СтрРазделить(СтрЗаменить(ЭлементыСтатейДляПереименования, Символы.ПС, ""),",",Ложь);
	
	Для каждого пИмяЭлФормы Из мЭлементовПереименования Цикл
	
		пЭлФормы = Элементы.Найти(пИмяЭлФормы);
		Если пЭлФормы = Неопределено Тогда 
			Продолжить;
		КонецЕсли;
		пЭлФормы.Заголовок = НСтр("ru = 'Статья ДДС'");
	
	КонецЦикла; 
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2019-02-27 (#3240)
...
Процедура ПриОткрытии(Отказ)
	...
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2019-10-05 (#3491)
	УправлениеФормой();
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2019-10-05 (#3491)
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//СБ Ванюков К. + //Изменения из 1.6.9.4.		
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеПодотчетномуЛицу") тогда
		Если Объект.СуммаДокумента<>Объект.ПодотчетныеЛица.Итог("Сумма") тогда //Светов - начало
			Сообщить("Сумма платежки не совпадает с суммой по подотчетникам, документ не проведется!");
			Отказ=Истина;
		КонецЕсли;	       ///Светов - окончание
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеЗП") тогда
		Если Объект.СуммаДокумента<>Объект.ПеречислениеЗаработнойПлаты.Итог("СуммаПлатежа") тогда //Светов - начало
			Сообщить("Не совпадают сумма документа и ее расшифровка!");
			Отказ=Истина;
		КонецЕсли;	       ///Светов - окончание
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеДепонентов") тогда
		Если Объект.СуммаДокумента<>Объект.ПеречислениеДепонентов.Итог("СуммаПлатежа") тогда //Светов - начало
			Сообщить("Не совпадают сумма документа и ее расшифровка!");
			Отказ=Истина;
		КонецЕсли;	       ///Светов - окончание
	Иначе
		Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСписаниеДенежныхСредств.ОплатаПоставщику")
			...
			ИЛИ Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПрочиеРасчетыСКонтрагентами") тогда
	...
	Если Не Отказ 
		И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ПлатежноеПоручение")
		И Объект.СуммаДокумента <> ОК_ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Объект.ДокументОснование,"СуммаДокумента").СуммаДокумента Тогда
		Ответ = Вопрос("Сумма в СсРС не совпадает с суммой в ПП. Продолжить?",РежимДиалогаВопрос.ДаНет);
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-10-11 (#3066)	
	//Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
	//	КонтрагентПриИзмененииСервер();
	//КонецЕсли;	
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-10-11 (#3066)
...
Процедура УправлениеФормой()
	...
	Элементы.ок_ПоИсполнительнымЛистам.ТолькоПросмотр = ДокументСвязанСЗнРДС(Объект.Ссылка);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ДокументСвязанСЗнРДС(Документ)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		//1С-ИжТиСи, Кондратьев, 03.2020, обновление (
		//"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		//|	СписаниеСРасчетногоСчетаРасшифровкаПлатежа.бит_ПлатежнаяПозиция.ДокументОснование КАК ЗнРДС
		//|ИЗ
		//|	Документ.СписаниеСРасчетногоСчета.РасшифровкаПлатежа КАК СписаниеСРасчетногоСчетаРасшифровкаПлатежа
		//|ГДЕ
		//|	СписаниеСРасчетногоСчетаРасшифровкаПлатежа.Ссылка = &Документ
		//|	И ЕСТЬNULL(СписаниеСРасчетногоСчетаРасшифровкаПлатежа.бит_ПлатежнаяПозиция.Ссылка, НЕОПРЕДЕЛЕНО) <> НЕОПРЕДЕЛЕНО
		//|	И ЕСТЬNULL(СписаниеСРасчетногоСчетаРасшифровкаПлатежа.бит_ПлатежнаяПозиция.ДокументОснование, НЕОПРЕДЕЛЕНО) <> НЕОПРЕДЕЛЕНО
		//|	И СписаниеСРасчетногоСчетаРасшифровкаПлатежа.бит_ПлатежнаяПозиция.ДокументОснование.Проведен
		//|	И НЕ СписаниеСРасчетногоСчетаРасшифровкаПлатежа.бит_ПлатежнаяПозиция.ДокументОснование.ПометкаУдаления";
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СписаниеСРасчетногоСчетаРасшифровкаПлатежа.ДокументПланирования.ДокументОснование КАК ЗнРДС
		|ИЗ
		|	Документ.СписаниеСРасчетногоСчета.бит_РаспределениеБюджета КАК СписаниеСРасчетногоСчетаРасшифровкаПлатежа
		|ГДЕ
		|	СписаниеСРасчетногоСчетаРасшифровкаПлатежа.Ссылка = &Документ
		|	И ЕСТЬNULL(СписаниеСРасчетногоСчетаРасшифровкаПлатежа.ДокументПланирования.Ссылка, НЕОПРЕДЕЛЕНО) <> НЕОПРЕДЕЛЕНО
		|	И ЕСТЬNULL(СписаниеСРасчетногоСчетаРасшифровкаПлатежа.ДокументПланирования.ДокументОснование, НЕОПРЕДЕЛЕНО) <> НЕОПРЕДЕЛЕНО
		|	И СписаниеСРасчетногоСчетаРасшифровкаПлатежа.ДокументПланирования.ДокументОснование.Проведен
		|	И НЕ СписаниеСРасчетногоСчетаРасшифровкаПлатежа.ДокументПланирования.ДокументОснование.ПометкаУдаления";
	    //1С-ИжТиСи, Кондратьев, 03.2020, обновление )
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ЗнРДС) Тогда
...
//ОК Ванюков К. +// Перенос настроек // 2012-04-13
&НаСервере
Процедура ПересчитатьСуммуДокументаПодотчетников()
	Объект.СуммаДокумента = Объект.ПодотчетныеЛица.Итог("Сумма");
КонецПроцедуры	

&НаКлиенте
Процедура ПодотчетныеЛицаПослеУдаления(Элемент)
	ПересчитатьСуммуДокументаПодотчетников();
КонецПроцедуры

&НаКлиенте
Процедура ПодотчетныеЛицаПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	ПересчитатьСуммуДокументаПодотчетников();
КонецПроцедуры
...
Процедура СБ_ПрофильРазноскиОбработатьИзменение() Экспорт
	...
	Для каждого СтрокаПлатеж Из Объект.РасшифровкаПлатежа Цикл
		...
		Если СписокОрганизаций_НеПогашатьЗадолженность <> Неопределено 
			И СписокОрганизаций_НеПогашатьЗадолженность.НайтиПоЗначению(Объект.Организация) <> Неопределено Тогда
...
//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-08-20 (#3066)
...
Процедура РасшифровкаПлатежаок_СтатьяОборотовБДРПриИзменении(Элемент)
	...
	//КонецЕсли; 	
	НайденныеСтроки = Объект.бит_РаспределениеБюджета.НайтиСтроки(Новый Структура("бит_КлючСтроки", ТекущиеДанные.бит_КлючСтроки));
	Для Каждого СтрокаТЧ ИЗ НайденныеСтроки Цикл
		СтрокаТЧ.СтатьяОборотов = ПолучитьСоответствующуюСтатьюБДДС(ТекущиеДанные.Ок_СтатьяОборотовБДР);
	КонецЦикла;		
	Если НайденныеСтроки.Количество() > 0 Тогда
		МассивСтатей = бит_Бюджетирование.СвязанныеСтатьиОборотовИСтатьиРегл(НайденныеСтроки[0].СтатьяОборотов, "СправочникСсылка.СтатьиДвиженияДенежныхСредств");
		Если МассивСтатей.Количество() > 0 Тогда  		
			ТекущиеДанные.СтатьяДвиженияДенежныхСредств = МассивСтатей[0];		
		КонецЕсли	
	КонецЕсли;	
...
КонецФункции
//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-08-20 (#3066)
