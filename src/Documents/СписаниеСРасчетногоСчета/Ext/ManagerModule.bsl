...
// +СБ. Широков Николай. 2016-05-30. #2489
...
Функция КонтрагентДолженБытьЗаполнен(ВидОперации) Экспорт
	...
	Возврат ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ОплатаПоставщику
		...
		//++ СВВ {[+](фрагмент добавлен), Сапожников Вадим 07.08.2015 14:05:38
		//Или ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеПодотчетномуЛицу
		//-- СВВ}Сапожников Вадим 07.08.2015 14:05:38		
...
Функция ПодготовитьСообщенияПроверкиЗаполненияРасшифровкаПлатежа(Объект, ФормаРасшифровкаПлатежа = Неопределено, Отказ, ПроверяемыеРеквизиты, ПроверятьСчетаУчета = Истина) Экспорт
	...
	СписокВидовСРасшифровкойПлатежаБезПроверкиЗаполнения = Новый СписокЗначений();
	СписокВидовСРасшифровкойПлатежаБезПроверкиЗаполнения.Добавить(Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога);
	
	Если СписокВидовСРасшифровкойПлатежаБезПроверкиЗаполнения.НайтиПоЗначению(ВидОперации) = Неопределено
		И ЕстьРасшифровкаПлатежа(ВидОперации) Тогда
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2021-09-09 (#4338)
...
Функция ТекстЗапросаТаблицыДокумента(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	//Заменено на:
	|	СУММА(ТаблицаРасшифровкаПлатежа.СуммаНДС) КАК СуммаНДС,
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-10-08 (#3066)
	...
	//Заменено на:
	|	СУММА(ТаблицаРасшифровкаПлатежа.СуммаПлатежа) КАК СуммаПлатежа,
	|	СУММА(ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.СуммаПлатежа * &КоэффициентРуб КАК ЧИСЛО(15, 2))) КАК СуммаРуб,
	|	СУММА(ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.СуммаНДС * &КоэффициентРуб КАК ЧИСЛО(15, 2))) КАК СуммаНДСРуб,
	...
	|	ТаблицаРасшифровкаПлатежа.СпособПогашенияЗадолженности,	
	|	ТаблицаРасшифровкаПлатежа.СчетНаОплату,
	|	ТаблицаРасшифровкаПлатежа.СтавкаНДС,
	|	ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента,
	|	ТаблицаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств,
	|	ТаблицаРасшифровкаПлатежа.Сделка,
	|	ТаблицаРасшифровкаПлатежа.СчетУчетаРасчетовСКонтрагентом,
	|	ТаблицаРасшифровкаПлатежа.СчетУчетаРасчетовПоАвансам,
	|	ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента.ВалютаВзаиморасчетов,
	|	ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента.РасчетыВУсловныхЕдиницах,
	|	ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента.ВидДоговора,
	|	ТаблицаРасшифровкаПлатежа.КурсВзаиморасчетов,
	|	ТаблицаРасшифровкаПлатежа.КратностьВзаиморасчетов,
	|	ВЫБОР
	|		КОГДА ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента.ВалютаВзаиморасчетов = &ВалютаРеглУчета
...
Функция ТекстСодержанияПроводокДокумента(Реквизиты)
	...
	Содержание = СокрЛП(СтрЗаменить(ТекстНазначение, Символы.ПС, " "))
		...
		//СБ Ванюков К.+
		Если Реквизиты.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПрочиеРасчетыСКонтрагентами Тогда
			Содержание = "Расчеты с контрагентом";
		ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВозвратПокупателю Тогда
			Содержание = "Расчеты с контрагентом";
		ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ОплатаПоставщику Тогда
			Содержание = "Расчеты с контрагентом";
		ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.РасчетыПоКредитамИЗаймам тогда
			Содержание = "Расчеты по кредитам и займам";
		ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога тогда
			Содержание = "Перечисление налога";
		ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПрочееСписание тогда
			Содержание = "Прочее списание денежных средств";
		ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеПодотчетномуЛицу Тогда
			Содержание = "Перечисление денежных средств подотчетнику";
		ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПереводНаДругойСчет Тогда
			Содержание = "Перевод средств на другой счет организации";
		КонецЕсли; 
		//СБ Ванюков К.-
...
//ОКЕЙ Михайлова М.В.(ПервыйБИТ) Начало 2021-07-15 (#ФР-МП-11)

// Изменен реквизит Сделка в ТЧ РасшифровкаПлатежа - расширен тип на документы 
// - бит_ок_ОперацияАксапты, 
// - бит_ок_ОперацияАксапты12

//ОКЕЙ Михайлова М.В.(ПервыйБИТ) Конец  2021-07-15 (#ФР-МП-11)

//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Начало 2021-09-07 (#ТП_БП05_ФР12)

// Изменен реквизит ДокументОснование - расширен тип на документ бит_ПлатежнаяПозиция
// Добавлены движения по РегистрНакопления.ок_ВзаиморасчетыСПодотчетнымиЛицами

//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Конец 2021-09-07 (#ТП_БП05_ФР12)