...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ОКЕЙ Беднягин Е.А. (ПервыйБИТ) Начало 2021-04-20 (#ТП_БП11_ФР05) (#ТП_БП05_ФР15 - Завьялова)
	ок_МодификацияКонфигурацииПереопределяемый.ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка);
	//ОКЕЙ Беднягин Е.А. (ПервыйБИТ) Конец 2021-04-20 (#ТП_БП11_ФР05) (#ТП_БП05_ФР15 - Завьялова)	
...
Процедура СчетУчетаЗадержкаЗарплатыПриИзменении(Элемент)
	...
	//ОКЕЙ Беднягин Е.А. (ПервыйБИТ) Начало 2021-04-23 (#ТП_БП11_ФР05)
	ок_ЗаполнитьОбъектПоДоговоруИзСубконто(Элемент);
	//ОКЕЙ Беднягин Е.А. (ПервыйБИТ) Конец 2021-04-23 (#ТП_БП11_ФР05)	
...
Процедура РасшифровкаПлатежаДоговорКонтрагентаПриИзмененииНаСервере(ИдСтроки)
	...
	Если СтрокаПлатеж.НомерСтроки = 1 Тогда
		Если ЗначениеЗаполнено(СтрокаПлатеж.ДоговорКонтрагента) Тогда 
			Объект.ок_Объект = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПлатеж.ДоговорКонтрагента, "Объект");
		Иначе
			Объект.ок_Объект = Неопределено;
		КонецЕсли;
	КонецЕсли;
...
//ОКЕЙ Беднягин Е.А. (ПервыйБИТ) Начало 2021-04-23 (#ТП_БП11_ФР05)
&НаКлиенте
Процедура ок_ЗаполнитьОбъектПоДоговоруИзСубконто(Элемент)
	
	СчетаУчета = Новый Массив;
	СчетаУчета.Добавить(ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.РасчетыСПокупателями"));
	СчетаУчета.Добавить(ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.РасчетыПоАвансамПолученным"));
	СчетаУчета.Добавить(ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.РасчетыСПрочимиПокупателямиИЗаказчиками"));
	СчетаУчета.Добавить(ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПрочиеРасчетыСРазнымиДебиторамиИКредиторами"));
	
	Если СчетаУчета.Найти(Объект.СчетУчетаРасчетовСКонтрагентом) = Неопределено Тогда		
		Объект.ок_Объект = Неопределено;
		Возврат;
	ИначеЕсли Элемент = Элементы.СчетУчета И ЗначениеЗаполнено(Объект.ок_Объект) Тогда 
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.СубконтоДт2) Тогда 
		ок_ЗаполнитьОбъектПоДоговоруИзСубконтоНаСервере(Объект.СубконтоДт2);
	Иначе
		Объект.ок_Объект = Неопределено;
	КонецЕсли;
...
&НаСервере
Процедура ок_ЗаполнитьОбъектПоДоговоруИзСубконтоНаСервере(Договор)
	
	Объект.ок_Объект = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "Объект");
...
//ОКЕЙ Завьялова Е.А. (ПервыйБИТ) Начало 2021-05-12 (#ТП_БП05_ФР15)
&НаКлиенте
Процедура Подключаемый_КонтрагентНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	Если КонтрагентЮрФизЛицо = ПредопределенноеЗначение("Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо") Тогда
		СтандартнаяОбработка = Ложь;
