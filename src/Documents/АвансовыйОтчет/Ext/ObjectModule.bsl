...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// Убираем условие, т.к по #ТП_БП05_ФР12, #ТП_БП05_ФР13 требуются суточные
...
//Ванюков К. + // Проверка заполненности ОК_РазнесениеОборотов при проведении документов// 2012-02-13
	ОК_ОбщегоНазначения.ПроверитьЗаполнениеОК_РазнесениеОборотов(ЭтотОбъект, РежимЗаписи, Отказ);
//Ванюков К. -
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	пИменаНеобходимыхТаблиц = "Товаров,Прочее";
	мИменаТаблиц = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(пИменаНеобходимыхТаблиц,",");
	
	пИменаКолонокТаблиц = "Период,НомерСтроки,ЦФО,СтатьяОборотов,ВидСтатьи,Проект,Аналитика_2,СуммаБезНДСРуб,СуммаНДСРуб";
	...
	Для Каждого пИмяТаблицы Из мИменаТаблиц Цикл 
		...
		Если ПараметрыПроведения.Свойство("Поступление" + пИмяТаблицы + "Таблица") Тогда 
			Если ПараметрыПроведения["Поступление" + пИмяТаблицы + "Таблица"]<>Неопределено  
					И ПараметрыПроведения["Поступление" + пИмяТаблицы + "Таблица"].Количество()>0
				Тогда 
				Если ПараметрыПроведения["Поступление" + пИмяТаблицы + "Таблица"].Колонки.Найти("ОК_ID_Разноска")<>Неопределено Тогда 
				...
				КонецЕсли;
				пИмяКолонкиСчета = "";
				Если ПараметрыПроведения["Поступление" + пИмяТаблицы + "Таблица"].Колонки.Найти("СчетЗатрат") <> Неопределено Тогда 
					пИмяКолонкиСчета = "СчетЗатрат"; 
				ИначеЕсли ПараметрыПроведения["Поступление" + пИмяТаблицы + "Таблица"].Колонки.Найти("СчетУчета") <> Неопределено Тогда 
					пИмяКолонкиСчета = "СчетУчета"; 
				ИначеЕсли ПараметрыПроведения["Поступление" + пИмяТаблицы + "Таблица"].Колонки.Найти("СчетРасчетов") <> Неопределено Тогда 
					пИмяКолонкиСчета = "СчетРасчетов"; 					
				КонецЕсли;
				
				пКопияТаблицы = ПараметрыПроведения["Поступление" + пИмяТаблицы + "Таблица"].Скопировать(,пИменаКолонокТаблиц + ?(ПустаяСтрока(пИмяКолонкиСчета),"",","+пИмяКолонкиСчета));
	...
	Если стрТаблицыДляПроведенияПоБК.Количество()>0 Тогда 
		Если НЕ пРасчетыВВалюте Тогда 
			Для Каждого КлючЗнчТаб Из стрТаблицыДляПроведенияПоБК Цикл 
				...
				пТаблица.Колонки.Добавить("СуммаРуб",               Новый ОписаниеТипов("Число"));
				пТаблица.Колонки.Добавить("Ссылка",                 Новый ОписаниеТипов("ДокументСсылка.АвансовыйОтчет"));
				Для Каждого СтрТаб Из пТаблица Цикл 
					СтрТаб.СуммаВзаиморасчетов	  = СтрТаб.СуммаБезНДСРуб;
					СтрТаб.СуммаРуб          	  = СтрТаб.СуммаБезНДСРуб;
					СтрТаб.СуммаНДСВзаиморасчетов = СтрТаб.СуммаНДСРуб;
					СтрТаб.Ссылка                 = Ссылка;
				КонецЦикла;
			КонецЦикла;
		Иначе
			Для Каждого КлючЗнчТаб Из стрТаблицыДляПроведенияПоБК Цикл 
...
//Ванюков К. + //При копировании документа ОК_ID_Разноска и ОК_РазнесениеОборотов не должены копироваться //2012-02-13
	ОК_ID_Разноска = "";
