...
Функция ПолучитьСуммуВыданныхАвансов(Ссылка) Экспорт
	...
	//ОКЕЙ Завьялова Е.А. (ПервыйБИТ) Начало 2021-08-27 (#ТП_БП05_ФР12,#ТП_БП05_ФР13)
	// Закомментировано - сумма аванса определяется как сумма значений реквизита ок_СуммаАванса ТЧ ВыданныеАвансы
	//Запрос.Текст =
	//"ВЫБРАТЬ
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СУММА(АвансовыйОтчетВыданныеАвансы.ок_СуммаАванса) КАК ДокументАвансаСуммаДокумента
	|ИЗ
	|	Документ.АвансовыйОтчет.ВыданныеАвансы КАК АвансовыйОтчетВыданныеАвансы
	|ГДЕ
	|	АвансовыйОтчетВыданныеАвансы.Ссылка = &Ссылка";
	//ОКЕЙ Завьялова Е.А. (ПервыйБИТ) Конец 2021-08-27 (#ТП_БП05_ФР12,#ТП_БП05_ФР13)
...
Функция ПараметрыЗаполненияПечатнойФормыАвансовыйОтчет(ДанныеДокумента, ДанныеВалюты = Неопределено) Экспорт
	...
	Если ДанныеВалюты = Неопределено Тогда
		ДляРасчета = ДанныеДокумента.СуммаДокумента;
	Иначе 
		ДляРасчета = ДанныеДокумента.СуммаДокумента * ДанныеВалюты.Курс / ДанныеВалюты.Кратность;
	КонецЕсли;
...
Функция ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц, Реквизиты)
	...
	Если Реквизиты.ЕстьПрочее Тогда
		...
		|	Реквизиты.ФизЛицо КАК РеквизитыФизЛицо
		//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Начало 2021-06-01 (#ТП_БП05_ФР12, #ТП_БП05_ФР13)
		|	,ТаблицаПрочее.ок_Период КАК Период,		
		|	ТаблицаПрочее.ок_СтатьяОборотов КАК СтатьяОборотов, 
		|	ТаблицаПрочее.ок_СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
		|	ТаблицаПрочее.ок_ЦФО КАК ЦФО,
		|	"""" КАК ОК_ID_Разноска, 
		|	ТаблицаПрочее.ок_Проект КАК Проект,
		|	ТаблицаПрочее.ок_Аналитика_2 КАК Аналитика_2
		//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Конец 2021-06-01 (#ТП_БП05_ФР12, #ТП_БП05_ФР13)
...
Функция ТекстЗапросаРасчетыВВалютеПрочее()
	...
	//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Начало 2021-06-01 (#ТП_БП05_ФР12, #ТП_БП05_ФР13)
	|	,ВременнаяТаблицаПрочее.Период КАК Период,		
	|	ВременнаяТаблицаПрочее.СтатьяОборотов КАК СтатьяОборотов, 
	|	ВременнаяТаблицаПрочее.СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
	|	ВременнаяТаблицаПрочее.ЦФО КАК ЦФО,
	|	"""" КАК ОК_ID_Разноска, 
	|	ВременнаяТаблицаПрочее.Проект КАК Проект,
	|	ВременнаяТаблицаПрочее.Аналитика_2 КАК Аналитика_2
	//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Конец 2021-06-01 (#ТП_БП05_ФР12, #ТП_БП05_ФР13)
	|ПОМЕСТИТЬ ТаблицаПрочее
	|ИЗ
...
Функция ТекстЗапросаПоступлениеТоваров(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ТаблицаТовары.СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
...
Функция ТекстЗапросаПоступлениеПрочее(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Начало 2021-06-01 (#ТП_БП05_ФР12, #ТП_БП05_ФР13)
	|	,ТаблицаПрочее.СуммаНДСВзаиморасчетов
	|	,ТаблицаПрочее.Период КАК Период,		
	|	ТаблицаПрочее.СтатьяОборотов КАК СтатьяОборотов, 
	|	ТаблицаПрочее.СтатьяОборотов.ВидСтатьи КАК ВидСтатьи,
	|	ТаблицаПрочее.ЦФО КАК ЦФО,
	|	"""" КАК ОК_ID_Разноска, 
	|	ТаблицаПрочее.Проект КАК Проект,
	|	ТаблицаПрочее.Аналитика_2 КАК Аналитика_2
	//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Конец 2021-06-01 (#ТП_БП05_ФР12, #ТП_БП05_ФР13)
	|ИЗ
	|	ТаблицаПрочее КАК ТаблицаПрочее
	|
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Начало 2021-05-26 (#ТП_БП05_ФР12, #ТП_БП05_ФР13) 	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор  = "Аванс_ОтчетСОстатком";
	КомандаПечати.МенеджерПечати = "Обработка.ок_ПечатьАвансовогоОтчета";
	КомандаПечати.Представление  = НСтр("ru = 'АО с учетом остатка'");
	КомандаПечати.Порядок        = 11;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор  = "Аванс_ОтчетБезОстатка";
	КомандаПечати.МенеджерПечати = "Обработка.ок_ПечатьАвансовогоОтчета";
	КомандаПечати.Представление  = НСтр("ru = 'АО без учета остатка'");
	КомандаПечати.Порядок        = 12;    
	//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Конец 2021-05-26 (#ТП_БП05_ФР12, #ТП_БП05_ФР13)
КонецПроцедуры
...
Функция ПечатьАвансовогоОтчета(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		ОбщиеПараметры = ПараметрыЗаполненияПечатнойФормыАвансовыйОтчет(СтруктураДанных, ДанныеВал);
...
//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Начало 2021-07-16 (#ТП_БП05_ФР12, #ТП_БП05_ФР13)

// Изменен реквизит НазначениеАванса - изменен тип на СправочникСсылка.ок_НазначениеАвансовыхОтчетов
// Изменен реквизит ВидДокВходящий в ТЧ прочее - изменен тип на СправочникСсылка.ок_ВидыРасходовАвансовогоОтчета 
// Изменена периодичность нумерации - в пределах дня

//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Конец 2021-07-16 (#ТП_БП05_ФР12, #ТП_БП05_ФР13)

//ОКЕЙ Завьялова Е.А. (ПервыйБИТ) Начало 2021-10-08 (#ТП_БП05_ФР06)

// Добавлены движения по РН ок_ВзаиморасчетыСПодотчетнымиЛицами

//ОКЕЙ Завьялова Е.А. (ПервыйБИТ) Конец 2021-10-08 (#ТП_БП05_ФР06)