...
Процедура МПЗОтгруженныеНоменклатураПриИзменении(Элемент)
	...
	СтрокаТаблицы = Элементы.МПЗОтгруженные.ТекущиеДанные;
	Если Не (СтрокаТаблицы.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ГотоваяПродукцияОтгруженная")
		Или СтрокаТаблицы.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости")) Тогда
		
		РеквизитыПрослеживаемости = Новый Структура("СтранаПроисхождения, ПрослеживаемыйТовар");
...
Процедура МПЗОтгруженныеВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "МПЗОтгруженныеРНПТ" Тогда
		...
		Если ТекущиеДанные.ПрослеживаемыйТовар
			И НЕ (ТекущиеДанные.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ГотоваяПродукцияОтгруженная")
			...
			ПрослеживаемостьФормыКлиентБП.ОткрытьФормуПодбораРНПТ(ЭтаФорма, "МПЗОтгруженные");
		КонецЕсли;
	КонецЕсли;
	...
	Если Поле.Имя = "МПЗОтгруженныеСтранаПроисхождения" Тогда
		...
		Если НЕ ТекущиеДанные.ПрослеживаемыйТовар 
			Или ТекущиеДанные.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ГотоваяПродукцияОтгруженная")
			Или ТекущиеДанные.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости")
			Или ТекущиеДанные.ДоговорПокупателяВидДоговора <> ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.СКомиссионером") Тогда
			СтандартнаяОбработка = Ложь;
		КонецЕсли;
...
Процедура ПолучитьРеквизитыПрослеживаемости(ПараметрыСтроки, ПараметрыОбъекта)
	...
	Если ПараметрыОбъекта.ВедетсяУчетПрослеживаемыхТоваров Тогда
		...
		ПараметрыОбъекта.Номенклатура, "СтранаПроисхождения, ПрослеживаемыйТовар");
