...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВедетсяУчетПрослеживаемыхТоваров И ОтражатьВБухгалтерскомУчете 
		И (РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.Товары 
...
Процедура ПроверитьЗаполнениеРеквизитовПоПрослеживаемости(ИмяТабличнойЧасти, Отказ)
	...
	Иначе
		ПредставлениеТабличнойЧасти = ЭтотОбъект.Метаданные().ТабличныеЧасти[ИмяТабличнойЧасти].Синоним;
	КонецЕсли;
	СчетаИсключенияПриПроверке = Новый Массив;
	СчетаИсключенияПриПроверке.Добавить(ПланыСчетов.Хозрасчетный.ГотоваяПродукцияОтгруженная);   // 45.02
	СчетаИсключенияПриПроверке.Добавить(ПланыСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости); // 45.04
	Для Каждого ТекущаяСтрока Из ЭтотОбъект[ИмяТабличнойЧасти] Цикл
		НужнаПроверка = Ложь;
		Если ИмяТабличнойЧасти = "МПЗОтгруженные" Тогда 
			Если ТекущаяСтрока.ПрослеживаемыйТовар
				...
				И СчетаИсключенияПриПроверке.Найти(ТекущаяСтрока.СчетУчета) = Неопределено Тогда
		...
		Если НужнаПроверка Тогда 
			Префикс = "" + ИмяТабличнойЧасти + "[" + Формат(ТекущаяСтрока.НомерСтроки - 1, "ЧН=0; ЧГ=") + "].";
			Если Не ЗначениеЗаполнено(ТекущаяСтрока.СтранаПроисхождения) Тогда
