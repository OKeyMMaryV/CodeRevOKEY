...
&НаКлиенте
Процедура ОК_ЗаполнитьДопАналитику(Команда)
	Если Объект.Проведен Тогда
		Ответ = Вопрос("Заполнение возможно только в непроведенном документе
	               |Отменить проведение документа?", РежимДиалогаВопрос.ОКОтмена);
	...
	ОК_ЗаполнитьДопАналитику_НаСервере();
КонецПроцедуры

&НаСервере
Процедура ОК_ЗаполнитьДопАналитику_НаСервере()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ХозрасчетныйОбороты.Субконто1 КАК Объект,
	               |	ХозрасчетныйОбороты.Подразделение,
	               |	ХозрасчетныйОбороты.Субконто2 КАК СтатьяЗатрат,
	               |	ХозрасчетныйОбороты.СуммаОборотДт - ХозрасчетныйОбороты.СуммаОборотКт КАК СуммаБУ,
	               |	ХозрасчетныйОбороты.СуммаНУОборотДт - ХозрасчетныйОбороты.СуммаНУОборотКт КАК СуммаНУ,
	               |	ХозрасчетныйОбороты.СуммаПРОборотДт - ХозрасчетныйОбороты.СуммаПРОборотКт КАК СуммаПР,
	               |	ХозрасчетныйОбороты.СуммаВРОборотДт - ХозрасчетныйОбороты.СуммаВРОборотКт КАК СуммаВР
	               |ИЗ
	               |	РегистрБухгалтерии.Хозрасчетный.Обороты(
	               |			,
	               |			&КонецПериода,
	               |			,
	               |			Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.СтроительствоОбъектовОсновныхСредств),
	               |			,
	               ...
	               |ГДЕ
	               |	ХозрасчетныйОбороты.СуммаОборотДт - ХозрасчетныйОбороты.СуммаОборотКт + ХозрасчетныйОбороты.СуммаНУОборотДт - ХозрасчетныйОбороты.СуммаНУОборотКт <> 0";
	
	Запрос.УстановитьПараметр("КонецПериода", 	Объект.Дата);
	Запрос.УстановитьПараметр("Организация", 	Объект.Организация);
	Запрос.УстановитьПараметр("Объект", 		Объект.ОбъектСтроительства);
	Запрос.УстановитьПараметр("Счет", 			Объект.СчетУчетаВнеоборотногоАктива);
	
	ТЗРезультат 								= Запрос.Выполнить().Выгрузить();
	Объект.Аналитика.Загрузить(ТЗРезультат);
	
	Если Объект.Аналитика.Количество() > 0 Тогда 
		Объект.ОК_ВРазрезеСтатейЗатрат = Истина;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// +СБ Кузнецова С. 2016-09-16 Redmine 2651
	НоваяСтраница = Элементы.Добавить("ГруппаСБ_ДокументыПоступления", Тип("ГруппаФормы"), Элементы.ГруппаСтраницы);
	НоваяСтраница.Вид = ВидГруппыФормы.Страница;
	НоваяСтраница.Заголовок = "Документы поступления";
	
	НовоеТаблПоле = Элементы.Добавить("СБ_ДокументыПоступления", Тип("ТаблицаФормы"), НоваяСтраница);
	НовоеТаблПоле.ПутьКДанным = "Объект.СБ_ДокументыПоступления";
	НовоеТаблПоле.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	
	Колонка1 = Элементы.Добавить("СБ_ДокументыПоступленияНомерСтроки", Тип("ПолеФормы"), НовоеТаблПоле);
	Колонка1.Вид = ВидПоляФормы.ПолеНадписи;
	Колонка1.ПутьКДанным = "Объект.СБ_ДокументыПоступления.НомерСтроки";
	
	Колонка2 = Элементы.Добавить("СБ_ДокументыПоступленияДокументПоступления", Тип("ПолеФормы"), НовоеТаблПоле);
	Колонка2.Вид = ВидПоляФормы.ПолеВвода;
	Колонка2.ПутьКДанным = "Объект.СБ_ДокументыПоступления.ДокументПоступления";
	
	Колонка3 = Элементы.Добавить("СБ_ДокументыПоступленияДатаДокументаПоступления", Тип("ПолеФормы"), НовоеТаблПоле);
	Колонка3.Вид = ВидПоляФормы.ПолеВвода;
	Колонка3.ПутьКДанным = "Объект.СБ_ДокументыПоступления.ДатаДокументаПоступления";
	
	Колонка4 = Элементы.Добавить("СБ_ДокументыПоступленияСумма", Тип("ПолеФормы"), НовоеТаблПоле);
	Колонка4.Вид = ВидПоляФормы.ПолеВвода;
	Колонка4.ПутьКДанным = "Объект.СБ_ДокументыПоступления.Сумма";
	// -СБ Кузнецова С.
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Справочник.НомераГТД.Форма.ФормаПодбораРНПТДляОС" Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//СБ Ванюков К. + //Проверка на заполненность реквизита нового ТЧ "объект" и запись его в элемент справочника ОС
	СчетчикСтрок = 0;
	Для Каждого СтрокаОС из Объект.ОС цикл
		СчетчикСтрок = СчетчикСтрок + 1;
		Если не ЗначениеЗаполнено(СтрокаОС.Объект) тогда
			Сообщить("Строка №" + СчетчикСтрок + ": Не заполнено значение реквизита Объект");
			Продолжить;
		КонецЕсли;
		
		текОбъект = ИнтеграцияС1СДокументооборотВызовСервера.ЗначениеРеквизитаОбъекта(СтрокаОС.ОсновноеСредство, "Объект");
		Если ЗначениеЗаполнено(текОбъект) тогда
			Если текОбъект = СтрокаОС.Объект тогда 
				Продолжить;
			Иначе
				Ответ =Вопрос("Для основного средства " + СтрокаОС.ОсновноеСредство + " установлен объект " + текОбъект + " заменить его на " + СтрокаОС.Объект, РежимДиалогаВопрос.ДаНет, 20, КодВозвратаДиалога.Нет, "Изменить объект основного средства?"); 
				Если Ответ = КодВозвратаДиалога.Да Тогда
					УстановитьОбъектОсновногоСредства(СтрокаОС.ОсновноеСредство, текОбъект, СчетчикСтрок);
				КонецЕсли;
			КонецЕсли;
		иначе
			УстановитьОбъектОсновногоСредства(СтрокаОС.ОсновноеСредство, текОбъект, СчетчикСтрок);
...
//СБ Ванюков К. + //Проверка на заполненность реквизита нового ТЧ "объект" и запись его в элемент справочника ОС
&НаСервере
Процедура УстановитьОбъектОсновногоСредства(текОС, текОбъект, СчетчикСтрок = 0)
	ОбъектОС = текОС.ПолучитьОбъект();
	ОбъектОС.Объект	= текОбъект;
	ОбъектОС.Записать();
	Сообщить("Строка №" + СчетчикСтрок + ": Для Основного средства " + текОС + " установлен реквизит Объект = " + текОбъект);
КонецПроцедуры
//СБ Ванюков К. - //Проверка на заполненность реквизита нового ТЧ "объект" и запись его в элемент справочника ОС
...
Процедура ОСОсновноеСредствоПриИзменении(Элемент)
	...
	Если ВедетсяУчетПрослеживаемыхТоваров Тогда
		ЭлементКоллекции.ПрослеживаемыйТовар = ПрослеживаемостьФормыВызовСервера.ПолучитьПризнакПрослеживаемости(ЭлементКоллекции.ОсновноеСредство);
	КонецЕсли;
	...
	Если Поле.Имя = "ОСРНПТ" Тогда
		...
		Если ТекущиеДанные.ПрослеживаемыйТовар Тогда
			...
			ИначеЕсли ПринятиеКУчетуПослеИнвентаризации И Не Объект.ОтражатьВПрослеживаемости Тогда
				СтандартнаяОбработка = Ложь;
			Иначе
				ПрослеживаемостьФормыКлиентБП.ОткрытьФормуПодбораРНПТ(ЭтаФорма, "ОС", Ложь);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура СпособыОтраженияРасходовПоАмортизацииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	//ОК Калинин М. 130313
	//Если ТипЗНЧ(ИнтеграцияЕГАИСВызовСервера.ЗначениеРеквизитаОбъекта(Объект.ПодразделениеОрганизации, "ОК_КонтрольОбъекта")) = Тип("Неопределено") Тогда
	//    Возврат;
	//КонецЕсли;
	//Если ИнтеграцияЕГАИСВызовСервера.ЗначениеРеквизитаОбъекта(Объект.ПодразделениеОрганизации, "ОК_КонтрольОбъекта") тогда
	//	//дополнительный фильтр в отборе открываемой формы (по первому объекту)
	//	Если Объект.ОС.Количество()>0 тогда
	//		бит_БК_ОбщийКлиент.НачалоВыбораЗначенияСпособовОтраженияРасходовПоАмортизации(Элемент, Объект.Организация, СтандартнаяОбработка, получитьСсылкуНаОбъект(), Объект.СпособыОтраженияРасходовПоАмортизации);
	//	конецесли;	
	//	возврат;
	//конецесли;
	//ОК Калинин М.
	
	РеквизитыПодразделения = РеквизитыПодразделения(Объект.ПодразделениеОрганизации, "ОК_КонтрольОбъекта, ОК_ОбъектСтроительства"); 
	Если РеквизитыПодразделения.ОК_КонтрольОбъекта = Истина И ЗначениеЗаполнено(РеквизитыПодразделения.ОК_ОбъектСтроительства) Тогда
		бит_БК_ОбщийКлиент.НачалоВыбораЗначенияСпособовОтраженияРасходовПоАмортизации(Элемент, Объект.Организация, СтандартнаяОбработка, РеквизитыПодразделения.ОК_ОбъектСтроительства, Объект.СпособыОтраженияРасходовПоАмортизации);	
	КонецЕсли;	
...
//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-10-22 (#4405)
&НаСервере
Функция ПоместитьВХранилище(ИдентификаторСтроки)
...
Процедура ОткрытьСписокРНПТДляРедактирования(ТекущаяСтрока)
	...
	АдресВременногоХранилища = ПоместитьВХранилище(ТекущаяСтрока.ИдентификаторСтроки);
	...
	ПараметрыОткрытияФормы.Сумма                    = Объект.СтоимостьБУ;
