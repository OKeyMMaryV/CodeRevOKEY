...
Функция ТекстСодержанияПроводокДокумента(Реквизиты)
	...
	Содержание = СокрЛП(СтрЗаменить(ТекстНазначение, Символы.ПС, " "))
		...
		//СБ Ванюков К.+
		//Содержание = "";
		Если Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.РасчетыПоКредитамИЗаймам Тогда
			Содержание = "Расчеты по кредитам и займам";
		ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочиеРасчетыСКонтрагентами тогда
			Содержание = "Расчеты с контрагентом";
		ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам
			 И Реквизиты.Регистратор.СуммаУслуг > 0 тогда
			//ОКЕЙ Завьялова Е.А.(ПервыйБИТ) Начало 2021-10-07 (#ТП_БП11_ФР17)
			// изменен текст
			//Содержание = "Услуги банка по платежным картам и кредитам";
			Содержание = "Поступление выручки от розничной торговли";
			//ОКЕЙ Завьялова Е.А.(ПервыйБИТ) Конец  2021-10-07 (#ТП_БП11_ФР17)
		ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам тогда
			Содержание = "Расчеты по платежным картам и банковским кредитам";
		ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступление Тогда
			Содержание = "Прочее поступление денежных средств";
		КонецЕсли; 
		//СБ Ванюков К.-	
...
Функция ТекстЗапросаОстаткиПрочихРасчетов()
	...
	|		ПО ОплатаПлатежнойКартойРасшифровкаПлатежа.Ссылка = ВТОстаткиПрочихРасчетов.РасчетныйДокумент
	|ГДЕ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОстаткиПрочихРасчетов КАК ВТОстаткиПрочихРасчетов
	|		ПО РеализацияТоваровУслуг.Ссылка = ВТОстаткиПрочихРасчетов.РасчетныйДокумент
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
// Процедура устанавливает указанное значение в табличной части расшифровка платежа в заданном субконто
// Объект - Ссылка на документ Поступление на расчтеный счте
// НомерСубконто - число, номер заполняемого субконто
// ЗначениеСубконто - значение заполняемого субконто
//
Функция ок_УстановитьЗначенияСубконтоВРасшифровке(Объект, НомерСубконто, ЗначениеСубконто) Экспорт
	
	Для Каждого СтрокаТЧ Из Объект.РасшифровкаПлатежа Цикл
		СтрокаТЧ["СубконтоКт"+НомерСубконто] = ЗначениеСубконто;		
	КонецЦикла;
...
//ОКЕЙ Балыков А.Г.(ПервыйБИТ) Начало 2021-04-29 (#ТП_БП11_ФР17)
Процедура ок_ПрофильРазноскиОбработатьИзменение(Объект) Экспорт
	...
	Объект.СтатьяДвиженияДенежныхСредств = Обработки.ок_НастройкиОтборовДляОчисткиСубконто3.ПолучитьЗначениеКонстанты("СтатьяДДСКомиссияЭквайринг");
	Для Каждого СтрокаТЧ Из Объект.РасшифровкаПлатежа Цикл
...
// Изменен реквизит Сделка в ТЧ РасшифровкаПлатежа - расширен тип на документ бит_ок_ОперацияАксапты

//ОКЕЙ Балыков А.Г.(ПервыйБИТ) Конец 2021-05-14 (#НТП_МП_04)

//ОКЕЙ Михайлова М.В.(ПервыйБИТ) Начало 2021-07-15 (#ФР-МП-11)

// Изменен реквизит Сделка в ТЧ РасшифровкаПлатежа - расширен тип на документы бит_ок_ОперацияАксапты12

//ОКЕЙ Михайлова М.В.(ПервыйБИТ) Конец  2021-07-15 (#ФР-МП-11)

//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Начало 2021-08-06 (#ТП_БП09_ФР01)

// Изменен реквизит Сделка в ТЧ РасшифровкаПлатежа - расширен тип на документ ок_ОперацияДилс
// Изменен реквизит ДокументОснование - расширен тип на документ ок_ОперацияДилс  

//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Конец 2021-08-06 (#ТП_БП09_ФР01)
