...
Процедура СчетУчетаПриИзменении(Элемент)
	...
	КонецЕсли;
	//ОКЕЙ Беднягин Е.А. (ПервыйБИТ) Начало 2021-04-23 (#ТП_БП11_ФР01)
	ок_ЗаполнитьОбъектПоДоговоруИзСубконто(Элемент);
	//ОКЕЙ Беднягин Е.А. (ПервыйБИТ) Конец 2021-04-23 (#ТП_БП11_ФР01)	
...
Процедура ок_ЗаполнитьОбъектПоДоговоруИзСубконто(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.СубконтоКт2) Тогда 
		ок_ЗаполнитьОбъектПоДоговоруИзСубконтоНаСервере(Объект.СубконтоКт2);
	Иначе
		Объект.ок_Объект = Неопределено;
	КонецЕсли;
