...
// Процедура получает статус документа и отображает его на форме. 
// 
&НаСервере
Процедура ПолучитьВывестиСтатус()
	
	РезСтруктура = бит_Визирование.ПолучитьСтатусОбъекта(Объект.Ссылка);
	
	Статус = ?(ЗначениеЗаполнено(РезСтруктура.Статус), РезСтруктура.Статус, Справочники.бит_СтатусыОбъектов.СправкаОВалютныхОперациях_Черновик);
...
КонецПроцедуры // ПолучитьВывестиСтатус()
...
//
&НаКлиенте
Процедура УстановитьСтатусВФорме(Статус,ДатаИзмененияСтатуса)
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "бит_ИзмененСтатус" Тогда
		...
		Иначе 
			...
			ДатаИзмененияСтатуса = Параметр.ДатаИзмененияСтатуса;
			УстановитьСтатусВФорме(Статус,ДатаИзмененияСтатуса);
		КонецЕсли;
