...
// бит_DKravchenko Процедура выполняет первоначальное заполнение созданного/скопированного документа.
//
// Параметры:
...
// бит_DKravchenko Функция получает структуру шапки документа.
//
// Параметры:
//  Нет.
// 
...
Функция ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, СтруктураКурсыВалют)  Экспорт
	...
	СтруктураПараметров.Вставить("ЦФО");
	СтруктураПараметров.Вставить("ПоВсемОрганизациям"     , Ложь);
	СтруктураПараметров.Вставить("ПоВсемЦФО"              , Истина);
	//1С-ИжТиСи, Кондратьев, 03.2020, обновление )
...
// бит_DKravchenko Процедура выполняет проверку данных документа.
//
// Параметры:
//  СтруктураШапкиДокумента - Структура.
//  СтруктураТаблиц 		- Структура.
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		...
		Для Каждого ТекИмяРесурса Из РесурсыПереоценки Цикл
			...
			Если ЗначениеРесурса > 0 Тогда
				...
				СформироватьЗаписьПоДоходу(СтруктураШапкиДокумента, СтрокаТаблицы, ТекИмяРесурса, СтруктураКурсыВалют.МУ); //БИТ Тртилек 22.02.2012 Добавлен параметр СтруктураКурсыВалют.МУ для заполнения суммы регл
			...
			ИначеЕсли ЗначениеРесурса < 0 Тогда
				...
				СформироватьЗаписьПоРасходу(СтруктураШапкиДокумента, СтрокаТаблицы, ТекИмяРесурса, СтруктураКурсыВалют.МУ); //БИТ Тртилек 22.02.2012 Добавлен параметр СтруктураКурсыВалют.МУ для заполнения суммы регл
...
// бит_DKravchenko Процедура формирует движение курсовой разницы по доходу.
//
// Параметры:
//  СтруктураШапкиДокумента - Структура.
...
//  ИмяРесурса 				- Строка.
// 
Процедура СформироватьЗаписьПоДоходу(СтруктураШапкиДокумента, СтрокаТаблицы, ИмяРесурса, ВалютаМУ) //БИТ Тртилек 22.02.2012 Добавлен параметр ВалютаМУ для заполнения суммы регл
...
// бит_DKravchenko Процедура формирует движение курсовой разницы по расходу.
//
// Параметры:
//  СтруктураШапкиДокумента - Структура.
//  СтрокаТаблицы 			- СтрокаТаблицыЗначений.
//  ИмяРесурса 				- Строка.
// 
Процедура СформироватьЗаписьПоРасходу(СтруктураШапкиДокумента, СтрокаТаблицы, ИмяРесурса, ВалютаМУ) //БИТ Тртилек 22.02.2012 Добавлен параметр ВалютаМУ для заполнения суммы регл
...
// бит_DKravchenko Процедура заполняет атрибуты записи.
//
// Параметры:
...
Процедура ЗаполнитьАтрибутыЗаписи(Запись, СтруктураШапкиДокумента, СтрокаТаблицы, ЭтоДоход = Истина)
	...
	Если Запись.СчетДт.Валютный Тогда
		Запись.ВалютаДт        = СтрокаТаблицы.Валюта;
		Запись.ВалютнаяСуммаДт = 0;
	КонецЕсли; 
	
	Если Запись.СчетКт.Валютный Тогда
		Запись.ВалютаКт        = СтрокаТаблицы.Валюта;
		Запись.ВалютнаяСуммаКт = 0;
	КонецЕсли; 
	
	//БИТ Тртилек 21122013
	Если НЕ Метаданные.РегистрыБухгалтерии[СтруктураШапкиДокумента.ИмяРегистраБухгалтерии].Измерения.Найти("ВидДвиженияМСФО") = Неопределено Тогда
...
// бит_DKravchenko Процедура заполняет аналитику счета Дт и Кт записи.
//
// Параметры:
//  Запись					- РегистрБухгалтерииЗапись.
//  СтруктураШапкиДокумента - Структура.
...
Процедура ЗаполнитьАналитикуЗаписи(Запись, СтруктураШапкиДокумента, СтрокаТаблицы, ЭтоДоход = Истина)
	...
	//БИТ Тртилек 21122013
	ТрансляционнаяРазница = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("666666");
	Если НЕ ТрансляционнаяРазница = Неопределено Тогда
		Если Запись.СчетДт = ПланыСчетов.бит_Дополнительный_2.ВалютныйСчет ИЛИ Запись.СчетДт = ПланыСчетов.бит_Дополнительный_2.РасчетныйСчет Тогда
			бит_му_ОбщегоНазначения.УстановитьСубконто(Запись.СчетДт, Запись.СубконтоДт, 2, ТрансляционнаяРазница);
		КонецЕсли;
		Если Запись.СчетКт = ПланыСчетов.бит_Дополнительный_2.ВалютныйСчет ИЛИ Запись.СчетКт = ПланыСчетов.бит_Дополнительный_2.РасчетныйСчет Тогда
			бит_му_ОбщегоНазначения.УстановитьСубконто(Запись.СчетКт, Запись.СубконтоКт, 2, ТрансляционнаяРазница);
		КонецЕсли;
	КонецЕсли;
	///БИТ Тртилек
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ВидыКурсов = Новый Структура("Упр,МУ");
