...
Процедура ПодобратьДокументЗавершить(Знач Результат, Знач СпособОбработки) Экспорт
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		Если СпособОбработки = "ПоступлениеТоваровУслуг"
			И ЗначениеЗаполнено(Результат)
			И ТипЗнч(Результат) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
			ПерепривязатьЭлектронныйДокументКПодчиненнойСчетФактуре(Результат);
		КонецЕсли; 
...
Процедура ОбработатьДокументыОснования(СсылкаНаДокумент,СпособОбработки = "",Удаление = Ложь)
	...
	//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-05-14 (#3746)
		И Удаление
	//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Конец 2020-05-14 (#3746)
		Тогда
		УчетныйОбъект = СсылкаНаДокумент.ПолучитьОбъект();
		Если Не УчетныйОбъект.ПометкаУдаления Тогда
			УчетныйОбъект.УстановитьПометкуУдаления(Истина);
		КонецЕсли;	
	КонецЕсли;	
...
//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-05-20 (#3732)
&НаСервере
Процедура ПерепривязатьЭлектронныйДокументКПодчиненнойСчетФактуре(ДокументПоступления)
	
	СчетФактура = УчетНДСПереопределяемый.НайтиПодчиненныйСчетФактуруПолученный(ДокументПоступления);
			
	СтруктураОтбора = Новый Структура("ДокументОснование", СчетФактура);
	СтрокиОснования = ЭлектронныйДокумент.ДокументыОснования.НайтиСтроки(СтруктураОтбора);
	Если ЗначениеЗаполнено(СчетФактура)
		И СтрокиОснования.Количество() = 0 Тогда
		ПерепривязатьЭлектронныйДокумент(СчетФактура,"СчетФактураПолученный");
	КонецЕсли;
