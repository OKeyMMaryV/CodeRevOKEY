...
Процедура ок_СтатусПриИзменении(Элемент)
	...
	Если ок_ТекущийСтатусДокумента <> ПредопределенноеЗначение("Перечисление.ок_СтатусыВходящегоЭлектронногоДокументооборота.НаСогласование")
		И Объект.ок_Статус = ПредопределенноеЗначение("Перечисление.ок_СтатусыВходящегоЭлектронногоДокументооборота.ЗавершенБезПодписания") Тогда 
		Возврат;
	КонецЕсли;	
...
Процедура ок_ИнициаторыИнициаторОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВыбранноеЗначение) 
		И Объект.ок_Инициаторы.НайтиСтроки(Новый Структура("Инициатор", ВыбранноеЗначение)).Количество() > 0 Тогда
...
Процедура ок_ЦФОНачалоВыбораЗаершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.ок_ФункциональныйДиректор) Тогда
		...
		ШаблонОшибки 		= "Согласующее лицо %1 уволен(а) от %2. Рекомендовано заменить согласующее лицо.";
		РеквизитыИнициатора = ОК_ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Объект.ок_ФункциональныйДиректор, "ок_Уволен,ок_ДатаУвольнения");
		
		Если РеквизитыИнициатора.ок_Уволен Тогда
			
			ПоказатьПредупреждение(,СтрШаблон(ШаблонОшибки, Объект.ок_ФункциональныйДиректор, Формат(РеквизитыИнициатора.ок_ДатаУвольнения,"ДФ=dd.MM.yyyy")));
			
			Объект.ок_ЦФО 						= ПредопределенноеЗначение("Справочник.Подразделения.ПустаяСсылка");
...
Функция СписокДоступныхСтатусов(Текст = Неопределено)
	...
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2021-10-14 (#4324)
	Если Не ок_ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоБумажныйДокумент(Объект.Ссылка) Тогда 
		ДанныеВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ок_СтатусыВходящегоЭлектронногоДокументооборота.ЗавершенБезПодписания"));
	КонецЕсли;	
	// ОКЕЙ Сафронов А.А. (СофтЛаб) Конец 2021-10-14 (#4324)
