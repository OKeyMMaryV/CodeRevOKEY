...
Функция ок_ИзменитьСтатусыДокументовНаСервере(МассивДокументов, ПараметрыИзменения) Экспорт
	БылиОшибкиИзмененияСтатусов = Ложь;
	
	Для Каждого ТекДокумент Из МассивДокументов Цикл
		ДокОбъект = ТекДокумент.ПолучитьОбъект();
		ДокОбъект.ок_Статус = ПараметрыИзменения.Статус;
		ДокОбъект.ДополнительныеСвойства.Вставить("НовыйДокумент", Истина);
		Попытка
			ДокОбъект.Записать(РежимЗаписиДокумента.Запись);
		Исключение
			ШаблонСообщения = НСтр("ru = 'При изменении статуса документа: %1 произошла ошибка:'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ТекДокумент) + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ТекДокумент);
		КонецПопытки;	
	КонецЦикла;
	
	Возврат Не БылиОшибкиИзмененияСтатусов;
КонецФункции

Функция ЕдинственныйСтатусВСпискеЭлектронныхДокументов(МассивДокументов) Экспорт
	
	ПолученныйСтатус = Неопределено;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЭлектронныйДокументВходящий.ок_Статус КАК Статус
		|ИЗ
		|	Документ.ЭлектронныйДокументВходящий КАК ЭлектронныйДокументВходящий
		|ГДЕ
		|	ЭлектронныйДокументВходящий.Ссылка В(&МассивДокументов)
		|
		|СГРУППИРОВАТЬ ПО
		|	ЭлектронныйДокументВходящий.ок_Статус";
	
	Запрос.УстановитьПараметр("МассивДокументов", МассивДокументов);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Количество() = 1 Тогда
		Выборка.Следующий();
		ПолученныйСтатус = Выборка.Статус;		
	КонецЕсли;	
