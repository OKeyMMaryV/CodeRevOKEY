...
Функция ОстаткиОСПоСписку()
	...
	ВыборкаОС = Запрос.Выполнить().Выгрузить();
	...
	Для Каждого СтрокаОС Из Объект.ОС Цикл
		...
		Если НЕ (ОстатокОС = Неопределено) Тогда
			...
			//izhtc-Kir 05.10.2015 (
			СтрокаОС.СтоимостьПоРезультатамИнвентаризации = ОстатокОС.ВосстановительнаяСтоимость;
			//izhtc-Kir 05.10.2015 )
		Иначе
			...
			//izhtc-Kir 05.10.2015 (
			СтрокаОС.СтоимостьПоРезультатамИнвентаризации = 0;
			//izhtc-Kir 05.10.2015 )
...
&НаКлиенте
Процедура ДобавитьОСПоОбъектуСтроительства(Команда)
	Если Объект.ОС.Количество() > 0 Тогда
	...
	ФормаПодбораОСПоОбъекту	= ПолучитьФорму("Документ.ИнвентаризацияОС.Форма.ФормаПодбораОСПоОбъекту");
	//ФормаПодбораОСПоОбъекту.РежимВыбора=Истина;
	ФормаПодбораОСПоОбъекту.имяКолонки="ОсновноеСредство";
	ФормаПодбораОСПоОбъекту.Организация=Объект.Организация;
	ФормаПодбораОСПоОбъекту.Дата=Объект.Дата;
	ФормаПодбораОСПоОбъекту.ВладелецФормы=ЭтаФорма;
	ФормаПодбораОСПоОбъекту.ОткрытьМодально();	
	
	ВыборкаОС = ОстаткиОСПоСписку();
	ЗаполнитьРеквизитыТЧ();
КонецПроцедуры	
