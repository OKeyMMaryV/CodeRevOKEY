...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МетаданныеОбъекта = Метаданные.Документы.бит_му_ВыработкаВНА;
	...
	Если Параметры.Свойство("Дата") Тогда
		...
		ЭлементОтбора.ПравоеЗначение = Параметры.Дата;
...
&НаКлиенте
Процедура НРПВНА(Команда)
	Если Элементы.Список.ТекущиеДанные = Неопределено Тогда Возврат КонецЕсли;
	ОткрытьФорму("РегистрСведений.бит_му_НРП_ВНА.ФормаСписка", Новый Структура("Отбор", Новый Структура("мРегистратор", Элементы.Список.ТекущиеДанные.Ссылка)));
КонецПроцедуры
///БИТ Тртилек 06092013
