...
Процедура ОбработкаПроведения(Отказ)
	...
	РеквизитыПрослеживаемости = ПрослеживаемостьБП.ПодготовитьОбщиеПараметры(ПараметрыПроведения.РеквизитыПоступление);
	Если ЗначениеЗаполнено(РеквизитыПрослеживаемости.Реквизиты) Тогда
		ПрослеживаемостьБП.СформироватьДвиженияОперацииСПрослеживаемымиТоварами(
			ПараметрыПроведения.СторноПрослеживаемыеОперации, 
			РеквизитыПрослеживаемости.Реквизиты[0],
			Движения);
...
Процедура ОпределениеПараметровСчетаФактуры() Экспорт
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.КорректировкаПоступления") И 
		ТипЗнч(ДокументОснование.ДокументПоступления) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") И 
		ДокументОснование.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеСобственнойОшибки Тогда
		ЗаполнитьСуммуДокументаНаОснованииОперацияАксапты(ДокументОснование);
		Возврат;
	КонецЕсли;	
...
Процедура ЗаполнитьНаОснованииИсправления(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.КорректировкаПоступления") И 
		ТипЗнч(Основание.ДокументПоступления) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		ЗаполнитьНаОснованииОперацияАксапты(Основание);
		Возврат;
	КонецЕсли;	
...
//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-05-17 (#НТП-МП-03)	
Процедура ЗаполнитьНаОснованииОперацияАксапты(Основание)
	...
	Если ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.Корректировочный Тогда	
		// Корректировка счета-фактуры	
		СтрокаОснования.НомерИсходногоДокумента = Основание.Номер;
		СтрокаОснования.ДатаИсходногоДокумента  = Основание.Дата;			
	Иначе		
		// Исправление обычного счета-фактуры	
...
//ОКЕЙ Лещенко Д.Ю. (ПервыйБИТ) Начало 2021-07-31 (#НТП-МП-03)      
Процедура ЗаполнитьСуммуДокументаНаОснованииОперацияАксапты(ДокументОснование)
	
	СуммаПоКорректировке = ДокументОснование.Услуги.Итог("Сумма");
	Если СуммаПоКорректировке = 0 Тогда
		СуммаДокумента = ДокументОснование.ок_СуммаПоСчетуФактуре;		
	КонецЕсли; 
