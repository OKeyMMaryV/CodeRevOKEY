...
Функция ПолучитьРеквизитыИсходногоСчетаФактурыДляКорректировки(ДокументОснование, СчетФактура, ПолучатьПредставление = Ложь, 
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.КорректировкаПоступления") И ТипЗнч(ДокументПоступления) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		Возврат ПолучитьРеквизитыИсходногоСчетаФактурыДляКорректировкиОперацияАксапты(ДокументОснование, ДокументПоступления);
	КонецЕсли;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	// 1С-ИжТиСи Полина 18.08.2015 (
	Если Реквизиты.Период >= '20150101'
		И Реквизиты.БланкСтрогойОтчетности Тогда
		Реквизиты.Вставить("КодВидаОперации", "23");
	КонецЕсли;
	// 1С-ИжТиСи Полина 18.08.2015 )
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура534") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур534(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактурНовый(ПорядокПримененияФормСФ["СчетФактура534"]),
			ПараметрыПечати);
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура534Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур534Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактурНовый(ПорядокПримененияФормСФ["СчетФактура534Прослеж"], Истина),
			ПараметрыПечати);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура534") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур534(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактурНовый(ПорядокПримененияФормСФ["КорректировочныйСчетФактура534"]), 
			ПараметрыПечати);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура534Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур534Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактурНовый(ПорядокПримененияФормСФ["КорректировочныйСчетФактура534Прослеж"],Истина),
			ПараметрыПечати);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
...
// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-06-09 (#4208)
Функция ТекстЗапросаПечатьСчетовФактурНовый(ВерсияПостановленияНДС1137 = Неопределено, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
...
Функция ТекстЗапросаПечатьКорректировочныхСчетовФактурНовый(ВерсияПостановленияНДС1137 = Неопределено, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
...
Функция СоздатьАктуализироватьИсправлениеСобственнойОшибки(Параметры) Экспорт
	...
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-05-17 (#НТП-МП-03)
	Если ТипЗнч(Параметры.ДокументОснование) = Тип("ДокументСсылка.КорректировкаПоступления") И ТипЗнч(Параметры.ДокументОснование.ДокументПоступления) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		Результат = ПолучитьСФПоОперацииАксапта(Параметры);
		Возврат Результат;
	КонецЕсли;
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Конец 2021-05-17 (#НТП-МП-03)
...
Функция ТекстЗапросаКорректировка()
	...
	ТекстЗапроса = ТекстЗапросаДляИсправленияКорректировки() + "ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СчетФактураШапка.Ссылка КАК СчетФактура,
	|	НЕОПРЕДЕЛЕНО КАК Субкомиссионер,
	|	СчетФактураШапка.Контрагент КАК Покупатель,
	|	СУММА(ВЫБОР
...
//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-05-17 (#НТП-МП-03)	
Функция ПолучитьРеквизитыИсходногоСчетаФактурыДляКорректировкиОперацияАксапты(ДокументОснование, ДокументПоступления)
	
	РеквизитыОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОснование, "ок_НомерИсходногоСФ, ок_ДатаИсходногоСФ"); 
	РеквизитыСчетаФактуры = Новый Структура("СчетФактура, СчетФактураПредставление, СчетФактураКраткоеПредставление, НомерСчетаФактуры, ДатаСчетаФактуры, Исправление, НомерИсправления, ДатаИсправления");
	РеквизитыСчетаФактуры.Вставить("СчетФактура", ДокументПоступления);
	РеквизитыСчетаФактуры.Вставить("СчетФактураПредставление", СтрШаблон("%1 от %2", РеквизитыОснования.ок_НомерИсходногоСФ, РеквизитыОснования.ок_ДатаИсходногоСФ));
	РеквизитыСчетаФактуры.Вставить("СчетФактураКраткоеПредставление", СтрШаблон("%1 от %2", РеквизитыОснования.ок_НомерИсходногоСФ, РеквизитыОснования.ок_ДатаИсходногоСФ));
	РеквизитыСчетаФактуры.Вставить("НомерСчетаФактуры", РеквизитыОснования.ок_НомерИсходногоСФ);
	РеквизитыСчетаФактуры.Вставить("ДатаСчетаФактуры", РеквизитыОснования.ок_ДатаИсходногоСФ);
	
	Возврат РеквизитыСчетаФактуры;
	
КонецФункции

Функция ПолучитьСФПоОперацииАксапта(Параметры)
		
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("СчетФактураИсправляемый", Параметры.ДокументОснование);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СчетФактураПолученный.Организация,
	|	СчетФактураПолученный.ок_ВидСчетаФактуры КАК ВидСчетаФактуры,
	|	СчетФактураПолученный.Контрагент,
	|	СчетФактураПолученный.ДоговорКонтрагента,
	|	ЛОЖЬ КАК Исправление,
	|	СчетФактураПолученный.НомерИсправления,
	|	СчетФактураПолученный.ДатаИсправления,
	|	СчетФактураПолученный.ок_НомерИсходногоСФ КАК НомерВходящегоДокумента,
	|	СчетФактураПолученный.ок_ДатаИсходногоСФ КАК ДатаВходящегоДокумента,	
	|	СчетФактураПолученный.ок_КодВидаОперации КАК КодВидаОперации,
	|	СчетФактураПолученный.ок_КодВидаОперацииНаУменьшение КАК КодВидаОперацииНаУменьшение,
	|	СчетФактураПолученный.ВалютаДокумента,
	|	СчетФактураПолученный.Контрагент КАК Продавец
	|ИЗ
	|	Документ.КорректировкаПоступления КАК СчетФактураПолученный
	|ГДЕ
	|	СчетФактураПолученный.Ссылка = &СчетФактураИсправляемый";
