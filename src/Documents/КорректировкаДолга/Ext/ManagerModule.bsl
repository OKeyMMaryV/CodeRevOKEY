...
Функция ок_ОбработатьКЗДляПроведенияПоБК(КредиторскаяЗадолженность, ДоговорКонтрагента=Неопределено, КорДоговорКонтрагента=Неопределено, ФВБ=Неопределено) Экспорт
	
	ТЗКредиторскаяЗадолженность = ПолучитьИзВременногоХранилища(КредиторскаяЗадолженность);
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТЗКредиторскаяЗадолженность.*
		|ПОМЕСТИТЬ ВТКредиторскаяЗадолженность
		|ИЗ
		|	&ТЗКредиторскаяЗадолженность КАК ТЗКредиторскаяЗадолженность
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТКредиторскаяЗадолженность.*,
		|	ВЫБОР
		|		КОГДА &КорДоговорКонтрагента = НЕОПРЕДЕЛЕНО
		|			ТОГДА ВТКредиторскаяЗадолженность.КорДоговорКонтрагента
		|		ИНАЧЕ &КорДоговорКонтрагента
		|	КОНЕЦ КАК КорДоговорКонтрагента,
		|	бит_ДополнительныеАналитики.ЗначениеАналитики КАК ок_ФВБ
		|ИЗ
		...
		|				бит_ВидыДополнительныхАналитик.Ссылка КАК АналитикаНомерЗаявки
		|			ИЗ
		|				ПланВидовХарактеристик.бит_ВидыДополнительныхАналитик КАК бит_ВидыДополнительныхАналитик
		|			ГДЕ
		|				бит_ВидыДополнительныхАналитик.Код = ""NЗаявки"") КАК ВЗАналитикаНомерЗаявки
		|			ПО бит_ДополнительныеАналитики.Аналитика = ВЗАналитикаНомерЗаявки.АналитикаНомерЗаявки
		|		ПО ВТКредиторскаяЗадолженность.Сделка = бит_ДополнительныеАналитики.Объект
		|ГДЕ
		|	ВЫБОР
		|			КОГДА &ДоговорКонтрагента = НЕОПРЕДЕЛЕНО
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ ВТКредиторскаяЗадолженность.ДоговорКонтрагента = &ДоговорКонтрагента
		|		КОНЕЦ
		|	И ВЫБОР
		|			КОГДА &ФВБ = НЕОПРЕДЕЛЕНО
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ бит_ДополнительныеАналитики.ЗначениеАналитики = &ФВБ
		|		КОНЕЦ";
	
	Запрос.УстановитьПараметр("ТЗКредиторскаяЗадолженность", ТЗКредиторскаяЗадолженность);	
	Запрос.УстановитьПараметр("ДоговорКонтрагента", ДоговорКонтрагента);
	Запрос.УстановитьПараметр("КорДоговорКонтрагента", КорДоговорКонтрагента);
	Запрос.УстановитьПараметр("ФВБ", ФВБ);
	
	РезультатЗапроса = Запрос.Выполнить();
	ТЗКредиторскаяЗадолженность.Очистить();
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(ТЗКредиторскаяЗадолженность.Добавить(), ВыборкаДетальныеЗаписи);
	КонецЦикла;
	Возврат ПоместитьВоВременноеХранилище(ТЗКредиторскаяЗадолженность, КредиторскаяЗадолженность);
...
// Изменен реквизит Сделака в ТЧ ДебиторскаяЗадолженность – расширен тип на документ бит_ок_ОперацияАксапты

//ОКЕЙ Балыков А.Г.(ПервыйБИТ) Конец 2021-05-14 (#НТП_МП_04)

//ОКЕЙ Первухин В.С. (ПервыйБИТ) Начало 2021-07-13 (ФР-МП-11)      

// Изменен реквизит Сделка в ТЧ ДебиторскаяЗадолженность - расширен тип на документ бит_ок_ОперацияАксапты12
// Изменен реквизит Сделка в ТЧ КредиторскаяЗадолженность - расширен тип на документы бит_ок_ОперацияАксапты, бит_ок_ОперацияАксапты12
