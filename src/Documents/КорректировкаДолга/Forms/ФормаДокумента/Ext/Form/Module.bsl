...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.ок_ЭтоПереносЗадолженностиПоставщику = ок_ЭтоПереносЗадолженностиПоставщику(Объект.ВидОперации, Объект.ВидАвансаЗадолженности);
	ок_УстановитьВидимостьКомандТЧЗП(ЭтаФорма);
	
	Если ЭтаФорма.ок_ЭтоПереносЗадолженностиПоставщику Тогда 
		ЭтаФорма.ок_ФВБ = ок_ОбновитьФВБПоТЧ();
	КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.ДополнительныеСвойства.Свойство("ПередатьИнфоДляБК") И ТекущийОбъект.ДополнительныеСвойства.ПередатьИнфоДляБК Тогда
		ПараметрыЗаписи.Вставить("ПередатьИнфоДляБК",Истина);
	КонецЕсли;	
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	//ОКЕЙ Рычаков А.С.(СофтЛаб) Начало 2019-10-01 (#3490)
	Если ПараметрыЗаписи.Свойство("ПередатьИнфоДляБК") И ПараметрыЗаписи.ПередатьИнфоДляБК Тогда
		ТестСообщения = НСтр("ru = 'Необходимо передать информацию в БК по факту оформления данного документа в контуре БУ'");
		Предупреждение(ТестСообщения);
	КонецЕсли;
	//ОКЕЙ Рычаков А.С.(СофтЛаб) Конец 2019-10-01 (#3490)
...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	//ОКЕЙ Беднягин Е.А. (ПервыйБИТ) Начало 2021-04-03 (#НТП_БП06_ФР01, #НТП_БП06_ФР02)
	ок_ЭтоПереносЗадолженностиПоставщику = ок_ЭтоПереносЗадолженностиПоставщику(Объект.ВидОперации, Объект.ВидАвансаЗадолженности);
	ок_УстановитьВидимостьКомандТЧЗП(ЭтаФорма);
	//ОКЕЙ Беднягин Е.А. (ПервыйБИТ) Конец 2021-04-03 (#НТП_БП06_ФР01, #НТП_БП06_ФР02)
...
#Область НТПБП06ФР0102 

//ОКЕЙ Беднягин Е.А. (ПервыйБИТ) Начало 2021-04-03 (#НТП_БП06_ФР01, #НТП_БП06_ФР02)
&НаКлиенте
Процедура Подключаемый_ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	
	Если ИсточникВыбора.ИмяФормы = "Документ.КорректировкаДолга.Форма.ок_ФормаПодбораОстатковПоВзаиморасчетам" Тогда
		ок_ПодборОстатковПоВзаиморасчетамОбработкаВыбораНаСервере(ВыбранноеЗначение)
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ТаблицаДебиторскаяЗадолженностьСделкаПриИзменении(Элемент)
	
	ок_ТаблицаДебиторскаяЗадолженностьСделкаПриИзмененииНаСервере();

КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ПодборОстатковПоВзаиморасчетам(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение",
		НСтр("ru = 'Организация'"),,, СтрокаСообщения);
	...
	Если Объект.КредиторскаяЗадолженность.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ru = 'Перед подбором кредиторская задолженность будет очищена. Заполнить?'");
		...
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да );
	Иначе
		ЭтаФорма["ок_ФВБ"] = Неопределено;
		ок_ОткрытьФормуПодбораОстатков();
	КонецЕсли;
...
&НаСервере
Функция ок_ОбновитьФВБПоТЧ()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТЧКредиторскаяЗадолженность.ок_ФВБ КАК ок_ФВБ
		|ПОМЕСТИТЬ ВТКредиторскаяЗадолженность
		|ИЗ
		|	&ТЧКредиторскаяЗадолженность КАК ТЧКредиторскаяЗадолженность
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МАКСИМУМ(ВТКредиторскаяЗадолженность.ок_ФВБ) КАК ок_ФВБ
		|ИЗ
		|	ВТКредиторскаяЗадолженность КАК ВТКредиторскаяЗадолженность";
	
	Запрос.УстановитьПараметр("ТЧКредиторскаяЗадолженность", Объект.КредиторскаяЗадолженность.Выгрузить(, "ок_ФВБ"));
	...
	Возврат ВыборкаДетальныеЗаписи.ок_ФВБ;
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ок_ЭтоПереносЗадолженностиПоставщику(ВидОперации, ВидАвансаЗадолженности)
	
	Возврат ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкаДолга.ПереносЗадолженности")
	  И ВидАвансаЗадолженности = ПредопределенноеЗначение("Перечисление.ВидыАвансаЗадолженности.ЗадолженностьПоставщику");
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Процедура ок_УстановитьВидимостьКомандТЧЗП(Форма)
	
	Форма.Элементы.ок_ПодборОстатковПоВзаиморасчетам.Видимость = Форма.ок_ЭтоПереносЗадолженностиПоставщику;
	Форма.Элементы.ТаблицаКредиторскаяЗадолженностьЗаполнитьКредиторку.Видимость = Не Форма.ок_ЭтоПереносЗадолженностиПоставщику;
	
КонецПроцедуры

&НаСервере
Функция ПолучитьПараметрыЗаполненияВоВременномХранилище(ИмяКомандыЗаполнения)
	
	ПараметрыЗаполнения = СформироватьПараметрыЗаполнения(ИмяКомандыЗаполнения);
	АдресХранилищаСОшибками = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
	ПараметрыЗаполнения.Вставить("АдресХранилищаСОшибками", АдресХранилищаСОшибками);

	Возврат ПоместитьВоВременноеХранилище(ПараметрыЗаполнения, УникальныйИдентификатор);
	
КонецФункции

&НаСервере
Процедура ок_ТаблицаДебиторскаяЗадолженностьСделкаПриИзмененииНаСервере()
	
	ТекущаяСтрока = Элементы.ТаблицаКредиторскаяЗадолженность.ТекущаяСтрока;
	ТекущиеДанные = Объект.КредиторскаяЗадолженность.НайтиПоИдентификатору(ТекущаяСтрока);
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
		
	КонецЕсли;
	
	ФВБ_ТекущаяСделка = Документы.бит_ФормаВводаБюджета.ПустаяСсылка;
	Если ТекущиеДанные.Сделка.Пустая() Тогда 
		ТекущиеДанные.ок_ФВБ = ФВБ_ТекущаяСделка;
		
	Иначе
		ТЗ_ТекущаяСтрока = Объект.КредиторскаяЗадолженность.Выгрузить(ТекущиеДанные);
		АдресТЗ_ТекущаяСтрока = ПоместитьВоВременноеХранилище(ТЗ_ТекущаяСтрока, УникальныйИдентификатор);
		АдресТЗ_ТекущаяСтрока = Документы.КорректировкаДолга.ок_ОбработатьКЗДляПроведенияПоБК(АдресТЗ_ТекущаяСтрока);
		ТЗ_ТекущаяСтрока = ПолучитьИзВременногоХранилища(АдресТЗ_ТекущаяСтрока);
		
		Если ТЗ_ТекущаяСтрока.Количество() Тогда 
			ФВБ_ТекущаяСделка = ТЗ_ТекущаяСтрока.ок_ФВБ;
		КонецЕсли;
		
	КонецЕсли;
	Если ЗначениеЗаполнено(ЭтаФорма.ок_ФВБ) И ЗначениеЗаполнено(ФВБ_ТекущаяСделка) И ЭтаФорма.ок_ФВБ <> ФВБ_ТекущаяСделка Тогда 		
		...
		ТекущиеДанные.ок_ФВБ = Неопределено;
		
	Иначе
		ТекущиеДанные.ок_ФВБ = ФВБ_ТекущаяСделка;
		Если Не ЗначениеЗаполнено(ФВБ_ТекущаяСделка) Тогда 
			ЭтаФорма.ок_ФВБ = ок_ОбновитьФВБПоТЧ();
		КонецЕсли;
...
&НаСервере
Процедура ок_ПодборОстатковПоВзаиморасчетамОбработкаВыбораНаСервере(ВыбранноеЗначение)
	
	Если Объект.КонтрагентКредитор <> ВыбранноеЗначение.КонтрагентКредитор Тогда 
		Объект.КонтрагентКредитор = ВыбранноеЗначение.КонтрагентКредитор;
		КонтрагентКредиторПриИзмененииСервер();
	КонецЕсли;
	
	ДанныеПодбора = ПолучитьИзВременногоХранилища(ВыбранноеЗначение.АдресВХранилище);
	Для Каждого СтрокаДанныеПодбора Из ДанныеПодбора Цикл
		СтрокаКредиторскаяЗадолженность = Объект.КредиторскаяЗадолженность.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаКредиторскаяЗадолженность, СтрокаДанныеПодбора);
	КонецЦикла;
...
&НаКлиенте
Процедура ок_ОткрытьФормуПодбораОстатков()
	
	ПараметрыПодбора = Новый Структура("Организация, КонтрагентДебитор, КонтрагентКредитор, 
		|ВалютаДокумента, КурсДокумента, КратностьДокумента");
	ЗаполнитьЗначенияСвойств(ПараметрыПодбора, Объект);
	
	ПараметрыПодбора.Вставить("ПараметрыЗаполнения", ПолучитьПараметрыЗаполненияВоВременномХранилище("ЗаполнитьКредиторку"));
	ПараметрыПодбора.Вставить("ПараметрыВыбораДоговора", Элементы.ТаблицаКредиторскаяЗадолженностьДоговорКонтрагента.ПараметрыВыбора);

	ОткрытьФорму("Документ.КорректировкаДолга.Форма.ок_ФормаПодбораОстатковПоВзаиморасчетам", ПараметрыПодбора, ЭтаФорма, УникальныйИдентификатор, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
