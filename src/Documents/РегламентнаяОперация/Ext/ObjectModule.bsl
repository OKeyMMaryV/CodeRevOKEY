...
Функция ВыполнитьОперацию(БлокировкаУстановленаНеявно = Ложь) Экспорт
	...
	// 1С-ИжТиСи Полина 01.09.2015 (
	Если бит_ОбщегоНазначения.ЕстьМеханизмКонтроляЗакрытогоПериода() Тогда
		
		//БИТ Тртилек 15.01.2014 учитываем аналитику в документе
		Если КонтролироватьЗакрытыйПериод(ЭтотОбъект) Тогда
			// при наличиии механизма контроля закрытого периода сохраним движения для последующего анализа
			бит_КонтрольЗакрытогоПериода.КонсервироватьДвижения(ЭтотОбъект,Ложь,РежимЗаписиДокумента.Запись,РежимПроведенияДокумента.Неоперативный);
		...
		//Начало изменений Широков Н. <SoftBalance> 31/01/14-15:31 
		СБ_МеханизмКонтроляТрансляцииСервер.СБ_МеханизмКонтроляТрансляцииПередЗаписьюПередЗаписью(ЭтотОбъект, Ложь, РежимЗаписиДокумента.Запись, РежимПроведенияДокумента.Неоперативный);
		//Конец изменений Широков Н. 
		
	КонецЕсли;
	// 1С-ИжТиСи Полина 01.09.2015 )	
	
	// бит_Финанс изменение кода. Начало. 13.12.2012{{
	бит_КонтрольЗакрытогоПериода.КонсервироватьДвижения(ЭтотОбъект, Ложь, РежимЗаписиДокумента.ОтменаПроведения, РежимПроведенияДокумента.Неоперативный);
	// бит_Финанс изменение кода. Конец. 13.12.2012}}
	...
	Если РучнаяКорректировка Тогда
		...
		бит_МеханизмТрансляции.бит_ТрансляцияОперацияПриЗаписиПриЗаписи(ЭтотОбъект, Ложь);
		бит_КонтрольЗакрытогоПериода.КонтрольПриОбработкеПроведения(ЭтотОбъект, Ложь, РежимЗаписиДокумента.Проведение, Истина);
		// бит_Финанс добавление кода конец }}  
		
		Возврат Истина;
	КонецЕсли;
	
	// бит_Финанс изменения кода. Начало: 03.08.2018.
	Если НЕ РучнаяКорректировка Тогда
		бит_ОбщегоНазначения.ПриЗаписиОперацииРучнаяКорректировкаФакта(ЭтотОбъект);
	КонецЕсли;	
	// бит_Финанс изменения кода. Конец 03.08.2018. 
	...
	Если бит_ОбщегоНазначения.ЕстьМеханизмКонтроляЗакрытогоПериода() Тогда
		...
		Если КонтролироватьЗакрытыйПериод(ЭтотОбъект) Тогда
			// при наличиии механизма контроля закрытого периода сохраним движения для последующего анализа
			бит_КонтрольЗакрытогоПериода.КонтрольПриОбработкеПроведения(ЭтотОбъект,Ложь,РежимЗаписиДокумента.Запись,Истина);
			
		КонецЕсли;
		//Выполнить("бит_КонтрольЗакрытогоПериода.КонсервироватьДвижения(ДокументОбъект,Отказ,РежимЗаписиДокумента.Проведение,РежимПроведенияДокумента.Неоперативный)");
		///БИТ Тртилек
		////Начало изменений Широков Н. <SoftBalance> 31/01/14-15:31 
		//СБ_МеханизмКонтроляТрансляцииСервер.СБ_МеханизмКонтроляТрансляцииПриЗаписиПриЗаписи(ЭтотОбъект, Ложь);
		////Конец изменений Широков Н. 
	КонецЕсли;
	// 1С-ИжТиСи Полина 01.09.2015 )
	// бит_Финанс изменение кода. Конец. 13.12.2012}}	
...
Процедура ОтменитьОперацию(БлокировкаУстановленаНеявно = Ложь, Отказ = Ложь, Знач ПараметрыОтмены = Неопределено) Экспорт
	...
	// бит_Финанс изменения кода. Начало: 03.08.2018.
	// 1С-ИжТиСи Полина 01.09.2015 (
	//БИТ Тртилек 16.10.2012 требуется сохранить движения перед их удалением
	//бит_КонтрольЗакрытогоПериода.КонсервироватьДвижения(ЭтотОбъект, Отказ, РежимЗаписиДокумента.ОтменаПроведения, РежимПроведенияДокумента.Неоперативный);
	бит_КонтрольЗакрытогоПериода.КонсервироватьДвижения(ЭтотОбъект, Ложь, РежимЗаписиДокумента.ОтменаПроведения, РежимПроведенияДокумента.Оперативный);
	///БИТ Тртилек
	//Начало изменений Широков Н. <SoftBalance> 12/09/13
	СБ_МеханизмКонтроляТрансляцииСервер.СБ_МеханизмКонтроляТрансляцииПередЗаписьюПередЗаписью(ЭтотОбъект, Ложь, РежимЗаписиДокумента.Запись, РежимПроведенияДокумента.Неоперативный);
	//Конец изменений Широков Н. 
	// 1С-ИжТиСи Полина 01.09.2015 )	
	бит_ОбщегоНазначения.ПриЗаписиОперацииРучнаяКорректировкаФакта(ЭтотОбъект, ?(ПометкаУдаления, Ложь, Истина));
	// бит_Финанс изменения кода. Конец 03.08.2018. 
	...
	// бит_Финанс изменения кода. Начало: 03.08.2018.
	бит_КонтрольЗакрытогоПериода.КонтрольПриУдаленииПроведения(ЭтотОбъект, Отказ);
	бит_МеханизмТрансляции.КонтрольПоРучнойКорректировкеФакта(ЭтотОбъект, Истина);
	// бит_Финанс изменения кода. Конец 03.08.2018. 
	
	// 1С-ИжТиСи Полина 01.09.2015 (
	//БИТ Тртилек 16.10.2012 обработаем удаление записей, создадим сторнирующую запись для записи прошлого периода
	//бит_КонтрольЗакрытогоПериода.КонтрольПриУдаленииПроведения(ЭтотОбъект, Ложь);
	ЭтотОбъект.Движения.бит_Дополнительный_2.Записать();
	//БИТ Тртилек
	// 1С-ИжТиСи Полина 01.09.2015 )
...
Процедура ПропуститьОперацию(БлокировкаУстановленаНеявно = Ложь, Отказ = Ложь) Экспорт
	...
	// 1С-ИжТиСи Полина 01.09.2015 (
	//БИТ Тртилек 23012014 требуется сохранить движения перед их удалением
	бит_КонтрольЗакрытогоПериода.КонсервироватьДвижения(ЭтотОбъект, Ложь, РежимЗаписиДокумента.ОтменаПроведения, РежимПроведенияДокумента.Оперативный);
	///БИТ Тртилек
	...
	// 1С-ИжТиСи Полина 01.09.2015 (
	//БИТ Тртилек 23012014 обработаем удаление записей, создадим сторнирующую запись для записи прошлого периода
	бит_КонтрольЗакрытогоПериода.КонтрольПриУдаленииПроведения(ЭтотОбъект, Ложь);
	ЭтотОбъект.Движения.бит_Дополнительный_2.Записать();
	//БИТ Тртилек
	
	////Начало изменений Широков Н. <SoftBalance> 31/01/14-15:48 
	//СБ_МеханизмКонтроляТрансляцииСервер.СБ_МеханизмКонтроляТрансляцииОбработкаУдаленияПроведенияОбработкаУдаленияПроведения(ЭтотОбъект, Ложь);
	////Конец изменений Широков Н.
	// 1С-ИжТиСи Полина 01.09.2015 )	
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// бит_Финанс изменения кода. Начало: 03.08.2018.
	Если ПометкаУдаления И Состояние = Перечисления.ВидыСостоянийРегламентныхОпераций.НеВыполнено Тогда
		Если бит_МеханизмДопИзмерений.ЕстьРучнаяКорректировкаФакта(Ссылка) Тогда
			ОбщегоНазначенияБПВызовСервера.ИзменитьАктивностьПоРегистратору(ЭтотОбъект, Отказ, Ложь);
		КонецЕсли; 
	КонецЕсли; 
	// бит_Финанс изменения кода. Конец 03.08.2018. 
...
Процедура СформироватьДвижения(СтруктураШапкиДокумента, Отказ, Ошибки)
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.ЗакрытиеСчетов20_23_25_26 Тогда
		...
		//	Отказ,
		//	Ошибки);
		// Для организации Окей не нужен как таковой расчет себестоимости, используем упрощенное закрытие счетов 20 и 26.
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-10-29 (#4434)
		//Если СтруктураШапкиДокумента.Организация = Справочники.Организации.НайтиПоКоду("000000045") Тогда // Это Окей
		ОрганизацияОкей = бит_БК_Общий.ПолучитьЗначениеНастройкиМеханизмаИмпортаДанных("Организации", "Организация ОКЕЙ");
		ДатаНачалаТиповогоЗакрытияОкей = ок_ОбщегоНазначения.ПолучитьЗначениеУниверсальнойНастройки("ЗакрытиеПериода", "ПериодТиповоеЗакрытиеОкей", Дата(2099,1,1));
		Если СтруктураШапкиДокумента.Организация = ОрганизацияОкей
			И СтруктураШапкиДокумента.Дата < ДатаНачалаТиповогоЗакрытияОкей Тогда
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-10-29 (#4434)
			ЗакрытиеСчетов20_26_Окей(СтруктураШапкиДокумента,Отказ);
		Иначе
			ЗакрытиеСчетов20_23_25_26(
				СтруктураШапкиДокумента.Дата,
				СтруктураШапкиДокумента.Организация,
...
// 1С-ИжТиСи Полина 01.09.2015 (
//БИТ Тртилек 15.01.2013
Функция КонтролироватьЗакрытыйПериод(ДокументОбъект)
	
	Отбор = Новый Структура("Объект, Аналитика");
	Отбор.Объект = ДокументОбъект.Ссылка;
	//ОК+ Довбешка Т. 17.07.2017 принято решение использовать типовую аналитику #2843#
	...
	Возврат Контролируем;
	
КонецФункции
// 1С-ИжТиСи Полина 01.09.2015 )

// БИТ_YK_н #2204 02/16
Процедура ЗакрытиеСчетов20_26_Окей(СтруктураШапкиДокумента,Отказ)
	
	// 26 закроем на 90.08 по НУ и БУ, 20 на 90.07 - при таком закрытии последовательность не важна.
	Для Итерация = 1 по 2 Цикл
		...
		|ПОМЕСТИТЬ ОстаткиПоОборотнымСубконто
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоМесяца, &КонецМесяца, , СчетДт = &Счет, , , , Организация = &Организация) КАК ХозрасчетныйОборотыДтКт
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоМесяца, &КонецМесяца, , , , СчетКт = &Счет, , Организация = &Организация) КАК ХозрасчетныйОборотыДтКт
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.Остатки(&НачалоМесяца, Счет = &Счет, , Организация = &Организация) КАК ХозрасчетныйОстатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ОстаткиПоОборотнымСубконто.Счет КАК Счет,
		|	ОстаткиПоОборотнымСубконто.Субконто1,
		|	ОстаткиПоОборотнымСубконто.Субконто2,
		|	ОстаткиПоОборотнымСубконто.Субконто3,
		|	ОстаткиПоОборотнымСубконто.Подразделение КАК Подразделение,
		|	СУММА(ОстаткиПоОборотнымСубконто.Сумма) КАК Сумма,
		|	СУММА(ОстаткиПоОборотнымСубконто.СуммаНУ) КАК СуммаНУ,
		|	СУММА(ОстаткиПоОборотнымСубконто.СуммаПР) КАК СуммаПР,
		|	СУММА(ВЫБОР
		|			КОГДА &Это20Счет
		|				ТОГДА ВЫБОР
		|						КОГДА ОстаткиПоОборотнымСубконто.Сумма = 0
		|								И ОстаткиПоОборотнымСубконто.СуммаВР <> 0
		|							ТОГДА ОстаткиПоОборотнымСубконто.СуммаВР
		|						ИНАЧЕ -ОстаткиПоОборотнымСубконто.СуммаНУ
		|					КОНЕЦ
		|			ИНАЧЕ ОстаткиПоОборотнымСубконто.СуммаВР
		|		КОНЕЦ) КАК СуммаВР
		|ИЗ
		|	ОстаткиПоОборотнымСубконто КАК ОстаткиПоОборотнымСубконто
		|
		|СГРУППИРОВАТЬ ПО
		|	ОстаткиПоОборотнымСубконто.Субконто3,
		|	ОстаткиПоОборотнымСубконто.Субконто2,
		|	ОстаткиПоОборотнымСубконто.Подразделение,
		|	ОстаткиПоОборотнымСубконто.Счет,
		|	ОстаткиПоОборотнымСубконто.Субконто1
		|ИТОГИ ПО
		|	Счет,
		|	Подразделение
		| %ДопИтоги%";
		Запрос.УстановитьПараметр("НачалоМесяца",СтруктураШапкиДокумента.НачДата);
		Запрос.УстановитьПараметр("КонецМесяца", СтруктураШапкиДокумента.КонДата);
		Запрос.УстановитьПараметр("Организация", СтруктураШапкиДокумента.Организация);
		
		Если Итерация = 1 Тогда
			Счет = ПланыСчетов.Хозрасчетный.ОсновноеПроизводство;
			Запрос.Текст = СтрЗаменить(Запрос.Текст,"%ДопИтоги%",",Субконто1");
			Запрос.УстановитьПараметр("Это20Счет", Истина);
		Иначе
			Счет = ПланыСчетов.Хозрасчетный.ОбщехозяйственныеРасходы;
			Запрос.Текст = СтрЗаменить(Запрос.Текст,"%ДопИтоги%","");
			Запрос.УстановитьПараметр("Это20Счет", Ложь);
		КонецЕсли;
		...
		Если НЕ Результат.Пустой() Тогда
			...
			Пока ВыборкаСчет.Следующий() Цикл
				...
				ВыборкаПодразд = ВыборкаСчет.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				
				Пока ВыборкаПодразд.Следующий() Цикл
					
					Если Итерация = 1 Тогда
						ВыборкаНомГр = ВыборкаПодразд.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
						Пока ВыборкаНомГр.Следующий() Цикл
							
							// Здесь общие проводки по ВР полож. и БУ
							НоваяПроводка = Проводки.Добавить();
							НоваяПроводка.Период          = Дата;
							НоваяПроводка.Организация     = СтруктураШапкиДокумента.Организация;
							НоваяПроводка.СчетДт          = ПланыСчетов.Хозрасчетный.Продажи_РасходыНаПродажуНеЕНВД;
							НоваяПроводка.ПодразделениеДт = ВыборкаНомГр.Подразделение;
							НоваяПроводка.СчетКт          = ВыборкаНомГр.Счет;
							НоваяПроводка.ПодразделениеКт = ВыборкаНомГр.Подразделение;
							БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетКт, НоваяПроводка.СубконтоКт, "НоменклатурныеГруппы", ВыборкаНомГр.Субконто1);
							НоваяПроводка.Сумма           = ВыборкаНомГр.Сумма;
							НоваяПроводка.СуммаВРДт       = ВыборкаНомГр.СуммаВР*-1;
							НоваяПроводка.СуммаВРКт       = ВыборкаНомГр.СуммаВР*-1;
							//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-03-30 (#3689)
							НоваяПроводка.СуммаПРДт       = ВыборкаНомГр.СуммаПР;
							НоваяПроводка.СуммаПРКт       = ВыборкаНомГр.СуммаПР;
							//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2020-03-30 (#3689)
							НоваяПроводка.Содержание      = "Закрытие счетов косвенных расходов";
							
							ДетЗаписи = ВыборкаНомГр.Выбрать();
							Пока ДетЗаписи.Следующий() Цикл
								
								// Здесь НУ и ВР отриц. (по статьям затрат)
								НоваяПроводка = Проводки.Добавить();
								НоваяПроводка.Период          = Дата;
								НоваяПроводка.Организация     = СтруктураШапкиДокумента.Организация;
								НоваяПроводка.СчетДт          = ПланыСчетов.Хозрасчетный.Продажи_УправленческиеРасходыНеЕНВД;
								НоваяПроводка.ПодразделениеДт = ДетЗаписи.Подразделение;
								НоваяПроводка.СчетКт          = ДетЗаписи.Счет;
								НоваяПроводка.ПодразделениеКт = ДетЗаписи.Подразделение;
								БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетКт, НоваяПроводка.СубконтоКт, "НоменклатурныеГруппы", ДетЗаписи.Субконто1);
								БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетКт, НоваяПроводка.СубконтоКт, "СтатьиЗатрат",         ДетЗаписи.Субконто2);
								БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетКт, НоваяПроводка.СубконтоКт, "ОК_Функции",           ДетЗаписи.Субконто3);
								
								НоваяПроводка.СуммаНУДт       = ДетЗаписи.СуммаНУ;
								НоваяПроводка.СуммаНУКт       = ДетЗаписи.СуммаНУ;	
								НоваяПроводка.СуммаВРДт       = ДетЗаписи.СуммаВР;
								НоваяПроводка.СуммаВРКт       = ДетЗаписи.СуммаВР;
						...
						КонецЦикла;
					Иначе
						ДетЗаписи = ВыборкаПодразд.Выбрать();
						Пока ДетЗаписи.Следующий() Цикл
							...
							НоваяПроводка.СчетКт          = ДетЗаписи.Счет;
							НоваяПроводка.ПодразделениеКт = ДетЗаписи.Подразделение;
							БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетКт, НоваяПроводка.СубконтоКт, "СтатьиЗатрат",         ДетЗаписи.Субконто1);
							БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетКт, НоваяПроводка.СубконтоКт, "ОбъектыСтроительства", ДетЗаписи.Субконто2);	
							БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетКт, НоваяПроводка.СубконтоКт, "ОК_Функции",           ДетЗаписи.Субконто3);
							
							НоваяПроводка.Сумма           = ДетЗаписи.Сумма;
							НоваяПроводка.СуммаНУДт       = ДетЗаписи.СуммаНУ;
							НоваяПроводка.СуммаНУКт       = ДетЗаписи.СуммаНУ;	
							НоваяПроводка.СуммаВРДт       = ДетЗаписи.СуммаВР;
							НоваяПроводка.СуммаВРКт       = ДетЗаписи.СуммаВР;
							НоваяПроводка.СуммаПРДт       = ДетЗаписи.СуммаПР;
							НоваяПроводка.СуммаПРКт       = ДетЗаписи.СуммаПР;
		...
		КонецЕсли;
	КонецЦикла;
	// Можно еще сделать движения во вспомогательные регистры сведений, но пока это не востребовано.	
