...
Функция ВыполнитьОперациюНаСервере(ИБФайловая, СсылкаНаЗаписанныйОбъект)
	...
	// ОКЕЙ Наумов А.Н.(СофтЛаб) Начало 2017-05-05 (#2824)
	// при записи вызывается подписчик ОбщийМодуль.бит_КонтрольЗакрытогоПериода.Модуль.бит_КонтрольЗакрытогоПериодаПередЗаписьюПередЗаписью
	// он затирает движения, чтобы этого не происходило взведем признак
	ДокОбъект.ДополнительныеСвойства.Вставить("бит_ДвиженияЗаконсервированы", Истина);
	// ОКЕЙ Наумов А.Н.(СофтЛаб) Конец 2017-05-05 (#2824)	
	...
	// ОКЕЙ Наумов А.Н.(СофтЛаб) Начало 2017-05-05 (#2824)
	ДокОбъект.ДополнительныеСвойства.Вставить("бит_ДвиженияЗаконсервированы", Ложь);
	// ОКЕЙ Наумов А.Н.(СофтЛаб) Конец 2017-05-05 (#2824)	
