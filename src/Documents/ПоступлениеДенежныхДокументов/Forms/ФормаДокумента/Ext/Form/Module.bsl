...
Процедура ДенежныеДокументыДенежныйДокументПриИзменении(Элемент)
	...
	//ОКЕЙ Завьялова Е.А. (ПервыйБИТ) Начало 2021-02-17 (#ТП_БП05_ФР04)
	СтрокаПлатеж.ок_ПодотчетноеЛицо = ок_ПолучитьПодотчетноеЛицоИзДенежногоДокумента(СтрокаПлатеж.ДенежныйДокумент);
	//ОКЕЙ Завьялова Е.А. (ПервыйБИТ) Конец 2021-02-17 (#ТП_БП05_ФР04)
...
//ОКЕЙ Завьялова Е.А. (ПервыйБИТ) Начало 2021-02-17 (#ТП_БП05_ФР04)
&НаСервереБезКонтекста
Функция ок_ПолучитьПодотчетноеЛицоИзДенежногоДокумента(Знач ДенежныйДокумент)
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДенежныйДокумент, "ок_ПодотчетноеЛицо");
...
// Процедура Подключаемый_СоздатьНаОснованииВыдачаДД
// 1. Количество созданных документов "Выдача ДД" должно быть равно количеству строк в ТЧ "Поступление ДД".
// 2. Документ "Выдача ДД" должен быть проведен сразу после создания.
// 3. Реквизиты документа "Выдача ДД" должны наследоваться из документа "Поступление ДД".
&НаКлиенте
Процедура Подключаемый_СоздатьНаОснованииВыдачаДД(Команда)
	
	Если Модифицированность Тогда
		ТекстВопроса = НСтр("ru = 'Данные еще не записаны.
		|Выполнение команды ""Создать документы ""Выдача денежных документов """" возможно только после записи данных.
		|Данные будут записаны. Продолжить?'");
		
		Оповещение = Новый ОписаниеОповещения("ок_ВопросСоздатьДокументыВыдачаДД", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
		
	Иначе
		
		ок_СоздатьДокументыВыдачаДД();
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура ок_СоздатьДокументыВыдачаДД()
	
	//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Начало 2021-07-13 (#ТП_БП05_ФР04)
	СообщениеДляВывода = НСтр("ru='Созданы документы ""Выдача денежных документов"":'");
	ВыводитьСообщение = Ложь;
	//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Конец 2021-07-113 (#ТП_БП05_ФР04)
	Для Каждого Строка Из Объект.ДенежныеДокументы Цикл
		ДокументВыдачаДД = Документы.ВыдачаДенежныхДокументов.СоздатьДокумент();
		ДокументВыдачаДД.Дата					=	Объект.Дата;
		ДокументВыдачаДД.УстановитьНовыйНомер();
		ДокументВыдачаДД.Организация			=	Объект.Организация;
		//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Начало 2021-07-13 (#ТП_БП05_ФР04)
		ДокументВыдачаДД.ПодразделениеОрганизации =	Объект.ПодразделениеОрганизации;
		//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Конец 2021-07-113 (#ТП_БП05_ФР04)
		ДокументВыдачаДД.ок_ДокументОснование	=	Объект.Ссылка;
		ДокументВыдачаДД.ВидОперации			=	Перечисления.ВидыОперацийВыдачаДенежныхДокументов.ВыдачаПодотчетномуЛицу;
		ДокументВыдачаДД.Выдано 				=	Строка(Строка.ок_ПодотчетноеЛицо);
		ДокументВыдачаДД.Контрагент				=	Строка.ок_ПодотчетноеЛицо;
		ДокументВыдачаДД.СчетУчетаДенежныхДокументов = Объект.СчетУчетаДенежныхДокументов;
		ДокументВыдачаДД.ВалютаДокумента		=	Объект.ВалютаДокумента;
		ДокументВыдачаДД.КратностьВзаиморасчетов = Объект.КратностьВзаиморасчетов;
		ДокументВыдачаДД.КурсВзаиморасчетов		= Объект.КурсВзаиморасчетов;
		
		НоваяСтрока = ДокументВыдачаДД.ДенежныеДокументы.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
		НоваяСтрока.Стоимость = Строка.Сумма/Строка.Количество;
		//НовДокумент.Записать(РежимЗаписиДокумента.Проведение,РежимПроведенияДокумента.Оперативный);
		Попытка
			ДокументВыдачаДД.записать(РежимЗаписиДокумента.Проведение);
		Исключение
			
			ДокументВыдачаДД.записать(РежимЗаписиДокумента.Запись);
		КонецПопытки;
		//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Начало 2021-07-13 (#ТП_БП05_ФР04)
		СообщениеДляВывода = СообщениеДляВывода + Символы.ПС + ДокументВыдачаДД.Номер + " от " +  Формат(ДокументВыдачаДД.Дата, "ДФ=dd.MM.yyyy");
		ВыводитьСообщение = Истина;
		//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Конец 2021-07-113 (#ТП_БП05_ФР04)
	КонецЦикла;
	//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Начало 2021-07-13 (#ТП_БП05_ФР04)
	Если ВыводитьСообщение Тогда
		ОбщегоНазначения.СообщитьПользователю(СообщениеДляВывода);
	КонецЕсли;
	//ОКЕЙ Дмитриева В.В. (ПервыйБИТ) Конец 2021-07-113 (#ТП_БП05_ФР04)
...
&НаКлиенте
Процедура ок_ВопросСоздатьДокументыВыдачаДД(Результат, ДополнительныеПараметры) Экспорт
