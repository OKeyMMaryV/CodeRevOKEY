...
Функция ПолучитьРеквизитыИсходногоСчетаФактурыДляКорректировки(
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		...
		Если СчетФактура.Исправление Тогда 
			...
			КонецЕсли; 
		Иначе
			ИсходныйДокументРеализации = ?(Не ИсходныйДокумент = Неопределено, ИсходныйДокумент, ДокументРеализации);
		КонецЕсли;
...
Функция ТекстЗапросаСуммыАвансовПоВзаиморасчетам(НомераТаблиц)
	...
	|				И СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоАвансамПолученнымВСчетОтгрузкиУЕ)
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура534Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур534Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактурНовый(ПорядокПримененияФормСФ["СчетФактура534Прослеж"], Ложь, Истина),
			ПараметрыПечати);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура534Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур534Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактурНовый(ПорядокПримененияФормСФ["КорректировочныйСчетФактура534Прослеж"],Ложь,Истина),
			ПараметрыПечати);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
...
// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-06-09 (#4208)
Функция ТекстЗапросаПечатьСчетовФактурНовый(ВерсияПостановленияНДС1137 = Неопределено, ДляУПД = Ложь, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
...
Функция ТекстЗапросаПечатьКорректировочныхСчетовФактурНовый(ВерсияПостановленияНДС1137 = Неопределено, ДляУКД = Ложь, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
...
Функция ТекстЗапросаНачислениеНДС(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ВидСчетаФактуры <> Перечисления.ВидСчетаФактурыВыставленного.НаАванс
		...
		И Реквизиты.ВидСчетаФактуры <> Перечисления.ВидСчетаФактурыВыставленного.НаАвансКомитента 
		И Реквизиты.ВидСчетаФактуры <> Перечисления.ВидСчетаФактурыВыставленного.НаАвансКомитентаНаЗакупку
		// 1С-ИжТиСи Полина 18.08.2015 )		
