...
Процедура ОпределениеПараметровСчетаФактурыНаРеализацию() Экспорт
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации") И 
		ТипЗнч(ДокументОснование.ДокументРеализации) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		ЗаполнитьНаОснованииОперацияАксапты(ДокументОснование);
		Возврат;
	КонецЕсли;	
...
//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-05-17 (#НТП-МП-03)
Процедура ЗаполнитьНаОснованииОперацияАксапты(ДокументОснование)
	
	РеквизитыОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОснование, "ок_НомерИсходногоСФ, ок_ДатаИсходногоСФ");
	
	НомерИсходногоДокумента = РеквизитыОснования.ок_НомерИсходногоСФ;
	ДатаИсходногоДокумента  = РеквизитыОснования.ок_ДатаИсходногоСФ;
	Для Каждого СтрокаОснования из ДокументыОснования Цикл
		СтрокаОснования.НомерИсходногоДокумента = РеквизитыОснования.ок_НомерИсходногоСФ;
		СтрокаОснования.ДатаИсходногоДокумента  = РеквизитыОснования.ок_ДатаИсходногоСФ;	
	КонецЦикла;
