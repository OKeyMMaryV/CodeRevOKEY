...
Функция ТекстЗапросаАвансыПолученные() Экспорт
	...
	// ОКЕЙ Первухин В.С. (ПервыйБИТ) Начало 2021-10-06 (#Исправление)
	|	И НЕ ХозрасчетныйОстатки.Субконто3 = НЕОПРЕДЕЛЕНО
	// ОКЕЙ Первухин В.С. (ПервыйБИТ) Конец 2021-10-06 (#Исправление)
	|
	|СГРУППИРОВАТЬ ПО
	...
	Для Каждого СтрокаСчетФактура Из ТаблицаНДСкВычету Цикл
		...
		Если СписокСчетовФактур.НайтиПоЗначению(СтрокаСчетФактура.СчетФактура) <> Неопределено 
			ИЛИ (СтрокаСчетФактура.ВидЦенности = Перечисления.ВидыЦенностей.Возврат 
				...
				//ТипЗнч(СтрокаСчетФактура.СчетФактура) <> Тип("ДокументСсылка.ВозвратТоваровОтПокупателя"))
				ИЛИ (ТипЗнч(СтрокаСчетФактура.СчетФактура) <> Тип("ДокументСсылка.ВозвратТоваровОтПокупателя")
				ИЛИ ТипЗнч(СтрокаСчетФактура.СчетФактура) <> Тип("ДокументСсылка.ВозвратТоваровПоставщику")))
				// 1С-ИжТиСи Полина 18.08.2015 )
...
Функция ПолучитьДанныеОЗачтенныхАвансахПоСпискуСФ_УпрощенныйУчетНДС(СтруктураПараметров, МенеджерВТ_СписокСчетовФактур)
	...
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-09-22 (#4350)
	|	И НЕ ОборотыПоДебету62.СчетФактура.Дата ЕСТЬ NULL
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-09-22 (#4350)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Покупатель,
	|	СчетФактура,
