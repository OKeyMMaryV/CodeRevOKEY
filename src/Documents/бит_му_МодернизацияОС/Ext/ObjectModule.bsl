...
Функция ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, СтруктураКурсыВалют)  Экспорт
	...
	|	ОсновныеСредства.ДатаМодернизации КАК ДатаМодернизации,
...
Процедура ЗаполнитьОсновныеСредства(ВыбранноеЗначение, МожноПерезаполнятьДанныеПоОС) Экспорт
	...
	Для каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		...
		Если НайденнаяСтрока = Неопределено Тогда
			...
			////izhtc-Kir 12.10.2015)	
			Если ВыбранноеЗначение.Режим = Перечисления.бит_му_РежимыПодбораВНА.ок_МодернизацияОборудования Тогда 
				
				ЭтотОбъект.ДополнительныеСвойства.Вставить("РежимПодбораОсновныхСредств", ВыбранноеЗначение.Режим);
		...
		Для Каждого СтрокаТаблицы ИЗ ОсновныеСредства Цикл
			Если ЗначениеЗаполнено(СтрокаТаблицы.СчетНезавершенногоСтроительства) Тогда
				...
				Для Ном = 1 По КоличествоСубконто Цикл
					...
					Если СтрокаТаблицы.СчетНезавершенногоСтроительства.ВидыСубконто[Ном-1].ВидСубконто = ПланыВидовХарактеристик.бит_ВидыСубконтоДополнительные_2.Номенклатура Тогда 
...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, СтруктураТаблиц, СтруктураКурсыВалют, ИсторическиеКурсы, Отказ, Заголовок)
	...
	///БИТ Тртилек
		Для Каждого СтрокаТаблицы Из ТаблицаДанных Цикл
			...
			Запись.ОсновноеСредство = СтрокаТаблицы.ОсновноеСредство;
			// ОКЕЙ Конаков Ю.М. (СофтЛаб) Начало 2021-11-09 (#4297) 
			Запись.Период			= СтрокаТаблицы.ДатаМодернизации;
			// ОКЕЙ Конаков Ю.М. (СофтЛаб) Конец 2021-11-09 (#4297) 
		КонецЦикла;
