...
Функция ПроверитьПодключениеСУЗНаСервере()
	...
	Если ПараметрыСУЗ.ФорматОбмена = ПредопределенноеЗначение("Перечисление.ВерсииФорматаОбменаСУЗ.V1") Тогда
	...
	Иначе
		СтруктураРезультат = ИнтерфейсСУЗ.ПроверитьДоступностьСУЗ_V2(
			ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Обувная"), ПараметрыСУЗ);
	КонецЕсли;
