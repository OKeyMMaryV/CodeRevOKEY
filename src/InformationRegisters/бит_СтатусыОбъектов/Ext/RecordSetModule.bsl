...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	СтатусОбъекта = Неопределено;
	Если ТипОбъекта = Тип("ДокументСсылка.ЭлектронныйДокументВходящий") Тогда 
		СтатусОбъекта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Отбор.Объект.Значение, "ок_статус");
	КонецЕсли;
	...
	Для каждого Запись Из ЭтотОбъект Цикл
		...
		// ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2021-10-14 (#4324)
		Если СтатусОбъекта <> Неопределено
			И СтатусОбъекта <> Запись.Статус Тогда 
			
			ДокОбъект = Отбор.Объект.Значение.ПолучитьОбъект();
			Попытка
				ДокОбъект.Заблокировать();
				ДокОбъект.ок_статус = Запись.Статус;
				ДокОбъект.ДополнительныеСвойства.Вставить("СинхронизацияСтатуса", Истина);
