...
Функция ДействияФормыЗагрузитьАналитику_Сервер(Отказ)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	(бит_ок_СопоставлениеАналитики.КодАналитикиАксапты В (&СтатьиЗатрат)
	               |			ИЛИ бит_ок_СопоставлениеАналитики.КодАналитикиАксапты В (&СтатьиДДС))
	               |	И бит_ок_СопоставлениеАналитики.Организация = &Организация"; //***БИТ***Теплова***
...
Функция ДействияФормыЗагрузитьАналитику_Сервер_1(ДанныеДляСозданияЗаписей)
	...
	Для каждого текЭлемент из ДанныеДляСозданияЗаписей.МассивКодовСтатейЗатрат Цикл
		Запись = РегистрыСведений.бит_ок_СопоставлениеАналитики.СоздатьМенеджерЗаписи();
		//***БИТ***Теплова***(
...
Процедура ДействияФормыЗагрузитьАналитику(Команда)
	Перем ТабСтатьиЗатрат, НовСтр, ТабСтатьиДДС, МассивИсключений;
	 //***БИТ***Теплова***(
	Форма = ПолучитьФорму("РегистрСведений.бит_ок_СопоставлениеАналитики.Форма.ФормаВыбораОрганизации",, ЭтаФорма);
	...
	//Вывод окна выбора
	Форма = ПолучитьФорму("РегистрСведений.бит_ок_СопоставлениеАналитики.Форма.ФормаВыбораСтатейЗатратИДДСДляДобавления",, ЭтаФорма);
