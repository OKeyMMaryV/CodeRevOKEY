...
Функция Отбор(Параметры) Экспорт
	...
	Если ТипЗнч(Параметры) = Тип("Структура")
		И Параметры.Свойство("ок_Объект") Тогда
		ОтборЛисты.Вставить("ок_Объект", Параметры.ок_Объект);	
	КонецЕсли;
...
Функция ПолучитьНомераЛистовЗаВесьПериод(ДатаПоследнегоЛиста, Знач ПараметрыОтчета, Знач СписокПодразделений)
	...
	|	И НомераЛистовКассовойКниги.ок_Объект = &ок_Объект
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Конец 2021-09-22 (#ТП_БП11_ФР14)
	...
	Если ТипЗнч(ПараметрыОтчета) = Тип("Структура")
		И ПараметрыОтчета.Свойство("ок_Объект") Тогда
		ЗапросЛисты.УстановитьПараметр("ок_Объект", ПараметрыОтчета.ок_Объект);
	Иначе
		ЗапросЛисты.УстановитьПараметр("ок_Объект", Справочники.ОбъектыСтроительства.ПустаяСсылка());	
	КонецЕсли;
	//ОКЕЙ Балыков А.Г. (ПервыйБИТ) Конец 2021-09-22 (#ТП_БП11_ФР14)
