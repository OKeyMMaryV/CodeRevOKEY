...
Процедура ПроверкаУчетнойЗаписиПродолжить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	//ОК(СофтЛаб) Вдовиченко Г.В. 20181101 №3103
	Если ЗначениеЗаполнено(Запись.ок_СпособОбменаЭД) Тогда
		
		Ответ = "";
		Результат = ок_ОбменСКонтрагентамиСлужебныйВызовСервера.ТестПодключения(Запись.ИдентификаторЭДО, Ответ);
		Сообщить(?(Результат, "Тест удачно пройден", "Тест не пройден") + ?(ПустаяСтрока(Ответ), "", ": " + Ответ));
		Возврат;
	КонецЕсли;	
	//ОК(СофтЛаб) Вдовиченко Г.В. 20181101 №3103		
...
Функция ПроверитьЗаполнениеРеквизитов()
	...
	Иначе
		...
		Для каждого СтрокаСертификата Из ВыбранныеСертификаты Цикл
			Если СтрокаСертификата.СрокДействияВДнях > 0 Тогда
...
&НаКлиенте
Процедура НачатьПолучениеВсехСертификатовИСамогоНового(Оповещение, СписокСертификатов) Экспорт
...
Процедура ЗагрузитьНастройкиУведомленийИзСервиса()
	...
	Оповещение = Новый ОписаниеОповещения("ЗагрузитьНастройкиУведомленийИзСервисаПослеПолученияСерификатов", ЭтотОбъект);
	НачатьПолучениеВсехСертификатовИСамогоНового(Оповещение, СписокСертификатов);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьНастройкиУведомленийИзСервисаПослеПолученияСерификатов(ДанныеСертификатов, ДополнительныеПараметры) Экспорт
...
Процедура ок_ПроверитьИдентификаторУчастникаОбмена(Команда) Экспорт
	...
	Если Модифицированность Тогда
		Сообщить("Сохраните учетную запись перед проверкой");
		Возврат;
	КонецЕсли;		
	...
	Ответ = "";
	Результат = ок_ОбменСКонтрагентамиСлужебныйВызовСервера.ПроверитьИдентификаторУчастникаОбмена(Запись.ИдентификаторЭДО, Новый Структура("ИдентификаторКонтрагента, Контрагент", Запись.ИдентификаторЭДО, Запись.Организация), Ответ);
	Если Не Результат Тогда
		Сообщить("Не удалось получить идентификатор участника обмена" + ?(ПустаяСтрока(Ответ), "", ": " + Ответ));
	Иначе
		Модифицированность = Истина;
		Запись.ок_ИдентификаторУчастникаОбмена = Ответ;
	КонецЕсли;	
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(ЭтаФорма["ок_Логин"]) Тогда
		ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(Запись.ИдентификаторЭДО, ЭтаФорма["ок_Логин"], "ЛогинFTP");
	КонецЕсли;
	Если ЗначениеЗаполнено(ЭтаФорма["ок_Пароль"]) Тогда
		ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(Запись.ИдентификаторЭДО, ЭтаФорма["ок_Пароль"], "ПарольFTP");
	КонецЕсли;
