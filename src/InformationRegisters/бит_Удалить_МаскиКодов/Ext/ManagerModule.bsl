...
#Область ЗаполнениеМасокПоУмолчанию
...
Процедура ПеренестиДанныеПриОбновлении() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	МаскиКодов.ОбъектСистемы КАК ОбъектСистемы,
	|	МаскиКодов.ИмяРеквизита КАК ИмяРеквизита,
	|	МаскиКодов.МаскаКода КАК МаскаКода,
	|	МаскиКодов.МаскаТекущая КАК МаскаТекущая,
	|	МаскиКодов.Автозаполнение КАК Автозаполнение
	|ИЗ
	|	РегистрСведений.бит_Удалить_МаскиКодов КАК МаскиКодов
	|ГДЕ
	|	МаскиКодов.ОбъектСистемы ССЫЛКА Справочник.бит_ОбъектыСистемы
	|ИТОГИ ПО
	|	ОбъектСистемы";
	...
	Пока ВыборкаОбъект.Следующий() Цикл
		НаборЗаписей = РегистрыСведений.бит_МаскиКодов.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.ОбъектСистемы.Установить(ВыборкаОбъект.ОбъектСистемы);
...
#Область ЧтениеИИзменениеДанныхРегистра
...
#Область ЗаполнениеКодификатораПоУмолчанию
