		...
		Для Каждого СтрокаНоменклатура Из ЭлементыКорня Цикл
			Если ЗначениеЗаполнено(СтрокаНоменклатура.КоличествоПечать) И ВидимостьШаблонаЭтикетки Тогда
				...
				Если Не ЗначениеЗаполнено(СтрокаНоменклатура.ШаблонПотребительскойЭтикетки) Тогда
					ТекстСообщения = СтрШаблон(НСтр("ru = 'В строке %1 не заполнено поле ""Шаблон потребительской этикетки""'"), НомерСтроки);
				...
				Если Не ЗначениеЗаполнено(СтрокаНоменклатура.ШаблонЛогистическойЭтикетки) Тогда
					ТекстСообщения = СтрШаблон(НСтр("ru = 'В строке %1 не заполнено поле ""Шаблон логистической этикетки""'"), НомерСтроки);
					Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
...
Процедура Печать(Команда)
	...
	ПоказатьВопрос(ОповещениеВопросПечати, 
		НСтр("ru = 'Распечатать этикетки по выбранным данным можно только один раз. Продолжить?'"),
...
Функция ДанныеДляПечатиАгрегированныеКоды()
	...
	ИтоговаяСтруктура = Новый Структура;
	ИтоговаяСтруктура.Вставить("Документ",                   Документ);
	ИтоговаяСтруктура.Вставить("РежимПечати",                "АгрегированныеКоды");
	ИтоговаяСтруктура.Вставить("КаждаяЭтикеткаНаНовомЛисте", КаждаяЭтикеткаНаНовомЛисте());
	...
	Для Каждого СтрокаДерева Из ДеревоАгрегированныхКодов.ПолучитьЭлементы() Цикл
		...
		Если СтрокаДерева.КоличествоПечать = 0 
			Или Не ЗначениеЗаполнено(СтрокаДерева.ШаблонПотребительскойЭтикетки)
			Или Не ЗначениеЗаполнено(СтрокаДерева.ШаблонЛогистическойЭтикетки) 
		...
		СтруктураПараметровПечатиУпаковок.Вставить("Шаблон",                СтрокаДерева.Шаблон);
		СтруктураПараметровПечатиУпаковок.Вставить("СпособВводаВОборот",    СтрокаДерева.СпособВводаВОборот);
	...
	Если Отказ Тогда
		ВызватьИсключение "Невозможно выполнить печать агрегированных кодов";
	КонецЕсли;
...
Функция МассивДанныхДляПечатиЭтикетокПоАгрегированнымКодам(Документ, МассивДанныхПоХешам, Отказ)
	...
	ТаблицаХешей.Колонки.Добавить("Содержимое",                    Новый ОписаниеТипов("Строка"));
	...
	ТаблицаХешей.Колонки.Добавить("Шаблон",                        Новый ОписаниеТипов("ПеречислениеСсылка.ШаблоныКодовМаркировкиСУЗ"));
	ТаблицаХешей.Колонки.Добавить("СпособВводаВОборот",            Новый ОписаниеТипов("ПеречислениеСсылка.СпособыВводаВОборотСУЗ"));
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ТаблицаХешей.Организация,
		|	ТаблицаХешей.Содержимое,
		|	ТаблицаХешей.ХешСуммаУпаковки КАК ХешСуммаУпаковки,
		|	ТаблицаХешей.Количество,
		|	ТаблицаХешей.КоличествоЭкземпляров,
		|	ТаблицаХешей.Шаблон,
		|	ТаблицаХешей.СпособВводаВОборот,
		|	ТаблицаХешей.ШаблонПотребительскойЭтикетки,
		|	ТаблицаХешей.ШаблонЛогистическойЭтикетки
		...
		|			И ТаблицаПараметровУпаковок.Шаблон             = ПулКодовМаркировкиСУЗ.Шаблон
		|			И ТаблицаПараметровУпаковок.СпособВводаВОборот = ПулКодовМаркировкиСУЗ.СпособВводаВОборот
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	ХешСуммаУпаковки,
		...
		|	КоличестваПоХешу.ШтрихкодУпаковки            КАК ШтрихкодУпаковки,
		|	СУММА(КоличестваПоХешу.СодержимоеКоличество) КАК СодержимоеКоличество,
		|	МАКСИМУМ(ВЫБОР
		|		КОГДА КоличестваПоХешу.ДатаПечатиУниверсальная = ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА 0
		|		ИНАЧЕ 1
		|	КОНЕЦ)                                       КАК РаспечатаноКоличество
		|ПОМЕСТИТЬ ТаблицаКоличестваПоХешуИУпаковке
		|ИЗ
		|	ТаблицаПула КАК КоличестваПоХешу
		|СГРУППИРОВАТЬ ПО
		|	КоличестваПоХешу.Организация,
		|	КоличестваПоХешу.ШтрихкодУпаковки,
		...
		|	КоличестваПоХешу.Организация                    КАК Организация,
		|	КоличестваПоХешу.ХешСуммаУпаковки               КАК ХешСуммаУпаковки,
		|	МАКСИМУМ(КоличестваПоХешу.СодержимоеКоличество) КАК СодержимоеКоличество,
		|	СУММА(КоличестваПоХешу.РаспечатаноКоличество)   КАК РаспечатаноКоличество
		|ПОМЕСТИТЬ ТаблицаКоличестваПоХешу
		|ИЗ
		|	ТаблицаКоличестваПоХешуИУпаковке КАК КоличестваПоХешу
		|СГРУППИРОВАТЬ ПО
		|	КоличестваПоХешу.Организация,
		|	КоличестваПоХешу.ХешСуммаУпаковки
		|ИНДЕКСИРОВАТЬ ПО
		...
		|	ТаблицаПараметровУпаковок.ШаблонПотребительскойЭтикетки    КАК ШаблонПотребительскойЭтикетки,
		|	ТаблицаПараметровУпаковок.ШаблонЛогистическойЭтикетки      КАК ШаблонЛогистическойЭтикетки,
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.ШтрихкодУпаковки, НЕОПРЕДЕЛЕНО)                   КАК ШтрихкодУпаковки,
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.ШтрихкодУпаковки.Номенклатура, НЕОПРЕДЕЛЕНО)      КАК НоменклатураУпаковка,
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.ШтрихкодУпаковки.Характеристика, НЕОПРЕДЕЛЕНО)    КАК ХарактеристикаУпаковка,
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.Шаблон, НЕОПРЕДЕЛЕНО)                             КАК Шаблон,
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.СпособВводаВОборот, НЕОПРЕДЕЛЕНО)                 КАК СпособВводаВОборот,
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.ШтрихкодУпаковки.ЗначениеШтрихкода, НЕОПРЕДЕЛЕНО) КАК ЗначениеШтрихкодаУпаковки,
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.ШтрихкодУпаковки.ТипШтрихкода, НЕОПРЕДЕЛЕНО)      КАК ТипШтрихкодаУпаковки,
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.ВидПродукции, НЕОПРЕДЕЛЕНО)                       КАК ВидПродукции,
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.Номенклатура, НЕОПРЕДЕЛЕНО)                       КАК Номенклатура,
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.Характеристика, НЕОПРЕДЕЛЕНО)                     КАК Характеристика,
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.КодМаркировки, НЕОПРЕДЕЛЕНО)                      КАК КодМаркировки,
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.ПолныйКодМаркировки, НЕОПРЕДЕЛЕНО)                КАК ПолныйКодМаркировки,
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.ДатаПечатиУниверсальная, ДАТАВРЕМЯ(1, 1, 1))      КАК ДатаПечатиУниверсальная,
		|	ЕСТЬNULL(ТаблицаКоличестваПоХешу.СодержимоеКоличество, 0)                        КАК СодержимоеКоличество,
		|	ЕСТЬNULL(ТаблицаКоличестваПоХешу.РаспечатаноКоличество, 0)                       КАК РаспечатаноКоличество,
		|	ТаблицаКоличестваПоХешу.ХешСуммаУпаковки                                         КАК ХешСуммаУпаковки
		|ИЗ
		|	ТаблицаПараметровУпаковок КАК ТаблицаПараметровУпаковок
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПула КАК ПулКодовМаркировкиСУЗ
		|		ПО ТаблицаПараметровУпаковок.Организация        = ПулКодовМаркировкиСУЗ.Организация
		|		И ТаблицаПараметровУпаковок.ХешСуммаУпаковки    = ПулКодовМаркировкиСУЗ.ХешСуммаУпаковки
		|		И ПулКодовМаркировкиСУЗ.ДатаПечатиУниверсальная = ДАТАВРЕМЯ(1, 1, 1)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаКоличестваПоХешу КАК ТаблицаКоличестваПоХешу
		|		ПО ТаблицаПараметровУпаковок.Организация     = ТаблицаКоличестваПоХешу.Организация
		|		И ТаблицаПараметровУпаковок.ХешСуммаУпаковки = ТаблицаКоличестваПоХешу.ХешСуммаУпаковки
		|УПОРЯДОЧИТЬ ПО
		|	ДатаПечатиУниверсальная
		|ИТОГИ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ШтрихкодУпаковки)  КАК ШтрихкодУпаковки,
		|	МАКСИМУМ(ВидПродукции)                  КАК ВидПродукции,
		|	МАКСИМУМ(НоменклатураУпаковка)          КАК НоменклатураУпаковка,
		|	МАКСИМУМ(ХарактеристикаУпаковка)        КАК ХарактеристикаУпаковка,
		|	МАКСИМУМ(Шаблон)                        КАК Шаблон,
		|	МАКСИМУМ(СпособВводаВОборот)            КАК СпособВводаВОборот,
		|	МАКСИМУМ(ЗначениеШтрихкодаУпаковки)     КАК ЗначениеШтрихкодаУпаковки,
		|	МАКСИМУМ(ШаблонЛогистическойЭтикетки)   КАК ШаблонЛогистическойЭтикетки,
		|	МАКСИМУМ(ШаблонПотребительскойЭтикетки) КАК ШаблонПотребительскойЭтикетки,
		|	МАКСИМУМ(ТипШтрихкодаУпаковки)          КАК ТипШтрихкодаУпаковки,
		|	МАКСИМУМ(СодержимоеКоличество)          КАК СодержимоеКоличество,
		|	МАКСИМУМ(Количество)                    КАК Количество,
		|	МАКСИМУМ(КоличествоЭкземпляров)         КАК КоличествоЭкземпляров,
		|	МАКСИМУМ(РаспечатаноКоличество)         КАК РаспечатаноКоличество
		|ПО
		|	Организация,
		|	ХешСуммаУпаковки,
		|	ШтрихкодУпаковки";
	...
	РегистрыСведений.ПулКодовМаркировкиСУЗ.ДобавитьВЗапросПечатиУсловияПоДокументу(
		Запрос, СтруктураОтбораПоДокументу, 1);
		...
		Пока ВыборкаХешУпаковки.Следующий() Цикл
			...
			Пока ВыборкаУпаковка.Следующий() И ДобавленоУпаковок < НужноДобавить Цикл
				...
				Пока ВыборкаКодМаркировки.Следующий() Цикл
					...
					ПараметрыШтрихкода.ВидПродукции         = ВыборкаКодМаркировки.ВидПродукции;
					...
					ПараметрыШтрихкода.Штрихкод             = ВыборкаКодМаркировки.КодМаркировки;
					ПараметрыШтрихкода.Шаблон               = ВыборкаКодМаркировки.Шаблон;
					ПараметрыШтрихкода.СпособВводаВОборот   = ВыборкаКодМаркировки.СпособВводаВОборот;
					ПараметрыШтрихкода.ТипШтрихкода         = Перечисления.ТипыШтрихкодов.DataMatrix;
				...
				ПараметрыШтрихкода.ВидПродукции         = ВыборкаУпаковка.ВидПродукции;
				...
				ПараметрыШтрихкода.Штрихкод             = ВыборкаУпаковка.ЗначениеШтрихкодаУпаковки;
				ПараметрыШтрихкода.ТипШтрихкода         = ВыборкаУпаковка.ТипШтрихкодаУпаковки;
				ПараметрыШтрихкода.НомерВГруппе         = НомерВГруппе;
				ПараметрыШтрихкода.СодержимоеКоличество = ВыборкаХешУпаковки.СодержимоеКоличество;
...
Процедура ОбработкаОтветаПользователяПриПечати(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если ДанныеПечати.ОбъектыПечати.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,, "СписокКодовМаркировки.КоличествоПечать");
		Возврат;
	КонецЕсли;
