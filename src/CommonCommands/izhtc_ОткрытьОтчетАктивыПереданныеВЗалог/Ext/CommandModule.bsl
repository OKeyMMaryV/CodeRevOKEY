...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	//ОК_ОбщегоНазначения.ОткрытьОтчетАктивыПереданныеВЗалог();
	ИмяОбработки = Неопределено;
	ОткрытьОтчетАктивыПереданныеВЗалог(ИмяОбработки);
	Если ИмяОбработки <> Неопределено Тогда
		ОткрытьФорму("ВнешнийОтчет."+ ИмяОбработки +".Форма");	
	КонецЕсли;
	// -СБ Кузнецова С.
	//Форма = ПолучитьФорму("Отчет.СБ_АктивыПереданныеВЗалог.Форма.ФормаОтчетаУправляемая");	 //ОК Довбешка Т. 01.04.2017
	//Форма.Открыть();
КонецПроцедуры

// +СБ Кузнецова С. 2016-07-27 
&НаСервере
Процедура ОткрытьОтчетАктивыПереданныеВЗалог(ИмяОбработки) 
	имяотчета_="Активы, переданные в залог или использование которых ограничено";
	попытка
