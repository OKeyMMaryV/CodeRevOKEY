...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	//ОК_ОбщегоНазначения.ОткрытьОтчетФинансовыеАктивыИОбязательства();
	ИмяОбработки = Неопределено;
	ОткрытьОтчетФинансовыеАктивыИОбязательства(ИмяОбработки);
	Если ИмяОбработки <> Неопределено Тогда
		ОткрытьФорму("ВнешнийОтчет."+ ИмяОбработки +".Форма");	
	КонецЕсли;
	// -СБ Кузнецова С.
	//Форма = ПолучитьФорму("Отчет.СБ_ФинансовыеАктивыИОбязательства.Форма.ФормаОтчетаУправляемая");	 //ОК Довбешка Т. 01.04.2017
	//Форма.Открыть();
КонецПроцедуры

// +СБ Кузнецова С. 2016-07-27 
&НаСервере
Процедура ОткрытьОтчетФинансовыеАктивыИОбязательства(ИмяОбработки) 
